...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "thinwedge-app-server";
const AGENT_IDENTITY_JWT_ISSUER: &str = "https://chatgpt.com/thinwedge-backend/agent-identity";
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "thinwedge-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "thinwedge-cli".to_string(),
        },
...
mod tests {
    ...
    fn test_rsa_encoding_key() -> EncodingKey {
        EncodingKey::from_rsa_pem(
            br#"PUBLIC_HISTORY_TEST_PRIVATE_KEY_REMOVED"#,
        )
    ...
    #[test]
    fn agent_identity_jwks_url_uses_thinwedge_api_base_url() {
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/thinwedge"),
            "http://localhost:8080/api/thinwedge/agent-identities/jwks"
        );
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/thinwedge/"),
            "http://localhost:8080/api/thinwedge/agent-identities/jwks"
