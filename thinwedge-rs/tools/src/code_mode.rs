            ...
            for tool in &mut namespace.tools {
                match tool {
                    ResponsesApiNamespaceTool::Function(tool) => {
                        ...
                        tool.description =
                            thinwedge_code_mode::augment_tool_definition(definition).description;
                    }
                }
            }
...
pub fn tool_spec_to_code_mode_tool_definition(spec: &ToolSpec) -> Option<CodeModeToolDefinition> {
    ...
    thinwedge_code_mode::is_code_mode_nested_tool(&definition.name)
        .then(|| thinwedge_code_mode::augment_tool_definition(definition))
}
...
pub fn collect_code_mode_tool_definitions<'a>(
    ...
    let mut tool_definitions = specs
        ...
        .filter(|definition| thinwedge_code_mode::is_code_mode_nested_tool(&definition.name))
        .map(thinwedge_code_mode::augment_tool_definition)
        .collect::<Vec<_>>();
...
pub fn create_wait_tool() -> ToolSpec {
    ...
    ToolSpec::Function(ResponsesApiTool {
        name: thinwedge_code_mode::WAIT_TOOL_NAME.to_string(),
        description: format!(
            "Waits on a yielded `{}` cell and returns new output or completion.\n{}",
            thinwedge_code_mode::PUBLIC_TOOL_NAME,
            thinwedge_code_mode::build_wait_tool_description().trim()
        ),
        strict: false,
...
"#;
    ...
    ToolSpec::Freeform(FreeformTool {
        name: thinwedge_code_mode::PUBLIC_TOOL_NAME.to_string(),
        description: thinwedge_code_mode::build_exec_tool_description(
            enabled_tools,
