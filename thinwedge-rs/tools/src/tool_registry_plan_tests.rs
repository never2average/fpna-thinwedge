...
#[test]
fn test_full_toolset_specs_for_gpt5_thinwedge_unified_exec_web_search() {
...
fn web_search_config_is_forwarded_to_tool_spec() {
    ...
    let web_search_config = WebSearchConfig {
        filters: Some(thinwedge_protocol::config_types::WebSearchFilters {
            allowed_domains: Some(vec!["example.com".to_string()]),
        }),
        user_location: Some(thinwedge_protocol::config_types::WebSearchUserLocation {
            r#type: thinwedge_protocol::config_types::WebSearchUserLocationType::Approximate,
            country: Some("US".to_string()),
            region: Some("California".to_string()),
            city: Some("San Francisco".to_string()),
            timezone: Some("America/Los_Angeles".to_string()),
        }),
        search_context_size: Some(thinwedge_protocol::config_types::WebSearchContextSize::High),
    };
...
fn namespaced_dynamic_specs_are_hidden_when_namespace_tools_are_disabled() {
    ...
    assert_lacks_tool_name(&tools, "thinwedge_app");
...
fn search_tool_description_lists_each_mcp_source_once() {
    ...
    let (tools, handlers) = build_specs(
        ...
        Some(HashMap::from([
            (
                ToolName::namespaced("mcp__thinwedge_apps__calendar", "_create_event"),
                mcp_tool(
    ...
    assert!(handlers.contains(&ToolHandlerSpec {
        name: ToolName::namespaced("mcp__thinwedge_apps__calendar", "_create_event"),
        kind: ToolHandlerKind::Mcp,
    }));
    assert!(handlers.contains(&ToolHandlerSpec {
...
fn search_tool_registers_for_deferred_dynamic_tools() {
    ...
    assert_eq!(
        tools
            .iter()
            .filter(|tool| tool.name() == "thinwedge_app")
            .count(),
        1
    );
    assert_eq!(
        namespace_function_names(&tools, "thinwedge_app"),
        vec![
    ...
    for tool_name in ["automation_update", "automation_list"] {
        let dynamic_tool = find_namespace_function_tool(&tools, "thinwedge_app", tool_name);
    ...
    assert!(handlers.contains(&ToolHandlerSpec {
        name: ToolName::namespaced("thinwedge_app", "automation_update"),
        kind: ToolHandlerKind::DynamicTool,
    }));
    assert!(handlers.contains(&ToolHandlerSpec {
        name: ToolName::namespaced("thinwedge_app", "automation_list"),
        kind: ToolHandlerKind::DynamicTool,
    }));
}
...
fn model_info() -> ModelInfo {
    serde_json::from_value(json!({
        "slug": "gpt-5-thinwedge",
        "display_name": "GPT-5 ThinWedge",
        "description": null,
