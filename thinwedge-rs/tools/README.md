...
`thinwedge-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `thinwedge-core`.
...
- code-mode compatibility shims that do not depend on `thinwedge-core`
...
1. Keep extension-owned executable-tool authoring in `thinwedge-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `thinwedge-core`.
