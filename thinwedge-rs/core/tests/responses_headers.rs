...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let thinwedge_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(request.header("x-thinwedge-parent-thread-id"), None);
    assert_eq!(
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    assert!(
        ...
        "turn_id should not be empty in x-thinwedge-turn-metadata"
    );
