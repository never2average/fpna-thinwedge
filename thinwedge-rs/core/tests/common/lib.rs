...
pub mod test_thinwedge;
pub mod test_thinwedge_exec;
pub mod tracing;
pub mod zsh_fork;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    thinwedge_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    thinwedge_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(thinwedge_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = thinwedge_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("thinwedge-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\thinwedge\AppData\Local\Temp"))
}
...
#[cfg(target_os = "linux")]
pub fn find_thinwedge_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
...
pub async fn wait_for_event<F>(
    ...
    predicate: F,
) -> thinwedge_protocol::protocol::EventMsg
where
    F: FnMut(&thinwedge_protocol::protocol::EventMsg) -> bool,
{
...
where
    F: Fn(&thinwedge_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(thinwedge, |ev| matcher(ev).is_some()).await;
...
pub fn sandbox_env_var() -> &'static str {
    thinwedge_core::spawn::THINWEDGE_SANDBOX_ENV_VAR
}
...
const REMOTE_ENV_ENV_VAR: &str = "THINWEDGE_TEST_REMOTE_ENV";
...
#[macro_export]
macro_rules! thinwedge_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_thinwedge_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("thinwedge-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
