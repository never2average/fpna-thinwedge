...
use core_test_support::test_thinwedge::RecordingUserInstructionsProvider;
...
async fn submit_thread_turn(
    thread: &Arc<thinwedge_core::ThinWedgeThread>,
    prompt: &str,
) -> Result<()> {
    thread
        .submit(Op::UserInput {
...
async fn agents_override_is_preferred_over_agents_md() -> Result<()> {
    let instructions =
        agents_instructions(test_thinwedge().with_workspace_setup(|cwd, fs| async move {
...
async fn loads_user_instructions_without_a_primary_environment() -> Result<()> {
    ...
    let provider = Arc::new(RecordingUserInstructionsProvider::new(Arc::new(
        ThinWedgeHomeUserInstructionsProvider::new(AbsolutePathBuf::try_from(
...
async fn fresh_thread_composes_global_before_project_and_reports_sources() -> Result<()> {
    ...
    assert_eq!(test.thinwedge.instruction_sources().await, creation_sources);
...
async fn global_loading_warning_surfaces_during_thread_creation() -> Result<()> {
    ...
    let mut builder = test_thinwedge().with_home(home);
    let test = builder.build(&server).await?;
    let warning = wait_for_event_match(&test.thinwedge, |event| match event {
...
async fn cold_resume_replays_rendered_instructions_but_reports_current_config_sources() -> Result<()>
{
    ...
    let mut initial_builder = test_thinwedge().with_home(Arc::clone(&home));
    let initial = initial_builder.build(&server).await?;

    // Assert the pre-resume thread reports the source used to create its snapshot.
    assert_eq!(
        initial.thinwedge.instruction_sources().await,
    ...
    let mut resume_builder = test_thinwedge().with_home(Arc::clone(&home));
...
async fn fork_replays_rendered_instructions_from_shared_history() -> Result<()> {
    ...
    let mut builder = test_thinwedge().with_home(Arc::clone(&home));
    let parent = builder.build(&server).await?;

    // Assert the parent reports the source used to create its snapshot.
    assert_eq!(
        parent.thinwedge.instruction_sources().await,
        vec![source.clone()],
        "parent reports the creation-time global source"
    );
    parent.submit_turn("persist instructions").await?;
    parent.thinwedge.ensure_rollout_materialized().await;
    parent.thinwedge.flush_rollout().await?;
    let rollout_path = parent.thinwedge.rollout_path().expect("rollout path");
    ...
    fork_config.thinwedge_self_exe = parent.config.thinwedge_self_exe.clone();
    let forked = parent
        .thread_manager
        .fork_thread(
