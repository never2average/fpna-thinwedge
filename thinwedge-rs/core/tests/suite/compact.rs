...
async fn assert_compaction_uses_turn_lifecycle_id(
    ...
    while turn_completed_id.is_none() {
        let event = thinwedge.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Shut down ThinWedge to flush rollout entries before inspecting the file.
    thinwedge.submit(Op::Shutdown).await.unwrap();
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    wait_for_event(&thinwedge, |ev| matches!(ev, EventMsg::Warning(_))).await;
    wait_for_event(&thinwedge, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let thinwedge = builder.build(&server).await.unwrap().thinwedge;
    ...
    let first = wait_for_event_match(&thinwedge, |ev| match ev {
    ...
    let last = wait_for_event_match(&thinwedge, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        thinwedge_protocol::models::ResponseItem::Message {
            id: None,
        ...
        thinwedge_protocol::models::ResponseItem::Compaction {
    ...
    let mut resume_builder = test_thinwedge().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    });
    ...
    wait_for_event(&resumed.thinwedge, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let previous_model = "gpt-5.3-thinwedge";
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.thinwedge).await;
...
async fn pre_sampling_compact_skips_when_either_comp_hash_is_missing() {
    ...
    let model_with_hash = "gpt-5.3-thinwedge";
...
async fn pre_sampling_compact_runs_after_resume_and_switch_to_smaller_model() {
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test thinwedge");
    let home = initial.home.clone();
    let rollout_path = initial
        .session_configured
        .rollout_path
    ...
    wait_for_event(&initial.thinwedge, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.thinwedge).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let task_error_message = wait_for_event_match(&thinwedge, |event| match event {
...
async fn manual_compact_twice_preserves_latest_user_messages() {
    ...
    assert_eq!(
        ...
        requests[1].header("x-thinwedge-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        requests[2].header("x-thinwedge-window-id").as_deref()
    );
    assert_ne!(
...
async fn manual_compaction_keeps_the_creation_time_global_instructions() -> Result<()> {
    ...
    assert_eq!(
        test.thinwedge.instruction_sources().await,
        vec![source.clone()],
...
-> Result<()> {
    ...
    test.thinwedge.flush_rollout().await?;
    ...
    let rollout_path = test.thinwedge.rollout_path().expect("rollout path");
    ...
    assert_eq!(
        resumed.thinwedge.instruction_sources().await,
        vec![source],
