...
use core_test_support::test_thinwedge::test_thinwedge as base_test_thinwedge;
...
const TURN_STATE_HEADER: &str = "x-thinwedge-turn-state";
...
fn test_thinwedge() -> TestThinWedgeBuilder {
    base_test_thinwedge().with_config(|config| {
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    })
}

fn remote_realtime_test_thinwedge_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestThinWedgeBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_thinwedge()
        .with_auth(ThinWedgeAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(thinwedge: &thinwedge_core::ThinWedgeThread) -> Result<()> {
    thinwedge
    ...
    wait_for_event_match(thinwedge, |msg| match msg {
...
async fn close_realtime_conversation(thinwedge: &thinwedge_core::ThinWedgeThread) -> Result<()> {
    thinwedge.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(thinwedge, |msg| match msg {
...
async fn wait_for_turn_complete(thinwedge: &thinwedge_core::ThinWedgeThread) {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    assert_eq!(
        compact_request
            .header("x-thinwedge-installation-id")
            .as_deref(),
    ...
    assert_eq!(
        ...
        compact_request.header("x-thinwedge-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        follow_up_request.header("x-thinwedge-window-id").as_deref()
    );
    assert_ne!(
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let mut builder = test_thinwedge().with_auth(auth);
    ...
    let harness = TestThinWedgeHarness::with_builder(builder).await?;
    let thinwedge = harness.test().thinwedge.clone();
    let image_url =
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    assert!(
        compact_request
            .header("x-thinwedge-beta-features")
            .as_deref()
    ...
    assert_eq!(
        compact_request.body_json()["client_metadata"]["x-thinwedge-window-id"].as_str(),
        compact_metadata["window_id"].as_str()
    );
    assert_eq!(
        compact_metadata["compaction"],
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let new_thread = test
        ...
        .await?;
    test.thinwedge = new_thread.thread;
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "thinwedge_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "thinwedge_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&thinwedge, |event| match event {
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&thinwedge, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestThinWedgeHarness::with_builder(
        test_thinwedge()
            .with_auth(ThinWedgeAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_thinwedge = baseline_harness.test().thinwedge.clone();
    ...
    baseline_thinwedge
        ...
        .await?;
    wait_for_event(&baseline_thinwedge, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    baseline_thinwedge.submit(Op::Compact).await?;
    wait_for_event(&baseline_thinwedge, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestThinWedgeHarness::with_builder(
        test_thinwedge()
            .with_auth(ThinWedgeAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_thinwedge = override_harness.test().thinwedge.clone();
    ...
    override_thinwedge
        ...
        .await?;
    wait_for_event(&override_thinwedge, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_thinwedge.submit(Op::Compact).await?;
    wait_for_event(&override_thinwedge, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    {
        let event = thinwedge.next_event().await.unwrap();
        match event.msg {
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    start_realtime_conversation(test.thinwedge.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_thinwedge_builder(&realtime_server).with_config({
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_thinwedge_builder(&realtime_server);
    let test = builder.build(&server).await?;
...
async fn snapshot_request_shape_remote_compact_resume_restates_realtime_end() -> Result<()> {
    ...
    start_realtime_conversation(initial.thinwedge.as_ref()).await?;
...
-> Result<()> {
    ...
    let next_model = "gpt-5.3-thinwedge";
    let harness = TestThinWedgeHarness::with_builder(
        test_thinwedge()
            .with_auth(ThinWedgeAuth::create_dummy_chatgpt_auth_for_testing())
...
async fn remote_mid_turn_compact_v2_sends_turn_state_over_websocket() -> Result<()> {
    ...
    test.thinwedge
        ...
        .await?;
    wait_for_turn_complete(&test.thinwedge).await;
