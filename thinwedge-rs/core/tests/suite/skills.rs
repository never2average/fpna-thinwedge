...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    let mut builder = test_thinwedge().with_workspace_setup(move |cwd, fs| async move {
...
async fn list_skills_includes_repo_and_home_skills_remote_aware() -> Result<()> {
    ...
    test.thinwedge
        ...
        .await?;
    let response =
        core_test_support::wait_for_event_match(test.thinwedge.as_ref(), |event| match event {
            thinwedge_protocol::protocol::EventMsg::ListSkillsResponse(response) => {
                Some(response.clone())
            }
            _ => None,
        })
...
async fn list_skills_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    ...
    let thread_manager = ThreadManager::new(
        &config,
        thinwedge_core::test_support::auth_manager_from_auth(ThinWedgeAuth::from_api_key("dummy")),
        SessionSource::Exec,
...
async fn list_skills_includes_system_cache_entries() -> Result<()> {
    ...
    let system_skill_path = system_skill_md_path(test.thinwedge_home_path(), SYSTEM_SKILL_NAME);
    assert!(
        system_skill_path.exists(),
