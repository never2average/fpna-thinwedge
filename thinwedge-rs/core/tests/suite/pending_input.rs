...
async fn build_thinwedge(server: &StreamingSseServer) -> Arc<ThinWedgeThread> {
    test_thinwedge()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .unwrap_or_else(|err| panic!("build streaming ThinWedge test session: {err}"))
        .thinwedge
}
...
async fn submit_danger_full_access_user_turn(test: &TestThinWedge, text: &str) {
...
async fn steer_user_input(thinwedge: &ThinWedgeThread, text: &str) {
    thinwedge
        .steer_input(
            vec![UserInput::Text {
                text: text.to_string(),
...
async fn submit_queue_only_agent_mail(thinwedge: &ThinWedgeThread, text: &str) {
    thinwedge
...
async fn wait_for_reasoning_item_started(thinwedge: &ThinWedgeThread) {
    wait_for_event(thinwedge, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(thinwedge: &ThinWedgeThread, text: &str) {
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let thinwedge = build_thinwedge(&server).await;

    submit_user_input(&thinwedge, "first prompt").await;

    wait_for_reasoning_item_started(&thinwedge).await;

    submit_queue_only_agent_mail(&thinwedge, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&thinwedge, "first answer").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    submit_user_input(&thinwedge, "first prompt").await;
    submit_user_input(&thinwedge, "second prompt").await;

    wait_for_agent_message(&thinwedge, "resumed old task").await;
    wait_for_turn_complete(&thinwedge).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&thinwedge, "processed steered prompt").await;
    wait_for_turn_complete(&thinwedge).await;
