...
const REQUESTED_MODEL: &str = "gpt-5.3-thinwedge";
...
async fn openai_model_header_mismatch_emits_warning_event() -> Result<()> {
    ...
    let reroute = wait_for_event(&test.thinwedge, |event| {
...
async fn cyber_policy_response_emits_typed_error_without_retry() -> Result<()> {
    ...
    let error = wait_for_event(&test.thinwedge, |event| matches!(event, EventMsg::Error(_))).await;
...
async fn openai_model_header_mismatch_only_emits_one_warning_per_turn() -> Result<()> {
    ...
    loop {
        let event = wait_for_event(&test.thinwedge, |_| true).await;
        match event {
