...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.thinwedge);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.thinwedge, "after resume").await?;
    shutdown_thread(&resumed.thinwedge).await?;
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        ...
        .expect("missing x-thinwedge-window-id header");
