...
use thinwedge_protocol::protocol::ElicitationAction;
...
fn set_calendar_approval_mode(config: &mut Config, approval_mode: AppToolApproval) {
    ...
    let user_config_path = config.thinwedge_home.join("config.toml").abs();
        ...
        .submit(Op::UserInput {
            ...
            thread_settings: thinwedge_protocol::protocol::ThreadSettingsOverrides {
                ...
                collaboration_mode: collaboration_mode.or({
                    Some(thinwedge_protocol::config_types::CollaborationMode {
...
async fn approved_mcp_tool_call_metadata_records_prior_user_input_request() -> Result<()> {
    ...
    let EventMsg::ElicitationRequest(request) = wait_for_event(&test.thinwedge, |event| {
        matches!(
            event,
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-thinwedge-turn-metadata/user_input_requested_during_turn"),
        Some(&json!(true))
    );
...
async fn apps_default_auto_review_routes_actual_mcp_approval_to_guardian() -> Result<()> {
    ...
    let route_event = wait_for_event(&test.thinwedge, |event| {
        matches!(
            event,
            EventMsg::ElicitationRequest(_) | EventMsg::TurnComplete(_)
        )
