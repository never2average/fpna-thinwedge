...
const REALTIME_BACKEND_PROMPT: &str = thinwedge_prompts::BACKEND_PROMPT;
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "THINWEDGE_REALTIME_CONVERSATION_TEST_SUBPROCESS";
...
async fn seed_recent_thread(
    ...
    let db = test.thinwedge.state_db().context("state db enabled")?;
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.thinwedge, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.thinwedge, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.thinwedge, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
    ...
    test.thinwedge.submit(Op::RealtimeConversationClose).await?;
    let closed = wait_for_event_match(&test.thinwedge, |msg| match msg {
...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    let created = wait_for_event_match(&test.thinwedge, |msg| match msg {
...
async fn conversation_webrtc_close_while_sideband_connecting_drops_pending_join() -> Result<()> {
    ...
    let sdp = wait_for_event_match(&test.thinwedge, |msg| match msg {
    ...
    let stale_event = timeout(Duration::from_millis(700), async {
        wait_for_event_match(&test.thinwedge, |msg| match msg {
            EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
...
async fn conversation_webrtc_sideband_connect_failure_closes_with_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.thinwedge, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_start_uses_openai_env_key_fallback_with_chatgpt_auth() -> Result<()> {
    ...
    test.thinwedge.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(&test.thinwedge, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.thinwedge
        ...
        .await?;
    let _ = wait_for_event_match(&test.thinwedge, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_uses_experimental_realtime_ws_base_url_override() -> Result<()> {
    ...
    let mut builder = test_thinwedge().with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
...
async fn conversation_rejects_voice_for_wrong_realtime_version() -> Result<()> {
    ...
    let error = wait_for_event_match(&test.thinwedge, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let history = user_turns
        ...
        .collect::<Vec<_>>();
    test.thinwedge.shutdown_and_wait().await?;
    let resumed_thread = test
        .thread_manager
        .resume_thread_with_history(
            test.config.clone(),
            InitialHistory::Forked(history),
            auth_manager_from_auth(ThinWedgeAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
        )
        .await?;
    let thinwedge = resumed_thread.thread;
    ...
    thinwedge.shutdown_and_wait().await?;
    realtime_server.shutdown().await;
    Ok(())
}
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("thinwedge-rs/core"))?;
    ...
    assert!(startup_context.contains("thinwedge-rs/"));
...
async fn conversation_user_text_turn_is_not_sent_to_realtime() -> Result<()> {
    ...
    let turn_complete = wait_for_event_match(&test.thinwedge, |event| match event {
...
async fn conversation_mirrors_assistant_message_text_to_realtime_handoff() -> Result<()> {
    ...
    wait_for_event(&test.thinwedge, |event| {
...
async fn delegated_turn_user_role_echo_does_not_redelegate_and_still_forwards_audio() -> Result<()>
{
    ...
    let mut builder = test_thinwedge().with_model("gpt-5.4").with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
