...
use thinwedge_web_search_extension::install as install_web_search_extension;
use wiremock::Mock;
...
"#,
    ...
    let auth = ThinWedgeAuth::from_api_key("dummy");
    let auth_manager = thinwedge_core::test_support::auth_manager_from_auth(auth.clone());
    ...
    assert_eq!(
        search_body["model"],
        serde_json::json!("test-gpt-5.1-thinwedge")
    );
    assert_eq!(
...
async fn run_code_mode_turn_with_rmcp(
    ...
    run_code_mode_turn_with_rmcp_model(server, prompt, code, "test-gpt-5.1-thinwedge").await
}
...
async fn run_code_mode_turn_with_rmcp_config(
    ...
    let test = builder.build(server).await?;
    wait_for_mcp_server(&test.thinwedge, "rmcp").await?;
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__thinwedge_apps__calendar_timezone_option_99"
);
if (!tool) {
...
"#,
        visible_tool_name = "mcp__thinwedge_apps__calendar_timezone_option_99",
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "thinwedge_app_hidden_dynamic_tool");
const out = await tools.thinwedge_app_hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
...
"#;
    ...
    test.thinwedge
        .submit(Op::UserInput {
            ...
            thread_settings: thinwedge_protocol::protocol::ThreadSettingsOverrides {
                environments: Some(
                    thinwedge_protocol::protocol::TurnEnvironmentSelections::new(cwd, Vec::new()),
                ),
    ...
    let turn_id = wait_for_event_match(&test.thinwedge, |event| match event {
        EventMsg::TurnStarted(event) => Some(event.turn_id.clone()),
        _ => None,
    })
    .await;
    let request = wait_for_event_match(&test.thinwedge, |event| match event {
    ...
    test.thinwedge
        ...
        .await?;
    wait_for_event(&test.thinwedge, |event| match event {
        ...
        parsed
            ...
            .is_some_and(|description| {
                description.contains("A hidden dynamic tool.")
                    ...
                    && description.contains("thinwedge_app_hidden_dynamic_tool(args:")
            })
    );
