...
async fn emits_warning_when_unstable_features_enabled_via_config() {
    ...
    let user_config_path =
        AbsolutePathBuf::from_absolute_path(config.thinwedge_home.join(CONFIG_TOML_FILE))
    ...
    let thread_manager = thinwedge_core::test_support::thread_manager_with_models_provider(
        ThinWedgeAuth::from_api_key("test"),
        config.model_provider.clone(),
    );
    let auth_manager =
        thinwedge_core::test_support::auth_manager_from_auth(ThinWedgeAuth::from_api_key("test"));
