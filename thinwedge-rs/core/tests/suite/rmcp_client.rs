...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/thinwedge-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn wait_for_mcp_tool(fixture: &TestThinWedge, tool_name: &str) -> anyhow::Result<()> {
    let tools_ready_deadline = Instant::now() + Duration::from_secs(30);
    loop {
        fixture.thinwedge.submit(Op::ListMcpTools).await?;
...
async fn call_cwd_tool(
    ...
    .await;
    let end_event = wait_for_event(&fixture.thinwedge, |ev| {
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.thinwedge, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.thinwedge, |ev| {
            matches!(
                ev,
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: thinwedge_protocol::thinwedge_models::ReasoningEffort::Medium,
...
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "THINWEDGE_TEST_REMOTE_EXEC_SERVER_URL";
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that ThinWedge should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: ThinWedge can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so ThinWedge will call the MCP echo tool
    ...
    // Phase 3: configure ThinWedge with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert ThinWedge begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.thinwedge, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
/// This test writes to a fallback credentials file in THINWEDGE_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire THINWEDGE_HOME through the code.
#[test]
#[serial(thinwedge_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so ThinWedge will call the OAuth-backed
    ...
    // Phase 3: seed an isolated THINWEDGE_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _thinwedge_home_guard = EnvVarGuard::set("THINWEDGE_HOME", temp_home.path().as_os_str());
    ...
    // Phase 4: configure ThinWedge with the OAuth-backed Streamable HTTP MCP
    ...
    // Phase 7: assert ThinWedge begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.thinwedge, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the ThinWedge fixture.
