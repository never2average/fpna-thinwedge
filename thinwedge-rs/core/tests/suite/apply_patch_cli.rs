...
use core_test_support::test_thinwedge::ApplyPatchModelOutput;
...
pub async fn apply_patch_harness() -> Result<TestThinWedgeHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestThinWedgeBuilder) -> TestThinWedgeBuilder,
) -> Result<TestThinWedgeHarness> {
    let builder = configure(test_thinwedge());
    // Box harness construction so apply_patch_cli tests do not inline the
    // full test-thread startup path into each test future.
    Box::pin(TestThinWedgeHarness::with_remote_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestThinWedgeHarness, prompt: &str) -> Result<()> {
...
async fn apply_patch_cli_uses_thinwedge_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let thinwedge_linux_sandbox_exe = harness
        .test()
        .config
        .thinwedge_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include thinwedge-linux-sandbox helper");
    assert_eq!(
        thinwedge_linux_sandbox_exe
            .file_name()
            .and_then(|name| name.to_str()),
        Some(THINWEDGE_LINUX_SANDBOX_ARG0),
    );
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&thinwedge_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
...
async fn apply_patch_turn_diff_tracks_local_and_remote_environment_paths() -> Result<()> {
    ...
    let shared_cwd = PathBuf::from(format!(
        "/tmp/thinwedge-remote-turn-diff-{}",
