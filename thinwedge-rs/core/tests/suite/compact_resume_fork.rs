...
//! request payload that ThinWedge would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(THINWEDGE_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn start_test_conversation(
    ...
    (test.home, test.config, test.thread_manager, test.thinwedge)
}

async fn user_turn(conversation: &Arc<ThinWedgeThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<ThinWedgeThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<ThinWedgeThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<ThinWedgeThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager = thinwedge_core::test_support::auth_manager_from_auth(
        thinwedge_login::ThinWedgeAuth::from_api_key("dummy"),
    );
