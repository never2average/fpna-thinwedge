...
use thinwedge_protocol::models::LocalShellAction;
use thinwedge_protocol::models::LocalShellExecAction;
...
fn assert_thinwedge_client_metadata(
    ...
    assert_eq!(
        client_metadata["x-thinwedge-installation-id"].as_str(),
        Some(installation_id)
    );
    assert_eq!(client_metadata["session_id"].as_str(), Some(session_id));
    assert_eq!(client_metadata["thread_id"].as_str(), Some(thread_id));
    let Some(turn_metadata_str) = client_metadata["x-thinwedge-turn-metadata"].as_str() else {
        panic!("missing x-thinwedge-turn-metadata client metadata");
    };
    let Ok(turn_metadata) = serde_json::from_str::<serde_json::Value>(turn_metadata_str) else {
        panic!("invalid x-thinwedge-turn-metadata json");
    };
    assert_eq!(
    ...
    assert_eq!(
        client_metadata["x-thinwedge-window-id"].as_str(),
        turn_metadata["window_id"].as_str()
    );
}

/// Writes an `auth.json` into the provided `thinwedge_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        thinwedge_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = thinwedge_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![thinwedge_protocol::models::ContentItem::InputText {
    ...
    let prior_system = thinwedge_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![thinwedge_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = thinwedge_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure ThinWedge to resume from our file
    let thinwedge_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_thinwedge()
        .with_home(thinwedge_home.clone())
        .with_pre_build_hook(|home| {
            std::fs::write(home.join("AGENTS.md"), "be nice").expect("write global instructions");
        });
    let test = builder
        .resume(&server, thinwedge_home, session_path.clone())
        .await
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let thinwedge_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_thinwedge().with_model("gpt-5.4");
    let test = builder
        .resume(&server, thinwedge_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_thinwedge().with_auth(ThinWedgeAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.thinwedge_home_path().join(INSTALLATION_ID_FILENAME))
    ...
    assert_thinwedge_client_metadata(
        &request_body,
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_thinwedge()
        .with_auth(ThinWedgeAuth::from_api_key("Test API Key"))
        .with_config(|config| {
            config.base_instructions = Some("test instructions".to_string());
        });
    let thinwedge = builder
        .build(&server)
        .await
        .expect("create new conversation")
        .thinwedge;
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut builder = test_thinwedge()
        .with_auth(create_dummy_thinwedge_auth())
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/thinwedge/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager = match ThinWedgeAuth::from_auth_storage(
        thinwedge_home.path(),
    ...
    .await
    {
        Ok(Some(auth)) => thinwedge_core::test_support::auth_manager_from_auth(auth),
        Ok(None) => panic!("No ThinWedgeAuth found in thinwedge_home"),
        Err(e) => panic!("Failed to load ThinWedgeAuth: {e}"),
    };
    let installation_id = resolve_installation_id(&config.thinwedge_home)
        .await
    ...
    let thread_manager = ThreadManager::new(
        ...
        Arc::new(thinwedge_core::test_support::EmptyUserInstructionsProvider),
...
async fn skills_append_to_developer_message() {
    ...
    let thinwedge_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = thinwedge_home.path().join("skills/demo");
    ...
    let mut builder = test_thinwedge()
        ...
        .with_auth(ThinWedgeAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = thinwedge_home_path.abs();
        });
    let thinwedge = builder
        .build(&server)
        .await
    ...
    let _thinwedge_home_guard = thinwedge_home;
}
...
async fn skills_use_aliases_in_developer_message_under_budget_pressure() {
    ...
    let thinwedge_home_parent = TempDir::new().unwrap();
    let long_home_parent = thinwedge_home_parent
        .path()
        .join("thinwedge-home-with-long-shared-prefix-for-skill-alias-budget-test");
    std::fs::create_dir_all(&long_home_parent).expect("create long home parent");
    let thinwedge_home = Arc::new(TempDir::new_in(long_home_parent).unwrap());
    let skill_root = thinwedge_home.path().join("skills");
    for index in 0..12 {
    ...
    let mut builder = test_thinwedge()
        ...
        .with_config(move |config| {
            config.cwd = thinwedge_home_path.abs();
            let user_config_path = thinwedge_home_path.join("config.toml").abs();
    ...
    let _thinwedge_home_guard = thinwedge_home;
    let _thinwedge_home_parent_guard = thinwedge_home_parent;
}
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-thinwedge-primary-used-percent", "12.5")
        .insert_header("x-thinwedge-secondary-used-percent", "40.0")
        .insert_header("x-thinwedge-primary-window-minutes", "10")
        .insert_header("x-thinwedge-secondary-window-minutes", "60")
        .insert_header("x-thinwedge-primary-reset-at", "1704069000")
        .insert_header("x-thinwedge-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_thinwedge()
        .with_auth(ThinWedgeAuth::from_api_key("test"))
        .with_config(move |config| {
    ...
    wait_for_event(&thinwedge, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-thinwedge-primary-used-percent", "100.0")
        .insert_header("x-thinwedge-secondary-used-percent", "87.5")
        .insert_header("x-thinwedge-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-thinwedge-primary-window-minutes", "15")
        .insert_header("x-thinwedge-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_thinwedge();
    let thinwedge_fixture = builder.build(&server).await?;
    let thinwedge = thinwedge_fixture.thinwedge.clone();
    ...
    let error_event = wait_for_event(&thinwedge, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&thinwedge, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&thinwedge, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = ThinWedgeErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_thinwedge_auth() -> ThinWedgeAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under ThinWedge sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
