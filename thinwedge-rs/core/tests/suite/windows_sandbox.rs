...
enum TestThinWedgeHome {
    Persistent(PathBuf),
    Temporary(TempDir),
}

impl TestThinWedgeHome {
    fn path(&self) -> &Path {
        match self {
...
fn thinwedge_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestThinWedgeHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        // The elevated backend provisions machine-local sandbox users. Bazel
        // retries run in the same Windows VM, so keep THINWEDGE_HOME stable within
        // the test temp root and let setup reconcile its persisted ACL state.
        let thinwedge_home = PathBuf::from(test_tmpdir).join(name);
        std::fs::create_dir_all(&thinwedge_home).with_context(|| {
            format!(
                "create stable test THINWEDGE_HOME {}",
                thinwedge_home.display()
            )
        })?;
        return Ok(TestThinWedgeHome::Persistent(thinwedge_home));
    }

    Ok(TestThinWedgeHome::Temporary(TempDir::new()?))
}
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("thinwedge-resources");
    ...
    for helper_name in [
        ...
        let helper = thinwedge_utils_cargo_bin::cargo_bin(helper_name)?;
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    let thinwedge_home = thinwedge_home_for_windows_sandbox_test(
        "windows-restricted-token-deny-read-thinwedge-home",
    )?;
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    let thinwedge_home =
        thinwedge_home_for_windows_sandbox_test("windows-elevated-deny-read-thinwedge-home")?;
    let _thinwedge_home_guard =
        EnvVarGuard::set("THINWEDGE_HOME", thinwedge_home.path().as_os_str());
    ...
    let public = cwd.join("public.txt");
    let setup_marker = thinwedge_home
        .path()
        .join(".sandbox")
    ...
    assert!(
        sandbox_setup_is_complete(thinwedge_home.path()),
