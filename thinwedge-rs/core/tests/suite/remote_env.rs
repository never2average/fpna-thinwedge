...
async fn unified_exec_test(server: &wiremock::MockServer) -> Result<TestThinWedge> {
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections = thinwedge_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn expect_patch_approval(
    ...
    let event = wait_for_event(&test.thinwedge, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_patch_approval(test: &TestThinWedge) {
    let event = wait_for_event(&test.thinwedge, |event| {
        matches!(
            event,
            EventMsg::ApplyPatchApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn remote_test_env_exposes_bash_shell_to_model() -> Result<()> {
    ...
    .await;
    let test = test_thinwedge().build_with_remote_env(&server).await?;
...
async fn remote_request_permissions_grant_unblocks_later_remote_exec() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/thinwedge-remote-request-permissions-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/thinwedge-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_path(&remote_cwd)?;
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/thinwedge-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_path(&remote_cwd)?;
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/thinwedge-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/thinwedge-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_path(&remote_cwd)?;
...
async fn remote_test_env_remove_removes_symlink_not_target() -> Result<()> {
    ...
    let root = PathBuf::from(format!(
        "/tmp/thinwedge-remote-remove-link-{}",
        std::process::id()
    ));
    let allowed_dir = root.join("allowed");
...
async fn remote_test_env_copy_preserves_symlink_source() -> Result<()> {
    ...
    let root = PathBuf::from(format!(
        "/tmp/thinwedge-remote-copy-link-{}",
        std::process::id()
    ));
    let allowed_dir = root.join("allowed");
...
fn remote_test_file_path() -> PathBuf {
    ...
    PathBuf::from(format!(
        "/tmp/thinwedge-remote-test-env-{}-{nanos}.txt",
        std::process::id()
    ))
}
