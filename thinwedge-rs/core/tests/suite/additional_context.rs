...
use thinwedge_protocol::protocol::AdditionalContextKind;
...
async fn additional_context_is_model_visible_but_not_a_user_message_item() -> Result<()> {
    ...
    let user_item = wait_for_event_match(&test.thinwedge, |event| match event {
    ...
    wait_for_event_match(&test.thinwedge, |event| {
