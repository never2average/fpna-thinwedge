...
fn extract_log_field_handles_empty_bare_values() {
    let line = "event.name=\"thinwedge.tool_result\" mcp_server= mcp_server_origin=";
...
fn extract_log_field_does_not_confuse_similar_keys() {
    let line = "event.name=\"thinwedge.tool_result\" mcp_server_origin=stdio";
...
async fn responses_api_emits_api_request_event() {
    ...
    let TestThinWedge { thinwedge, .. } = test_thinwedge().build(&server).await.unwrap();
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("thinwedge.api_request"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected thinwedge.api_request event".to_string()))
    });

    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("thinwedge.conversation_starts"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected thinwedge.conversation_starts event".to_string()))
    });
}
...
async fn process_sse_emits_tracing_for_output_item() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("thinwedge.sse_event")
...
async fn process_sse_emits_failed_event_on_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            ...
            .map(|_| Ok(()))
            .unwrap_or(Err("missing thinwedge.sse_event".to_string()))
    });
}
...
async fn process_sse_failed_event_logs_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("thinwedge.sse_event") && line.contains("event.kind=response.failed")
            })
            .map(|_| Ok(()))
            .unwrap_or(Err("missing thinwedge.sse_event".to_string()))
    });
    ...
    assert!(
        logs.lines().any(|line| {
            line.contains("handle_responses{")
                ...
                && line.contains("thinwedge.request.reasoning_effort=high")
                ...
                && line.contains("thinwedge.usage.reasoning_output_tokens=2")
                && line.contains("thinwedge.usage.total_tokens=9")
        }),
    ...
    assert!(
        logs.lines().any(|line| {
            line.contains("turn{otel.name=\"session_task.turn\"")
                && line.contains("thinwedge.turn.reasoning_effort=high")
                && line.contains("thinwedge.turn.token_usage.input_tokens=3")
                && line.contains("thinwedge.turn.token_usage.cached_input_tokens=1")
                && line.contains("thinwedge.turn.token_usage.non_cached_input_tokens=2")
                && line.contains("thinwedge.turn.token_usage.output_tokens=5")
                && line.contains("thinwedge.turn.token_usage.reasoning_output_tokens=2")
                && line.contains("thinwedge.turn.token_usage.total_tokens=9")
        }),
...
async fn handle_response_item_records_tool_result_for_custom_tool_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("thinwedge.tool_result") && line.contains("call_id=custom-tool-call")
            })
...
async fn handle_response_item_records_tool_result_for_function_call() {
    ...
    wait_for_event(&thinwedge, |ev| matches!(ev, EventMsg::TokenCount(_))).await;

    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("thinwedge.tool_result") && line.contains("call_id=function-call")
            })
            .ok_or_else(|| "missing thinwedge.tool_result event".to_string())?;
...
fn sandbox_outcome_assertion<'a>(
    ...
    move |lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("thinwedge.sandbox_outcome")
                    && line.contains(&format!("call_id={call_id}"))
            })
            .ok_or_else(|| format!("missing thinwedge.sandbox_outcome event for {call_id}"))?;
...
async fn handle_shell_command_user_approved_records_tool_decision() {
    let server = start_mock_server().await;
    let command = touch_command("thinwedge-otel-approval-test");
    mount_sse_once(
        &server,
        sse(vec![
