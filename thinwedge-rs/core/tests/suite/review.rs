...
use thinwedge_protocol::protocol::ReviewCodeLocation;
use thinwedge_protocol::protocol::ReviewFinding;
use thinwedge_protocol::protocol::ReviewLineRange;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under ThinWedge sandbox network restrictions.
    skip_if_no_network!();
    ...
    let thinwedge = new_conversation_for_server(&server, thinwedge_home.clone(), |_| {}).await;
    ...
    let path = thinwedge.rollout_path().expect("rollout path");
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let thinwedge = new_conversation_for_server(&server, thinwedge_home.clone(), |cfg| {
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = thinwedge_home.path().join("resume.jsonl");
    {
        ...
        let user = thinwedge_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = thinwedge_protocol::models::ResponseItem::Message {
            id: None,
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let thinwedge = new_conversation_for_server(&server, thinwedge_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, thinwedge_home, resume_path)
        .await
