...
async fn stop_hook_can_block_multiple_times_in_same_turn() -> Result<()> {
    ...
    let hook_inputs = read_stop_hook_inputs(test.thinwedge_home_path())?;
...
async fn session_start_hook_sees_materialized_transcript_path() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(test.thinwedge_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
    assert_eq!(
        hook_inputs[0]
            .get("transcript_path")
...
async fn session_start_runs_before_user_prompt_submit_on_first_turn() -> Result<()> {
    ...
    let hook_inputs = read_hook_order_inputs(test.thinwedge_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
...
async fn resumed_thread_runs_resume_then_compact_session_start_hooks() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(resumed.thinwedge_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
...
async fn resumed_thread_keeps_stop_continuation_prompt_in_history() -> Result<()> {
    ...
    let mut resume_builder = test_thinwedge().with_config(trust_discovered_hooks);
    let resumed = resume_builder.resume(&server, home, rollout_path).await?;
...
async fn blocked_user_prompt_submit_persists_additional_context_for_next_turn() -> Result<()> {
    ...
    let hook_inputs = read_user_prompt_submit_hook_inputs(test.thinwedge_home_path())?;
...
"#,
    ...
    let command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://thinwedge-network-test.invalid', timeout=2).read().decode(errors='replace'))""#;
    let args = serde_json::json!({ "command": command });
    ...
    assert!(
        timeout(
            ...
            wait_for_event(&test.thinwedge, |event| matches!(
                event,
    ...
    assert_single_permission_request_hook_input(
        test.thinwedge_home_path(),
        command,
        Some("network-access http://thinwedge-network-test.invalid:80"),
    )?;
...
async fn pre_tool_use_blocks_shell_command_before_execution() -> Result<()> {
    ...
    let hook_inputs = read_pre_tool_use_hook_inputs(test.thinwedge_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
"#
    ...
    let updated_input = serde_json::json!({ "command": rewritten_command });
    let mut builder = test_thinwedge()
        .with_model("test-gpt-5.1-thinwedge")
        .with_pre_build_hook(move |home| {
            if let Err(error) = write_updating_pre_tool_use_hook(home, "^Bash$", &updated_input) {
                panic!("failed to write updating pre tool use hook fixture: {error}");
            }
...
async fn plugin_pre_tool_use_blocks_shell_command_before_execution() -> Result<()> {
    ...
    fs::create_dir_all(plugin_root.join(".thinwedge-plugin"))
...
"#,
  ...
  "hooks": {
    ...
    let plugin_hook_sources = vec![PluginHookSource {
        ...
        hooks: serde_json::from_str::<thinwedge_config::HooksFile>(plugin_hooks_json)
...
async fn pre_tool_use_merges_hooks_json_and_config_toml() -> Result<()> {
    ...
    let json_hook_inputs = read_pre_tool_use_hook_inputs(test.thinwedge_home_path())?
        .into_iter()
        .map(|hook_input| {
            serde_json::json!({
...
async fn post_tool_use_records_additional_context_for_shell_command() -> Result<()> {
    ...
    let hook_inputs = read_post_tool_use_hook_inputs(test.thinwedge_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
async fn post_tool_use_blocks_when_exec_session_completes_via_write_stdin() -> Result<()> {
    ...
    let pre_hook_inputs = read_pre_tool_use_hook_inputs(test.thinwedge_home_path())?;
    ...
    let post_hook_inputs = read_post_tool_use_hook_inputs(test.thinwedge_home_path())?;
