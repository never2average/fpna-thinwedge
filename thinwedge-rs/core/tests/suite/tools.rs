...
async fn historical_unavailable_mcp_call_is_exposed_as_placeholder_tool() -> Result<()> {
    ...
    let mut builder = test_thinwedge()
        .with_model("gpt-5.4")
        .with_home(Arc::clone(&thinwedge_home))
    ...
    let rollout_path = test.thinwedge.rollout_path().context("rollout path")?;
...
async fn shell_escalated_permissions_rejected_then_ok() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_thinwedge().with_model("test-shell-json");
    let test = builder.build(&server).await?;
...
async fn shell_spawn_failure_truncates_exec_error() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_thinwedge().with_config(|cfg| {
        cfg.permissions
            .set_permission_profile(PermissionProfile::Disabled)
            .expect("set permission profile");
    });
