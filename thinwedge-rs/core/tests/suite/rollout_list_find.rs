...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(thinwedge_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(thinwedge_home, "sessions", id)
}

async fn upsert_thread_metadata(thinwedge_home: &Path, thread_id: ThreadId, rollout_path: PathBuf) {
    let runtime = StateRuntime::init(thinwedge_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
    ...
    builder.cwd = thinwedge_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_thinwedge_home_directory() {
    let repo = TempDir::new().unwrap();
    let thinwedge_home = repo.path().join(".thinwedge");
    std::fs::create_dir_all(&thinwedge_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".thinwedge/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&thinwedge_home, id);

    let found = find_thread_path_by_id_str(&thinwedge_home, &id.to_string())
        .await
        .unwrap();
