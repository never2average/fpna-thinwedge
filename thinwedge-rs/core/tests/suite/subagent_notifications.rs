...
const INHERITED_MODEL: &str = "gpt-5.3-thinwedge";
...
fn write_home_skill(thinwedge_home: &Path, dir: &str, name: &str, description: &str) -> Result<()> {
    let skill_dir = thinwedge_home.join("skills").join(dir);
...
async fn wait_for_spawned_thread_id(test: &TestThinWedge) -> Result<String> {
    let deadline = Instant::now() + Duration::from_secs(2);
    loop {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    configure_test: impl FnOnce(
        core_test_support::test_thinwedge::TestThinWedgeBuilder,
    ) -> core_test_support::test_thinwedge::TestThinWedgeBuilder,
) -> Result<(
    TestThinWedge,
    String,
    ...
    let mut builder = configure_test(test_thinwedge().with_config(|config| {
        config
            .features
...
async fn subagent_stop_replaces_stop_and_skips_internal_subagents() -> Result<()> {
    ...
    let stop_inputs = read_hook_log(test.thinwedge_home_path(), "stop_hook_log.jsonl")?;
    assert!(
        stop_inputs
            .iter()
    ...
    let subagent_stop_inputs_after_internal =
        read_hook_log(test.thinwedge_home_path(), "subagent_stop_hook_log.jsonl")?;
    assert_eq!(subagent_stop_inputs_after_internal, subagent_stop_inputs);

    let stop_inputs_after_internal =
        read_hook_log(test.thinwedge_home_path(), "stop_hook_log.jsonl")?;
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.thinwedge_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
