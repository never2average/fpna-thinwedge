...
async fn remote_tool_mode_selector_overrides_feature_flags() -> Result<()> {
    ...
    assert!(
        direct_tools
            .iter()
            .all(|name| name != thinwedge_code_mode::PUBLIC_TOOL_NAME
                && name != thinwedge_code_mode::WAIT_TOOL_NAME),
    ...
    assert_eq!(
        ...
        vec![
            ...
            thinwedge_code_mode::PUBLIC_TOOL_NAME.to_string(),
            thinwedge_code_mode::WAIT_TOOL_NAME.to_string(),
...
async fn remote_multi_agent_selector_uses_model_selected_before_first_turn() -> Result<()> {
    ...
    assert_eq!(
        (
            ...
            test.thinwedge.multi_agent_version(),
        ),
        (1, None)
    );
    ...
    .await?;
    assert_eq!(test.thinwedge.multi_agent_version(), None);
