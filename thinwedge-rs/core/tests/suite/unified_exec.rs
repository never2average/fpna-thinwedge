...
async fn wait_for_raw_unified_exec_output(
    ...
    let content = wait_for_event_match(&test.thinwedge, |event| match event {
...
async fn unified_exec_intercepts_apply_patch_exec_command() -> Result<()> {
    ...
    let builder = test_thinwedge().with_config(|config| {
    ...
    wait_for_event(&thinwedge, |event| match event {
...
async fn unified_exec_emits_exec_command_begin_event() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&test.thinwedge, |msg| match msg {
...
async fn unified_exec_emits_exec_command_end_event() -> Result<()> {
    ...
    let end_event = wait_for_event_match(&test.thinwedge, |msg| match msg {
...
async fn unified_exec_emits_output_delta_for_exec_command() -> Result<()> {
    ...
    let event = wait_for_event_match(&test.thinwedge, |msg| match msg {
        EventMsg::ExecCommandEnd(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
async fn unified_exec_full_lifecycle_with_background_end_event() -> Result<()> {
    ...
    loop {
        let msg = wait_for_event(&test.thinwedge, |_| true).await;
        match msg {
...
async fn unified_exec_network_denial_emits_failed_background_end_event() -> Result<()> {
    ...
    let args = json!({
        "cmd": "python3 -c \"import os, socket, time, urllib.parse; time.sleep(0.3); proxy = urllib.parse.urlparse(os.environ['HTTP_PROXY']); sock = socket.create_connection((proxy.hostname, proxy.port), timeout=2); sock.sendall(b'GET http://thinwedge-network-denied.invalid/ HTTP/1.1\\r\\nHost: thinwedge-network-denied.invalid\\r\\n\\r\\n'); sock.recv(1024); time.sleep(5)\"",
        "yield_time_ms": 50,
    });
    let response_mock =
...
async fn unified_exec_short_lived_network_denial_emits_failed_end_event() -> Result<()> {
    ...
    let args = json!({
        "cmd": "python3 -c \"import os, socket, urllib.parse; proxy = urllib.parse.urlparse(os.environ['HTTP_PROXY']); sock = socket.create_connection((proxy.hostname, proxy.port), timeout=2); sock.sendall(b'GET http://thinwedge-short-network-denied.invalid/ HTTP/1.1\\r\\nHost: thinwedge-short-network-denied.invalid\\r\\n\\r\\n'); sock.recv(1024)\"",
        "yield_time_ms": 1000,
    });
    let response_mock =
        mount_unified_exec_network_denial_responses(&server, call_id, &args).await?;
...
) -> Result<(TestThinWedge, PermissionProfile)> {
    use std::sync::Arc;
...
async fn wait_for_unified_exec_end(
...
) -> (thinwedge_protocol::protocol::ExecCommandEndEvent, bool) {
    ...
    let end_event = loop {
        ...
        let event = match tokio::time::timeout(remaining, test.thinwedge.next_event()).await {
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    let mut task_completed = false;
    loop {
        let event_msg = wait_for_event(&test.thinwedge, |_| true).await;
        match event_msg {
...
async fn unified_exec_keeps_long_running_session_after_turn_end() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&thinwedge, |msg| match msg {
...
async fn unified_exec_interrupt_preserves_long_running_session() -> Result<()> {
    ...
    let _begin_event = wait_for_event_match(&thinwedge, |msg| match msg {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
    ...
    thinwedge.submit(Op::CleanBackgroundTerminals).await?;
    wait_for_process_exit(&pid).await?;
...
async fn unified_exec_timeout_and_followup_poll() -> Result<()> {
    ...
    loop {
        let event = test.thinwedge.next_event().await.expect("event");
