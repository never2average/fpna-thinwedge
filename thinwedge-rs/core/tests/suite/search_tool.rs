...
async fn tool_search_returns_deferred_tools_without_follow_up_tool_injection() -> Result<()> {
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-thinwedge-turn-metadata/thread_id"),
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-thinwedge-turn-metadata/model")
            .and_then(Value::as_str),
        Some("gpt-5.4")
    );
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-thinwedge-turn-metadata/reasoning_effort")
            .and_then(Value::as_str),
