...
const CALENDAR_CREATE_TOOL: &str = "mcp__thinwedge_apps__calendar_create_event";
const CALENDAR_LIST_TOOL: &str = "mcp__thinwedge_apps__calendar_list_events";
const SEARCH_CALENDAR_NAMESPACE: &str = "mcp__thinwedge_apps__calendar";
...
fn configured_builder(apps_base_url: String) -> TestThinWedgeBuilder {
    test_thinwedge()
...
async fn tool_search_returns_deferred_tools_without_follow_up_tool_injection() -> Result<()> {
    ...
    let EventMsg::McpToolCallBegin(begin) = wait_for_event(&test.thinwedge, |event| {
    ...
    let EventMsg::McpToolCallEnd(end) = wait_for_event(&test.thinwedge, |event| {
    ...
    assert_eq!(
        ...
        McpInvocation {
            server: "thinwedge_apps".to_string(),
    ...
    let apps_tool_call = server
        ...
        .find_map(|request| {
            ...
            (request.url.path() == "/api/thinwedge/apps"
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/_thinwedge_apps"),
        Some(&json!({
            "call_id": "calendar-call-1",
            "resource_uri": CALENDAR_CREATE_EVENT_RESOURCE_URI,
            "contains_mcp_source": true,
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-thinwedge-turn-metadata/session_id"),
        Some(&json!(test.session_configured.session_id.to_string()))
    );
    assert!(
        apps_tool_call
            .pointer("/params/_meta/x-thinwedge-turn-metadata/turn_id")
            .and_then(Value::as_str)
            .is_some_and(|turn_id| !turn_id.is_empty()),
        "apps tools/call should include turn metadata turn_id: {apps_tool_call:?}"
    );
    let mcp_turn_started_at_unix_ms = apps_tool_call
        .pointer("/params/_meta/x-thinwedge-turn-metadata/turn_started_at_unix_ms")
...
async fn tool_search_returns_deferred_dynamic_tool_and_routes_follow_up_call() -> Result<()> {
    ...
    let mut builder = test_thinwedge().with_config(configure_search_capable_model);
    ...
    let EventMsg::DynamicToolCallRequest(request) = wait_for_event(&test.thinwedge, |event| {
    ...
    assert_eq!(request.namespace.as_deref(), Some("thinwedge_app"));
    ...
    assert_eq!(
        ...
        vec![json!({
            "type": "namespace",
            "name": "thinwedge_app",
            "description": "Tools in the thinwedge_app namespace.",
            "tools": [{
                "type": "function",
                "name": tool_name,
