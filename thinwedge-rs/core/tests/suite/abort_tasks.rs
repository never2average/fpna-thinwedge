...
async fn interrupt_long_running_tool_emits_turn_aborted() {
    ...
    wait_for_event(&thinwedge, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;

    thinwedge.submit(Op::Interrupt).await.unwrap();

    // Expect TurnAborted soon after.
    wait_for_event(&thinwedge, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
}
...
async fn interrupt_tool_records_history_entries() {
    ...
    let fixture = test_thinwedge()
        ...
        .await
        .unwrap();
    let thinwedge = Arc::clone(&fixture.thinwedge);
