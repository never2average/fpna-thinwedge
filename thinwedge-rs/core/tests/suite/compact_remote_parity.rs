...
const FIXED_CWD: &str = "/tmp/thinwedge_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> ThinWedgeAuth {
        match self {
            AuthCase::ChatGpt => ThinWedgeAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => ThinWedgeAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_session(
    ...
    for (idx, step) in scenario.steps.iter().enumerate() {
        submit_user_input(&thinwedge, user_input_for_step(scenario.name, idx, *step)).await?;
    }
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().thinwedge_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestThinWedgeHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness_inner(
    ...
    TestThinWedgeHarness::with_builder(builder.with_config(move |config| {
        config.cwd = thinwedge_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
fn rollout_path(harness: &TestThinWedgeHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
async fn capture_from_requests(
    mode: Mode,
    thinwedge: &thinwedge_core::ThinWedgeThread,
    rollout_path: &Path,
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<THINWEDGE_HOME>");
            search_start = start_index + "<THINWEDGE_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <THINWEDGE_HOME>/skills/.system/imagegen/SKILL.md and \
         <THINWEDGE_HOME>/skills/custom/SKILL.md"
    );
}
