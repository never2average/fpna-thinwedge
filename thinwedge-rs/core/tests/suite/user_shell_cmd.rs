...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    thinwedge
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&thinwedge, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let fixture = builder
        ...
        .await
        .expect("create new conversation");
    let thinwedge = &fixture.thinwedge;
    ...
    let _begin = wait_for_event_match(thinwedge, |ev| match ev {
    ...
    .await;
    thinwedge.submit(Op::Interrupt).await.unwrap();
...
async fn user_shell_command_does_not_replace_active_turn() -> anyhow::Result<()> {
    ...
    let _ = wait_for_event_match(&fixture.thinwedge, |ev| match ev {
    ...
    for _ in 0..200 {
        let event = timeout(Duration::from_secs(20), fixture.thinwedge.next_event())
            .await
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:THINWEDGE_SANDBOX; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${THINWEDGE_SANDBOX:-not-set}\"""#.to_string();
    ...
    let begin_event = wait_for_event_match(&test.thinwedge, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.thinwedge, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.thinwedge, |ev| match ev {
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let mut builder = core_test_support::test_thinwedge::test_thinwedge().with_config(|config| {
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:THINWEDGE_SANDBOX_NETWORK_DISABLED; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command =
        r#"sh -c "printf '%s' \"${THINWEDGE_SANDBOX_NETWORK_DISABLED:-not-set}\"""#.to_string();
    ...
    let ExecCommandEndEvent {
        ...
        stderr,
        ..
    } = wait_for_event_match(&test.thinwedge, |ev| match ev {
        EventMsg::ExecCommandEnd(event) => Some(event.clone()),
        _ => None,
    })
    .await;
...
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = core_test_support::test_thinwedge::test_thinwedge();
    let test = builder
        .with_config(|config| {
