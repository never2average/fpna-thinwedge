...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestThinWedge) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured ThinWedge helper binary
...
impl Expectation {
    fn verify(&self, test: &TestThinWedge, result: &CommandResult) -> Result<()> {
        match self {
...
async fn wait_for_completion_without_approval(test: &TestThinWedge) {
    let event = wait_for_event(&test.thinwedge, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestThinWedge) {
...
async fn wait_for_spawned_thread(test: &TestThinWedge) -> Result<Arc<ThinWedgeThread>> {
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://thinwedge-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
