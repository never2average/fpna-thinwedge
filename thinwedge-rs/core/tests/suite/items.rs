...
async fn user_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let TestThinWedge { thinwedge, .. } = test_thinwedge().build(&server).await?;
    ...
    let started_item = wait_for_event_match(&thinwedge, |ev| match ev {
    ...
    let legacy_message = wait_for_event_match(&thinwedge, |ev| match ev {
...
async fn agent_message_content_delta_has_item_metadata() -> anyhow::Result<()> {
    ...
    let (started_turn_id, started_item) = wait_for_event_match(&thinwedge, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
            turn_id,
            item: TurnItem::AgentMessage(item),
            ..
    ...
    let delta_event = wait_for_event_match(&thinwedge, |ev| match ev {
...
async fn plan_mode_emits_plan_item_from_proposed_plan_block() -> anyhow::Result<()> {
    ...
    let plan_delta = wait_for_event_match(&thinwedge, |ev| match ev {
        EventMsg::PlanDelta(event) => Some(event.clone()),
        _ => None,
    })
    .await;

    let plan_completed = wait_for_event_match(&thinwedge, |ev| match ev {
        EventMsg::ItemCompleted(ItemCompletedEvent {
...
async fn plan_mode_strips_plan_from_agent_messages() -> anyhow::Result<()> {
    ...
    while plan_delta.is_none() || agent_item.is_none() || plan_item.is_none() {
        let ev = wait_for_event(&thinwedge, |_| true).await;
        match ev {
...
async fn reasoning_content_delta_has_item_metadata() -> anyhow::Result<()> {
    ...
    let reasoning_item = wait_for_event_match(&thinwedge, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
            item: TurnItem::Reasoning(item),
            ..
        }) => Some(item.clone()),
