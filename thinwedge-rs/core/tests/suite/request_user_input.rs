...
async fn request_user_input_round_trip_for_mode(
    ...
    let request = wait_for_event_match(&thinwedge, |event| match event {
    ...
    assert!(
        timeout(Duration::from_millis(200), async {
            loop {
                let event = match thinwedge.next_event().await {
                    Ok(event) => event,
    ...
    wait_for_event(&thinwedge, |event| matches!(event, EventMsg::TokenCount(_))).await;
...
async fn request_user_input_interrupt_emits_deferred_token_count() -> anyhow::Result<()> {
    ...
    let token_count = wait_for_event_match(&thinwedge, |event| match event {
