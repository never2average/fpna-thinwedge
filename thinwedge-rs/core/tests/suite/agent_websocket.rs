...
async fn websocket_test_thinwedge_shell_chain() -> Result<()> {
    ...
    let mut builder = test_thinwedge().with_windows_cmd_shell();
...
async fn websocket_first_turn_uses_startup_prewarm_and_create() -> Result<()> {
    ...
    assert_eq!(
        ...
        warmup["client_metadata"]["x-thinwedge-window-id"].as_str()
    );
    assert!(
        turn["tools"]
            .as_array()
    ...
    let turn_metadata: Value = serde_json::from_str(
        turn["client_metadata"]["x-thinwedge-turn-metadata"]
            .as_str()
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn websocket_v2_test_thinwedge_shell_chain() -> Result<()> {
    skip_if_no_network!(Ok(()));
