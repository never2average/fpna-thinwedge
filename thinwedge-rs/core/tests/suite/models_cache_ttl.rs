...
const REMOTE_MODEL: &str = "thinwedge-test-ttl";
const VERSIONED_MODEL: &str = "thinwedge-test-versioned";
const MISSING_VERSION_MODEL: &str = "thinwedge-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "thinwedge-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let cache_path = config.thinwedge_home.join(CACHE_FILE);
