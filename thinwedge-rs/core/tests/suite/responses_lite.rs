...
use thinwedge_image_generation_extension::install as install_image_generation_extension;
...
const RESPONSES_LITE_HEADER: &str = "x-openai-internal-thinwedge-responses-lite";

fn responses_extensions(auth: &ThinWedgeAuth) -> Arc<ExtensionRegistry<Config>> {
...
fn configure_image_capable_model(model_info: &mut thinwedge_protocol::openai_models::ModelInfo) {
...
async fn responses_lite_strips_data_image_detail_without_resize_all_images() -> Result<()> {
    ...
    let mut builder = test_thinwedge().with_model_info_override("gpt-5.4", |model_info| {
