...
fn repo_root() -> std::path::PathBuf {
    ...
    thinwedge_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
fn personal_access_token_exec_command(server: &MockServer, home: &TempDir) -> Command {
    let bin = thinwedge_utils_cargo_bin::cargo_bin("thinwedge").unwrap();
    let mut cmd = Command::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-c")
        .arg(format!(
            "openai_base_url=\"{}/api/thinwedge\"",
            server.uri()
        ))
        .arg("-c")
    ...
    cmd.env("THINWEDGE_HOME", home.path())
        .env(THINWEDGE_ACCESS_TOKEN_ENV_VAR, PERSONAL_ACCESS_TOKEN)
        .env("THINWEDGE_AUTHAPI_BASE_URL", server.uri())
        .env_remove(THINWEDGE_API_KEY_ENV_VAR)
...
impl Drop for ChildProcessCleanupGuard {
    fn drop(&mut self) {
        #[cfg(unix)]
        {
            let _ = thinwedge_utils_pty::process_group::kill_process_group(self.0);
        }
...
// Use this for new `thinwedge exec` subprocess tests in this file. These commands
...
async fn responses_mode_stream_cli_does_not_attempt_oauth_refresh_for_personal_access_tokens_after_401()
 {
    ...
    Mock::given(method("POST"))
        .and(path("/api/thinwedge/responses"))
        .and(header("authorization", PERSONAL_ACCESS_TOKEN_AUTHORIZATION))
        .and(header(
            "chatgpt-account-id",
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // ThinWedge to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `thinwedge exec --profile ...` preserves the active user config
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the thinwedge CLI and invoke `exec`, which is what records a session.
    let bin = thinwedge_utils_cargo_bin::cargo_bin("thinwedge").unwrap();
    let mut cmd = Command::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
        ...
        .arg(&prompt);
    cmd.env("THINWEDGE_HOME", home.path())
        .env(THINWEDGE_API_KEY_ENV_VAR, "dummy");
    ...
    let bin2 = thinwedge_utils_cargo_bin::cargo_bin("thinwedge").unwrap();
    ...
    assert!(output2.status.success(), "resume thinwedge-cli run failed");
