...
fn write_skill_metadata(home: &Path, name: &str, contents: &str) -> Result<()> {
    ...
    fs::write(metadata_dir.join("thinwedge.yaml"), contents)?;
    Ok(())
}
...
fn skill_script_command(test: &TestThinWedge, script_name: &str) -> Result<String> {
...
async fn wait_for_exec_approval_request(test: &TestThinWedge) -> Option<ExecApprovalRequestEvent> {
    wait_for_event_match(test.thinwedge.as_ref(), |event| match event {
...
async fn wait_for_turn_complete(test: &TestThinWedge) {
    wait_for_event(test.thinwedge.as_ref(), |event| {
...
async fn shell_zsh_fork_still_enforces_workspace_write_sandbox() -> Result<()> {
    ...
    let outside_path = "/tmp/thinwedge-zsh-fork-workspace-write-deny.txt";
    let workspace_write_profile = restrictive_workspace_write_profile();
