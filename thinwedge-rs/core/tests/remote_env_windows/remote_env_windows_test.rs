...
async fn windows_exec_server_records_host_shell_mismatch() -> Result<()> {
    let executable = thinwedge_utils_cargo_bin::cargo_bin("wine-windows-exec-server")?;
    let mut exec_server = WineTestCommand::new(executable)
        .env("THINWEDGE_HOME", r"C:\thinwedge-home")
        .spawn()?;
    ...
    exec_server
        .scope(async move {
            ...
            let mut end = None;
            loop {
                match wait_for_event(&test.thinwedge, |_| true).await {
