...
use thinwedge_analytics::CompactionStrategy;
...
use thinwedge_utils_string::to_ascii_json_string;

use crate::client::X_OPENAI_SUBAGENT_HEADER;
use crate::client::X_THINWEDGE_INSTALLATION_ID_HEADER;
use crate::client::X_THINWEDGE_PARENT_THREAD_ID_HEADER;
use crate::client::X_THINWEDGE_TURN_METADATA_HEADER;
use crate::client::X_THINWEDGE_WINDOW_ID_HEADER;
...
const RESERVED_METADATA_KEYS: &[&str] = &[
    ...
    X_THINWEDGE_TURN_METADATA_HEADER,
...
pub(crate) enum ThinWedgeResponsesRequestKind {
    Turn,
    Prewarm,
...
impl ThinWedgeResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            ThinWedgeResponsesRequestKind::Turn => ("turn", None),
            ThinWedgeResponsesRequestKind::Prewarm => ("prewarm", None),
            ThinWedgeResponsesRequestKind::Compaction(metadata) => ("compaction", Some(metadata)),
            ThinWedgeResponsesRequestKind::Memory => ("memory", None),
        }
    }

    fn has_turn_identity(self) -> bool {
        !matches!(self, ThinWedgeResponsesRequestKind::Memory)
    }
}
...
/// Caller-owned snapshot of ThinWedge metadata sent to ResponsesAPI.
///
/// The full ThinWedge turn metadata blob is transported canonically as
/// `client_metadata["x-thinwedge-turn-metadata"]`. Flat `client_metadata` keys and direct HTTP/ws
/// headers are generated compatibility projections of this snapshot, not separate sources of
/// truth.
#[derive(Clone, Debug)]
pub struct ThinWedgeResponsesMetadata {
    pub(crate) installation_id: String,
    pub(crate) session_id: String,
    pub(crate) thread_id: String,
    pub(crate) turn_id: Option<String>,
    pub(crate) window_id: String,
    pub(crate) request_kind: Option<ThinWedgeResponsesRequestKind>,
...
impl ThinWedgeResponsesMetadata {
    pub(crate) fn new(
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        insert_header(&mut headers, X_THINWEDGE_WINDOW_ID_HEADER, &self.window_id);
        // Direct x-thinwedge-turn-metadata is compatibility output. New per-request consumers should
        // prefer client_metadata["x-thinwedge-turn-metadata"], which is rendered from this same object.
        if self.has_turn_metadata()
            && let Some(turn_metadata_json) = self.turn_metadata_json()
        {
            insert_header(
    ...
    fn turn_metadata_payload(&self) -> ThinWedgeTurnMetadataPayload<'_> {
        ...
        let has_turn_identity =
            request_kind.is_none_or(ThinWedgeResponsesRequestKind::has_turn_identity);
        let has_request_identity =
            request_kind.is_some_and(ThinWedgeResponsesRequestKind::has_turn_identity);
        ThinWedgeTurnMetadataPayload {
            ...
            compaction,
            // responsesapi_client_metadata enriches the ThinWedge turn metadata blob, not literal
            // top-level Responses client_metadata. Reserved ThinWedge-owned keys are filtered when
...
#[derive(Serialize)]
struct ThinWedgeTurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]
