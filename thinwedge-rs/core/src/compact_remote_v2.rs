...
use thinwedge_rollout_trace::CompactionCheckpointTracePayload;
...
async fn run_remote_compact_task_inner(
    ...
    if result.is_ok() {
        ...
        if let PostCompactHookOutcome::Stopped = post_compact_outcome {
            attempt
                .track(sess.as_ref(), status, thinwedge_error, analytics_details)
                .await;
    ...
    if let Err(err) = result {
        sess.track_turn_thinwedge_error(turn_context, &err);
...
) -> ThinWedgeResult<RemoteCompactionV2Output> {
...
mod tests {
    ...
    fn response_stream(events: Vec<ThinWedgeResult<ResponseEvent>>) -> ResponseStream {
