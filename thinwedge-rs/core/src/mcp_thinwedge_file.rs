...
//!
//! Model-visible schema masking is owned by `thinwedge-mcp` alongside MCP tool
...
async fn build_uploaded_local_argument_value(
    ...
    let Some(auth) = auth else {
        return Err(
            "ChatGPT auth is required to upload local files for ThinWedge Apps tools".to_string(),
        );
    };
    ...
    let upload_auth = thinwedge_model_provider::auth_provider_from_auth(auth);
