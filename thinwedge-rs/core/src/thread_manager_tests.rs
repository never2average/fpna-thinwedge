...
async fn shutdown_all_threads_bounded_submits_shutdown_to_every_thread() {
    ...
    config.thinwedge_home = temp_dir.path().join("thinwedge-home").abs();
    config.cwd = config.thinwedge_home.abs();
    std::fs::create_dir_all(&config.thinwedge_home).expect("create thinwedge home");
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl thinwedge_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
            input: thinwedge_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl thinwedge_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        ...
        fn contribute<'a>(
            &'a self,
            context: thinwedge_extension_api::McpServerContributionContext<'a, Config>,
        ) -> thinwedge_extension_api::ExtensionFuture<
            'a,
            Vec<thinwedge_extension_api::McpServerContribution>,
        > {
            Box::pin(async move {
                ...
                let mut server = thinwedge_mcp::thinwedge_apps_mcp_server_config(
                ...
                vec![thinwedge_extension_api::McpServerContribution::Set {
    ...
    let mut extensions = thinwedge_extension_api::ExtensionRegistryBuilder::new();
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = thinwedge_extension_api::ExtensionDataInit::new();
    ...
    let selected_servers = |config: &thinwedge_mcp::McpConfig| {
        thinwedge_mcp::configured_mcp_servers(config)
            .into_iter()
...
#[tokio::test]
async fn explicit_installation_id_skips_thinwedge_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.thinwedge_home = temp_dir.path().join("thinwedge-home").abs();
    config.cwd = config.thinwedge_home.abs();
    ...
    assert_eq!(
        thread.thread.thinwedge.session.installation_id,
        installation_id
    );
