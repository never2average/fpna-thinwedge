...
use thinwedge_utils_pty::process_group::kill_child_process_group;
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: thinwedge_protocol::config_types::WindowsSandboxLevel,
...
pub fn build_exec_request(
    ...
    let mut exec_req = manager
        .transform(SandboxTransformRequest {
            ...
            thinwedge_linux_sandbox_exe: thinwedge_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
    ...
    .map_err(ThinWedgeErr::UnsupportedOperation)?;
    Ok(exec_req)
}
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        thinwedge_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = thinwedge_otel::global() {
        let _ = metrics.counter(
            "thinwedge.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_thinwedge_home;
    use thinwedge_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use thinwedge_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            thinwedge_windows_sandbox::WindowsSandboxCancellationToken::new(move || {
                token.is_cancelled()
            })
        });
    ...
    let thinwedge_home = find_thinwedge_home().map_err(|err| {
        ThinWedgeErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve thinwedge_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                thinwedge_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
                    permission_profile: &permission_profile,
                    workspace_roots: workspace_roots.as_slice(),
                    thinwedge_home: thinwedge_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(ThinWedgeErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, ThinWedgeErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(ThinWedgeErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(ThinWedgeErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(ThinWedgeErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
/// THINWEDGE_SANDBOX_NETWORK_DISABLED=1 is added to the environment of the spawned
/// process.
///
...
pub(crate) fn resolve_windows_restricted_token_filesystem_overrides(
    ...
    let additional_deny_read_paths = thinwedge_windows_sandbox::resolve_windows_deny_read_paths(
        &file_system_sandbox_policy,
        sandbox_policy_cwd,
    )?;
...
pub(crate) fn resolve_windows_elevated_filesystem_overrides(
    ...
    let split_readable_roots: Vec<PathBuf> = file_system_sandbox_policy
        ...
        .into_iter()
        .map(thinwedge_utils_absolute_path::AbsolutePathBuf::into_path_buf)
        .map(&normalize_path)
        .collect();
...
fn has_reopened_writable_descendant(
    writable_roots: &[thinwedge_protocol::protocol::WritableRoot],
) -> bool {
...
async fn consume_output(
    ...
    let stdout_reader = child.stdout.take().ok_or_else(|| {
        ThinWedgeErr::Io(io::Error::other(
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        thinwedge_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                thinwedge_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }
