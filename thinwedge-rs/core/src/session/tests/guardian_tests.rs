    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                thinwedge_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_command_additional_permissions_requests_past_policy_validation() {
    ...
    let (mut session, mut turn_context_raw) = make_session_and_context().await;
    turn_context_raw.thinwedge_linux_sandbox_exe = thinwedge_linux_sandbox_exe_or_skip!();
    turn_context_raw
        .approval_policy
        .set(AskForApproval::OnRequest)
        .expect("test setup should allow updating approval policy");
    ...
    turn_context_raw.permission_profile = thinwedge_protocol::models::PermissionProfile::Disabled;
    let mut config = (*turn_context_raw.config).clone();
    config.model_provider.base_url = Some(format!("{}/v1", server.uri()));
    let config = Arc::new(config);
    let models_manager = models_manager_with_provider(
    ...
    let handler = crate::tools::handlers::ShellCommandHandler::from(
        thinwedge_tools::ShellCommandBackendConfig::Classic,
    );
    #[allow(deprecated)]
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: thinwedge_tools::ToolName::plain("shell_command"),
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let thinwedge_home = tempdir().expect("create thinwedge home");
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_thinwedge_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(thinwedge_thread_store::LocalThreadStore::new(
        thinwedge_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));

    let ThinWedgeSpawnOk { thinwedge, .. } = ThinWedge::spawn(ThinWedgeSpawnArgs {
        config,
        ...
        plugins_manager,
        mcp_manager,
        extensions: thinwedge_extension_api::empty_extension_registry(),
