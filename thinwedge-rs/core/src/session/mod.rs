...
use thinwedge_analytics::TurnThinWedgeErrorFact;
...
use thinwedge_extension_api::ExtensionDataInit;
use thinwedge_extension_api::LoadedUserInstructions;
use thinwedge_extension_api::PromptSlot;
use thinwedge_features::FEATURES;
use thinwedge_features::Feature;
use thinwedge_features::unstable_features_warning_event;
use thinwedge_hooks::Hooks;
use thinwedge_hooks::HooksConfig;
...
use thinwedge_mcp::McpResourceClient;
...
use thinwedge_network_proxy::NetworkProxyAuditMetadata;
use thinwedge_network_proxy::normalize_host;
use thinwedge_otel::current_span_trace_id;
...
use thinwedge_protocol::protocol::AdditionalContextEntry;
use thinwedge_protocol::protocol::FileChange;
use thinwedge_protocol::protocol::HasLegacyEvent;
...
use thinwedge_protocol::protocol::RawResponseItemEvent;
...
use thinwedge_rollout_trace::ThreadTraceContext;
...
use thinwedge_thread_store::CreateThreadParams;
use thinwedge_thread_store::LiveThread;
use thinwedge_thread_store::LiveThreadInitGuard;
use thinwedge_thread_store::LocalThreadStore;
use thinwedge_thread_store::ReadThreadParams;
use thinwedge_thread_store::ResumeThreadParams;
use thinwedge_thread_store::ThreadPersistenceMetadata;
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                thinwedge_error_info: Some(ThinWedgeErrorInfo::BadRequest),
            },
...
use thinwedge_mcp::effective_mcp_servers_from_configured;
use thinwedge_mcp::host_owned_thinwedge_apps_enabled;
use thinwedge_otel::SessionTelemetry;
use thinwedge_otel::THREAD_STARTED_METRIC;
...
use thinwedge_protocol::models::LocalImagePreparation;
...
use thinwedge_protocol::protocol::ModelRerouteEvent;
use thinwedge_protocol::protocol::ModelRerouteReason;
use thinwedge_protocol::protocol::ModelVerification;
use thinwedge_protocol::protocol::ModelVerificationEvent;
...
use thinwedge_protocol::protocol::RequestUserInputEvent;
...
/// Wrapper returned by [`ThinWedge::spawn`] containing the spawned [`ThinWedge`] and
/// the unique session id.
pub struct ThinWedgeSpawnOk {
    pub thinwedge: ThinWedge,
    pub thread_id: ThreadId,
}

pub(crate) struct ThinWedgeSpawnArgs {
    ...
    pub(crate) extensions: Arc<thinwedge_extension_api::ExtensionRegistry<crate::config::Config>>,
...
impl ThinWedge {
    ...
    pub(crate) async fn spawn(args: ThinWedgeSpawnArgs) -> ThinWedgeResult<ThinWedgeSpawnOk> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if thinwedge_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
        ...
        Self::spawn_internal(ThinWedgeSpawnArgs {
            parent_trace,
            ..args
        })
    ...
    async fn spawn_internal(args: ThinWedgeSpawnArgs) -> ThinWedgeResult<ThinWedgeSpawnOk> {
        let ThinWedgeSpawnArgs {
            mut config,
            user_instructions,
            installation_id,
            auth_manager,
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| ThinWedgeErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            thinwedge_models_manager::manager::RefreshStrategy::Offline
        } else {
            thinwedge_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        config
            ...
            .map_err(|err| ThinWedgeErr::InvalidRequest(err.to_string()))?;
        ...
        // Generate a unique ID for the lifetime of this ThinWedge session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.thinwedge_home)
        })?;
    ...
    pub async fn submit(&self, op: Op) -> ThinWedgeResult<String> {
    ...
    /// Use sparingly: prefer `submit()` so ThinWedge is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, mut sub: Submission) -> ThinWedgeResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| ThinWedgeErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub async fn set_thread_memory_mode(
        &self,
        mode: thinwedge_protocol::protocol::ThreadMemoryMode,
    ) -> anyhow::Result<()> {
        handlers::persist_thread_memory_mode_update(&self.session, mode).await
    }

    pub async fn shutdown_and_wait(&self) -> ThinWedgeResult<()> {
    ...
    pub async fn next_event(&self) -> ThinWedgeResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> ThinWedgeResult<PermissionProfileState> {
...
impl Session {
    ...
    /// Builds the `x-thinwedge-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &thinwedge_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn thinwedge_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn thinwedge_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn thinwedge_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.thinwedge_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect ThinWedge performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn update_settings(
        ...
        let (
            ...
            let thinwedge_home = updated.thinwedge_home.clone();
    ...
    pub(crate) async fn user_instructions(
        &self,
    ) -> Option<thinwedge_extension_api::UserInstructions> {
        let state = self.state.lock().await;
        state
            .session_configuration
            .loaded_agents_md
    ...
    /// Record a terminal ThinWedgeErr before the app-server completion notification is reduced.
    pub(crate) fn track_turn_thinwedge_error(
        &self,
        turn_context: &TurnContext,
        error: &ThinWedgeErr,
    ) {
        self.services
            .analytics_events_client
            .track_turn_thinwedge_error(TurnThinWedgeErrorFact::from_thinwedge_err(
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        self.services
            ...
            .record_thinwedge_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &thinwedge_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| thinwedge_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&thinwedge_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        if crate::guardian::routes_approval_to_guardian(turn_context.as_ref()) {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                request,
                /*retry_reason*/ None,
                thinwedge_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-thinwedge, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(thinwedge_hooks::SessionStartSource::Compact);
        }
    }
    ...
    pub(crate) async fn notify_stream_error(
        ...
        thinwedge_error: ThinWedgeErr,
    ) {
        let additional_details = thinwedge_error.to_string();
        let thinwedge_error_info = ThinWedgeErrorInfo::ResponseStreamDisconnected {
            http_status_code: thinwedge_error.http_status_code_value(),
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<thinwedge_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_for_config(
    ...
    Hooks::new(HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::ThinWedgeHooks),
