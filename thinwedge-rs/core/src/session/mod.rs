...
use thinwedge_protocol::protocol::SkillDependencies as ProtocolSkillDependencies;
use thinwedge_protocol::protocol::SkillErrorInfo;
use thinwedge_protocol::protocol::SkillInterface as ProtocolSkillInterface;
...
use thinwedge_utils_readiness::ReadinessFlag;
#[cfg(test)]
...
async fn thread_title_from_state_db(
    ...
    find_thread_name_by_id(thinwedge_home, &conversation_id)
        .await
        .ok()
        .flatten()
}
...
impl Session {
    ...
    pub(crate) async fn update_settings(
        ...
        let (previous_cwd, permission_profile_changed, next_cwd, thinwedge_home, session_source) = {
            let mut state = self.state.lock().await;
    ...
    pub(crate) async fn record_model_warning(&self, message: impl Into<String>, ctx: &TurnContext) {
        self.services
            .session_telemetry
            .counter("thinwedge.model_warning", /*inc*/ 1, &[]);
    ...
    pub(crate) async fn build_initial_context(
        ...
        if turn_context.features.enabled(Feature::MemoryTool)
            ...
            && let Some(memory_prompt) =
                build_memory_tool_developer_instructions(&turn_context.config.thinwedge_home).await
        {
        ...
        if turn_context.features.enabled(Feature::ThinWedgeGitCommit)
...
use thinwedge_memories_read::build_memory_tool_developer_instructions;
