...
use thinwedge_mcp::collect_mcp_snapshot_from_manager;
...
use thinwedge_protocol::protocol::SkillsListEntry;
...
use thinwedge_protocol::mcp::RequestId as ProtocolRequestId;
...
pub async fn resolve_elicitation(
    ...
    decision: thinwedge_protocol::approvals::ElicitationAction,
    content: Option<Value>,
    meta: Option<Value>,
) {
    let action = match decision {
        thinwedge_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
        thinwedge_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
        thinwedge_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
    };
    ...
    tokio::spawn(async move {
        ...
        let event = Event {
            ...
            msg: EventMsg::GetHistoryEntryResponse(
                thinwedge_protocol::protocol::GetHistoryEntryResponseEvent {
                    offset,
                    log_id,
                    entry: entry_opt.map(|e| thinwedge_protocol::message_history::HistoryEntry {
...
pub async fn list_skills(sess: &Session, sub_id: String, cwds: Vec<PathBuf>, force_reload: bool) {
    ...
    let thinwedge_home = sess.thinwedge_home().await;
    ...
    if let Err(err) = sess.flush_rollout().await {
        sess.send_event(
            ...
            EventMsg::Warning(WarningEvent {
                message: format!(
                    "Rolled the thread back, but failed to save the rollback marker. ThinWedge will continue retrying. Error: {err}"
                ),
            }),
        )
        .await;
...
pub async fn set_thread_name(sess: &Arc<Session>, sub_id: String, name: String) {
    ...
    let thinwedge_home = sess.thinwedge_home().await;
    if let Err(err) =
        crate::rollout::append_thread_name(&thinwedge_home, sess.conversation_id, &name).await
    {
...
pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
    ...
    info!("Shutting down ThinWedge instance");
    ...
    sess.services.session_telemetry.counter(
        "thinwedge.conversation.turn.count",
    ...
    sess.services
        .rollout_thread_trace
        .record_ended(thinwedge_rollout_trace::RolloutStatus::Completed);
    true
}
