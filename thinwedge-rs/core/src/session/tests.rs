...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let mut builder = test_thinwedge().with_config(move |config| {
        ...
        config.permissions.permission_profile =
            thinwedge_config::Constrained::allow_any(PermissionProfile::from_runtime_permissions(
    ...
    let expected_permission_profile =
        thinwedge_protocol::models::PermissionProfile::from_legacy_sandbox_policy(
...
async fn session_configuration_apply_preserves_profile_file_system_policy_on_cwd_only_update() {
    ...
    session_configuration.permission_profile = thinwedge_config::Constrained::allow_any(
...
async fn session_new_fails_when_zsh_fork_enabled_without_zsh_path() {
    ...
    let result = Session::new(
        ...
        Arc::new(thinwedge_thread_store::LocalThreadStore::new(
            thinwedge_rollout::RolloutConfig::from_view(config.as_ref()),
        )),
...
async fn unknown_turn_environment_returns_error() {
    ...
    assert!(matches!(err, ThinWedgeErr::InvalidRequest(_)));
...
async fn upsert_goal_test_thread(session: &Session) {
    ...
    let mut builder = thinwedge_state::ThreadMetadataBuilder::new(
...
async fn interrupt_accounts_active_goal_before_pausing() -> anyhow::Result<()> {
    ...
    assert_eq!(
        thinwedge_protocol::protocol::ThreadGoalStatus::Paused,
        goal.status
    );
...
async fn active_goal_continuation_runs_to_completion_after_turn() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(std::time::Duration::from_secs(8), async {
        loop {
            let event = test.thinwedge.next_event().await?;
...
async fn external_goal_mutation_accounts_active_turn_before_status_change() -> anyhow::Result<()> {
    ...
    state_db
        .update_thread_goal(
            ...
            thinwedge_state::ThreadGoalUpdate {
                status: Some(thinwedge_state::ThreadGoalStatus::Complete),
                token_budget: None,
    ...
    sess.goal_runtime_apply(GoalRuntimeEvent::ExternalSet {
        status: thinwedge_state::ThreadGoalStatus::Complete,
    })
    .await?;
    ...
    let goal = state_db
        ...
        .await?
        .expect("goal should remain persisted");
    assert_eq!(thinwedge_state::ThreadGoalStatus::Complete, goal.status);
    assert_eq!(70, goal.tokens_used);
...
async fn external_active_goal_set_marks_current_turn_for_accounting() -> anyhow::Result<()> {
    ...
    state_db
        ...
        .await?;
    sess.goal_runtime_apply(GoalRuntimeEvent::ExternalSet {
        status: thinwedge_state::ThreadGoalStatus::Active,
    })
    .await?;
    ...
    let goal = state_db
        ...
        .await?
        .expect("goal should remain persisted");
    assert_eq!(thinwedge_state::ThreadGoalStatus::Active, goal.status);
    assert_eq!(25, goal.tokens_used);
...
async fn create_goal_tool_rejects_existing_goal() {
    ...
    handler
        .handle(ToolInvocation {
            ...
            tool_name: thinwedge_tools::ToolName::plain("create_goal"),
...
async fn update_goal_tool_rejects_pausing_goal() {
    ...
    let response = handler
        .handle(ToolInvocation {
            ...
            tool_name: thinwedge_tools::ToolName::plain("update_goal"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "status": "paused",
