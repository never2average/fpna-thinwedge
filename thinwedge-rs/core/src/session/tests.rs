...
use crate::thinwedge_thread::TryStartTurnIfIdleRejectionReason;
...
use thinwedge_models_manager::model_info;
...
use thinwedge_protocol::openai_models::ModelServiceTier;
...
use thinwedge_protocol::protocol::NonSteerableTurnKind;
...
use thinwedge_config::permissions_toml::NetworkToml;
use thinwedge_config::permissions_toml::PermissionProfileToml;
use thinwedge_config::permissions_toml::PermissionsToml;
...
use thinwedge_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use thinwedge_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use thinwedge_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use thinwedge_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use thinwedge_protocol::protocol::MultiAgentVersion;
...
use thinwedge_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
...
use thinwedge_protocol::protocol::ThreadSettingsOverrides;
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("thinwedge-core-tests");
    ...
    let response = session
        .request_mcp_server_elicitation(
            ...
            McpServerElicitationRequestParams {
                ...
                server_name: "thinwedge_apps".to_string(),
...
fn write_project_hooks(dot_thinwedge: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_thinwedge)?;
    std::fs::write(
        dot_thinwedge.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        thinwedge_home.join(thinwedge_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<thinwedge_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&thinwedge_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: thinwedge_hooks::StartHookTarget::SessionStart {
                source: thinwedge_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> thinwedge_sandboxing::SandboxablePreference {
            thinwedge_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(thinwedge_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: thinwedge_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let thinwedge_home = session.thinwedge_home().await;
    std::fs::create_dir_all(&thinwedge_home).expect("create thinwedge home");
    let config_toml_path = thinwedge_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = thinwedge_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let thinwedge_home = session.thinwedge_home().await;
    std::fs::create_dir_all(&thinwedge_home).expect("create thinwedge home");
    let base_config_path = thinwedge_home.join(CONFIG_TOML_FILE);
    let profile_config_path = thinwedge_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(thinwedge_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            ...
            .expect("enable ThinWedge hooks");
    })
    .await?;
    let thinwedge_home = session.thinwedge_home().await;
    std::fs::create_dir_all(&thinwedge_home)?;
    let config_toml_path = thinwedge_home.join(CONFIG_TOML_FILE);
    let user_config: thinwedge_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = thinwedge_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = thinwedge_hooks::list_hooks(thinwedge_hooks::HooksConfig {
        feature_enabled: true,
    ...
    assert_eq!(
        ...
        thinwedge_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: thinwedge_config::TomlValue =
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: thinwedge_config::MatcherGroup,
    }
    let trusted_hash = {
        ...
        let identity = thinwedge_config::TomlValue::try_from(identity)?;
        thinwedge_config::version_for_toml(&identity)
    };
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl thinwedge_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a thinwedge_extension_api::ExtensionData,
            thread_store: &'a thinwedge_extension_api::ExtensionData,
            turn_store: &'a thinwedge_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> thinwedge_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let mut builder =
        thinwedge_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl thinwedge_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: thinwedge_extension_api::TurnStartInput<'a>,
        ) -> thinwedge_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl thinwedge_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: thinwedge_extension_api::TurnErrorInput<'a>,
        ) -> thinwedge_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let expected = RecordedTurnError {
        ...
        turn_level_id: turn_context.sub_id.clone(),
        turn_id: turn_context.sub_id.clone(),
        error: ThinWedgeErrorInfo::UsageLimitExceeded,
        saw_session_store: true,
        saw_thread_store: true,
    };

    session
        .emit_turn_error_lifecycle(&turn_context, ThinWedgeErrorInfo::UsageLimitExceeded)
        .await;
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl thinwedge_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &thinwedge_extension_api::ExtensionData,
            thread_store: &thinwedge_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: thinwedge_protocol::protocol::NetworkAccess::Restricted,
    };
...
async fn session_permission_profile_rebinds_runtime_workspace_roots() -> anyhow::Result<()> {
    let thinwedge_home = tempfile::TempDir::new()?;
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_thinwedge().with_config(|config| {
        config.permissions.approval_policy =
            thinwedge_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        thinwedge_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "thinwedge_core__thinwedge_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: thinwedge_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            thinwedge_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            thinwedge_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            thinwedge_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.thinwedge_error_info,
...
async fn set_rate_limits_retains_previous_credits() {
    let thinwedge_home = tempfile::tempdir().expect("create temp dir");
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(thinwedge_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("thinwedge_other".to_string()),
        limit_name: Some("thinwedge_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        credits: None,
        individual_limit: None,
        plan_type: Some(thinwedge_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    assert_eq!(
        state.latest_rate_limits,
        Some(RateLimitSnapshot {
            limit_id: Some("thinwedge".to_string()),
            limit_name: None,
...
async fn build_test_config(thinwedge_home: &Path) -> Config {
...
async fn emit_subagent_session_started_includes_fork_lineage_from_session_configuration() {
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(ThinWedgeAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("thinwedge-tui".to_string()),
            ...
            if let Some(requests) = server.received_requests().await {
                for request in requests {
                    ...
                    if let Some(event) = payload["events"].as_array().and_then(|events| {
                        events
                            .iter()
                            .find(|event| event["event_type"] == "thinwedge_thread_initialized")
                    }) {
...
fn turn_environments_for_tests(
    environment: &Arc<thinwedge_exec_server::Environment>,
    cwd: &thinwedge_utils_absolute_path::AbsolutePathBuf,
) -> crate::environment_selection::ResolvedTurnEnvironments {
    crate::environment_selection::ResolvedTurnEnvironments {
        turn_environments: vec![TurnEnvironment::new(
            thinwedge_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        thinwedge_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let thinwedge_home = parent_config.thinwedge_home.clone();
    let skill_dir = thinwedge_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&thinwedge_exec_server::LOCAL_FS));
    ...
    let role_path = thinwedge_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.thinwedge_home.to_path_buf()));
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(thinwedge_extension_api::ExtensionRegistryBuilder::new().build()),
        thinwedge_extension_api::ExtensionDataInit::default(),
        AgentControl::default(),
        Arc::new(thinwedge_exec_server::EnvironmentManager::default_for_tests()),
        /*analytics_events_client*/ None,
        Arc::new(thinwedge_thread_store::LocalThreadStore::new(
            thinwedge_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        thinwedge_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: thinwedge_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(thinwedge_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: thinwedge_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: thinwedge_extension_api::ExtensionData::new(thread_id.to_string()),
        mcp_thread_init: thinwedge_extension_api::ExtensionDataInit::default(),
        agent_control,
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(thinwedge_thread_store::LocalThreadStore::new(
            thinwedge_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            thinwedge_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(thinwedge_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(thinwedge_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            thinwedge_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(thinwedge_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = thinwedge_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(thinwedge_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    &turn_context,
                    call_id,
                    thinwedge_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(thinwedge_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: thinwedge_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(thinwedge_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserInput {
            ...
            thread_settings: thinwedge_protocol::protocol::ThreadSettingsOverrides {
                ...
                approvals_reviewer: Some(thinwedge_config::types::ApprovalsReviewer::AutoReview),
    ...
    assert_eq!(
        ...
        thinwedge_config::types::ApprovalsReviewer::AutoReview
    );
}
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(thinwedge_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn thinwedge_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        thinwedge_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_emits_thread_stop_lifecycle() {
    ...
    impl thinwedge_extension_api::ThreadLifecycleContributor<crate::config::Config>
        for ThreadStopRecorder
    {
        fn on_thread_stop<'a>(
            &'a self,
            input: thinwedge_extension_api::ThreadStopInput<'a>,
        ) -> thinwedge_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl thinwedge_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: thinwedge_extension_api::TurnAbortInput<'a>,
        ) -> thinwedge_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn shutdown_and_wait_allows_multiple_waiters() {
    ...
    let thinwedge = Arc::new(ThinWedge {
        tx_sub,
        rx_event,
        agent_status,
        session: Arc::new(session),
    ...
    let waiter_1 = {
        let thinwedge = Arc::clone(&thinwedge);
        tokio::spawn(async move { thinwedge.shutdown_and_wait().await })
    };
    let waiter_2 = {
        let thinwedge = Arc::clone(&thinwedge);
        tokio::spawn(async move { thinwedge.shutdown_and_wait().await })
...
async fn shutdown_and_wait_shuts_down_cached_guardian_subagent() {
    ...
    let parent_thinwedge = ThinWedge {
    ...
    parent_session
        ...
        .cache_for_test(child_thinwedge)
        .await;
...
async fn shutdown_and_wait_shuts_down_tracked_ephemeral_guardian_review() {
    ...
    parent_session
        .guardian_review_session
        .register_ephemeral_for_test(child_thinwedge)
        .await;
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(thinwedge_home).await;
...
impl thinwedge_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute<'a>(
        &'a self,
        _session_store: &'a thinwedge_extension_api::ExtensionData,
        thread_store: &'a thinwedge_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<Output = Vec<thinwedge_extension_api::PromptFragment>>
                + Send
                + 'a,
        >,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    thinwedge_extension_api::PromptFragment::developer_policy(
...
-> Arc<thinwedge_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = thinwedge_extension_api::ExtensionRegistryBuilder::new();
...
async fn handle_output_item_done_records_image_save_history_message() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(thinwedge_extension_api::ExtensionData::new(
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = thinwedge_protocol::user_input::TextElement::new(
        thinwedge_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
        ...
        for ThreadIdleRecorder
    {
        fn on_thread_idle<'a>(
            &'a self,
            input: thinwedge_extension_api::ThreadIdleInput<'a>,
        ) -> thinwedge_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn thread_idle_lifecycle_waits_for_trigger_turn_mailbox_work() {
        ...
        for ThreadIdleRecorder
    {
        fn on_thread_idle<'a>(
            &'a self,
            _input: thinwedge_extension_api::ThreadIdleInput<'a>,
        ) -> thinwedge_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            })
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use thinwedge_protocol::protocol::AskForApproval;
    use thinwedge_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: thinwedge_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: thinwedge_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let thinwedge_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_thinwedge = project_root.join(".thinwedge");
    let nested_dot_thinwedge = nested.join(".thinwedge");

    std::fs::create_dir_all(&thinwedge_home)?;
    std::fs::create_dir_all(&nested_dot_thinwedge)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_thinwedge)?;
    write_project_hooks(&nested_dot_thinwedge)?;
    write_project_trust_config(&thinwedge_home, &[(&nested, TrustLevel::Trusted)]).await?;
    ...
    let expected_source_path = thinwedge_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        nested_dot_thinwedge.join("hooks.json"),
    )?;
    assert_eq!(
        hook_list
            .hooks
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_thinwedge)?;
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let thinwedge_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&thinwedge_home)?;
        write_project_trust_config(&thinwedge_home, &trust_entries).await?;
