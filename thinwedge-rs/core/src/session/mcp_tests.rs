...
fn guardian_meta(tool_params: Option<Value>) -> Option<Meta> {
    let mut value = json!({
        "thinwedge_approval_kind": "mcp_tool_call",
        "thinwedge_request_type": "approval_request",
...
fn plugin_install_elicitation_telemetry_metadata_requires_install_tool_suggestion() {
    let event = EventMsg::ElicitationRequest(ElicitationRequestEvent {
        ...
        id: thinwedge_protocol::mcp::RequestId::String("request-1".to_string()),
        request: thinwedge_protocol::approvals::ElicitationRequest::Form {
            meta: Some(json!({
    ...
    let enable_event = EventMsg::ElicitationRequest(ElicitationRequestEvent {
        turn_id: Some("turn-1".to_string()),
        server_name: "thinwedge_apps".to_string(),
        id: thinwedge_protocol::mcp::RequestId::String("request-2".to_string()),
        request: thinwedge_protocol::approvals::ElicitationRequest::Form {
            meta: Some(json!({
                "thinwedge_approval_kind": "tool_suggestion",
                "suggest_type": "enable",
