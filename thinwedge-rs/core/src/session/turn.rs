...
use thinwedge_analytics::TurnResolvedConfigFact;
use thinwedge_analytics::build_track_events_context;
use thinwedge_async_utils::OrCancelExt;
use thinwedge_core_skills::injection::InjectedHostSkillPrompts;
use thinwedge_extension_api::TurnInputContext;
use thinwedge_extension_api::TurnInputEnvironment;
use thinwedge_features::Feature;
use thinwedge_git_utils::get_git_repo_root_with_fs;
...
use thinwedge_tools::filter_request_plugin_install_discoverable_tools_for_client;
use thinwedge_utils_stream_parser::AssistantTextChunk;
use thinwedge_utils_stream_parser::AssistantTextStreamParser;
...
pub(crate) async fn run_turn(
    ...
    turn_extension_data: Arc<thinwedge_extension_api::ExtensionData>,
    input: Vec<TurnInput>,
    ...
    if let Err(err) = run_pre_sampling_compact(&sess, &turn_context, &mut client_session).await {
        let error = err.to_thinwedge_protocol_error();
    ...
    // Although from the perspective of thinwedge.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
            ...
            window_id,
            ThinWedgeResponsesRequestKind::Turn,
        );
        ...
        .await
        {
            ...
            Err(ThinWedgeErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(thinwedge_error @ ThinWedgeErr::InvalidImageRequest()) => {
                {
                ...
                sess.track_turn_thinwedge_error(turn_context.as_ref(), &thinwedge_error);
                let error = ThinWedgeErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                let event = EventMsg::Error(ErrorEvent {
                    message: "Invalid image in your last message. Please remove it and try again."
                        .to_string(),
                    thinwedge_error_info: Some(error),
                });
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_thinwedge_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                sess.track_turn_thinwedge_error(turn_context.as_ref(), &e);
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = thinwedge_connectors::merge::merge_plugin_connectors_with_accessible(
            loaded_plugins
                .effective_apps()
                .into_iter()
...
async fn run_sampling_request(
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
    turn_store: Arc<thinwedge_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    responses_metadata: &ThinWedgeResponsesMetadata,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> ThinWedgeResult<SamplingRequestResult> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(ThinWedgeErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(ThinWedgeErr::ContextWindowExceeded);
            }
            Err(ThinWedgeErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(ThinWedgeErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
...
pub(crate) async fn built_tools(
    ...
    turn_context: &TurnContext,
    cancellation_token: &CancellationToken,
) -> ThinWedgeResult<Arc<ToolRouter>> {
...
fn agent_message_text(item: &thinwedge_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: thinwedge_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(thinwedge_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &thinwedge_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, ThinWedgeResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> ThinWedgeResult<()> {
...
async fn try_run_sampling_request(
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, ThinWedgeResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: ThinWedgeResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            thinwedge.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            thinwedge.usage.reasoning_output_tokens = field::Empty,
            thinwedge.usage.total_tokens = field::Empty,
        );
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![thinwedge_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
