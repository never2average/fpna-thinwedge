...
use thinwedge_hooks::HookEventAfterAgent;
        ...
        .await
        {
            Ok(sampling_request_output) => {
                ...
                if !needs_follow_up {
                    ...
                    .to_string();
                    let stop_request = thinwedge_hooks::StopRequest {
                    ...
                    for run in sess.hooks().preview_stop(&stop_request) {
                        sess.send_event(
                            &turn_context,
                            EventMsg::HookStarted(thinwedge_protocol::protocol::HookStartedEvent {
            ...
            Err(ThinWedgeErr::InvalidImageRequest()) => {
                {
...
fn connector_inserted_in_messages(
    ...
    let mention_slug = thinwedge_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    ...
    loop {
        ...
        if retries < max_retries {
            retries += 1;
            let delay = match &err {
                ThinWedgeErr::Stream(_, requested_delay) => {
