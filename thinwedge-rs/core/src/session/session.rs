...
pub(crate) struct SessionConfiguration {
    ...
    /// Directory containing all ThinWedge state for this session.
    pub(super) thinwedge_home: AbsolutePathBuf,
...
impl SessionConfiguration {
    ...
    pub(crate) fn thinwedge_home(&self) -> &AbsolutePathBuf {
        &self.thinwedge_home
    }
            ...
            if let Some(active_permission_profile) = next_configuration.active_permission_profile()
            {
                ...
                config.permissions.network = config
                    ...
                    .map_err(|err| ConstraintError::InvalidValue {
                        ...
                        requirement_source: thinwedge_config::RequirementSource::Unknown,
                    })?;
                config
                    .permissions
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        extensions: Arc<thinwedge_extension_api::ExtensionRegistry<crate::config::Config>>,
        ...
        let thread_extension_data = thinwedge_extension_api::ExtensionData::new_with_init(
            thread_id.to_string(),
        ...
        let auth_and_mcp_fut = async move {
            ...
            let mcp_config = mcp_manager_for_mcp
                ...
                .await;
            let mcp_servers = thinwedge_mcp::effective_mcp_servers(&mcp_config, auth.as_ref());
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let trace_agent_path = session_configuration
                .session_source
                .get_agent_path()
                .unwrap_or_else(thinwedge_protocol::AgentPath::root);
            let trace_task_name =
            ...
            let config_path = config.thinwedge_home.join(CONFIG_TOML_FILE);
            ...
            let auth_env_telemetry = collect_auth_env_telemetry(
                &session_configuration.provider,
                auth_manager.thinwedge_api_key_env_enabled(),
            );
            ...
            let session_extension_data =
                thinwedge_extension_api::ExtensionData::new(session_id.to_string());
            session_extension_data.insert(McpResourceClient::new(Arc::clone(
                &mcp_connection_manager,
            )));
            for contributor in extensions.thread_lifecycle_contributors() {
                contributor.on_thread_start(thinwedge_extension_api::ThreadStartInput {
            ...
            let host_owned_thinwedge_apps_enabled = config
                .features
                .apps_enabled_for_auth(auth.as_ref().is_some_and(|auth| auth.uses_thinwedge_backend()));
            ...
            .await
            .map_err(|err| {
                ThinWedgeErr::InvalidRequest(err.to_string().replace(
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Resumed(_) => thinwedge_hooks::SessionStartSource::Resume,
                InitialHistory::New | InitialHistory::Forked(_) => {
                    thinwedge_hooks::SessionStartSource::Startup
                }
                InitialHistory::Cleared => thinwedge_hooks::SessionStartSource::Clear,
            };
