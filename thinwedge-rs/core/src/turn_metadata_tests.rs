...
fn test_turn_responses_metadata_json(state: &TurnMetadataState, window_id: &str) -> String {
    test_responses_metadata_json(state, window_id, ThinWedgeResponsesRequestKind::Turn)
}
...
fn test_compaction_responses_metadata_json(
    ...
    test_responses_metadata_json(
        state,
        window_id,
        ThinWedgeResponsesRequestKind::Compaction(compaction),
    )
}
...
fn turn_metadata_state_merges_client_metadata_without_replacing_reserved_fields() {
    ...
    state.set_responsesapi_client_metadata(HashMap::from([
        ...
        (
            "x-thinwedge-installation-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
            "x-thinwedge-parent-thread-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
    ...
    assert!(json.get("x-thinwedge-installation-id").is_none());
    assert!(json.get("x-thinwedge-parent-thread-id").is_none());
