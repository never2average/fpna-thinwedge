...
use crate::thinwedge_thread::BackgroundTerminalInfo;
use thinwedge_analytics::TurnProfileFact;
use thinwedge_analytics::TurnTokenUsageFact;
...
use thinwedge_otel::TURN_E2E_DURATION_METRIC;
use thinwedge_otel::TURN_MEMORY_METRIC;
use thinwedge_otel::TURN_NETWORK_PROXY_METRIC;
use thinwedge_otel::TURN_TOKEN_USAGE_METRIC;
use thinwedge_otel::TURN_TOOL_CALL_METRIC;
...
const TASK_COMPACT_METRIC: &str = "thinwedge.task.compact";
...
///
/// Implementations encapsulate a specific ThinWedge workflow (regular chat,
...
impl Session {
    ...
    pub(crate) async fn start_task<T: SessionTask>(
        ...
        let task_span = info_span!(
            ...
            thinwedge.turn.reasoning_effort = %reasoning_effort,
            thinwedge.turn.token_usage.input_tokens = field::Empty,
            thinwedge.turn.token_usage.cached_input_tokens = field::Empty,
            thinwedge.turn.token_usage.non_cached_input_tokens = field::Empty,
            thinwedge.turn.token_usage.output_tokens = field::Empty,
            thinwedge.turn.token_usage.reasoning_output_tokens = field::Empty,
            thinwedge.turn.token_usage.total_tokens = field::Empty,
        );
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; ThinWedge will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
    ...
    pub async fn on_task_finished(
        ...
        // Emit token usage metrics.
        {
            ...
            current_span.record(
                "thinwedge.turn.token_usage.input_tokens",
                turn_token_usage.input_tokens,
            );
            current_span.record(
                "thinwedge.turn.token_usage.cached_input_tokens",
                turn_token_usage.cached_input(),
            );
            current_span.record(
                "thinwedge.turn.token_usage.non_cached_input_tokens",
                turn_token_usage.non_cached_input(),
            );
            current_span.record(
                "thinwedge.turn.token_usage.output_tokens",
                turn_token_usage.output_tokens,
            );
            current_span.record(
                "thinwedge.turn.token_usage.reasoning_output_tokens",
                turn_token_usage.reasoning_output_tokens,
            );
            current_span.record(
                "thinwedge.turn.token_usage.total_tokens",
