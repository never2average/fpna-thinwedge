...
use thinwedge_config::merge_toml_values;
...
use thinwedge_config::permissions_toml::overlay_network_domain_permissions;
...
use thinwedge_network_proxy::NetworkProxyConstraintError;
...
async fn build_config_state_with_mtimes() -> Result<(ConfigState, Vec<LayerMtime>)> {
    ...
    let (exec_policy, warning) = match load_exec_policy(&config_layer_stack).await {
        ...
        Err(err @ ExecPolicyError::ParsePolicy { .. }) => {
            (thinwedge_execpolicy::Policy::empty(), Some(err))
        }
...
fn apply_exec_policy_network_rules(
    ...
    for host in allowed_domains {
        upsert_network_domain(
            config,
            host,
            thinwedge_network_proxy::NetworkDomainPermission::Allow,
        );
    }
    for host in denied_domains {
        upsert_network_domain(
            config,
            host,
            thinwedge_network_proxy::NetworkDomainPermission::Deny,
        );
    }
}
...
fn upsert_network_domain(
    config: &mut NetworkProxyConfig,
    host: String,
    permission: thinwedge_network_proxy::NetworkDomainPermission,
) {
    config
        .network
