...
pub(crate) struct ConfigLockReplayOptions {
    pub allow_thinwedge_version_mismatch: bool,
}
...
pub(crate) fn validate_config_lock_replay(
    ...
    if !options.allow_thinwedge_version_mismatch
        && expected_lock.thinwedge_version != actual_lock.thinwedge_version
    {
        return Err(config_lock_error(format!(
            "config lock ThinWedge version mismatch: lock was generated by {}, current version is {}; set debug.config_lockfile.allow_thinwedge_version_mismatch=true to ignore this",
            expected_lock.thinwedge_version, actual_lock.thinwedge_version
        )));
    }
...
fn config_lock_for_comparison(
    ...
    if options.allow_thinwedge_version_mismatch {
        lockfile.thinwedge_version.clear();
    }
    lockfile
}
