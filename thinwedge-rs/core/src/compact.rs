...
use thinwedge_analytics::ThinWedgeCompactionEvent;
use thinwedge_analytics::now_unix_seconds;
...
pub use thinwedge_prompts::SUMMARIZATION_PROMPT;
pub use thinwedge_prompts::SUMMARY_PREFIX;
...
async fn run_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            let error = ThinWedgeErr::TurnAborted;
            attempt
                .track(
                    sess.as_ref(),
    ...
    let thinwedge_error = result.as_ref().err();
    if result.is_ok() {
...
async fn run_compact_task_inner_impl(
    ...
    let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
        ...
        window_id,
        ThinWedgeResponsesRequestKind::Compaction(compaction_metadata),
    );
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(ThinWedgeErr::Interrupted) => {
                return Err(ThinWedgeErr::Interrupted);
            }
            Err(e @ ThinWedgeErr::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        thinwedge_error: Option<&ThinWedgeErr>,
        ...
        sess.services
            ...
            .track_compaction(ThinWedgeCompactionEvent {
                ...
                status,
                thinwedge_error_kind: thinwedge_error.map(Into::into),
                thinwedge_error_http_status_code: thinwedge_error
                    .and_then(ThinWedgeErr::http_status_code_value),
                active_context_tokens_before,
...
pub(crate) fn compaction_status_from_result<T>(result: &ThinWedgeResult<T>) -> CompactionStatus {
    match result {
        Ok(_) => CompactionStatus::Completed,
        Err(ThinWedgeErr::Interrupted | ThinWedgeErr::TurnAborted) => CompactionStatus::Interrupted,
