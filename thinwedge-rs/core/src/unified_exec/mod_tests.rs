...
use std::sync::Arc;
use thinwedge_exec_server::ExecProcess;
use thinwedge_exec_server::ExecProcessEventReceiver;
use thinwedge_exec_server::ExecProcessFuture;
use thinwedge_exec_server::ProcessId;
use thinwedge_exec_server::ProcessSignal;
use thinwedge_exec_server::ReadResponse;
use thinwedge_exec_server::StartedExecProcess;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, thinwedge_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, thinwedge_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), thinwedge_exec_server::ExecServerError> {
...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        "export THINWEDGE_INTERACTIVE_SHELL_VAR=thinwedge\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "echo $THINWEDGE_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    assert!(
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        format!("export THINWEDGE_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "sleep 5 && echo $THINWEDGE_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 10,
    )
    .await?;
    assert!(
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = thinwedge_exec_server::Environment::default_for_tests();
