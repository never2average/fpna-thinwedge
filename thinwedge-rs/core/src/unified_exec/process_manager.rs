...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the ThinWedge sandbox network proxy.";
...
) -> thinwedge_exec_server::ExecEnvPolicy {
    thinwedge_exec_server::ExecEnvPolicy {
...
fn exec_server_env_for_request(
    ...
    Option<thinwedge_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn exec_server_params_for_request(
    ...
    request: &ExecRequest,
    tty: bool,
) -> thinwedge_exec_server::ExecParams {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::ThinWedge(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        if request.sandbox == thinwedge_sandboxing::SandboxType::WindowsRestrictedToken {
            let thinwedge_home = crate::config::find_thinwedge_home().map_err(|err| {
            ...
            let spawned = match request.windows_sandbox_level {
                thinwedge_protocol::config_types::WindowsSandboxLevel::Elevated => {
                    thinwedge_windows_sandbox::spawn_windows_sandbox_session_elevated_for_permission_profile(
                ...
                thinwedge_protocol::config_types::WindowsSandboxLevel::RestrictedToken
                | thinwedge_protocol::config_types::WindowsSandboxLevel::Disabled => {
                    thinwedge_windows_sandbox::spawn_windows_sandbox_session_legacy(
                        &request.permission_profile,
                        request.windows_sandbox_workspace_roots.as_slice(),
                        thinwedge_home.as_ref(),
                        request.command.clone(),
        ...
        let spawn_result = if tty {
            thinwedge_utils_pty::pty::spawn_process_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
            ...
            thinwedge_utils_pty::pipe::spawn_process_no_stdin_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
