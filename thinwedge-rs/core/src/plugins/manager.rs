...
use thinwedge_core_plugins::loader::installed_plugin_telemetry_metadata;
use thinwedge_core_plugins::loader::load_plugin_apps;
use thinwedge_core_plugins::loader::load_plugin_mcp_servers;
use thinwedge_core_plugins::loader::load_plugin_skills;
use thinwedge_core_plugins::loader::load_plugins_from_layer_stack;
use thinwedge_core_plugins::loader::log_plugin_load_errors;
use thinwedge_core_plugins::loader::materialize_marketplace_plugin_source;
use thinwedge_core_plugins::loader::plugin_telemetry_metadata_from_root;
use thinwedge_core_plugins::loader::refresh_curated_plugin_cache;
use thinwedge_core_plugins::loader::refresh_non_curated_plugin_cache;
use thinwedge_core_plugins::loader::refresh_non_curated_plugin_cache_force_reinstall;
use thinwedge_core_plugins::manifest::PluginManifestInterface;
use thinwedge_core_plugins::manifest::load_plugin_manifest;
use thinwedge_core_plugins::marketplace::MarketplaceError;
use thinwedge_core_plugins::marketplace::MarketplaceInterface;
use thinwedge_core_plugins::marketplace::MarketplaceListError;
use thinwedge_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use thinwedge_core_plugins::marketplace::MarketplacePluginPolicy;
use thinwedge_core_plugins::marketplace::MarketplacePluginSource;
use thinwedge_core_plugins::marketplace::ResolvedMarketplacePlugin;
use thinwedge_core_plugins::marketplace::find_installable_marketplace_plugin;
use thinwedge_core_plugins::marketplace::find_marketplace_plugin;
use thinwedge_core_plugins::marketplace::list_marketplaces;
use thinwedge_core_plugins::marketplace::load_marketplace;
use thinwedge_core_plugins::marketplace::plugin_interface_with_marketplace_category;
use thinwedge_core_plugins::marketplace_upgrade::ConfiguredMarketplaceUpgradeError;
use thinwedge_core_plugins::marketplace_upgrade::ConfiguredMarketplaceUpgradeOutcome;
use thinwedge_core_plugins::marketplace_upgrade::configured_git_marketplace_names;
use thinwedge_core_plugins::marketplace_upgrade::upgrade_configured_git_marketplaces;
use thinwedge_core_plugins::remote::RemotePluginServiceConfig;
use thinwedge_core_plugins::remote_legacy::RemotePluginFetchError;
...
fn featured_plugin_ids_cache_key(
    ...
    FeaturedPluginIdsCacheKey {
        chatgpt_base_url: config.chatgpt_base_url.clone(),
        account_id: auth.and_then(ThinWedgeAuth::get_account_id),
        chatgpt_user_id: auth.and_then(ThinWedgeAuth::get_chatgpt_user_id),
        is_workspace_account: auth.is_some_and(ThinWedgeAuth::is_workspace_account),
    }
}
...
impl PluginsManager {
    pub fn new(thinwedge_home: PathBuf) -> Self {
        Self::new_with_restriction_product(thinwedge_home, Some(Product::ThinWedge))
    }
    ...
    pub fn new_with_restriction_product(
        ...
        // Product restrictions are enforced at marketplace admission time for a given THINWEDGE_HOME:
        ...
        // contents of that THINWEDGE_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single THINWEDGE_HOME is only used by one product.
        Self {
            thinwedge_home: thinwedge_home.clone(),
            store: PluginStore::new(thinwedge_home),
    ...
    pub async fn featured_plugin_ids_for_config(
        ...
        let featured_plugin_ids =
            thinwedge_core_plugins::remote_legacy::fetch_remote_featured_plugin_ids(
    ...
    pub async fn install_plugin_with_remote_sync(
        ...
        thinwedge_core_plugins::remote_legacy::enable_remote_plugin(
            &remote_plugin_service_config(config),
            auth,
            &plugin_id,
        )
    ...
    async fn install_resolved_plugin(
        ...
        let thinwedge_home = self.thinwedge_home.clone();
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(thinwedge_home.as_path(), &resolved.source)
        ...
        ConfigEditsBuilder::new(&self.thinwedge_home)
    ...
    pub async fn uninstall_plugin_with_remote_sync(
        ...
        thinwedge_core_plugins::remote_legacy::uninstall_remote_plugin(
            &remote_plugin_service_config(config),
            auth,
            &plugin_key,
        )
    ...
    pub async fn sync_plugins_from_remote(
        ...
        let remote_plugins = thinwedge_core_plugins::remote_legacy::fetch_remote_plugin_status(
            &remote_plugin_service_config(config),
            auth,
        )
        .await
        ...
        let curated_marketplace_root = curated_plugins_repo_path(self.thinwedge_home.as_path());
    ...
    pub(crate) async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path =
            if matches!(plugin.source, MarketplacePluginSource::Git { .. }) && plugin.installed {
                ...
                let materialized = tokio::task::spawn_blocking(move || {
                    materialize_marketplace_plugin_source(thinwedge_home.as_path(), &source)
                })
                .await
                .map_err(|err| {
                    MarketplaceError::InvalidPlugin(format!(
        ...
        let resolved_skills = load_plugin_skills(
            ...
            &manifest.paths,
            self.restriction_product,
            &thinwedge_core_skills::config_rules::skill_config_rules_from_stack(
                &config.config_layer_stack,
            ),
        )
        .await;
    ...
    fn run_non_curated_plugin_cache_refresh_loop(self: Arc<Self>) {
        loop {
            ...
            let refresh_result = match request.mode {
                NonCuratedCacheRefreshMode::IfVersionChanged => {
                    refresh_non_curated_plugin_cache(self.thinwedge_home.as_path(), &request.roots)
                }
    ...
    fn marketplace_roots(
        ...
        let curated_repo_root = curated_plugins_repo_path(self.thinwedge_home.as_path());
