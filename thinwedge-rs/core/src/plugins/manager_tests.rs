...
async fn load_plugins_from_config(config_toml: &str, thinwedge_home: &Path) -> PluginLoadOutcome {
    write_file(&thinwedge_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(thinwedge_home, thinwedge_home).await;
    PluginsManager::new(thinwedge_home.to_path_buf())
        .plugins_for_config(&config)
        .await
}

async fn load_config(thinwedge_home: &Path, cwd: &Path) -> crate::config::Config {
...
enabled = true
"#;
    let outcome = load_plugins_from_config(config_toml, thinwedge_home.path()).await;
