...
use thinwedge_core_plugins::startup_sync::has_local_curated_plugins_snapshot;
...
pub(crate) fn start_startup_remote_plugin_sync_once(
    ...
    let marker_path = startup_remote_plugin_sync_marker_path(thinwedge_home.as_path());
    ...
    tokio::spawn(async move {
        ...
        if !wait_for_startup_remote_plugin_sync_prerequisites(thinwedge_home.as_path()).await {
            warn!(
                thinwedge_home = %thinwedge_home.display(),
            ...
            .await
        {
            Ok(sync_result) => {
                ...
                if let Err(err) =
                    write_startup_remote_plugin_sync_marker(thinwedge_home.as_path()).await
                {
                    warn!(
                        error = %err,
...
fn startup_remote_plugin_sync_marker_path(thinwedge_home: &Path) -> PathBuf {
    thinwedge_home.join(STARTUP_REMOTE_PLUGIN_SYNC_MARKER_FILE)
}

async fn wait_for_startup_remote_plugin_sync_prerequisites(thinwedge_home: &Path) -> bool {
    let deadline = tokio::time::Instant::now() + STARTUP_REMOTE_PLUGIN_SYNC_PREREQUISITE_TIMEOUT;
    loop {
        if has_local_curated_plugins_snapshot(thinwedge_home) {
            return true;
        }
...
async fn write_startup_remote_plugin_sync_marker(thinwedge_home: &Path) -> std::io::Result<()> {
    let marker_path = startup_remote_plugin_sync_marker_path(thinwedge_home);
