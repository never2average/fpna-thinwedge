...
async fn list_tool_suggest_discoverable_plugins_omits_installed_curated_plugins() {
    ...
    PluginsManager::new(thinwedge_home.path().to_path_buf())
...
async fn list_tool_suggest_discoverable_plugins_does_not_reload_marketplace_per_plugin() {
  ...
  "interface": {{
    ...
    assert_eq!(
        normalized_logs
            .matches("build-ios-apps/.thinwedge-plugin/plugin.json")
            .count(),
        1
    );
    assert_eq!(
        normalized_logs
            .matches("life-science-research/.thinwedge-plugin/plugin.json")
            .count(),
        1
    );
}
