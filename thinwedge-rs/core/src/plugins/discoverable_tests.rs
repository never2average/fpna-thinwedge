...
use thinwedge_core_plugins::remote::fetch_and_cache_global_remote_plugin_catalog;
...
async fn list_discoverable_plugins_with_auth(
    ...
    let plugins_manager = PluginsManager::new_with_options(
        ...
        auth.map(thinwedge_login::ThinWedgeAuth::api_auth_mode),
    );
...
"#,
    ...
    let mut config = load_plugins_config(thinwedge_home.path()).await;
...
"#,
    ...
    let mut config_with_disabled_remote_plugin = load_plugins_config(thinwedge_home.path()).await;
