...
fn external_sandbox_auto_approves_in_on_request() {
    ...
    let policy = SandboxPolicy::ExternalSandbox {
        network_access: thinwedge_protocol::protocol::NetworkAccess::Enabled,
    };
...
#[test]
fn missing_project_dot_thinwedge_config_requires_approval() {
    let tmp = TempDir::new().unwrap();
    let cwd = tmp.path().abs();
    let config_path = cwd.join(".thinwedge").join("config.toml");
