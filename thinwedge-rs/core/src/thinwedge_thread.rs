...
use thinwedge_thread_store::ThreadStoreResult;
...
/// Explains why `ThinWedgeThread::try_start_turn_if_idle` rejected an automatic
/// idle turn.
...
impl ThreadConfigSnapshot {
    ...
    pub fn sandbox_policy(&self) -> SandboxPolicy {
        thinwedge_sandboxing::compatibility_sandbox_policy_for_permission_profile(
...
pub struct ThinWedgeThreadSettingsOverrides {
...
pub struct ThinWedgeThread {
    pub(crate) thinwedge: ThinWedge,
...
/// (formerly called a conversation) in ThinWedge.
impl ThinWedgeThread {
    pub(crate) fn new(
        thinwedge: ThinWedge,
        session_configured: SessionConfiguredEvent,
        rollout_path: Option<PathBuf>,
    ...
    pub async fn submit(&self, op: Op) -> ThinWedgeResult<String> {
        self.thinwedge.submit(op).await
    }

    /// Returns the session telemetry handle for thread-scoped production instrumentation.
    pub fn session_telemetry(&self) -> SessionTelemetry {
        self.thinwedge.session.services.session_telemetry.clone()
    }

    pub async fn shutdown_and_wait(&self) -> ThinWedgeResult<()> {
        self.thinwedge.shutdown_and_wait().await
    }

    /// Wait until the underlying session loop has terminated.
    pub async fn wait_until_terminated(&self) {
        self.thinwedge.session_loop_termination.clone().await;
    }
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(thinwedge_extension_api::ThreadResumeInput {
                    session_store: &self.thinwedge.session.services.session_extension_data,
                    thread_store: &self.thinwedge.session.services.thread_extension_data,
                })
                .await;
        }
    }
    ...
    pub async fn ensure_rollout_materialized(&self) {
        self.thinwedge.session.ensure_rollout_materialized().await;
    }

    #[doc(hidden)]
    pub async fn flush_rollout(&self) -> std::io::Result<()> {
        self.thinwedge.session.flush_rollout().await
    }
    ...
    pub async fn submit_with_trace(
        ...
        self.thinwedge.submit_with_trace(op, trace).await
    }
    ...
    pub async fn set_thread_memory_mode(&self, mode: ThreadMemoryMode) -> anyhow::Result<()> {
        self.thinwedge.set_thread_memory_mode(mode).await
    }
    ...
    /// callers that only hold a `ThinWedgeThread`.
    /// It returns the unchanged items when this thread has no active turn.
    pub async fn inject_if_running(
        &self,
        items: Vec<ResponseItem>,
    ) -> Result<(), Vec<ResponseItem>> {
        self.thinwedge.session.inject_if_running(items).await
    }
    ...
    pub async fn try_start_turn_if_idle(
        ...
        self.thinwedge.session.try_start_turn_if_idle(items).await
    }
    ...
    pub async fn preview_thread_settings_overrides(
        &self,
        overrides: ThinWedgeThreadSettingsOverrides,
    ) -> ConstraintResult<ThreadConfigSnapshot> {
        let updates = self.thread_settings_update(overrides).await;
        self.thinwedge.session.preview_settings(&updates).await
    }
    ...
    async fn thread_settings_update(
        ...
        let ThinWedgeThreadSettingsOverrides {
            environments,
            workspace_roots,
    ...
    pub async fn submit_with_id(&self, sub: Submission) -> ThinWedgeResult<()> {
        self.thinwedge.submit_with_id(sub).await
    }

    pub async fn next_event(&self) -> ThinWedgeResult<Event> {
        self.thinwedge.next_event().await
    }

    pub async fn agent_status(&self) -> AgentStatus {
        self.thinwedge.agent_status().await
    }

    pub async fn list_background_terminals(&self) -> Vec<BackgroundTerminalInfo> {
        self.thinwedge.session.list_background_terminals().await
    }
    ...
    pub(crate) fn subscribe_status(&self) -> watch::Receiver<AgentStatus> {
        self.thinwedge.agent_status.clone()
    }
    ...
    pub async fn token_usage_info(&self) -> Option<TokenUsageInfo> {
        self.thinwedge.session.token_usage_info().await
    }
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> ThinWedgeResult<()> {
        if items.is_empty() {
            return Err(ThinWedgeErr::InvalidRequest(
    ...
    pub(crate) fn is_running(&self) -> bool {
        !self.thinwedge.tx_sub.is_closed()
    }
    ...
    pub fn state_db(&self) -> Option<StateDbHandle> {
        self.thinwedge.state_db()
    }

    pub async fn config_snapshot(&self) -> ThreadConfigSnapshot {
        self.thinwedge.thread_config_snapshot().await
    }
    ...
    pub async fn instruction_sources(&self) -> Vec<AbsolutePathBuf> {
        self.thinwedge.instruction_sources().await
    }

    pub async fn config(&self) -> Arc<crate::config::Config> {
        self.thinwedge.session.get_config().await
    }
    ...
    ) -> thinwedge_mcp::McpConfig {
        self.thinwedge.session.runtime_mcp_config(config).await
    }

    pub fn multi_agent_version(&self) -> Option<MultiAgentVersion> {
        self.thinwedge.session.multi_agent_version()
    }
    ...
    pub async fn environment_selections(&self) -> Vec<TurnEnvironmentSelection> {
        self.thinwedge.thread_environment_selections().await
    }
    ...
    pub fn enabled(&self, feature: Feature) -> bool {
        self.thinwedge.enabled(feature)
    }

    pub async fn increment_out_of_band_elicitation_count(&self) -> ThinWedgeResult<u64> {
        let mut guard = self.out_of_band_elicitation_count.lock().await;
        let was_zero = *guard == 0;
        *guard = guard.checked_add(1).ok_or_else(|| {
            ThinWedgeErr::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> ThinWedgeResult<u64> {
        let mut guard = self.out_of_band_elicitation_count.lock().await;
        if *guard == 0 {
            return Err(ThinWedgeErr::InvalidRequest(
