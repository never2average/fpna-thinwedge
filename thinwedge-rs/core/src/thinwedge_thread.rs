...
pub struct ThinWedgeThreadTurnContextOverrides {
...
impl ThinWedgeThread {
    ...
    pub async fn apply_external_goal_set(&self, status: thinwedge_state::ThreadGoalStatus) {
        if let Err(err) = self
            .thinwedge
            .session
    ...
    pub async fn validate_turn_context_overrides(
        &self,
        overrides: ThinWedgeThreadTurnContextOverrides,
    ) -> ConstraintResult<()> {
        let ThinWedgeThreadTurnContextOverrides {
            cwd,
            approval_policy,
            approvals_reviewer,
            sandbox_policy,
        ...
        self.thinwedge.session.validate_settings(&updates).await
    }
    ...
    #[cfg(test)]
    pub(crate) async fn append_message(&self, message: ResponseItem) -> ThinWedgeResult<String> {
...
fn pending_message_input_item(message: &ResponseItem) -> ThinWedgeResult<ResponseInputItem> {
    match message {
        ...
        _ => Err(ThinWedgeErr::InvalidRequest(
