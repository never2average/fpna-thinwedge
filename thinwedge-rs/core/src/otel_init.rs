...
/// Filter predicate for exporting only ThinWedge-owned events via OTEL.
/// Keeps events that originated from thinwedge_otel module
pub fn thinwedge_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("thinwedge_otel")
}
...
pub fn record_process_start(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let _ = thinwedge_otel::record_process_start_once(metrics, originator);
}
...
pub fn install_sqlite_telemetry(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let telemetry = thinwedge_rollout::sqlite_telemetry_recorder(metrics.clone(), originator);
