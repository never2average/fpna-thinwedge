...
use thinwedge_exec_server::ExecutorFileSystemFuture;
use thinwedge_exec_server::FileMetadata;
use thinwedge_exec_server::FileSystemSandboxContext;
use thinwedge_exec_server::LOCAL_FS;
use thinwedge_exec_server::ReadDirectoryEntry;
...
async fn make_config(root: &TempDir, limit: usize, instructions: Option<&str>) -> TestConfig {
    ...
    let user_instructions = instructions.map(|text| UserInstructions {
        text: text.to_owned(),
        source: config.thinwedge_home.join(DEFAULT_AGENTS_MD_FILENAME),
    });
    TestConfig {
        config,
        user_instructions,
...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".thinwedge-root"), "").unwrap();
...
async fn project_layers_do_not_override_project_root_markers() {
    ...
    config.cwd = nested.abs();
    let project_layer = |dot_thinwedge_folder: AbsolutePathBuf, marker: &str| {
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![
            project_layer(root.path().join(".thinwedge").abs(), ".ignored-root-marker"),
            project_layer(config.cwd.join(".thinwedge"), ".ignored-nested-marker"),
        ],
...
async fn child_agents_message_after_global_instructions_uses_plain_separator() {
    ...
    let loaded = load_agents_md(&cfg, &mut warnings)
        .await
        .expect("instructions expected");
    let global_agents = cfg.thinwedge_home.join(DEFAULT_AGENTS_MD_FILENAME);
    let expected = LoadedAgentsMd {
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.thinwedge_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.thinwedge_home).unwrap();
...
async fn skills_are_not_appended_to_agents_md() {
    ...
    let cfg = make_config(&tmp, /*limit*/ 4096, /*instructions*/ None).await;
    create_skill(
        cfg.thinwedge_home.to_path_buf(),
        "pdf-processing",
        "extract from pdfs",
    );
...
fn create_skill(thinwedge_home: PathBuf, name: &str, description: &str) {
    let skill_dir = thinwedge_home.join(format!("skills/{name}"));
