...
command transformation live in the thinwedge-sandboxing crate; this module keeps
...
pub(crate) struct ExecServerEnvConfig {
    pub(crate) policy: thinwedge_exec_server::ExecEnvPolicy,
...
impl ExecRequest {
    ...
    pub(crate) fn from_sandbox_exec_request(
        ...
        if !network_sandbox_policy.is_enabled() {
            env.insert(
                THINWEDGE_SANDBOX_NETWORK_DISABLED_ENV_VAR.to_string(),
                "1".to_string(),
            );
        }
        #[cfg(target_os = "macos")]
...
) -> thinwedge_protocol::error::Result<ExecToolCallOutput> {
