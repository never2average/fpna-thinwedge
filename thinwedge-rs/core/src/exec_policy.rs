...
use thinwedge_execpolicy::AmendError;
use thinwedge_execpolicy::Decision;
use thinwedge_execpolicy::Error as ExecPolicyRuleError;
...
use thinwedge_execpolicy::blocking_append_network_rule;
...
use thinwedge_shell_command::is_dangerous_command::command_might_be_dangerous;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(thinwedge_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: thinwedge_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(thinwedge_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &thinwedge_execpolicy::Error) -> String {
...
pub(crate) fn render_decision_for_unmatched_command(
    ...
    let is_known_safe = match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            thinwedge_shell_command::is_safe_command::is_safe_powershell_words(command)
        }
    };
    ...
    let command_is_dangerous = match command_origin {
        ...
        #[cfg(windows)]
        ExecPolicyCommandOrigin::PowerShell => {
            thinwedge_shell_command::is_dangerous_command::is_dangerous_powershell_words(command)
        }
    };
...
fn commands_for_exec_policy(command: &[String]) -> ExecPolicyCommands {
    ...
    #[cfg(windows)]
    {
        if let Some(commands) =
            thinwedge_shell_command::powershell::parse_powershell_command_into_plain_commands(
                command,
            )
            && !commands.is_empty()
        {
...
/// - Note: we only use this amendment when the command fails to run in sandbox and thinwedge prompts the user to run outside the sandbox
