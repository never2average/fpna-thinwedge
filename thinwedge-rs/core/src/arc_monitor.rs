...
const THINWEDGE_ARC_MONITOR_ENDPOINT_OVERRIDE: &str = "THINWEDGE_ARC_MONITOR_ENDPOINT_OVERRIDE";
const THINWEDGE_ARC_MONITOR_TOKEN: &str = "THINWEDGE_ARC_MONITOR_TOKEN";
...
pub(crate) async fn monitor_action(
    ...
    let auth = match turn_context.auth_manager.as_ref() {
        Some(auth_manager) => match auth_manager.auth().await {
            Some(auth) if auth.uses_thinwedge_backend() => Some(auth),
            _ => None,
        },
        None => None,
    };
    let env_token = read_non_empty_env_var(THINWEDGE_ARC_MONITOR_TOKEN);
...
async fn build_arc_monitor_request(
    ...
    ArcMonitorRequest {
        metadata: ArcMonitorMetadata {
            thinwedge_thread_id: conversation_id.clone(),
