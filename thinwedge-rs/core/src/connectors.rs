...
pub use thinwedge_app_server_protocol::AppMetadata;
...
use thinwedge_mcp::tool_plugin_provenance;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = thinwedge_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        thinwedge_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_cached_accessible_connectors_from_mcp_tools(
    ...
    read_cached_accessible_connectors(&cache_key).map(|connectors| {
        thinwedge_connectors::filter::filter_disallowed_connectors(
            connectors,
            originator().value.as_str(),
        )
    })
...
pub(crate) fn refresh_accessible_connectors_cache_from_mcp_tools(
    ...
    let accessible_connectors = thinwedge_connectors::filter::filter_disallowed_connectors(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let environment_manager = EnvironmentManager::from_thinwedge_home(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_mcp_manager(
    ...
    {
        let cached_connectors = thinwedge_connectors::filter::filter_disallowed_connectors(
            cached_connectors,
            originator().value.as_str(),
        );
    ...
    mcp_servers.retain(|name, _| name == THINWEDGE_APPS_MCP_SERVER_NAME);
    let host_owned_thinwedge_apps_enabled =
        host_owned_thinwedge_apps_enabled(&mcp_config, auth.as_ref());
    ...
    let mcp_connection_manager = McpConnectionManager::new(
        ...
        thinwedge_apps_tools_cache_key(auth.as_ref()),
    ...
    let refreshed_tools = if force_refetch {
        match mcp_connection_manager
            .hard_refresh_thinwedge_apps_tools_cache()
            .await
        {
            Ok(tools) => Some(tools),
            Err(err) => {
                warn!(
                    "failed to force-refresh tools for MCP server '{THINWEDGE_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
                );
                None
            }
        }
    ...
    let thinwedge_apps_ready = if refreshed_tools_succeeded {
        true
    } else if let Some(cfg) = mcp_servers.get(THINWEDGE_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_connection_manager
            .wait_for_server_ready(THINWEDGE_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_connection_manager
                .wait_for_server_ready(THINWEDGE_APPS_MCP_SERVER_NAME, timeout)
                .await;
    ...
    if thinwedge_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
...
fn accessible_connectors_cache_key(
    ...
    let chatgpt_user_id = auth.and_then(ThinWedgeAuth::get_chatgpt_user_id);
    let is_workspace_account = auth.is_some_and(ThinWedgeAuth::is_workspace_account);
    AccessibleConnectorsCacheKey {
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + thinwedge_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    let Some(auth) = auth.filter(|auth| auth.uses_thinwedge_backend()) else {
        return Vec::new();
    };
    ...
    thinwedge_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
pub(crate) fn accessible_connectors_from_mcp_tools(mcp_tools: &[ToolInfo]) -> Vec<AppInfo> {
    ...
    let tools = mcp_tools.iter().filter_map(|tool| {
        if tool.server_name != THINWEDGE_APPS_MCP_SERVER_NAME {
            return None;
        }
        let connector_id = tool.connector_id.as_deref()?;
        Some(thinwedge_connectors::accessible::AccessibleConnectorTool {
    ...
    thinwedge_connectors::accessible::collect_accessible_connectors(tools)
}
...
pub(crate) fn thinwedge_app_tool_is_enabled(config: &Config, tool_info: &ToolInfo) -> bool {
    if tool_info.server_name != THINWEDGE_APPS_MCP_SERVER_NAME {
        return true;
    }
...
pub(crate) fn mcp_approvals_reviewer(
    ...
    let app_reviewer = if server_name == THINWEDGE_APPS_MCP_SERVER_NAME {
