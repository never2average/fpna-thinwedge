...
pub async fn list_accessible_connectors_from_mcp_tools_with_environment_manager(
    ...
    let mcp_servers = with_thinwedge_apps_mcp(HashMap::new(), auth.as_ref(), &mcp_config);
...
async fn tool_suggest_connector_ids(config: &Config) -> HashSet<String> {
    let mut connector_ids = PluginsManager::new(config.thinwedge_home.to_path_buf())
...
async fn list_directory_connectors_for_tool_suggest_with_auth(
    ...
    thinwedge_connectors::list_all_connectors_with_options(
        cache_key,
        is_workspace_account,
        /*force_refetch*/ false,
        |path| {
