...
use thinwedge_config::types::AppsDefaultConfig;
use thinwedge_connectors::filter::filter_disallowed_connectors;
use thinwedge_connectors::filter::filter_tool_suggest_discoverable_connectors;
use thinwedge_connectors::merge::merge_connectors;
use thinwedge_connectors::merge::plugin_connector_to_app_info;
use thinwedge_connectors::metadata::connector_install_url;
use thinwedge_connectors::metadata::connector_mention_slug;
...
fn thinwedge_app_tool(
    ...
    let tool_namespace = connector_name
        .map(sanitize_name)
        .map(|connector_name| format!("mcp__{THINWEDGE_APPS_MCP_SERVER_NAME}__{connector_name}"))
        .unwrap_or_else(|| THINWEDGE_APPS_MCP_SERVER_NAME.to_string());
...
async fn cloud_requirements_disable_connector_applies_without_user_apps_table() {
    let thinwedge_home = tempdir().expect("tempdir should succeed");
    std::fs::write(thinwedge_home.path().join(CONFIG_TOML_FILE), "").expect("write config");
...
async fn local_requirements_disable_connector_overrides_user_apps_config() {
    ...
    let config_toml_path =
        AbsolutePathBuf::try_from(thinwedge_home.path().join(CONFIG_TOML_FILE)).expect("abs path");
    let mut config = ConfigBuilder::default()
        .thinwedge_home(thinwedge_home.path().to_path_buf())
        .fallback_cwd(Some(thinwedge_home.path().to_path_buf()))
        .build()
...
fn filter_disallowed_connectors_allows_non_disallowed_connectors() {
    let filtered =
        filter_disallowed_connectors(vec![app("asdk_app_hidden"), app("alpha")], "thinwedge_cli");
