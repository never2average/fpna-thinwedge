...
use thinwedge_config::AppToolRequirementToml;
use thinwedge_config::AppToolsRequirementsToml;
...
"#
        ...
        assert_eq!(
            mcp_approvals_reviewer(&config, THINWEDGE_APPS_MCP_SERVER_NAME, Some("calendar")),
            expected_app
        );
        assert_eq!(
            mcp_approvals_reviewer(&config, THINWEDGE_APPS_MCP_SERVER_NAME, Some("drive")),
            expected_default
        );
        assert_eq!(
...
"#,
    ...
    let plugins_manager = PluginsManager::new(config.thinwedge_home.to_path_buf());
