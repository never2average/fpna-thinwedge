...
/// (thinwedge-linux-sandbox), which defaults to bubblewrap for filesystem
    ...
    P: AsRef<Path>,
{
    ...
    let thinwedge_linux_sandbox_exe = thinwedge_linux_sandbox_exe.as_ref();
    ...
    let arg0 = if thinwedge_linux_sandbox_exe
        .file_name()
        ...
        == Some(THINWEDGE_LINUX_SANDBOX_ARG0)
    {
        ...
        thinwedge_linux_sandbox_exe.to_string_lossy().into_owned()
    } else {
        THINWEDGE_LINUX_SANDBOX_ARG0.to_string()
    };
    spawn_child_async(SpawnChildRequest {
        program: thinwedge_linux_sandbox_exe.to_path_buf(),
        args,
        arg0: Some(&arg0),
        cwd: command_cwd,
