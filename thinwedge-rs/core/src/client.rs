...
//!
//! `ModelClient` is intended to live for the lifetime of a ThinWedge session and holds the stable
...
//! per-turn state such as the `x-thinwedge-turn-state` token used for sticky routing.
//!
...
use thinwedge_api::CompactClient as ApiCompactClient;
use thinwedge_api::CompactionInput as ApiCompactionInput;
use thinwedge_api::Compression;
use thinwedge_api::MemoriesClient as ApiMemoriesClient;
use thinwedge_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use thinwedge_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use thinwedge_api::Provider as ApiProvider;
use thinwedge_api::RawMemory as ApiRawMemory;
use thinwedge_api::RealtimeCallClient as ApiRealtimeCallClient;
use thinwedge_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use thinwedge_api::Reasoning;
use thinwedge_api::ReasoningContext;
use thinwedge_api::RequestTelemetry;
use thinwedge_api::ReqwestTransport;
use thinwedge_api::ResponseCreateWsRequest;
use thinwedge_api::ResponsesApiRequest;
use thinwedge_api::ResponsesClient as ApiResponsesClient;
use thinwedge_api::ResponsesOptions as ApiResponsesOptions;
use thinwedge_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use thinwedge_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use thinwedge_api::ResponsesWsRequest;
use thinwedge_api::SharedAuthProvider;
use thinwedge_api::SseTelemetry;
use thinwedge_api::TransportError;
use thinwedge_api::WebsocketTelemetry;
use thinwedge_api::auth_header_telemetry;
use thinwedge_api::build_session_headers;
use thinwedge_api::create_text_param_for_request;
use thinwedge_api::response_create_client_metadata;
...
use thinwedge_protocol::protocol::RealtimeConversationArchitecture;
use thinwedge_protocol::protocol::SessionSource;
use thinwedge_protocol::protocol::W3cTraceContext;
use thinwedge_rollout_trace::CompactionTraceContext;
use thinwedge_rollout_trace::InferenceTraceAttempt;
use thinwedge_rollout_trace::InferenceTraceContext;
use thinwedge_tools::create_tools_json_for_responses_api;
use tokio::sync::mpsc;
...
#[cfg(test)]
use thinwedge_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
...
use thinwedge_response_debug_context::extract_response_debug_context_from_api_error;
...
const X_THINWEDGE_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
    "x-thinwedge-ws-stream-request-start-ms";
const WS_REQUEST_HEADER_RESPONSES_LITE_CLIENT_METADATA_KEY: &str =
    "ws_request_header_x_openai_internal_thinwedge_responses_lite";
const RESPONSES_WEBSOCKETS_V2_BETA_HEADER_VALUE: &str = "responses_websockets=2026-02-06";
const X_OPENAI_INTERNAL_THINWEDGE_RESPONSES_LITE_HEADER: &str =
    "x-openai-internal-thinwedge-responses-lite";
...
///
/// This holds configuration and state that should be shared across turns within a ThinWedge session
...
/// - The `x-thinwedge-turn-state` sticky-routing token, which must be replayed for all requests within
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each ThinWedge turn. Reusing it across turns would replay
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-thinwedge-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-thinwedge-turn-state` request header for
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a ThinWedge session. Per-turn values
    ...
    pub fn new(
        ...
        let thinwedge_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.thinwedge_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), thinwedge_api_key_env_enabled);
    ...
    pub(crate) fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "thinwedge.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    pub(crate) async fn compact_conversation_history(
        ...
        responses_metadata: &ThinWedgeResponsesMetadata,
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(ThinWedgeAuth::auth_mode),
        ...
        if let Ok(header_value) = HeaderValue::from_str(&responses_metadata.installation_id) {
            extra_headers.insert(X_THINWEDGE_INSTALLATION_ID_HEADER, header_value);
        }
    ...
    fn build_responses_request(
        &self,
        provider: &thinwedge_api::Provider,
        prompt: &Prompt,
        model_info: &ModelInfo,
        effort: Option<ReasoningEffortConfig>,
        summary: ReasoningSummaryConfig,
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: thinwedge_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &ThinWedgeResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth).connect(
                headers,
                thinwedge_login::default_client::default_headers(),
                /*turn_state*/ None,
...
impl ModelClientSession {
    ...
    async fn stream_responses_websocket(
        ...
        loop {
            ...
            if let Some(turn_state) = self.turn_state.get() {
                client_metadata.insert(
                    X_THINWEDGE_TURN_STATE_HEADER.to_string(),
                    turn_state.clone(),
                );
            }
    ...
    /// Permanently disables WebSockets for this ThinWedge session and resets WebSocket state.
    ///
...
fn stamp_ws_stream_request_start_ms(request: &mut ResponsesWsRequest) {
    ...
    payload
        ...
        .insert(
            X_THINWEDGE_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY.to_string(),
...
///
/// These headers implement ThinWedge-specific conventions:
///
/// - `x-thinwedge-beta-features`: comma-separated beta feature keys enabled for the session.
/// - `x-thinwedge-turn-state`: sticky routing token captured earlier in the turn.
fn build_responses_headers(
    ...
    {
        headers.insert("x-thinwedge-beta-features", header_value);
    }
    if let Some(turn_state) = turn_state
        && let Some(state) = turn_state.get()
        && let Ok(header_value) = HeaderValue::from_str(state)
    {
        headers.insert(X_THINWEDGE_TURN_STATE_HEADER, header_value);
    }
    headers
}
...
fn add_responses_lite_header(headers: &mut ApiHeaderMap, use_responses_lite: bool) {
    if use_responses_lite {
        headers.insert(
            X_OPENAI_INTERNAL_THINWEDGE_RESPONSES_LITE_HEADER,
            HeaderValue::from_static("true"),
        );
    }
}
...
fn map_response_stream(
    api_stream: thinwedge_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let thinwedge_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = thinwedge_api::ResponseStream {
        rx_event,
...
/// the mapped `ThinWedgeErr` is returned to the caller.
...
struct WebsocketConnectParams<'a> {
    ...
    api_provider: thinwedge_api::Provider,
    api_auth: SharedAuthProvider,
    responses_metadata: &'a ThinWedgeResponsesMetadata,
    auth_context: AuthRequestTelemetryContext,
    request_route_telemetry: RequestRouteTelemetry,
}
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(ThinWedgeErr::RefreshTokenFailed(failed))
            }
