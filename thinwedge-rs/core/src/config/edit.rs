...
pub fn apply_blocking(thinwedge_home: &Path, edits: &[ConfigEdit]) -> anyhow::Result<()> {
...
///
pub async fn apply(thinwedge_home: &Path, edits: Vec<ConfigEdit>) -> anyhow::Result<()> {
...
impl ConfigEditsBuilder {
    pub fn new(thinwedge_home: &Path) -> Self {
        Self::for_config_path(&thinwedge_home.join(CONFIG_TOML_FILE))
    }
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .map(thinwedge_utils_absolute_path::AbsolutePathBuf::to_path_buf)
            .unwrap_or_else(|| config.thinwedge_home.join(CONFIG_TOML_FILE).to_path_buf());
