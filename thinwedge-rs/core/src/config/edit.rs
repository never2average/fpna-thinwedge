...
pub enum ConfigEdit {
    ...
    /// Toggle the opt-out marker for ThinWedge-managed fast defaults.
...
pub async fn apply(
    ...
    task::spawn_blocking(move || apply_blocking(&thinwedge_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.thinwedge_home, self.profile.as_deref(), &self.edits)
    }
