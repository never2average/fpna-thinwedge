...
use thinwedge_config::ProfileV2Name;
use thinwedge_config::RequirementSource;
use thinwedge_config::Sourced;
use thinwedge_config::config_toml::AgentRoleToml;
use thinwedge_config::config_toml::AgentsToml;
use thinwedge_config::config_toml::AutoReviewToml;
use thinwedge_config::config_toml::ConfigToml;
use thinwedge_config::config_toml::ExperimentalRequestUserInput;
use thinwedge_config::config_toml::ProjectConfig;
use thinwedge_config::config_toml::RealtimeArchitecture;
use thinwedge_config::config_toml::RealtimeConfig;
use thinwedge_config::config_toml::RealtimeToml;
use thinwedge_config::config_toml::RealtimeTransport;
use thinwedge_config::config_toml::RealtimeWsMode;
use thinwedge_config::config_toml::RealtimeWsVersion;
use thinwedge_config::config_toml::ToolsToml;
...
use thinwedge_config::permissions_toml::NetworkDomainPermissionToml;
use thinwedge_config::permissions_toml::NetworkDomainPermissionsToml;
use thinwedge_config::permissions_toml::NetworkMitmActionToml;
use thinwedge_config::permissions_toml::NetworkMitmHookToml;
use thinwedge_config::permissions_toml::NetworkMitmToml;
...
use thinwedge_config::permissions_toml::WorkspaceRootsToml;
use thinwedge_config::types::AppToolApproval;
use thinwedge_config::types::ApprovalsReviewer;
use thinwedge_config::types::BundledSkillsConfig;
use thinwedge_config::types::FeedbackConfigToml;
...
use thinwedge_config::types::MemoriesConfig;
use thinwedge_config::types::MemoriesToml;
use thinwedge_config::types::ModelAvailabilityNuxConfig;
use thinwedge_config::types::Notice;
use thinwedge_config::types::NotificationCondition;
use thinwedge_config::types::NotificationMethod;
use thinwedge_config::types::Notifications;
use thinwedge_config::types::OtelConfigToml;
use thinwedge_config::types::OtelExporterKind;
use thinwedge_config::types::SandboxWorkspaceWrite;
use thinwedge_config::types::SessionPickerViewMode;
use thinwedge_config::types::SkillsConfig;
use thinwedge_config::types::ToolSuggestDisabledTool;
use thinwedge_config::types::ToolSuggestDiscoverableType;
use thinwedge_config::types::Tui;
use thinwedge_config::types::TuiKeymap;
use thinwedge_config::types::TuiNotificationSettings;
use thinwedge_config::types::TuiPetAnchor;
...
[features.code_mode]
enabled = true
excluded_tool_namespaces = ["mcp__thinwedge_apps", "multi_agent_v1"]
"#,
    )
    .expect("TOML deserialization should succeed");
    let config = Config::load_from_base_config_with_overrides(
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        thinwedge_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let memories_root = thinwedge_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let thinwedge_home = temp_dir.path().join("thinwedge-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".thinwedge"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".thinwedge".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".thinwedge"), cwd.as_path()),
            "expected .thinwedge carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".thinwedge"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".thinwedge"] {
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of ThinWedge and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of ThinWedge and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of ThinWedge."
        )),
        "{:?}",
        config.startup_warnings
    );
...
fn filter_plugin_mcp_servers_by_allowlist_enforces_plugin_and_identity_rules() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            thinwedge_config::PluginRequirementsToml {
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, thinwedge_home.path());
    let project_dot_thinwedge =
        AbsolutePathBuf::resolve_path_against_base("project/.thinwedge", thinwedge_home.path());
    ...
    let requirements_toml = thinwedge_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = thinwedge_config::ConfigRequirements {
    ...
    let refreshed_layer_stack = ConfigLayerStack::new(
        vec![
            ConfigLayerEntry::new(
                thinwedge_app_server_protocol::ConfigLayerSource::User {
            ...
            ConfigLayerEntry::new(
                thinwedge_app_server_protocol::ConfigLayerSource::Project {
                    dot_thinwedge_folder: project_dot_thinwedge.clone(),
                },
                toml::toml! {
            ...
            ConfigLayerEntry::new(
                thinwedge_app_server_protocol::ConfigLayerSource::LegacyManagedConfigTomlFromMdm,
                toml::toml! {
                    [mcp_servers.managed_overrides_session]
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(thinwedge_home.path().to_path_buf()),
            ..Default::default()
        },
        thinwedge_home.abs(),
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                thinwedge_app_server_protocol::ConfigLayerSource::Project {
                    dot_thinwedge_folder: project_dot_thinwedge,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
            ...
            ConfigLayerEntry::new(
                thinwedge_app_server_protocol::ConfigLayerSource::SessionFlags,
                toml::toml! {
                    [mcp_servers.session_overrides_user]
...
-> anyhow::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let plugin_root = thinwedge_home
        .path()
        .join("plugins/cache")
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let default_zsh_path = thinwedge_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_thinwedge_home_for_workspace_write() -> std::io::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, thinwedge_home.path().to_path_buf());
...
-> std::io::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let memories_root = thinwedge_home.path().join("memories");
    let writable_root = thinwedge_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
"#,
    ...
    let project_config_dir = workspace.path().join(".thinwedge");
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let managed_path = thinwedge_home.path().join("managed_config.toml");
    let config_path = thinwedge_home.path().join(CONFIG_TOML_FILE);
    ...
    let cwd = thinwedge_home.path().abs();
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    ...
    let loaded = load_global_mcp_servers(thinwedge_home.path()).await?;
    ...
    match &docs.transport {
        McpServerTransportConfig::Stdio {
            ...
            assert_eq!(cwd, &Some(thinwedge_home.path().to_path_buf()));
        }
...
"#,
    ...
    assert_eq!(
        ...
        Some(&serde_json::json!("thinwedge"))
    );
    assert_eq!(
...
"#,
    ...
    let err = load_global_mcp_servers(thinwedge_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/thinwedge-mcp");
    let servers = BTreeMap::from([(
        "docs".to_string(),
        McpServerConfig {
            transport: McpServerTransportConfig::Stdio {
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/thinwedge-mcp""#),
    ...
    match &docs.transport {
        McpServerTransportConfig::Stdio { cwd, .. } => {
            assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/thinwedge-mcp")));
        }
        other => panic!("unexpected transport {other:?}"),
    }
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-thinwedge-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-thinwedge-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(thinwedge_home.path())
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let base_config = thinwedge_home.path().join(CONFIG_TOML_FILE);
    let selected_config = thinwedge_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(thinwedge_home.path(), &profile_name),
        thinwedge_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn thinwedge_home(&self) -> AbsolutePathBuf {
        self.thinwedge_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let workspace = thinwedge_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, thinwedge_home.abs()).await?;
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        thinwedge_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let missing_path = thinwedge_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let role_config_path = thinwedge_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = thinwedge_config::ConfigLayerStack::new(
        vec![thinwedge_config::ConfigLayerEntry::new(
            thinwedge_app_server_protocol::ConfigLayerSource::User {
                file: thinwedge_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        thinwedge_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".thinwedge").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let researcher_path = thinwedge_home.path().join("agents").join("researcher.toml");
    let reviewer_path = thinwedge_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = thinwedge_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let catalog_path = thinwedge_home.path().join("catalog.json");
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: thinwedge_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let thinwedge_home = fixture.thinwedge_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirement_source = thinwedge_config::RequirementSource::Unknown;
    ...
    let requirements = thinwedge_config::ConfigRequirements {
        web_search_mode: thinwedge_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        thinwedge_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
projects = { "/Users/mbolin/code/thinwedge4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/thinwedge3" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/thinwedge2");
...
[projects."/Users/mbolin/code/thinwedge4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/thinwedge3"]
trust_level = "trusted"
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let thinwedge_home = temp_dir.path();
    let config_path = thinwedge_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(thinwedge_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(thinwedge_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(thinwedge_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(thinwedge_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
"#,
    ...
    assert_eq!(
        ...
        Some(thinwedge_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; ThinWedge would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode."
    );
    Ok(())
}
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let cwd = thinwedge_home.path().join("workspace");
    let outside_root = thinwedge_home.path().join("outside-write");
...
export_dir = "locks"
allow_thinwedge_version_mismatch = true
...
"#,
    ...
    assert!(config.config_lock_allow_thinwedge_version_mismatch);
...
async fn debug_config_lockfile_load_path_loads_lock_from_nested_table() -> std::io::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let lock_path = thinwedge_home.path().join("session.config.lock.toml");
    std::fs::write(
        &lock_path,
        format!(
            r#"version = {}
thinwedge_version = "older-version"
...
-> std::io::Result<()> {
    let thinwedge_home = TempDir::new()?;
    let selected_config = thinwedge_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
