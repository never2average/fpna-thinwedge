...
async fn load_config_loads_global_agents_instructions() -> std::io::Result<()> {
    let thinwedge_home = tempdir()?;
    std::fs::write(
        thinwedge_home.path().join(DEFAULT_AGENTS_MD_FILENAME),
        "\n  global instructions  \n",
    )?;
...
async fn load_config_prefers_global_agents_override_instructions() -> std::io::Result<()> {
    ...
    let global_agents_override_path = thinwedge_home.path().join(LOCAL_AGENTS_MD_FILENAME);
...
-> std::io::Result<()> {
    ...
    let memories_root = AbsolutePathBuf::from_absolute_path(std::fs::canonicalize(
        thinwedge_home.path().join("memories"),
    )?)?;
    assert!(
        config
            .permissions
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `workspace` does not define any recognized filesystem entries for this version of ThinWedge."
        )),
        "{:?}",
        config.startup_warnings
    );
...
#[tokio::test]
async fn replace_mcp_servers_streamable_http_serializes_oauth_client_id() -> anyhow::Result<()> {
    let thinwedge_home = TempDir::new()?;
...
async fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
    ...
    assert_eq!(
        Config {
            ...
            thinwedge_home: fixture.thinwedge_home(),
            sqlite_home: fixture.thinwedge_home().to_path_buf(),
            log_dir: fixture.thinwedge_home().join("log").to_path_buf(),
            config_layer_stack: Default::default(),
            startup_warnings: Vec::new(),
            history: History::default(),
            ephemeral: false,
            file_opener: UriBasedFileOpener::VsCode,
            thinwedge_self_exe: None,
...
"#,
    ...
    let requirements = thinwedge_config::ConfigRequirementsToml {
        allowed_approval_policies: None,
        allowed_approvals_reviewers: None,
