...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![thinwedge_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(thinwedge_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(thinwedge_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            ...
            thinwedge_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(project_root.join(".thinwedge")).await?;
...
"#;
    ...
    .await?;
    tokio::fs::write(
        nested.join(".thinwedge").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let thinwedge_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&thinwedge_home).await?;
    tokio::fs::write(thinwedge_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn thinwedge_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
...
#[tokio::test]
async fn thinwedge_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_thinwedge = project_root.join(".thinwedge");
    ...
    make_config_for_test(
        &project_dot_thinwedge,
        &project_root,
        TrustLevel::Trusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let user_config_path = project_dot_thinwedge.join(CONFIG_TOML_FILE);
...
async fn project_layers_disabled_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    let thinwedge_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&thinwedge_home_untrusted).await?;
    make_config_for_test(
        &thinwedge_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = thinwedge_home_untrusted.join(CONFIG_TOML_FILE);
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".thinwedge")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".thinwedge").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let thinwedge_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&thinwedge_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let thinwedge_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&thinwedge_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &thinwedge_home,
...
mod requirements_exec_policy_tests {
    ...
    use thinwedge_config::ConfigRequirementsWithSources;
    use thinwedge_config::RequirementSource;
    use thinwedge_config::RequirementsExecPolicyDecisionToml;
    use thinwedge_config::RequirementsExecPolicyParseError;
    use thinwedge_config::RequirementsExecPolicyPatternTokenToml;
    use thinwedge_config::RequirementsExecPolicyPrefixRuleToml;
    use thinwedge_config::RequirementsExecPolicyToml;
    ...
    fn config_stack_for_dot_thinwedge_folder_with_requirements(
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_thinwedge_folder_with_requirements(temp_dir.path(), requirements);
