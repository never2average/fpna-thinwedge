...
async fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let thinwedge_home = temp_dir.path().join(".thinwedge");
    let zsh_path = temp_dir.path().join("runtime").join("zsh");
    let arg0_root = thinwedge_home.join("tmp").join("arg0");
    let allowed_arg0_dir = arg0_root.join("thinwedge-arg0-session");
    let sibling_arg0_dir = arg0_root.join("thinwedge-arg0-other-session");
    let execve_wrapper = allowed_arg0_dir.join("thinwedge-execve-wrapper");
    ...
    let config = Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(&thinwedge_home)?,
    )
    .await?;
...
fn network_toml_overlays_unix_socket_permissions_by_path() {
    ...
    assert_eq!(
        ...
        Some(thinwedge_network_proxy::NetworkUnixSocketPermissions {
            entries: BTreeMap::from([
                (
                    "/tmp/base.sock".to_string(),
