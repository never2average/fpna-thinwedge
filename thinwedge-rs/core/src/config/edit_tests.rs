...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = thinwedge_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let thinwedge_home = tmp.path();
    let link_a = thinwedge_home.join("a.toml");
    let link_b = thinwedge_home.join("b.toml");
    let config_path = thinwedge_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(thinwedge_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
#[test]
fn blocking_set_hide_gpt_5_1_thinwedge_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let thinwedge_home = tmp.path();
    std::fs::write(
        thinwedge_home.join(CONFIG_TOML_FILE),
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !thinwedge_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
    ...
    contents = std::fs::read_to_string(thinwedge_home.join(CONFIG_TOML_FILE)).expect("read config");
