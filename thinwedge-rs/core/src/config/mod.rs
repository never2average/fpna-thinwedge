...
use thinwedge_config::ConstrainedWithSource;
use thinwedge_config::FeatureRequirementsToml;
use thinwedge_config::McpServerIdentity;
use thinwedge_config::McpServerRequirement;
use thinwedge_config::PluginRequirementsToml;
use thinwedge_config::ProfileV2Name;
use thinwedge_config::ResidencyRequirement;
use thinwedge_config::SandboxModeRequirement;
use thinwedge_config::Sourced;
use thinwedge_config::ThreadConfigLoader;
use thinwedge_config::config_toml::ConfigLockfileToml;
use thinwedge_config::config_toml::ConfigToml;
use thinwedge_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use thinwedge_config::config_toml::ProjectConfig;
use thinwedge_config::config_toml::RealtimeAudioConfig;
use thinwedge_config::config_toml::RealtimeConfig;
use thinwedge_config::config_toml::ThreadStoreToml;
use thinwedge_config::config_toml::validate_model_providers;
...
use thinwedge_config::sandbox_mode_requirement_for_permission_profile;
...
use thinwedge_config::types::History;
...
use thinwedge_config::types::UriBasedFileOpener;
use thinwedge_config::types::WindowsSandboxModeToml;
use thinwedge_core_plugins::PluginsConfigInput;
use thinwedge_exec_server::ExecutorFileSystem;
use thinwedge_exec_server::LOCAL_FS;
use thinwedge_features::CodeModeConfigToml;
use thinwedge_features::Feature;
use thinwedge_features::FeatureConfigSource;
use thinwedge_features::FeatureOverrides;
use thinwedge_features::FeatureToml;
use thinwedge_features::Features;
use thinwedge_features::FeaturesToml;
use thinwedge_features::MultiAgentV2ConfigToml;
use thinwedge_features::NetworkProxyConfigToml;
use thinwedge_git_utils::resolve_root_git_project_for_trust;
use thinwedge_install_context::InstallContext;
use thinwedge_login::AuthManagerConfig;
use thinwedge_mcp::McpConfig;
use thinwedge_mcp::McpServerRegistration;
use thinwedge_mcp::ResolvedMcpCatalog;
use thinwedge_memories_read::memory_root;
...
use thinwedge_model_provider_info::merge_configured_model_providers;
use thinwedge_models_manager::ModelsManagerConfig;
use thinwedge_protocol::config_types::AltScreenMode;
...
pub use thinwedge_thread_store::ExtraConfig;
...
use crate::config::permissions::get_readable_roots_required_for_thinwedge_runtime;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(thinwedge_state::SQLITE_HOME_ENV).ok()?;
...
impl Permissions {
    ...
    /// selection. Internal ThinWedge-only writable roots are intentionally excluded.
...
// when ThinWedge is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<thinwedge_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, ThinWedge will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - ThinWedge
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.thinwedge/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "ThinWedge"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send ThinWedge '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the ThinWedge home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that ThinWedge can reach out to for tool calls.
    ...
    ///
    /// When unset, ThinWedge will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all ThinWedge state (defaults to `~/.thinwedge` but can be
    /// overridden by the `THINWEDGE_HOME` environment variable).
    pub thinwedge_home: AbsolutePathBuf,

    /// Directory where ThinWedge stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where ThinWedge writes log files (defaults to `$THINWEDGE_HOME/log`).
    pub log_dir: PathBuf,

    /// Directory where ThinWedge writes effective session config lock files.
    pub config_lock_export_dir: Option<AbsolutePathBuf>,

    /// Whether config lock replay ignores ThinWedge version drift between the
    /// lock metadata and the regenerated lock.
    pub config_lock_allow_thinwedge_version_mismatch: bool,
    ...
    /// Path to the current ThinWedge executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub thinwedge_self_exe: Option<PathBuf>,

    /// Path to the `thinwedge-linux-sandbox` executable. This must be set if
    /// [`thinwedge_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `thinwedge-linux-sandbox`.
    pub thinwedge_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `thinwedge-execve-wrapper` executable used for shell
    ...
    /// When `true`, checks for ThinWedge updates on startup and surfaces update prompts.
    /// Set to `false` only if your ThinWedge updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across ThinWedge product surfaces in this machine.
    ...
    pub otel: thinwedge_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn thinwedge_home(&self) -> PathBuf {
        self.thinwedge_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn thinwedge_home(mut self, thinwedge_home: PathBuf) -> Self {
        self.thinwedge_home = Some(thinwedge_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let thinwedge_home = match thinwedge_home {
            Some(thinwedge_home) => AbsolutePathBuf::from_absolute_path(thinwedge_home)?,
            None => find_thinwedge_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&thinwedge_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        {
            let allow_thinwedge_version_mismatch = config_lock_settings
                .and_then(|config_lock| config_lock.allow_thinwedge_version_mismatch)
                .unwrap_or(false);
            ...
            config.config_lock_allow_thinwedge_version_mismatch = allow_thinwedge_version_mismatch;
...
impl Config {
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &thinwedge_core_plugins::PluginsManager,
    ) -> McpConfig {
        ...
        McpConfig {
            ...
            thinwedge_home: self.thinwedge_home.to_path_buf(),
            ...
            thinwedge_linux_sandbox_exe: self.thinwedge_linux_sandbox_exe.clone(),
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.thinwedge_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_thinwedge_home(
    ...
    /// Load a default configuration for a specific ThinWedge home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_thinwedge_home(
        ...
        let cli_layer = thinwedge_config::build_cli_overrides_layer(&cli_overrides);
        thinwedge_config::merge_toml_values(&mut merged, &cli_layer);
        let thinwedge_home = AbsolutePathBuf::from_absolute_path_checked(thinwedge_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &thinwedge_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `thinwedge exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `thinwedge_self_exe`, `thinwedge_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(thinwedge_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, thinwedge_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .thinwedge/ folders.
...
/// Patch `THINWEDGE_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(thinwedge_home: &Path, provider: &str) -> std::io::Result<()> {
    thinwedge_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &thinwedge_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            thinwedge_protocol::permissions::FileSystemSandboxEntry {
                path: thinwedge_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: thinwedge_protocol::permissions::FileSystemAccessMode::Deny,
            }
        } else {
            ...
            thinwedge_protocol::permissions::FileSystemSandboxEntry {
                path: thinwedge_protocol::permissions::FileSystemPath::Path { path },
                access: thinwedge_protocol::permissions::FileSystemAccessMode::Deny,
            }
        };
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&thinwedge_home);
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(thinwedge_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| thinwedge_home.join("log").to_path_buf());
        let sqlite_home = cfg
            .sqlite_home
            .as_ref()
            .map(AbsolutePathBuf::to_path_buf)
            .or_else(|| resolve_sqlite_home_env(&resolved_cwd))
            .unwrap_or_else(|| thinwedge_home.to_path_buf());
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_thinwedge_runtime(
            &thinwedge_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            config_lock_allow_thinwedge_version_mismatch: cfg
                .debug
                .as_ref()
                .and_then(|debug| debug.config_lockfile.as_ref())
                .and_then(|config_lock| config_lock.allow_thinwedge_version_mismatch)
...
/// `~/.thinwedge`.
///
/// - If `THINWEDGE_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `THINWEDGE_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_thinwedge_home() -> std::io::Result<AbsolutePathBuf> {
    thinwedge_utils_home_dir::find_thinwedge_home()
}

/// Returns the path to the folder where ThinWedge logs are stored. Does not verify
