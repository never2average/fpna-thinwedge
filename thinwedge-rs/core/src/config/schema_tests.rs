...
fn config_schema_matches_fixture() {
    let fixture_path = thinwedge_utils_cargo_bin::find_resource!("config.schema.json")
...
Run `just write-config-schema` to overwrite with your changes.\n\n{diff}"
    ...
    // Make sure the version in the repo matches exactly: https://github.com/openai/thinwedge/pull/10977.
...
#[test]
fn config_schema_exposes_finance_integration_settings() {
    let schema_json = config_schema_json().expect("serialize config schema");
    let schema_value: serde_json::Value =
        serde_json::from_slice(&schema_json).expect("decode schema json");
    ...
    assert_eq!(
        (
            properties.contains_key("billing"),
            properties.contains_key("db_ops"),
            properties.contains_key("db_sandbox"),
            properties.contains_key("ardent"),
        ),
        (true, true, true, true),
    );
}
