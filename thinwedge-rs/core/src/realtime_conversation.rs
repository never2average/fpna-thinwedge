...
use thinwedge_api::RealtimeEvent;
use thinwedge_api::RealtimeEventParser;
use thinwedge_api::RealtimeSessionConfig;
use thinwedge_api::RealtimeSessionMode;
use thinwedge_api::RealtimeWebsocketClient;
use thinwedge_api::RealtimeWebsocketEvents;
use thinwedge_api::RealtimeWebsocketWriter;
...
use thinwedge_protocol::protocol::RealtimeConversationSdpEvent;
...
const STANDALONE_HANDOFF_ID: &str = "thinwedge";
...
impl RealtimeConversationManager {
    ...
    async fn start(&self, start: RealtimeStart) -> ThinWedgeResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> ThinWedgeResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            architecture,
            extra_headers,
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> ThinWedgeResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(ThinWedgeErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, mut params: ConversationTextParams) -> ThinWedgeResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(params)
            .await
            .map_err(|_| ThinWedgeErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }

    pub(crate) async fn handoff_out(&self, output_text: String) -> ThinWedgeResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn handoff_complete(&self) -> ThinWedgeResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn shutdown(&self) -> ThinWedgeResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> ThinWedgeResult<PreparedRealtimeConversationStart> {
...
) -> ThinWedgeResult<RealtimeSessionConfig> {
    let config = sess.get_config().await;
...
fn realtime_api_key(
    ...
    if let Some(api_key) = auth.and_then(ThinWedgeAuth::api_key) {
    ...
    Err(ThinWedgeErr::InvalidRequest(
...
fn realtime_request_headers(
    ...
    api_key: Option<&str>,
    version: RealtimeWsVersion,
) -> ThinWedgeResult<Option<HeaderMap>> {
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            ThinWedgeErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    thinwedge_error_info: ThinWedgeErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
