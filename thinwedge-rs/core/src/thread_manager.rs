...
struct TempThinWedgeHomeGuard {
    path: PathBuf,
}

impl Drop for TempThinWedgeHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created ThinWedge thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<ThinWedgeThread>,
...
pub struct ThreadManager {
    ...
    _test_thinwedge_home_guard: Option<TempThinWedgeHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<thinwedge_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<ThinWedgeThread>>>>,
...
pub fn thread_store_from_config(
    ...
    match &config.experimental_thread_store {
        ThreadStoreConfig::Local => {
            ...
            {
                thinwedge_rollout::spawn_rollout_compression_worker(
                    config.thinwedge_home.to_path_buf(),
                );
            }
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        Self {
            ...
            _test_thinwedge_home_guard: None,
        }
    }

    /// Construct with a dummy AuthManager containing the provided ThinWedgeAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let thinwedge_home = std::env::temp_dir().join(format!(
            "thinwedge-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&thinwedge_home)
            .unwrap_or_else(|err| panic!("temp thinwedge home dir create failed: {err}"));
    ...
    /// Construct with a dummy AuthManager containing the provided ThinWedgeAuth and thinwedge home.
    /// Used for integration tests: should not be used by ordinary business logic.
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(thinwedge_home, /*config_model_catalog*/ None),
                environment_manager,
                skills_manager,
                plugins_manager,
                mcp_manager,
    ...
    pub fn validate_environment_selections(
        ...
        for environment in environments {
            if !environment_ids.insert(environment.environment_id.as_str()) {
                return Err(ThinWedgeErr::InvalidRequest(format!(
            ...
            self.state
                ...
                .ok_or_else(|| {
                    ThinWedgeErr::InvalidRequest(format!(
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> ThinWedgeResult<Arc<ThinWedgeThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `ThinWedgeThread`/`LiveThread`, so metadata changes stay ordered
        ...
        self.state
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    ThinWedgeErr::ThreadNotFound(thread_id)
                }
    ...
    pub async fn start_thread(&self, config: Config) -> ThinWedgeResult<NewThread> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<thinwedge_protocol::dynamic_tools::DynamicToolSpec>,
    ) -> ThinWedgeResult<NewThread> {
    ...
    /// as `Arc<ThinWedgeThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<ThinWedgeThread>> {
    ...
    async fn initial_history_from_rollout_path(
        &self,
        rollout_path: PathBuf,
    ) -> ThinWedgeResult<InitialHistory> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(
        ...
        match threads.get(&thread_id) {
            ...
            Some(_) | None => Err(ThinWedgeErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => ThinWedgeErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }

    /// Send an operation to a thread by ID.
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> ThinWedgeResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<ThinWedgeThread>> {
    ...
    async fn user_instructions_for_spawn(
        ...
        let instructions = match inherited_thread_id {
            ...
            Some(thread_id) => match self.get_thread(thread_id).await {
                Ok(thread) => thread.thinwedge.session.user_instructions().await,
                Err(_) => None,
            },
            None => None,
        };
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        let ThinWedgeSpawnOk {
            ...
            thread_id,
            ..
        } = Box::pin(ThinWedge::spawn(ThinWedgeSpawnArgs {
            config,
            user_instructions,
        ...
        .await?;
        let new_thread = self
            .finalize_thread_spawn(thinwedge, thread_id, tracked_session_source)
            .await?;
    ...
    async fn finalize_thread_spawn(
        ...
        let event = thinwedge.next_event().await?;
        let session_configured = match event {
            ...
            _ => {
                return Err(ThinWedgeErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(ThinWedgeThread::new(
                    thinwedge,
        ...
        if let Err(err) = thinwedge.shutdown_and_wait().await {
            warn!("failed to shut down duplicate thread {thread_id}: {err}");
        }
        Err(ThinWedgeErr::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> thinwedge_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return thinwedge_rollout_trace::ThreadTraceContext::disabled();
        };
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> ThinWedgeErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => ThinWedgeErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => ThinWedgeErr::InvalidRequest(message),
        err => ThinWedgeErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> ThinWedgeErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => ThinWedgeErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => ThinWedgeErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => ThinWedgeErr::UnsupportedOperation(format!(
