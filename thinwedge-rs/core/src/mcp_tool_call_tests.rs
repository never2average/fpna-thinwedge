...
use thinwedge_config::types::AppToolConfig;
use thinwedge_config::types::AppToolsConfig;
use thinwedge_config::types::ApprovalsReviewer;
use thinwedge_config::types::AppsConfigToml;
...
use thinwedge_rollout_trace::ToolDispatchInvocation;
use thinwedge_rollout_trace::ToolDispatchPayload;
use thinwedge_rollout_trace::ToolDispatchRequester;
...
fn write_sample_plugin_mcp(thinwedge_home: &std::path::Path) {
    let plugin_root = thinwedge_home.join("plugins/cache/test/sample/local");
    ...
    let dispatch_trace = session
        ...
        .start_tool_dispatch_trace(|| {
            Some(ToolDispatchInvocation {
                ...
                thinwedge_turn_id: turn_context.sub_id.clone(),
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.thinwedge_home)
        .expect("create thinwedge home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.thinwedge_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = thinwedge_hooks::list_hooks(HooksConfig {
...
fn attach_trace_bundle(
    ...
    let rollout_thread_trace =
        thinwedge_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
            root,
    ...
    rollout_thread_trace.record_thinwedge_turn_started(turn_context.sub_id.as_str());
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("thinwedge.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("thinwedge.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("thinwedge.mcp.target.id=")
            && !invalid_logs.contains("thinwedge.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("thinwedge.mcp.target.id=")
            && !missing_logs.contains("thinwedge.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("thinwedge.mcp.target.id=")
            && !no_meta_logs.contains("thinwedge.mcp.server_user_flow.triggered="),
...
async fn approval_elicitation_request_uses_message_override_and_preserves_tool_params_keys() {
    ...
    assert_eq!(
        ...
        McpServerElicitationRequestParams {
            ...
            server_name: THINWEDGE_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn thinwedge_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        THINWEDGE_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_thinwedge_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        THINWEDGE_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn thinwedge_apps_tool_question_without_elicitation_omits_always_allow() {
    let session_key = McpToolApprovalKey {
        server: THINWEDGE_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn thinwedge_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: THINWEDGE_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    let turn_metadata = meta
        .get(crate::X_THINWEDGE_TURN_METADATA_HEADER)
    ...
    assert_eq!(
        meta,
        serde_json::json!({
            crate::X_THINWEDGE_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
#[tokio::test]
async fn thinwedge_apps_tool_call_request_meta_includes_turn_metadata_and_thinwedge_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
        .current_meta_value_for_mcp_request(mcp_turn_metadata_context(&turn_context))
    ...
    assert_eq!(
        ...
        Some(serde_json::json!({
            crate::X_THINWEDGE_TURN_METADATA_HEADER: expected_turn_metadata,
            MCP_TOOL_THINWEDGE_APPS_META_KEY: {
...
fn thinwedge_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn thinwedge_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
async fn install_host_owned_thinwedge_apps_manager(session: &Session, turn_context: &TurnContext) {
    let auth = session.services.auth_manager.auth().await;
    let manager = thinwedge_mcp::McpConnectionManager::new(
        &HashMap::new(),
        ...
        thinwedge_mcp::McpRuntimeContext::new(Arc::clone(&session.services.environment_manager), {
            #[allow(deprecated)]
            turn_context.cwd.to_path_buf()
        }),
        turn_context.config.thinwedge_home.to_path_buf(),
        thinwedge_mcp::thinwedge_apps_tools_cache_key(auth.as_ref()),
        /*host_owned_thinwedge_apps_enabled*/ true,
        turn_context.config.prefix_mcp_tool_names(),
        rmcp::model::ElicitationCapability::default(),
        thinwedge_mcp::ToolPluginProvenance::default(),
        auth.as_ref(),
...
#[tokio::test]
async fn thinwedge_apps_auth_elicitation_feature_disabled_returns_original_result() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_thinwedge_apps_manager(&session, &turn_context).await;
    let result = thinwedge_apps_auth_failure_result();
    let metadata = thinwedge_apps_auth_failure_metadata();

    let returned = maybe_request_thinwedge_apps_auth_elicitation(
        &session,
        &turn_context,
        "call_123",
        THINWEDGE_APPS_MCP_SERVER_NAME,
...
#[tokio::test]
async fn thinwedge_apps_auth_elicitation_non_host_owned_server_returns_original_result() {
...
#[tokio::test]
async fn thinwedge_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_thinwedge_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn thinwedge_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_thinwedge_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn thinwedge_apps_auth_elicitation_feature_enabled_requests_elicitation() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_thinwedge_apps_manager(&session, &turn_context).await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let request_task = tokio::spawn({
        ...
        async move {
            maybe_request_thinwedge_apps_auth_elicitation(
                &session,
                &turn_context,
                "call_123",
                THINWEDGE_APPS_MCP_SERVER_NAME,
    ...
    assert_eq!(request.server_name, THINWEDGE_APPS_MCP_SERVER_NAME);
    assert_eq!(
        request.id,
        thinwedge_protocol::mcp::RequestId::String("thinwedge_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
    ...
    session
        .resolve_elicitation(
            ...
            rmcp::model::RequestId::String("thinwedge_apps_auth_call_123".into()),
            ElicitationResponse {
...
async fn guardian_review_decision_maps_to_mcp_tool_decision() {
    ...
    session.services.guardian_rejections.lock().await.insert(
        ...
        crate::guardian::GuardianRejection {
            ...
            source: thinwedge_protocol::protocol::GuardianAssessmentDecisionSource::Agent,
        },
    );
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_thinwedge_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            THINWEDGE_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_thinwedge_app_tool_approval_writes_tool_override() {
    ...
    persist_thinwedge_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(thinwedge_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .thinwedge_home(thinwedge_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_thinwedge_dir = project_dir.path().join(".thinwedge");
    std::fs::create_dir_all(&project_thinwedge_dir).expect("create project .thinwedge dir");
    std::fs::write(
        project_thinwedge_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&thinwedge_home)
        .set_project_trust_level(
            project_dir.path(),
            thinwedge_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_thinwedge_dir.join(CONFIG_TOML_FILE))
...
async fn approve_mode_skips_guardian_in_every_permission_mode() {
    ...
    for approval_policy in [
        ...
        turn_context.auth_manager = Some(crate::test_support::auth_manager_from_auth(
            thinwedge_login::ThinWedgeAuth::create_dummy_chatgpt_auth_for_testing(),
        ));
        turn_context
            .approval_policy
            .set(approval_policy)
