...
use thinwedge_config::ConfigEditsBuilder;
use thinwedge_config::McpServerConfig;
use thinwedge_config::McpServerTransportConfig;
use thinwedge_config::load_global_mcp_servers;
use thinwedge_login::default_client::is_first_party_originator;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(thinwedge_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&thinwedge_home).await {
        Ok(servers) => servers,
        Err(err) => {
