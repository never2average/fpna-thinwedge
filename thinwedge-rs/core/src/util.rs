...
/// `thinwedge_feedback::ThinWedgeFeedback::metadata_layer()` is installed, these fields are captured and
...
///
/// ```rust
/// thinwedge_core::feedback_tags!(model = "gpt-5", cached = true);
/// thinwedge_core::feedback_tags!(provider = provider_id, request_id = request_id);
/// ```
#[macro_export]
...
pub fn resume_command(thread_name: Option<&str>, thread_id: Option<ThreadId>) -> Option<String> {
    ...
    resume_target.map(|target| {
        ...
        if needs_double_dash {
            format!("thinwedge resume -- {escaped}")
        } else {
            format!("thinwedge resume {escaped}")
        }
    })
