...
use thinwedge_analytics::GuardianApprovalRequestSource;
...
/// Start an interactive sub-ThinWedge thread and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_thinwedge_thread_interactive(
    config: Config,
    ...
    let ThinWedgeSpawnOk { thinwedge, .. } = Box::pin(ThinWedge::spawn(ThinWedgeSpawnArgs {
        config,
        user_instructions,
        ...
        parent_rollout_thread_trace: thinwedge_rollout_trace::ThreadTraceContext::disabled(),
        parent_trace: None,
        environment_selections: parent_ctx.environments.clone(),
        thread_extension_init: thinwedge_extension_api::ExtensionDataInit::default(),
    ...
    let thinwedge_for_events = Arc::clone(&thinwedge);
    ...
    let thinwedge_for_ops = Arc::clone(&thinwedge);
    tokio::spawn(async move {
        forward_ops(thinwedge_for_ops, rx_ops, cancel_token_ops).await;
    });

    Ok(ThinWedge {
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: thinwedge.agent_status.clone(),
        session: Arc::clone(&thinwedge.session),
        session_loop_termination: thinwedge.session_loop_termination.clone(),
    })
}
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_thinwedge_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
    ...
    let io = Box::pin(run_thinwedge_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn forward_events(
    ...
    loop {
        tokio::select! {
            _ = &mut cancelled => {
                shutdown_delegate(&thinwedge).await;
                break;
            }
            event = thinwedge.next_event() => {
                ...
                match event {
                    ...
                    other => {
                        if !forward_event_or_shutdown(&thinwedge, &tx_sub, &cancel_token, other).await
                        {
                            break;
                        }
                    }
...
async fn shutdown_delegate(thinwedge: &ThinWedge) {
    let _ = thinwedge.submit(Op::Interrupt).await;
    let _ = thinwedge.submit(Op::Shutdown {}).await;

    let _ = timeout(Duration::from_millis(500), async {
        while let Ok(event) = thinwedge.next_event().await {
            if matches!(
                event.msg,
...
async fn forward_ops(
    ...
    loop {
        ...
        let _ = thinwedge.submit_with_id(submission).await;
    }
}
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                thinwedge_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                thinwedge_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                thinwedge_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn handle_request_user_input(
    ...
    .await
    {
        let _ = thinwedge.submit(Op::UserInputAnswer { id, response }).await;
        return;
    }
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            thinwedge_protocol::request_user_input::RequestUserInputAnswer {
...
where
    F: core::future::Future<Output = thinwedge_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, thinwedge_protocol::protocol::ReviewDecision::Abort)
                .await;
...
#[cfg(test)]
#[path = "thinwedge_delegate_tests.rs"]
mod tests;
