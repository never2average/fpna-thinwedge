...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call(
    ...
    ) -> impl std::future::Future<Output = Result<ResponseInputItem, ThinWedgeErr>> {
        ...
        async move {
            match future.await {
                ...
                Err(FunctionCallError::Fatal(message)) => Err(ThinWedgeErr::Fatal(message)),
        ...
        match call.payload {
            ...
            ToolPayload::Custom { .. } => ResponseInputItem::CustomToolCallOutput {
                ...
                output: thinwedge_protocol::models::FunctionCallOutputPayload {
                    body: thinwedge_protocol::models::FunctionCallOutputBody::Text(message),
                    success: Some(false),
                },
            },
...
mod tests {
    ...
    impl thinwedge_extension_api::ToolLifecycleContributor for FinishRecorder {
        fn on_tool_finish<'a>(
            &'a self,
    ...
    impl thinwedge_extension_api::ToolLifecycleContributor for BlockingFinishContributor {
        fn on_tool_finish<'a>(
            &'a self,
            input: thinwedge_extension_api::ToolFinishInput<'a>,
        ) -> thinwedge_extension_api::ToolLifecycleFuture<'a> {
    ...
    -> anyhow::Result<()> {
        ...
        let tool_name = thinwedge_tools::ToolName::plain("test_tool");
    ...
    -> anyhow::Result<()> {
        ...
        let session = Arc::new(session);
        let turn_context = Arc::new(turn_context);
        let tool_name = thinwedge_tools::ToolName::plain("cleanup_tool");
