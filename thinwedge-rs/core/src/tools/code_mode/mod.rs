...
use thinwedge_tools::collect_code_mode_tool_definitions;
use thinwedge_utils_output_truncation::TruncationPolicy;
use thinwedge_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
...
pub(crate) const PUBLIC_TOOL_NAME: &str = thinwedge_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = thinwedge_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = thinwedge_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
pub(crate) struct CodeModeService {
    inner: thinwedge_code_mode::CodeModeService,
}

impl CodeModeService {
    pub(crate) fn new() -> Self {
        Self {
            inner: thinwedge_code_mode::CodeModeService::new(),
        }
    }
    ...
    pub(crate) async fn execute(
        &self,
        request: thinwedge_code_mode::ExecuteRequest,
    ...
    pub(crate) async fn wait(
        &self,
        request: thinwedge_code_mode::WaitRequest,
    ) -> Result<thinwedge_code_mode::WaitOutcome, String> {
    ...
    ) -> Option<thinwedge_code_mode::CodeModeTurnWorker> {
...
pub(super) async fn build_enabled_tools(
    exec: &ExecContext,
) -> Vec<thinwedge_code_mode::ToolDefinition> {
...
fn tool_kind_for_spec(spec: &ToolSpec) -> thinwedge_code_mode::CodeModeToolKind {
    if matches!(spec, ToolSpec::Freeform(_)) {
        thinwedge_code_mode::CodeModeToolKind::Freeform
    } else {
        thinwedge_code_mode::CodeModeToolKind::Function
    }
}
...
fn tool_kind_for_name(
    ...
    tool_name: &ToolName,
) -> Result<thinwedge_code_mode::CodeModeToolKind, String> {
    spec.as_ref()
