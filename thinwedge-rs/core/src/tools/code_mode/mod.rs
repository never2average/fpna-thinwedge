...
use thinwedge_code_mode::CellId;
use thinwedge_code_mode::CodeModeNestedToolCall;
use thinwedge_code_mode::CodeModeSession;
...
impl CodeModeService {
    pub(crate) fn new() -> Self {
        ...
        Self {
            session: Some(Arc::new(
                thinwedge_code_mode::CodeModeService::with_delegate(dispatch_broker.clone()),
            )),
            dispatch_broker,
        }
    }
    ...
    ) -> Result<thinwedge_code_mode::StartedCell, String> {
    ...
    pub(crate) fn mark_cell_ready_for_dispatch(&self, cell_id: &thinwedge_code_mode::CellId) {
