...
use thinwedge_models_manager::model_info::with_config_overrides;
...
use thinwedge_tools::TOOL_SUGGEST_TOOL_NAME;
...
use thinwedge_tools::ZshForkConfig;
...
fn deferred_responses_api_tool_serializes_with_defer_loading() {
    ...
    let serialized = serde_json::to_value(ToolSpec::Function(
        mcp_tool_to_deferred_responses_api_tool(
            &ToolName::namespaced("mcp__thinwedge_apps__", "lookup_order"),
            &tool,
        )
...
#[tokio::test]
async fn test_build_specs_gpt5_thinwedge_default() {
    let features = Features::with_defaults();
...
#[tokio::test]
async fn test_build_specs_gpt51_thinwedge_default() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4",
        &features,
...
#[tokio::test]
async fn test_build_specs_gpt5_thinwedge_unified_exec_web_search() {
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn test_build_specs_gpt51_thinwedge_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.4",
...
#[tokio::test]
async fn test_gpt_5_1_thinwedge_max_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4",
        &features,
...
#[tokio::test]
async fn test_thinwedge_5_1_mini_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4-mini",
        &features,
...
#[tokio::test]
async fn test_gpt_5_1_thinwedge_max_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.4",
        ...
        tools_config
            .with_unified_exec_shell_mode_for_session(
                ...
                Some(&PathBuf::from(if cfg!(windows) {
                    r"C:\opt\thinwedge\zsh"
                } else {
                    "/opt/thinwedge/zsh"
                })),
                Some(&PathBuf::from(if cfg!(windows) {
                    r"C:\opt\thinwedge\thinwedge-execve-wrapper"
                } else {
                    "/opt/thinwedge/thinwedge-execve-wrapper"
                })),
            )
            .unified_exec_shell_mode,
        if cfg!(unix) {
            UnifiedExecShellMode::ZshFork(ZshForkConfig {
                shell_zsh_path: AbsolutePathBuf::from_absolute_path("/opt/thinwedge/zsh").unwrap(),
...
async fn search_tool_description_falls_back_to_connector_name_without_description() {
    ...
    let (tools, _) = build_specs(
        ...
        /*mcp_tools*/ None,
        Some(HashMap::from([(
            "mcp__thinwedge_apps__calendar_create_event".to_string(),
            ToolInfo {
                server_name: THINWEDGE_APPS_MCP_SERVER_NAME.to_string(),
                callable_name: "_create_event".to_string(),
                callable_namespace: "mcp__thinwedge_apps__calendar".to_string(),
                server_instructions: None,
                tool: mcp_tool(
...
async fn search_tool_registers_namespaced_mcp_tool_aliases() {
    ...
    let (_, registry) = build_specs(
        ...
        Some(HashMap::from([
            ...
            (
                "mcp__thinwedge_apps__calendar_list_events".to_string(),
                ToolInfo {
                    server_name: THINWEDGE_APPS_MCP_SERVER_NAME.to_string(),
    ...
    let app_alias = ToolName::namespaced("mcp__thinwedge_apps__calendar", "_create_event");
...
async fn unavailable_mcp_tools_are_exposed_as_dummy_function_tools() {
    ...
    let unavailable_tool = ToolName::namespaced("mcp__thinwedge_apps__calendar", "_create_event");
    ...
    let tool = find_tool(&tools, "mcp__thinwedge_apps__calendar_create_event");
