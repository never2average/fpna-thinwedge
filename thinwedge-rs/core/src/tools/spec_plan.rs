...
use thinwedge_protocol::openai_models::ToolMode;
...
use thinwedge_tools::ToolExecutor;
...
use thinwedge_tools::can_request_original_image_detail;
use thinwedge_tools::collect_code_mode_exec_prompt_tool_definitions;
use thinwedge_tools::collect_request_plugin_install_entries;
use thinwedge_tools::default_namespace_description;
use thinwedge_tools::request_user_input_available_modes;
use thinwedge_tools::shell_command_backend_for_features;
use thinwedge_tools::shell_type_for_model_and_features;
...
fn spec_for_model_request(
    ...
    if matches!(
        ...
        && thinwedge_code_mode::is_code_mode_nested_tool(spec.name())
    {
        thinwedge_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
...
fn image_generation_runtime_enabled(turn_context: &TurnContext) -> bool {
    turn_context
        .auth_manager
        .as_deref()
        .is_some_and(AuthManager::current_auth_uses_thinwedge_backend)
...
fn is_hidden_by_code_mode_only(
    ...
    turn_context.tool_mode == ToolMode::CodeModeOnly
        && exposure != ToolExposure::DirectModelOnly
        && thinwedge_code_mode::is_code_mode_nested_tool(
            &thinwedge_tools::code_mode_name_for_tool_name(tool_name),
        )
}
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, thinwedge_code_mode::ToolNamespaceDescription> {
    ...
    for spec in specs {
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| thinwedge_code_mode::ToolNamespaceDescription {
...
fn append_extension_tool_executors(
    ...
    if matches!(
        ...
        reserved_tool_names.insert(ToolName::plain(thinwedge_code_mode::PUBLIC_TOOL_NAME));
        reserved_tool_names.insert(ToolName::plain(thinwedge_code_mode::WAIT_TOOL_NAME));
    }
...
fn compare_code_mode_tools(
    left: &thinwedge_code_mode::ToolDefinition,
    right: &thinwedge_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, thinwedge_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &thinwedge_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, thinwedge_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
