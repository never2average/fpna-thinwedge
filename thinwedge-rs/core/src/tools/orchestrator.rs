...
impl ToolOrchestrator {
        ...
        T: ToolRuntime<Rq, Out>,
    {
        ...
        let attempt_with_network_approval = SandboxAttempt {
            ...
            thinwedge_linux_sandbox_exe: attempt.thinwedge_linux_sandbox_exe,
        ...
        T: ToolRuntime<Rq, Out>,
    {
        ...
        let initial_attempt = SandboxAttempt {
            ...
            thinwedge_linux_sandbox_exe: turn_ctx.thinwedge_linux_sandbox_exe.as_ref(),
            use_legacy_landlock,
        ...
        match first_result {
            ...
            Err(ToolError::ThinWedge(ThinWedgeErr::Sandbox(SandboxErr::Denied {
                ...
                let retry_thinwedge_linux_sandbox_exe = if unsandboxed_allowed {
                    None
                } else {
                    turn_ctx.thinwedge_linux_sandbox_exe.as_ref()
                };
                let retry_attempt = SandboxAttempt {
                    ...
                    sandbox_cwd: &sandbox_policy_cwd,
                    workspace_roots: workspace_roots.as_slice(),
                    thinwedge_linux_sandbox_exe: retry_thinwedge_linux_sandbox_exe,
                    use_legacy_landlock,
                    windows_sandbox_level: turn_ctx.windows_sandbox_level,
                    windows_sandbox_private_desktop: turn_ctx
                        .config
    ...
    async fn request_approval<Rq, Out, T>(
        ...
        otel: &thinwedge_otel::SessionTelemetry,
...
fn sandbox_outcome_from_tool_error(err: &ToolError) -> Option<&'static str> {
    match err {
        ToolError::ThinWedge(ThinWedgeErr::Sandbox(SandboxErr::Denied { .. })) => Some("denied"),
        ToolError::ThinWedge(ThinWedgeErr::Sandbox(SandboxErr::Timeout { .. })) => {
            Some("timed_out")
        }
        ToolError::ThinWedge(ThinWedgeErr::Sandbox(SandboxErr::Signal(_))) => Some("signal"),
        ToolError::Rejected(_) | ToolError::ThinWedge(_) => None,
    }
}
