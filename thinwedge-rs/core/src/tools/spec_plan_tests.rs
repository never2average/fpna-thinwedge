...
use thinwedge_tools::ToolExposure;
use thinwedge_tools::ToolName;
use thinwedge_tools::ToolOutput;
...
fn zsh_fork_config_for_spec_plan_tests() -> thinwedge_tools::ZshForkConfig {
    let placeholder_exe = thinwedge_utils_absolute_path::AbsolutePathBuf::try_from(
...
impl ToolExecutor<ExtensionToolCall> for WebRunExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(thinwedge_tools::ResponsesApiNamespace {
            name: "web".to_string(),
    ...
    fn handle(&self, _call: ExtensionToolCall) -> thinwedge_tools::ToolExecutorFuture<'_> {
        Box::pin(async {
            Ok(Box::new(thinwedge_tools::JsonToolOutput::new(json!({}))) as Box<dyn ToolOutput>)
        })
    }
}
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Function(ResponsesApiTool {
            ...
            parameters: thinwedge_tools::JsonSchema::object(
                BTreeMap::from([(
                    "message".to_string(),
                    thinwedge_tools::JsonSchema::string(/*description*/ None),
                )]),
...
async fn request_user_input_stays_direct_in_code_mode_only() {
    ...
    plan.assert_visible_contains(&[
        "request_user_input",
        thinwedge_code_mode::PUBLIC_TOOL_NAME,
        thinwedge_code_mode::WAIT_TOOL_NAME,
    ]);
    ...
    let ToolSpec::Freeform(exec) = plan.visible_spec(thinwedge_code_mode::PUBLIC_TOOL_NAME) else {
...
async fn shell_zsh_fork_stays_standalone_until_unified_exec_composition_is_enabled() {
    ...
    if thinwedge_utils_pty::conpty_supported() {
...
async fn zsh_fork_unified_exec_hides_shell_parameter() {
    ...
    let plan = probe(|turn| {
        ...
        turn.unified_exec_shell_mode =
            thinwedge_tools::UnifiedExecShellMode::ZshFork(zsh_fork_config_for_spec_plan_tests());
    })
    .await;
    ...
    let plan = probe(|turn| {
        ...
        turn.environments.turn_environments.push(
            crate::session::turn_context::TurnEnvironment::new(
                "remote".to_string(),
                Arc::new(
                    thinwedge_exec_server::Environment::create_for_tests(Some(
...
async fn code_mode_only_exposes_code_executor_and_hides_nested_tools() {
    ...
    assert_eq!(
        plain.namespace_function_names("thinwedge_app"),
        &["lookup".to_string()]
    );
    ...
    assert_eq!(
        code_mode_only.namespace_function_names("thinwedge_app"),
