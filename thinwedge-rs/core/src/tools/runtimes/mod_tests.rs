...
async fn test_network_proxy() -> anyhow::Result<NetworkProxy> {
    let state = thinwedge_network_proxy::build_config_state(
...
fn runtime_path_prepends_records_runtime_path_prepend() {
    ...
    runtime_path_prepends.prepend(&mut env, PathBuf::from("/package/thinwedge-path").as_path());

    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/thinwedge-path:/usr/bin:/bin"),
        "runtime PATH prepend should update the live exec environment"
    );
    assert_eq!(
        runtime_path_prepends.entries,
        vec!["/package/thinwedge-path"],
...
fn runtime_path_prepends_drops_empty_path_entries() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        ":/usr/bin:/package/thinwedge-path::/bin:".to_string(),
    )]);
    let mut runtime_path_prepends = RuntimePathPrepends::default();
...
fn apply_zsh_fork_path_prepend_uses_shell_parent() {
    ...
    apply_zsh_fork_path_prepend(
        ...
        PathBuf::from("/package/thinwedge-resources/zsh/bin/zsh").as_path(),
    );

    let expected = "/package/thinwedge-resources/zsh/bin:/usr/bin:/bin";
    assert_eq!(env.get("PATH").map(String::as_str), Some(expected));
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
            entries: vec!["/package/thinwedge-resources/zsh/bin".to_string()]
        }
    );
}
...
fn apply_zsh_fork_path_prepend_moves_existing_shell_parent_to_front() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        "/usr/bin:/package/thinwedge-resources/zsh/bin:/bin:/package/thinwedge-resources/zsh/bin"
            .to_string(),
    )]);
    let mut runtime_path_prepends = RuntimePathPrepends::default();
    ...
    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/thinwedge-resources/zsh/bin:/usr/bin:/bin")
    );
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
...
#[test]
fn explicit_escalation_keeps_user_proxy_env_without_thinwedge_marker() {
    let env = HashMap::from([
        (
...
#[test]
fn maybe_wrap_shell_lc_with_snapshot_restores_thinwedge_thread_id_from_env() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport THINWEDGE_THREAD_ID='parent-thread'\n",
    )
    .expect("write snapshot");
    let session_shell = shell_with_snapshot(
        ShellType::Bash,
    ...
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printf '%s' \"$THINWEDGE_THREAD_ID\"".to_string(),
    ];
    let rewritten = maybe_wrap_shell_lc_with_snapshot(
        &command,
        &session_shell,
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env("THINWEDGE_THREAD_ID", "nested-thread")
        .output()
        .expect("run rewritten command");
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_refreshes_thinwedge_proxy_git_ssh_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{THINWEDGE_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
    );
    let fresh_command = format!(
        "{THINWEDGE_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:48081 %h %p'"
    );
    std::fs::write(
        &snapshot_path,
        format!(
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_clears_stale_thinwedge_git_ssh_command_without_live_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{THINWEDGE_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
...
fn run_snapshot_path_probe_with_runtime_path_prepend(
    ...
    let package_path_dir = dir.path().join("thinwedge-path");
...
fn maybe_wrap_shell_lc_with_snapshot_preserves_unset_override_variables() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport THINWEDGE_TEST_UNSET_OVERRIDE='snapshot-value'\n",
    )
    .expect("write snapshot");
    let session_shell = shell_with_snapshot(
        ShellType::Bash,
    ...
    let command = vec![
            "/bin/bash".to_string(),
            "-lc".to_string(),
            "if [ \"${THINWEDGE_TEST_UNSET_OVERRIDE+x}\" = x ]; then printf 'set:%s' \"$THINWEDGE_TEST_UNSET_OVERRIDE\"; else printf 'unset'; fi".to_string(),
        ];
    let explicit_env_overrides = HashMap::from([(
        "THINWEDGE_TEST_UNSET_OVERRIDE".to_string(),
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove("THINWEDGE_TEST_UNSET_OVERRIDE")
        .output()
        .expect("run rewritten command");
    assert!(output.status.success(), "command failed: {output:?}");
    assert_eq!(String::from_utf8_lossy(&output.stdout), "unset");
