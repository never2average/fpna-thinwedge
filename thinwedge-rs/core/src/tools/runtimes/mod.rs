...
*/
use crate::exec_env::THINWEDGE_THREAD_ID_ENV_VAR;
...
use thinwedge_network_proxy::CUSTOM_CA_ENV_KEYS;
...
use thinwedge_network_proxy::is_managed_mitm_ca_trust_bundle_path;
...
pub(crate) fn strip_managed_proxy_env(env: &mut HashMap<String, String>) {
    ...
    // Only macOS injects a ThinWedge-owned SSH wrapper for the managed SOCKS proxy.
    #[cfg(target_os = "macos")]
    if env
        .get(PROXY_GIT_SSH_COMMAND_ENV_KEY)
        .is_some_and(|command| command.starts_with(THINWEDGE_PROXY_GIT_SSH_COMMAND_MARKER))
    {
...
/// PATH entries owned by ThinWedge runtime setup.
///
...
/// runtime-only vars like `THINWEDGE_THREAD_ID` without pretending they came from
/// the explicit override policy.
///
/// `runtime_path_prepends` contains ThinWedge-owned PATH entries already applied to
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    if let Some(thread_id) = env.get(THINWEDGE_THREAD_ID_ENV_VAR) {
        override_env.insert(THINWEDGE_THREAD_ID_ENV_VAR.to_string(), thread_id.clone());
    }
...
fn build_override_exports(explicit_env_overrides: &HashMap<String, String>) -> (String, String) {
    ...
    build_override_exports_for_keys("__THINWEDGE_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports() -> (String, String) {
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__THINWEDGE_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__THINWEDGE_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__THINWEDGE_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
    let git_blocks = build_thinwedge_proxy_git_ssh_command_exports();
    (
...
#[cfg(target_os = "macos")]
fn build_thinwedge_proxy_git_ssh_command_exports() -> (String, String) {
    let key = PROXY_GIT_SSH_COMMAND_ENV_KEY;
    let marker_pattern = format!("{}\\ *", THINWEDGE_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
            "__THINWEDGE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET=\"${{{key}+x}}\"\n__THINWEDGE_SNAPSHOT_PROXY_GIT_SSH_COMMAND=\"${{{key}-}}\"\ncase \"$__THINWEDGE_SNAPSHOT_PROXY_GIT_SSH_COMMAND\" in\n  {marker_pattern}) __THINWEDGE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED=1 ;;\n  *) __THINWEDGE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED= ;;\nesac"
        ),
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __THINWEDGE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __THINWEDGE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__THINWEDGE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__THINWEDGE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__THINWEDGE_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__THINWEDGE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__THINWEDGE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__THINWEDGE_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}
