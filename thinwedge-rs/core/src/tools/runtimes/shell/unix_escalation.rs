...
use thinwedge_shell_escalation::EscalateServer;
use thinwedge_shell_escalation::EscalationDecision;
use thinwedge_shell_escalation::EscalationExecution;
use thinwedge_shell_escalation::EscalationPermissions;
use thinwedge_shell_escalation::EscalationPolicy;
use thinwedge_shell_escalation::EscalationPolicyFuture;
use thinwedge_shell_escalation::EscalationSession;
use thinwedge_shell_escalation::ExecParams;
use thinwedge_shell_escalation::ExecResult;
use thinwedge_shell_escalation::PreparedExec;
use thinwedge_shell_escalation::ResolvedPermissionProfile;
use thinwedge_shell_escalation::ShellCommandExecutor;
use thinwedge_shell_escalation::ShellCommandExecutorFuture;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        sandbox_policy_cwd,
        windows_sandbox_workspace_roots,
        thinwedge_linux_sandbox_exe: ctx.turn.thinwedge_linux_sandbox_exe.clone(),
...
struct CoreShellCommandExecutor {
    ...
    network: Option<thinwedge_network_proxy::NetworkProxy>,
    windows_sandbox_level: WindowsSandboxLevel,
    arg0: Option<String>,
...
impl CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["THINWEDGE_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            thinwedge_linux_sandbox_exe: self.thinwedge_linux_sandbox_exe.as_deref(),
