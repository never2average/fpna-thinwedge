...
use thinwedge_shell_escalation::EscalationExecution;
use thinwedge_shell_escalation::EscalationPermissions;
...
fn extract_shell_script_supports_wrapped_command_prefixes() {
    assert_eq!(
        extract_shell_script(&[
            "/usr/bin/env".into(),
            "THINWEDGE_EXECVE_WRAPPER=1".into(),
            "/bin/zsh".into(),
            "-lc".into(),
            "echo hello".into()
        ])
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(thinwedge_execpolicy::Policy::empty())),
        ...
        stopwatch: thinwedge_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = thinwedge_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = thinwedge_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.thinwedge_home)
        .context("recreate thinwedge home for hook fixtures")?;
    ...
    let action = tokio::time::timeout(
        ...
        thinwedge_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        thinwedge_shell_escalation::EscalationDecision::Escalate(
            thinwedge_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, thinwedge_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        thinwedge_shell_escalation::EscalationDecision::Deny {
