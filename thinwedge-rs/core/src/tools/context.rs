...
pub enum ToolCallSource {
    ...
    CodeMode {
        ...
        /// debugging the JS/runtime bridge, but it is not the ThinWedge tool call id
    ...
    match response {
        ResponseInputItem::Message { content, .. } => content_items_to_code_mode_result(
            &content
                .into_iter()
                .map(|item| match item {
                    thinwedge_protocol::models::ContentItem::InputText { text }
                    | thinwedge_protocol::models::ContentItem::OutputText { text } => {
                        FunctionCallOutputContentItem::InputText { text }
                    }
                    thinwedge_protocol::models::ContentItem::InputImage { image_url, detail } => {
