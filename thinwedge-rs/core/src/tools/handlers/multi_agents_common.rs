...
pub(crate) fn collab_spawn_error(err: ThinWedgeErr) -> FunctionCallError {
    match err {
        ThinWedgeErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        ThinWedgeErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: ThinWedgeErr) -> FunctionCallError {
    match err {
        ThinWedgeErr::ThreadNotFound(id) => {
...
pub(crate) fn apply_spawn_agent_runtime_overrides(
    ...
    config.thinwedge_linux_sandbox_exe = turn.thinwedge_linux_sandbox_exe.clone();
    #[allow(deprecated)]
