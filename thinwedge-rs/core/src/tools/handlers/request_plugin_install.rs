...
use thinwedge_tools::DiscoverableToolAction;
use thinwedge_tools::DiscoverableToolType;
use thinwedge_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use thinwedge_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use thinwedge_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use thinwedge_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use thinwedge_tools::RequestPluginInstallArgs;
use thinwedge_tools::RequestPluginInstallResult;
use thinwedge_tools::ToolName;
use thinwedge_tools::ToolSpec;
use thinwedge_tools::all_requested_connectors_picked_up;
use thinwedge_tools::build_request_plugin_install_elicitation_request;
use thinwedge_tools::filter_request_plugin_install_discoverable_tools_for_client;
use thinwedge_tools::verified_connector_install_completed;
use tracing::warn;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        if args.tool_type == DiscoverableToolType::Plugin
            && turn.app_server_client_name.as_deref() == Some("thinwedge-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in thinwedge-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.thinwedge_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    thinwedge_home: &thinwedge_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    turn: &crate::session::turn_context::TurnContext,
    tool: &DiscoverableTool,
    auth: Option<&thinwedge_login::ThinWedgeAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match manager.hard_refresh_thinwedge_apps_tools_cache().await {
        Ok(mcp_tools) => {
            let accessible_connectors = connectors::with_app_enabled_state(
                connectors::accessible_connectors_from_mcp_tools(&mcp_tools),
                &turn.config,
        ...
        Err(err) => {
            warn!(
                "failed to refresh thinwedge apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }
