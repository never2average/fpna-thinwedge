...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.thinwedge_home)
        .await
        .expect("thinwedge home should be created");
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    thinwedge_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn multi_agent_v2_spawn_fork_turns_all_rejects_agent_type_override() {
    ...
    let turn = TurnContext {
        ...
        multi_agent_version: thinwedge_protocol::protocol::MultiAgentVersion::V2,
        ..turn
    };
...
async fn spawn_agent_service_tier_override_validates_the_effective_child_model() {
...

    {
        ...
        assert_eq!(
            err,
            FunctionCallError::RespondToModel(
                "Service tier `priority` is not supported for model `gpt-5.3-thinwedge`. Supported service tiers: none"
                    .to_string()
            )
        );
    }
...
async fn multi_agent_v2_list_agents_returns_completed_status_without_encrypted_spawn_preview() {
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_turn = child_thread.thinwedge.session.new_default_turn().await;
    child_thread
        .thinwedge
        .session
        .send_event(
...
async fn multi_agent_v2_followup_task_completion_notifies_parent_on_every_turn() {
    ...
    root.thread.thinwedge.session.new_default_turn().await;
    session.services.agent_control = manager.agent_control();
    session.thread_id = root.thread_id;
    let session = Arc::new(session);
    let turn = Arc::new(turn);
    ...
    let first_turn = thread.thinwedge.session.new_default_turn().await;
    thread
        .thinwedge
        .session
        .send_event(
    ...
    let second_turn = thread.thinwedge.session.new_default_turn().await;
    thread
        .thinwedge
        .session
        .send_event(
...
async fn multi_agent_v2_interrupted_turn_does_not_notify_parent() {
    ...
    let aborted_turn = thread.thinwedge.session.new_default_turn().await;
    thread
        .thinwedge
        .session
        .send_event(
...
async fn resume_agent_restores_closed_agent_and_accepts_send_input() {
    ...
    let thread = manager
        .resume_thread_with_history(
            ...
            AuthManager::from_auth_for_testing(ThinWedgeAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
        )
        .await
        .expect("start thread");
...
async fn multi_agent_v2_interrupt_agent_accepts_task_name_target() {
    ...
    let worker_session = worker_thread.thinwedge.session.clone();
    SpawnAgentHandlerV2::default()
        .handle(invocation(
...
async fn tool_handlers_cascade_close_and_resume_and_keep_explicitly_closed_subtrees_closed() {
    ...
    let parent_session = parent.thread.thinwedge.session.clone();
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_session = child_thread.thinwedge.session.clone();
    ...
    let operator_session = operator.thread.thinwedge.session.clone();
    let _ = manager
        .agent_control()
    ...
    let parent_resume_output = ResumeAgentHandler
        .handle(invocation(
            operator_session,
            operator.thread.thinwedge.session.new_default_turn().await,
            "resume_agent",
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    turn.thinwedge_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    #[allow(deprecated)]
    ...
    expected.thinwedge_linux_sandbox_exe = turn.thinwedge_linux_sandbox_exe.clone();
    #[allow(deprecated)]
    {
