...
use thinwedge_tools::ConversationHistory;
use thinwedge_tools::ExtensionTurnItem;
use thinwedge_tools::ToolCall as ExtensionToolCall;
use thinwedge_tools::ToolEnvironment;
use thinwedge_tools::ToolName;
use thinwedge_tools::ToolSearchInfo;
use thinwedge_tools::ToolSpec;
use thinwedge_tools::TurnItemEmissionFuture;
use thinwedge_tools::TurnItemEmitter;
...
pub(crate) struct ExtensionToolAdapter(Arc<dyn thinwedge_tools::ToolExecutor<ExtensionToolCall>>);

impl ExtensionToolAdapter {
    pub(crate) fn new(executor: Arc<dyn thinwedge_tools::ToolExecutor<ExtensionToolCall>>) -> Self {
        Self(executor)
    }
}
    ...
    fn emit_completed<'a>(&'a self, item: ExtensionTurnItem) -> TurnItemEmissionFuture<'a> {
        Box::pin(async move {
            ...
            finalize_turn_item(
                ...
                &mut item,
                turn.collaboration_mode.mode == thinwedge_protocol::config_types::ModeKind::Plan,
            )
            .await;
...
mod tests {
    ...
    use thinwedge_extension_api::TurnItemContributor;
    use thinwedge_protocol::items::TurnItem;
    ...
    impl thinwedge_extension_api::ToolExecutor<thinwedge_tools::ToolCall> for StubExtensionExecutor {
        fn tool_name(&self) -> thinwedge_tools::ToolName {
            thinwedge_tools::ToolName::plain("extension_echo")
        }

        fn spec(&self) -> thinwedge_tools::ToolSpec {
            thinwedge_tools::ToolSpec::Function(thinwedge_tools::ResponsesApiTool {
                name: "extension_echo".to_string(),
                description: "Echoes arguments.".to_string(),
                strict: true,
                parameters: thinwedge_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle(
            &self,
            _call: thinwedge_tools::ToolCall,
        ) -> thinwedge_tools::ToolExecutorFuture<'_> {
            Box::pin(async {
                Ok(
                    Box::new(thinwedge_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn thinwedge_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<thinwedge_tools::ToolCall>>>,
    }

    impl thinwedge_extension_api::ToolExecutor<thinwedge_tools::ToolCall>
        for CapturingExtensionExecutor
    {
        ...
        fn spec(&self) -> thinwedge_tools::ToolSpec {
            thinwedge_tools::ToolSpec::Function(thinwedge_tools::ResponsesApiTool {
                ...
                strict: false,
                parameters: thinwedge_tools::JsonSchema::default(),
                output_schema: None,
                defer_loading: None,
            })
        }
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: thinwedge_tools::ToolCall,
        ) -> Result<Box<dyn thinwedge_tools::ToolOutput>, thinwedge_tools::FunctionCallError>
        {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: thinwedge_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = thinwedge_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    impl TurnItemContributor for RecordExtensionTurnItemContributor {
        ...
        ) -> thinwedge_extension_api::ExtensionFuture<'a, Result<(), String>> {
            Box::pin(async move {
    ...
    async fn extension_completion_runs_turn_item_contributors() {
        ...
        thinwedge_tools::TurnItemEmitter::emit_completed(
            &emitter,
    ...
    {
        fn tool_name(&self) -> thinwedge_tools::ToolName {
            thinwedge_tools::ToolName::namespaced("image_gen", "imagegen")
        }
        ...
        ) -> Result<Box<dyn thinwedge_tools::ToolOutput>, thinwedge_tools::FunctionCallError>
        {
            call.turn_item_emitter
                .emit_started(ExtensionTurnItem::ImageGeneration(
                    thinwedge_protocol::items::ImageGenerationItem {
                        id: call.call_id.clone(),
    ...
    async fn image_generation_publication_is_finalized_by_core() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: thinwedge_tools::ToolName::namespaced("image_gen", "imagegen"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: "{}".to_string(),
            },
