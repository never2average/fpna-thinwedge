...
use thinwedge_tools::TOOL_SUGGEST_PERSIST_ALWAYS_VALUE;
use thinwedge_tools::TOOL_SUGGEST_PERSIST_KEY;
use thinwedge_tools::TOOL_SUGGEST_TOOL_NAME;
use thinwedge_tools::ToolSuggestArgs;
use thinwedge_tools::ToolSuggestResult;
use thinwedge_tools::all_suggested_connectors_picked_up;
...
impl ToolHandler for ToolSuggestHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin tool suggestions are not available in thinwedge-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_tool_suggest_disable(
    ...
    if let Err(err) = persist_tool_suggest_disable(&turn.config.thinwedge_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn refresh_missing_suggested_connectors(
    ...
    match manager.hard_refresh_thinwedge_apps_tools_cache().await {
        ...
        Err(err) => {
            warn!(
                "failed to refresh thinwedge apps tools cache after tool suggestion for {tool_id}: {err:#}"
            );
            None
        }
    }
