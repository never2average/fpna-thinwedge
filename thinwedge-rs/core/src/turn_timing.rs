...
use thinwedge_otel::TURN_TTFM_DURATION_METRIC;
use thinwedge_otel::TURN_TTFT_DURATION_METRIC;
    ...
    match item {
        ...
        ResponseItem::Reasoning {
            ...
            summary.iter().any(|entry| match entry {
                thinwedge_protocol::models::ReasoningItemReasoningSummary::SummaryText { text } => {
                    !text.is_empty()
                }
            }) || content.as_ref().is_some_and(|entries| {
                entries.iter().any(|entry| match entry {
                    thinwedge_protocol::models::ReasoningItemContent::ReasoningText { text }
                    | thinwedge_protocol::models::ReasoningItemContent::Text { text } => {
                        !text.is_empty()
                    }
                })
            })
