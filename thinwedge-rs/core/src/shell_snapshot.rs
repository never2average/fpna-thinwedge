...
impl ShellSnapshot {
    ...
    fn spawn_snapshot_task(
        ...
        tokio::spawn(
            async move {
                let timer = session_telemetry.start_timer("thinwedge.shell_snapshot.duration_ms", &[]);
                let snapshot =
                    ShellSnapshot::try_new(&thinwedge_home, session_id, &session_cwd, &snapshot_shell)
                        .await
                        .map(Arc::new);
                ...
                session_telemetry.counter("thinwedge.shell_snapshot", /*inc*/ 1, &counter_tags);
    ...
    async fn try_new(
        ...
        let temp_path = thinwedge_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.tmp-{nonce}"));

        // Clean the (unlikely) leaked snapshot files.
        let thinwedge_home = thinwedge_home.clone();
        let cleanup_session_id = session_id;
        tokio::spawn(async move {
            if let Err(err) = cleanup_stale_snapshots(&thinwedge_home, cleanup_session_id).await {
...
async fn run_script_with_timeout(
    ...
    unsafe {
        handler.pre_exec(|| {
            thinwedge_utils_pty::process_group::detach_from_tty()?;
            Ok(())
        });
    }
...
pub async fn cleanup_stale_snapshots(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path = find_thread_path_by_id_str(thinwedge_home, session_id).await?;
