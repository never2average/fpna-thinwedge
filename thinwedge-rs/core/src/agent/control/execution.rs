...
impl AgentControl {
    pub(crate) async fn ensure_execution_capacity_for_op(
        ...
        if thread.thinwedge.session.active_turn.lock().await.is_some() {
            return Ok(());
        }
        let config = thread.thinwedge.session.get_config().await;
    ...
    pub(crate) fn ensure_execution_capacity(
        ...
        if self.agent_execution_limiter.has_capacity() {
            ...
            Err(ThinWedgeErr::AgentLimitReached { max_threads })
        }
    }
