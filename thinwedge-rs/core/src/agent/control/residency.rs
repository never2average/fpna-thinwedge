...
impl AgentControl {
    ...
    ) -> ThinWedgeResult<V2ResidencySlot> {
        let capacity = config
    ...
    async fn reserve_slot(
        ...
        loop {
            ...
            if !self
                ...
                .await
            {
                return Err(ThinWedgeErr::AgentLimitReached {
...
fn is_resident_candidate(thread: &ThinWedgeThread) -> bool {
...
async fn is_unloadable(thread: &ThinWedgeThread) -> bool {
    matches!(
        thread.agent_status().await,
        AgentStatus::Completed(_) | AgentStatus::Errored(_) | AgentStatus::Interrupted
    ) && thread.thinwedge.session.active_turn.lock().await.is_none()
        && !thread
            .thinwedge
            .session
            .input_queue
