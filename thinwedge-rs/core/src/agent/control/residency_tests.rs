...
async fn residency_slot_reservation_unloads_oldest_idle_v2_agent() {
    ...
    config.thinwedge_home = temp_home.path().to_path_buf().try_into().unwrap();
    ...
    match manager.get_thread(first.thread_id).await {
        Err(ThinWedgeErr::ThreadNotFound(thread_id)) => assert_eq!(thread_id, first.thread_id),
...
async fn interrupted_v2_agent_is_lost_after_residency_eviction() {
    ...
    match err {
        ThinWedgeErr::ThreadNotFound(thread_id) => assert_eq!(thread_id, first.thread_id),
...
async fn mark_thread_completed(thread: &ThinWedgeThread) {
    let turn = thread.thinwedge.session.new_default_turn().await;
    thread
        .thinwedge
        .session
        .send_event(
...
async fn mark_thread_interrupted(thread: &ThinWedgeThread) {
    let turn = thread.thinwedge.session.new_default_turn().await;
    thread
        .thinwedge
        .session
...
async fn clear_active_turn(thread: &ThinWedgeThread) {
    // The fixture has no task runner to clear the turn after the terminal event.
    *thread.thinwedge.session.active_turn.lock().await = None;
}
