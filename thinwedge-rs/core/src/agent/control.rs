...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> ThinWedgeThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    async fn spawn_agent_internal(
        ...
        )) = notification_source.as_ref()
        {
            ...
            let thread_config = new_thread.thread.thinwedge.thread_config_snapshot().await;
    ...
    async fn spawn_forked_thread(
    ...
    ) -> ThinWedgeResult<crate::thread_manager::NewThread> {
        if options.fork_parent_spawn_call_id.is_none() {
            return Err(ThinWedgeErr::Fatal(
        ...
        if let Some(parent_thread) = parent_thread.as_ref() {
            ...
            parent_thread
                ...
                .await;
            parent_thread.thinwedge.session.flush_rollout().await?;
        }
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> ThinWedgeResult<String> {
        let state = self.upgrade()?;
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: ThinWedgeResult<String>,
    ) -> ThinWedgeResult<String> {
        if matches!(result, Err(ThinWedgeErr::InternalAgentDied)) {
    ...
    pub(crate) async fn shutdown_live_agent(&self, agent_id: ThreadId) -> ThinWedgeResult<String> {
        let state = self.upgrade()?;
        let result = if let Ok(thread) = state.get_thread(agent_id).await {
            thread.thinwedge.session.ensure_rollout_materialized().await;
            thread.thinwedge.session.flush_rollout().await?;
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> ThinWedgeResult<String> {
        let state = self.upgrade()?;
    ...
    async fn shutdown_agent_tree(&self, agent_id: ThreadId) -> ThinWedgeResult<String> {
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        Err(ThinWedgeErr::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> ThinWedgeResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
    ...
    ) -> ThinWedgeResult<Vec<ListedAgent>> {
        let state = self.upgrade()?;
    ...
    ) -> ThinWedgeResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> ThinWedgeResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| ThinWedgeErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
    ...
    async fn inherited_exec_policy_for_source(
        ...
        let parent_thread = state.get_thread(*parent_thread_id).await.ok()?;
        let parent_config = parent_thread.thinwedge.session.get_config().await;
        if !crate::exec_policy::child_uses_parent_exec_policy(&parent_config, child_config) {
            return None;
        }

        Some(Arc::clone(
            &parent_thread.thinwedge.session.services.exec_policy,
        ))
    }
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> ThinWedgeResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> ThinWedgeResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
