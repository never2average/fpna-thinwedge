...
async fn test_config_with_cli_overrides(
    ...
    let config = ConfigBuilder::default()
        .thinwedge_home(home_path.clone())
...
async fn apply_role_preserves_unspecified_keys() {
    ...
    assert_eq!(config.model_reasoning_effort, Some(ReasoningEffort::High));
    assert_eq!(
        config.thinwedge_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/thinwedge-linux-sandbox"))
    );
    assert_eq!(
...
"#,
    ...
    let outcome = skills_manager
        .skills_for_config(
            &skills_input,
            Some(Arc::clone(&thinwedge_exec_server::LOCAL_FS)),
        )
        .await;
    let skill = outcome
        .skills
