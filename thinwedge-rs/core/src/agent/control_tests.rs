...
use thinwedge_thread_store::ArchiveThreadParams;
...
impl AgentControlHarness {
    ...
    async fn new_with_config(home: TempDir, config: Config) -> Self {
        ...
        let manager = ThreadManager::with_models_provider_home_and_state_for_tests(
            ...
            std::sync::Arc::new(thinwedge_exec_server::EnvironmentManager::default_for_tests()),
            state_db.clone(),
        );
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<ThinWedgeThread>) {
        let new_thread = self
            .manager
...
async fn wait_for_subagent_notification(parent_thread: &Arc<ThinWedgeThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<ThinWedgeThread>, message: &str) {
    thread
...
async fn assert_thread_not_loaded(manager: &ThreadManager, thread_id: ThreadId) {
    match manager.get_thread(thread_id).await {
        Err(ThinWedgeErr::ThreadNotFound(id)) => assert_eq!(id, thread_id),
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(err, ThinWedgeErr::ThreadNotFound(id) if id == thread_id);
}
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            if thread
                ...
                .session
                .input_queue
                .has_pending_input(&thread.thinwedge.session.active_turn)
                .await
            {
                break;
            }
...
async fn ensure_v2_agent_loaded_reloads_registered_unloaded_agent() {
    ...
    match harness.manager.get_thread(spawned_agent.thread_id).await {
        Err(ThinWedgeErr::ThreadNotFound(id)) => assert_eq!(id, spawned_agent.thread_id),
        Err(err) => panic!("expected ThreadNotFound, got {err:?}"),
...
async fn resume_agent_from_rollout_does_not_reopen_v2_descendants() {
    ...
    let resumed_manager = ThreadManager::with_models_provider_home_and_state_for_tests(
        ...
        harness.config.thinwedge_home.to_path_buf(),
        std::sync::Arc::new(thinwedge_exec_server::EnvironmentManager::default_for_tests()),
...
async fn spawn_agent_can_fork_parent_thread_history_with_sanitized_items() {
    ...
    parent_thread
        ...
        .await;
    let turn_context = parent_thread.thinwedge.session.new_default_turn().await;
    ...
    let history = child_thread.thinwedge.session.clone_history().await;
...
async fn spawn_agent_fork_last_n_turns_keeps_only_recent_turns() {
    ...
    let queued_turn_context = parent_thread.thinwedge.session.new_default_turn().await;
    parent_thread
        .thinwedge
        .session
        .record_conversation_items(
    ...
    let triggered_turn_context = parent_thread.thinwedge.session.new_default_turn().await;
    parent_thread
        .thinwedge
        .session
        .record_conversation_items(
    ...
    parent_thread
        ...
        .await;
    let spawn_turn_context = parent_thread.thinwedge.session.new_default_turn().await;
...
async fn spawn_agent_fork_last_n_turns_drops_parent_startup_prefix_when_under_limit() {
    let harness = AgentControlHarness::new().await;
    let (parent_thread_id, parent_thread) = harness.start_thread().await;
    let startup_turn_context = parent_thread.thinwedge.session.new_default_turn().await;
    parent_thread
        .thinwedge
        .session
        .record_conversation_items(
...
async fn spawn_agent_respects_max_threads_limit() {
    ...
    let ThinWedgeErr::AgentLimitReached {
        max_threads: seen_max_threads,
    } = err
    else {
        panic!("expected ThinWedgeErr::AgentLimitReached");
    };
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let ThinWedgeErr::AgentLimitReached { max_threads } = err else {
        panic!("expected ThinWedgeErr::AgentLimitReached");
    };
...
async fn multi_agent_v2_completion_ignores_dead_direct_parent() {
    ...
    let tester_turn = tester_thread.thinwedge.session.new_default_turn().await;
    tester_thread
        .thinwedge
        .session
        .send_event(
