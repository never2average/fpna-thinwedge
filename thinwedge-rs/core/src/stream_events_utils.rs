...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<thinwedge_protocol::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                thinwedge_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
...
async fn save_image_generation_result(
    ...
    let bytes = BASE64_STANDARD
        ...
        .map_err(|err| {
            ThinWedgeErr::InvalidRequest(format!("invalid image generation payload: {err}"))
        })?;
    let path = image_generation_artifact_path(thinwedge_home, session_id, call_id);
    ...
    .await
    {
        ...
        Err(err) => {
            ...
            let output_dir = output_path
                .parent()
                .unwrap_or_else(|| turn_context.config.thinwedge_home.clone());
            tracing::warn!(
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(item.clone()) {
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(ThinWedgeErr::Fatal(message));
        }
    }
...
pub(crate) async fn finalize_turn_item(
    ...
    if let TurnItem::AgentMessage(agent_message) = &mut *turn_item {
        ...
        agent_message.content =
            vec![thinwedge_protocol::items::AgentMessageContent::Text { text: stripped }];
