...
pub fn sandbox_setup_is_complete(thinwedge_home: &Path) -> bool {
    thinwedge_windows_sandbox::sandbox_setup_is_complete(thinwedge_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_thinwedge_home: &Path) -> bool {
    false
}

#[cfg(target_os = "windows")]
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    let failure = thinwedge_windows_sandbox::extract_setup_failure(err)?;
    let code = failure.code.as_str().to_string();
    let message = thinwedge_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
pub fn elevated_setup_failure_metric_name(err: &anyhow::Error) -> &'static str {
    if thinwedge_windows_sandbox::extract_setup_failure(err).is_some_and(|failure| {
        matches!(
            failure.code,
            thinwedge_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
        )
    }) {
        "thinwedge.windows_sandbox.elevated_setup_canceled"
    } else {
        "thinwedge.windows_sandbox.elevated_setup_failure"
    }
}
...
pub fn run_elevated_setup(
    ...
    let permissions =
        thinwedge_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
            permission_profile,
            workspace_roots,
        )?;
    thinwedge_windows_sandbox::run_elevated_setup(
        thinwedge_windows_sandbox::SandboxSetupRequest {
        ...
        thinwedge_windows_sandbox::SetupRootOverrides::default(),
    )
}
...
pub fn run_elevated_provisioning_setup(
    ...
    thinwedge_windows_sandbox::run_elevated_provisioning_setup(thinwedge_home, real_user)
}
...
pub fn run_legacy_setup_preflight(
    ...
    thinwedge_windows_sandbox::run_windows_sandbox_legacy_preflight(
        permission_profile,
        workspace_roots,
        thinwedge_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    thinwedge_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        permission_profile,
        workspace_roots,
        command_cwd,
        env_map,
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let thinwedge_home = request.thinwedge_home;
    let setup_thinwedge_home = thinwedge_home.clone();

    let setup_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        match mode {
            WindowsSandboxSetupMode::Elevated => {
                if !sandbox_setup_is_complete(setup_thinwedge_home.as_path()) {
                    run_elevated_setup(
                        &permission_profile,
                        workspace_roots.as_slice(),
                        command_cwd.as_path(),
                        &env_map,
                        setup_thinwedge_home.as_path(),
                    )?;
                }
            }
    ...
    ConfigEditsBuilder::new(thinwedge_home.as_path())
...
fn emit_windows_sandbox_setup_success_metrics(
    ...
    let Some(metrics) = thinwedge_otel::global() else {
        return;
    };
    let mode_tag = windows_sandbox_setup_mode_tag(mode);
    let _ = metrics.record_duration(
        "thinwedge.windows_sandbox.setup_duration_ms",
        duration,
        &[
            ("result", "success"),
    ...
    let _ = metrics.counter(
        "thinwedge.windows_sandbox.setup_success",
        /*inc*/ 1,
...
fn emit_windows_sandbox_setup_failure_metrics(
    ...
    let _ = metrics.counter(
        "thinwedge.windows_sandbox.setup_failure",
        /*inc*/ 1,
        &[("originator", originator_tag), ("mode", mode_tag)],
    );
    ...
    if matches!(mode, WindowsSandboxSetupMode::Elevated) {
        ...
        let _ = metrics.counter(
            "thinwedge.windows_sandbox.legacy_setup_preflight_failed",
            /*inc*/ 1,
            &[("originator", originator_tag)],
        );
    }
