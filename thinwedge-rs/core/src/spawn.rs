...
///
/// 1. The process was spawned by ThinWedge as part of a shell tool call.
/// 2. NetworkSandboxPolicy is restricted for the tool call.
///
/// We may try to have just one environment variable for all sandboxing
/// attributes, so this may change in the future.
pub const THINWEDGE_SANDBOX_NETWORK_DISABLED_ENV_VAR: &str = "THINWEDGE_SANDBOX_NETWORK_DISABLED";
...
pub const THINWEDGE_SANDBOX_ENV_VAR: &str = "THINWEDGE_SANDBOX";
...
/// `THINWEDGE_SANDBOX_NETWORK_DISABLED_ENV_VAR` environment variable.
...
pub(crate) async fn spawn_child_async(request: SpawnChildRequest<'_>) -> std::io::Result<Child> {
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(THINWEDGE_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }

    // If this ThinWedge process dies (including being killed via SIGKILL), we want
    ...
    unsafe {
        ...
        cmd.pre_exec(move || {
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // current parent dies."
                thinwedge_utils_pty::process_group::set_parent_death_signal(parent_pid)?;
            }
            Ok(())
        });
    }
