...
async fn forward_ops_preserves_submission_trace_context() {
    ...
    let cancel = CancellationToken::new();
    let forward = tokio::spawn(forward_ops(Arc::clone(&thinwedge), rx_ops, cancel));

    let submission = Submission {
        id: "sub-1".to_string(),
        op: Op::Interrupt,
        client_user_message_id: None,
        trace: Some(thinwedge_protocol::protocol::W3cTraceContext {
            traceparent: Some(
...
#[tokio::test]
async fn run_thinwedge_thread_interactive_respects_pre_cancelled_spawn() {
    ...
    let result = timeout(
        ...
        run_thinwedge_thread_interactive(
    ...
    assert!(matches!(result, Err(ThinWedgeErr::TurnAborted)));
}
