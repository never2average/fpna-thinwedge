...
use thinwedge_analytics::GuardianReviewDecision;
use thinwedge_analytics::GuardianReviewFailureReason;
use thinwedge_analytics::GuardianReviewTerminalStatus;
use thinwedge_analytics::GuardianReviewTrackContext;
...
impl GuardianReviewError {
    ...
    fn session_with_error_info(err: anyhow::Error, error_info: ThinWedgeErrorInfo) -> Self {
        Self::Session {
            message: err.to_string(),
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &thinwedge_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        thinwedge_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(label))
...
async fn run_guardian_review_session_before_deadline(
    ...
    let available_models = session
        .services
        .models_manager
        .list_models(thinwedge_models_manager::manager::RefreshStrategy::Offline)
        .await;
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = review_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset.supported_reasoning_efforts.iter().any(|effort| {
                effort.effort == thinwedge_protocol::openai_models::ReasoningEffort::Low
            }),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| {
                    preset.effort == thinwedge_protocol::openai_models::ReasoningEffort::Low
                }),
            ...
            GuardianReviewError::Session {
                error_info: Some(
                    ThinWedgeErrorInfo::ServerOverloaded
                        | ThinWedgeErrorInfo::HttpConnectionFailed { .. }
                        | ThinWedgeErrorInfo::ResponseStreamConnectionFailed { .. }
                        | ThinWedgeErrorInfo::InternalServerError
                        | ThinWedgeErrorInfo::ResponseStreamDisconnected { .. }
                ),
                ..
            } | GuardianReviewError::Parse { .. }
        )
