...
pub(crate) fn is_guardian_reviewer_source(
    ...
    matches!(
        session_source,
        thinwedge_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(name))
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = preferred_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset.supported_reasoning_efforts.iter().any(|effort| {
                effort.effort == thinwedge_protocol::thinwedge_models::ReasoningEffort::Low
            }),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| {
                    preset.effort == thinwedge_protocol::thinwedge_models::ReasoningEffort::Low
                }),
