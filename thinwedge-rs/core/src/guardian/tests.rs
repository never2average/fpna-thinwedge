...
fn guardian_shell_request(id: &str) -> GuardianApprovalRequest {
    GuardianApprovalRequest::Shell {
        ...
        cwd: test_path_buf("/repo/thinwedge-rs/core").abs(),
...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    let mut text = text;
    for canonical_path in ["/repo/thinwedge-rs/core", "/repo"] {
...
fn guardian_prompt_text(items: &[thinwedge_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            thinwedge_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
...
async fn build_guardian_prompt_full_mode_preserves_initial_review_format() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The ThinWedge agent has requested the following action:\n"));
...
async fn build_guardian_prompt_delta_mode_preserves_original_numbering() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The ThinWedge agent has requested the following next action:\n"));
...
async fn build_guardian_prompt_items_explains_network_access_review_scope() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "thinwedge_core__guardian__tests__network_access_guardian_prompt_layout",
...
async fn guardian_request_model_for_auto_review(
    ...
    String,
    String,
    thinwedge_analytics::GuardianReviewAnalyticsResult,
)> {
...
-> anyhow::Result<()> {
    ...
    assert!(matches!(
        ...
        Some(thinwedge_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    let request = request_log.single_request();
    ...
    settings.bind(|| {
        assert_snapshot!(
            "thinwedge_core__guardian__tests__guardian_review_request_layout",
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            thinwedge_protocol::user_input::UserInput::Text { text, .. } => text,
            thinwedge_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    assert!(matches!(
        ...
        Some(thinwedge_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "thinwedge_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
