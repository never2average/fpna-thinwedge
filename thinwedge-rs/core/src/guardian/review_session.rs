...
use thinwedge_analytics::GuardianReviewSessionAnalyticsParams;
use thinwedge_analytics::GuardianReviewSessionKind;
use thinwedge_extension_api::UserInstructions;
...
pub(crate) enum GuardianReviewSessionOutcome {
    ...
    SessionFailed {
        error: anyhow::Error,
        error_info: Option<ThinWedgeErrorInfo>,
    },
    TimedOut,
    Aborted,
}
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(
        ...
        Self {
            ...
            thinwedge_linux_sandbox_exe: spawn_config.thinwedge_linux_sandbox_exe.clone(),
...
impl GuardianReviewSession {
    async fn shutdown(&self) {
        ...
        let _ = self.thinwedge.shutdown_and_wait().await;
    }
    ...
    async fn refresh_last_committed_fork_snapshot(&self) {
        match load_rollout_items_for_fork(&self.thinwedge.session).await {
...
impl GuardianReviewSessionManager {
    pub(crate) async fn trunk_rollout_path(&self) -> Option<PathBuf> {
        ...
        trunk.thinwedge.session.ensure_rollout_materialized().await;
        match trunk.thinwedge.session.current_rollout_path().await {
            Ok(path) => path,
            Err(err) => {
    ...
    #[cfg(test)]
    pub(crate) async fn cache_for_test(&self, thinwedge: ThinWedge) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            thinwedge.session.get_config().await.as_ref(),
            thinwedge.session.user_instructions().await,
        );
    ...
    #[cfg(test)]
    pub(crate) async fn register_ephemeral_for_test(&self, thinwedge: ThinWedge) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            thinwedge.session.get_config().await.as_ref(),
            thinwedge.session.user_instructions().await,
        );
    ...
    pub(crate) async fn send_trunk_event_raw_for_test(&self, event: Event) {
        ...
        trunk.thinwedge.session.send_event_raw(event).await;
    }
...
async fn spawn_guardian_review_session(
    ...
    let thinwedge = Box::pin(run_thinwedge_thread_interactive(
        spawn_config,
...
async fn run_review_on_session(
    ...
    let mut analytics_result =
        GuardianReviewAnalyticsResult::from_session(GuardianReviewSessionAnalyticsParams {
            guardian_thread_id: review_session.thinwedge.session.thread_id.to_string(),
            guardian_session_kind,
        ...
        Box::pin(async {
            params
                ...
                .sync_session_approved_hosts_to(
                    &review_session.thinwedge.session.services.network_approval,
                )
                .await;
    ...
    let submit_result = run_before_review_deadline(
        deadline,
        params.external_cancel.as_ref(),
        Box::pin(review_session.thinwedge.submit(Op::UserInput {
            ...
            thread_settings: thinwedge_protocol::protocol::ThreadSettingsOverrides {
                environments: Some(
                    thinwedge_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn wait_for_guardian_review(
    ...
    loop {
        tokio::select! {
            ...
            event = review_session.thinwedge.next_event() => {
                match event {
                                ...
                                && let Some(error) = last_error
                            {
                                return (
                                    GuardianReviewSessionOutcome::SessionFailed {
                                        ...
                                        error_info: error.thinwedge_error_info,
                                    },
                                    true,
                                    true,
                                );
...
mod tests {
    ...
    async fn test_review_params() -> GuardianReviewSessionParams {
        ...
        GuardianReviewSessionParams {
            ...
            model,
            reasoning_effort,
            guardian_default_review_model_id: "thinwedge-auto-review".to_string(),
    ...
    async fn guardian_review_session_config_disables_hooks() {
        ...
        assert!(!guardian_config.features.enabled(Feature::ThinWedgeHooks));
    }
    ...
    async fn wait_for_guardian_review_preserves_structured_session_error() {
        ...
        tx_event
            .send(Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    thinwedge_error_info: Some(ThinWedgeErrorInfo::ServerOverloaded),
                }),
            })
            .await
        ...
        assert_eq!(error_info, Some(ThinWedgeErrorInfo::ServerOverloaded));
        assert!(keep_review_session);
        assert!(capture_token_usage);
    }
    ...
    async fn interrupt_and_drain_turn_ignores_prior_turn_completion() {
        ...
        interrupt_and_drain_turn(&review_session.thinwedge, "current-turn")
            .await
            .expect("drain current turn");

        assert!(review_session.thinwedge.rx_event.try_recv().is_err());
    }
}
