...
async fn run_review_on_session(
    ...
    let mut analytics_result = GuardianReviewAnalyticsResult::from_session(
        review_session.thinwedge.session.conversation_id.to_string(),
        guardian_session_kind,
        params.model.clone(),
    ...
    let submit_result = run_before_review_deadline(
        deadline,
        params.external_cancel.as_ref(),
        Box::pin(review_session.thinwedge.submit(Op::UserTurn {
            environments: None,
...
async fn append_guardian_followup_reminder(review_session: &GuardianReviewSession) {
    let turn_context = review_session.thinwedge.session.new_default_turn().await;
...
async fn wait_for_guardian_review(
    ...
    loop {
        tokio::select! {
            _ = &mut timeout => {
                let keep_review_session = interrupt_and_drain_turn(&review_session.thinwedge).await.is_ok();
...
async fn interrupt_and_drain_turn(thinwedge: &ThinWedge) -> anyhow::Result<()> {
