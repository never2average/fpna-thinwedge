...
use thinwedge_mcp::MCP_TOOL_THINWEDGE_APPS_META_KEY;
...
use thinwedge_mcp::auth_elicitation_completed_result;
use thinwedge_mcp::build_auth_elicitation_plan;
...
use thinwedge_protocol::items::McpToolCallError;
use thinwedge_protocol::items::McpToolCallItem;
use thinwedge_protocol::items::McpToolCallStatus;
use thinwedge_protocol::items::TurnItem;
use thinwedge_protocol::mcp::CallToolResult;
use thinwedge_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use thinwedge_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use thinwedge_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use thinwedge_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use thinwedge_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use thinwedge_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use thinwedge_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use thinwedge_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use thinwedge_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use thinwedge_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use thinwedge_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use thinwedge_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use thinwedge_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use thinwedge_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
const MCP_CALL_COUNT_METRIC: &str = "thinwedge.mcp.call";
const MCP_CALL_DURATION_METRIC: &str = "thinwedge.mcp.call.duration_ms";
const MCP_RESULT_TELEMETRY_META_KEY: &str = "thinwedge/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "thinwedge.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str =
    "thinwedge.mcp.server_user_flow.triggered";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == THINWEDGE_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == THINWEDGE_APPS_MCP_SERVER_NAME {
    ...
    if server == THINWEDGE_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
...
async fn handle_approved_mcp_tool_call(
    ...
    .await;
    maybe_track_thinwedge_app_used(sess, turn_context, &server, &tool_name).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        thinwedge.mcp.target.id = Empty,
        thinwedge.mcp.server_user_flow.triggered = Empty,
    );
...
async fn execute_mcp_tool_call(
    ...
    Ok(maybe_request_thinwedge_apps_auth_elicitation(
        sess,
        turn_context,
        call_id,
        &invocation.server,
...
async fn maybe_request_thinwedge_apps_auth_elicitation(
    sess: &Session,
    turn_context: &TurnContext,
    call_id: &str,
    server: &str,
    ...
    if !sess
        ...
        .mcp_connection_manager
        .load_full()
        .is_host_owned_thinwedge_apps_server(server)
    {
        return result;
    }
    ...
    let install_url = connector_id.map(|connector_id| {
        thinwedge_connectors::metadata::connector_install_url(
    ...
    refresh_thinwedge_apps_after_connector_auth(sess, turn_context).await;
    auth_elicitation_completed_result(&plan.auth_failure, result.meta)
}

async fn refresh_thinwedge_apps_after_connector_auth(sess: &Session, turn_context: &TurnContext) {
    let mcp_tools_result = {
        let manager = sess.services.mcp_connection_manager.load_full();
        manager.hard_refresh_thinwedge_apps_tools_cache().await
    };
    ...
    match mcp_tools_result {
        ...
        Err(err) => {
            tracing::warn!("failed to refresh ThinWedge Apps tools after connector auth: {err:#}");
        }
    }
}
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        ...
        thinwedge_linux_sandbox_exe: turn_context.thinwedge_linux_sandbox_exe.clone(),
        #[allow(deprecated)]
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                thinwedge_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
async fn maybe_track_thinwedge_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    tool_name: &str,
) {
    if server != THINWEDGE_APPS_MCP_SERVER_NAME {
        return;
    }
...
fn build_mcp_tool_call_request_meta(
    ...
    {
        request_meta.insert(
            crate::X_THINWEDGE_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }

    if server == THINWEDGE_APPS_MCP_SERVER_NAME {
        let mut thinwedge_apps_meta = metadata
            .and_then(|metadata| metadata.thinwedge_apps_meta.clone())
            .unwrap_or_default();
        thinwedge_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_THINWEDGE_APPS_META_KEY.to_string(),
            serde_json::Value::Object(thinwedge_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__thinwedge_mcp_decline__";
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == THINWEDGE_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    let connector_description = if server == THINWEDGE_APPS_MCP_SERVER_NAME {
    ...
    Some(McpToolApprovalMetadata {
        ...
        thinwedge_apps_meta: tool_info
            .tool
            .meta
            .as_ref()
            .and_then(|meta| meta.get(MCP_TOOL_THINWEDGE_APPS_META_KEY))
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == THINWEDGE_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_thinwedge_app_tool_approval(&turn_context.config, &connector_id, &tool_name).await
    } else {
...
async fn persist_thinwedge_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, thinwedge_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
