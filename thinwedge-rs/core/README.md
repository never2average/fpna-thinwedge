...
This crate implements the business logic for ThinWedge. It is designed to be used by the various ThinWedge UIs written in Rust.

## Dependencies

Note that `thinwedge-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.thinwedge` read-only.
...
Expects the binary containing `thinwedge-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--thinwedge-run-as-apply-patch`. See the
`thinwedge-arg0` crate for details.
