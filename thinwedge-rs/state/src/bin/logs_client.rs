...
#[command(about = "Tail ThinWedge logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to THINWEDGE_HOME. Defaults to $THINWEDGE_HOME or ~/.thinwedge.
    #[arg(long, env = "THINWEDGE_HOME")]
    thinwedge_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --thinwedge-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let thinwedge_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(thinwedge_home, "logs-client".to_string()).await?;
...
fn default_thinwedge_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".thinwedge");
    }
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["thinwedge-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["thinwedge-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["thinwedge-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["thinwedge-state-logs", "--level", "WARN"])
