...
//! orchestration and rollout scanning live in `thinwedge-core`.
...
pub const SQLITE_HOME_ENV: &str = "THINWEDGE_SQLITE_HOME";
...
pub const DB_ERROR_METRIC: &str = "thinwedge.db.error";
/// Metrics on backfill process. Tags: [status]
pub const DB_METRIC_BACKFILL: &str = "thinwedge.db.backfill";
/// Metrics on backfill duration. Tags: [status]
pub const DB_METRIC_BACKFILL_DURATION_MS: &str = "thinwedge.db.backfill.duration_ms";
/// SQLite initialization attempts. Tags: [status, phase, db, error]
pub const DB_INIT_METRIC: &str = "thinwedge.sqlite.init.count";
/// SQLite initialization latency. Tags: [status, phase, db, error]
pub const DB_INIT_DURATION_METRIC: &str = "thinwedge.sqlite.init.duration_ms";
/// Rollout fallback attempts. Tags: [caller, reason]
pub const DB_FALLBACK_METRIC: &str = "thinwedge.sqlite.fallback.count";
