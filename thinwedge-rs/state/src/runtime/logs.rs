...
mod tests {
    ...
    async fn insert_logs_use_dedicated_log_database() {
        ...
        let logs_count = log_row_count(logs_db_path(thinwedge_home.as_path()).as_path()).await;
    ...
    async fn init_recreates_legacy_logs_db_when_log_version_changes() {
        ...
        tokio::fs::create_dir_all(&thinwedge_home)
            .await
            .expect("create thinwedge home");
        let legacy_logs_path = thinwedge_home.join("logs_1.sqlite");
        let pool = SqlitePool::connect_with(
            SqliteConnectOptions::new()
        ...
        assert!(
            logs_db_path(thinwedge_home.as_path()).exists(),
    ...
    async fn init_configures_logs_db_with_incremental_auto_vacuum() {
        ...
        let pool = open_db_pool(logs_db_path(thinwedge_home.as_path()).as_path()).await;
