...
mod tests {
    ...
    async fn stage1_claim_skips_when_up_to_date() {
        ...
        let metadata = test_thread_metadata(&thinwedge_home, thread_id, thinwedge_home.join("a"));
        runtime
    ...
    async fn stage1_running_stale_can_be_stolen_but_fresh_running_is_skipped() {
        ...
        let owner_a = ThreadId::from_string(&Uuid::new_v4().to_string()).expect("owner id");
        let owner_b = ThreadId::from_string(&Uuid::new_v4().to_string()).expect("owner id");
        let cwd = thinwedge_home.join("workspace");
        runtime
            .upsert_thread(&test_thread_metadata(&thinwedge_home, thread_id, cwd))
            .await
            .expect("upsert thread");
    ...
    async fn stage1_concurrent_claims_respect_running_cap() {
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &thinwedge_home,
                thread_a,
                thinwedge_home.join("workspace-a"),
            ))
            .await
    ...
    async fn claim_stage1_jobs_filters_by_age_idle_and_current_thread() {
        ...
        let mut just_under_idle = test_thread_metadata(
            ...
            thinwedge_home.join("just-under-idle"),
        );
        ...
        let mut eligible_idle = test_thread_metadata(
            ...
            thinwedge_home.join("eligible-idle"),
        );
    ...
    async fn claim_stage1_jobs_prefilters_threads_with_up_to_date_memory() {
        ...
        let mut up_to_date = test_thread_metadata(
            ...
            thinwedge_home.join("up-to-date"),
        );
    ...
    async fn claim_stage1_jobs_enforces_global_running_cap() {
        ...
        for idx in 0..total_candidates {
            ...
            let mut metadata = test_thread_metadata(
                &thinwedge_home,
                thread_id,
                thinwedge_home.join(format!("thread-{idx}")),
            );
...
WHERE kind = 'memory_stage1'
  ...
  AND lease_until > ?
    ...
    async fn list_stage1_outputs_for_global_returns_latest_outputs() {
        ...
        assert_eq!(outputs[0].cwd, thinwedge_home.join("workspace-b"));
        ...
        assert_eq!(outputs[1].cwd, thinwedge_home.join("workspace-a"));
    ...
    async fn list_stage1_outputs_for_global_skips_empty_payloads() {
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &thinwedge_home,
                thread_id_non_empty,
                thinwedge_home.join("workspace-non-empty"),
            ))
            .await
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &thinwedge_home,
                thread_id_empty,
                thinwedge_home.join("workspace-empty"),
            ))
            .await
...
VALUES (?, ?, ?, ?, ?)
        ...
        assert_eq!(outputs[0].cwd, thinwedge_home.join("workspace-non-empty"));
    ...
    async fn get_phase2_input_selection_returns_current_selected_rows() {
        ...
        assert_eq!(
            ...
            thinwedge_home.join(format!("rollout-{thread_id_c}.jsonl"))
        );
        ...
        for (thread_id, updated_at) in [(thread_id_enabled, 100), (thread_id_polluted, 101)] {
            runtime
                .upsert_thread(&test_thread_metadata(
                    &thinwedge_home,
                    thread_id,
                    thinwedge_home.join(thread_id.to_string()),
                ))
                .await
                .expect("upsert thread");
