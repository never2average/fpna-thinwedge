...
mod tests {
    ...
    async fn init_removes_legacy_state_db_files() {
        ...
        for suffix in ["", "-wal", "-shm", "-journal"] {
            let path = thinwedge_home.join(format!("{unversioned_name}{suffix}"));
            tokio::fs::write(path, b"legacy")
                .await
                .expect("write legacy");
            let old_version_path = thinwedge_home.join(format!(
        ...
        let unrelated_path = thinwedge_home.join("state.sqlite_backup");
        tokio::fs::write(&unrelated_path, b"keep")
            .await
            .expect("write unrelated");
        let numeric_path = thinwedge_home.join("123");
        tokio::fs::write(&numeric_path, b"keep")
            .await
            .expect("write numeric");

        let _runtime = StateRuntime::init(thinwedge_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");

        for suffix in ["", "-wal", "-shm", "-journal"] {
            let legacy_path = thinwedge_home.join(format!("{unversioned_name}{suffix}"));
            assert_eq!(
        ...
        assert_eq!(
            tokio::fs::try_exists(thinwedge_home.join(current_name))
                .await
