...
//!
//! # async fn example(state_db: std::sync::Arc<thinwedge_state::StateRuntime>) {
    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: tracing_subscriber::layer::Context<'_, S>) {
        ...
        // were over 30% of retained logs in measured high-fanout ThinWedge environments.
...
mod tests {
    ...
    fn temp_thinwedge_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("thinwedge-state-log-db-{}", Uuid::new_v4()))
    }
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        let thinwedge_home = temp_thinwedge_home();
