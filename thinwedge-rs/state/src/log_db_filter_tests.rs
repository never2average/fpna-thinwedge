...
async fn sqlite_sink_drops_low_level_opentelemetry_sdk_logs() {
    let thinwedge_home =
        std::env::temp_dir().join(format!("thinwedge-state-log-db-filter-{}", Uuid::new_v4()));
    ...
    tracing::trace!(target: "thinwedge_state", "retained-trace");
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("TRACE", "thinwedge_state", Some("retained-trace")),
        ]
    );
