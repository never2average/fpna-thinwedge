...
impl StateRuntime {
    ...
    pub async fn init(
        ...
        let logs_path = logs_db_path(thinwedge_home.as_path());
...
pub fn state_db_path(thinwedge_home: &Path) -> PathBuf {
    thinwedge_home.join(state_db_filename())
}
...
pub fn logs_db_path(thinwedge_home: &Path) -> PathBuf {
    thinwedge_home.join(logs_db_filename())
}
...
async fn remove_legacy_db_files(
    ...
    let mut entries = match tokio::fs::read_dir(thinwedge_home).await {
        Ok(entries) => entries,
        Err(err) => {
            warn!(
                "failed to read thinwedge_home for {db_label} db cleanup {}: {err}",
