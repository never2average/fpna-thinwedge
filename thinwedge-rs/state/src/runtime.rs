...
impl RuntimeDbSpec {
    fn path(self, thinwedge_home: &Path) -> PathBuf {
        thinwedge_home.join(self.filename)
    }
}
...
impl StateRuntime {
    /// Initialize the state runtime using the provided ThinWedge home and default provider.
    ///
    ...
    pub(crate) async fn init_with_telemetry_for_tests(
        ...
        Self::init_inner(thinwedge_home, default_provider, Some(telemetry_override)).await
    }
    ...
    async fn init_inner(
        ...
        let state_path = STATE_DB.path(thinwedge_home.as_path());
        let logs_path = LOGS_DB.path(thinwedge_home.as_path());
        let goals_path = GOALS_DB.path(thinwedge_home.as_path());
        let memories_path = MEMORIES_DB.path(thinwedge_home.as_path());
    ...
    /// Return the configured ThinWedge home directory for this runtime.
    pub fn thinwedge_home(&self) -> &Path {
...
pub fn state_db_path(thinwedge_home: &Path) -> PathBuf {
    STATE_DB.path(thinwedge_home)
}
...
pub fn logs_db_path(thinwedge_home: &Path) -> PathBuf {
    LOGS_DB.path(thinwedge_home)
}
...
pub fn goals_db_path(thinwedge_home: &Path) -> PathBuf {
    GOALS_DB.path(thinwedge_home)
}
...
pub fn memories_db_path(thinwedge_home: &Path) -> PathBuf {
    MEMORIES_DB.path(thinwedge_home)
}

pub fn runtime_db_paths(thinwedge_home: &Path) -> Vec<RuntimeDbPath> {
    RUNTIME_DBS
        .iter()
        .map(|spec| RuntimeDbPath {
            label: spec.label,
            path: spec.path(thinwedge_home),
        })
        .collect()
}
...
mod tests {
    ...
    async fn sqlite_integrity_check_reports_ok_for_valid_db() {
        ...
        let path = state_db_path(thinwedge_home.as_path());
        ...
        let _ = tokio::fs::remove_dir_all(thinwedge_home).await;
    }
    ...
    async fn open_state_sqlite_tolerates_newer_applied_migrations() {
        ...
        tokio::fs::create_dir_all(&thinwedge_home)
            .await
            .expect("create thinwedge home");
        let state_path = state_db_path(thinwedge_home.as_path());
        let pool = SqlitePool::connect_with(
            SqliteConnectOptions::new()
                .filename(&state_path)
                .create_if_missing(true),
