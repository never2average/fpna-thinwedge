...
pub enum Feature {
    ...
    ChildAgentsMd,
    /// Compress request bodies (zstd) when sending streaming requests to thinwedge-backend.
    ...
    /// Requirements-only gate: this should be set from requirements, not user config.
    BrowserUse,
    /// Allow ThinWedge Computer Use.
    ///
    /// Requirements-only gate: this should be set from requirements, not user config.
    ComputerUse,
    /// Temporary internal-only flag for PS-backed remote plugin catalog development.
    RemotePlugin,
    /// Show the startup prompt for migrating external agent config into ThinWedge.
    ExternalMigration,
    ...
    match feature {
        ...
        _ => {
            ...
            let details = if alias == canonical {
                ...
                Some(format!(
                    "Enable it with `--enable {canonical}` or `[features].{canonical}` in config.toml. See https://developers.thinwedge.com/thinwedge/config-basic#feature-flags for details."
                ))
            };
            (summary, details)
        }
...
pub const FEATURES: &[FeatureSpec] = &[
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            ...
            menu_description: "Rebuild ThinWedge-owned transcript scrollback when the terminal width changes.",
            announcement: "",
        },
        default_enabled: true,
    },
    ...
    FeatureSpec {
        id: Feature::ThinWedgeGitCommit,
        key: "thinwedge_git_commit",
        stage: Stage::UnderDevelopment,
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            name: "Memories",
            menu_description: "Allow ThinWedge to create new memories from conversations and bring relevant memories into new conversations.",
            announcement: "NEW: ThinWedge can now generate and uses memories. Try is now with `/memories`",
        },
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            ...
            menu_description: "Show a startup prompt when ThinWedge detects migratable external agent config for this machine or project.",
            announcement: "",
        },
        default_enabled: false,
    },
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            name: "Goals",
            menu_description: "Set a persistent goal ThinWedge can continue over time",
            announcement: "",
        },
        default_enabled: false,
    },
    ...
    FeatureSpec {
        ...
        stage: if cfg!(any(
            ...
            Stage::Experimental {
                ...
                menu_description: "Keep your computer awake while ThinWedge is running a thread.",
