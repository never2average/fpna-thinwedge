...
#[test]
fn thinwedge_hooks_is_legacy_alias_for_hooks() {
    assert_eq!(feature_for_key("hooks"), Some(Feature::ThinWedgeHooks));
    assert_eq!(
        feature_for_key("thinwedge_hooks"),
        Some(Feature::ThinWedgeHooks)
    );
}
