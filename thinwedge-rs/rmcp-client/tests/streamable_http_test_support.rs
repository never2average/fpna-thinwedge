...
use thinwedge_exec_server::ExecServerClient;
use thinwedge_exec_server::HttpClient;
use thinwedge_exec_server::RemoteExecServerConnectArgs;
...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    thinwedge_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("thinwedge-test", "0.0.0-test")
            .with_title("ThinWedge rmcp recovery test"),
    )
...
pub(crate) async fn spawn_exec_server() -> anyhow::Result<ExecServerProcess> {
    ...
    let mut child = Command::new(thinwedge_utils_cargo_bin::cargo_bin("thinwedge")?)
...
/// Reads the websocket URL printed by `thinwedge exec-server --listen`.
