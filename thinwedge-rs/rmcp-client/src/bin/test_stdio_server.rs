...
const MEMO_URI: &str = "memo://thinwedge/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "thinwedge/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of ThinWedge TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (ThinWedge TUI):
    /// - Build this binary: `cargo build -p thinwedge-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `thinwedge mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in ThinWedge TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://thinwedge/{slug}".to_string(),
            name: "thinwedge-memo".to_string(),
            title: Some("ThinWedge Memo".to_string()),
            description: Some(
                "Template for memo://thinwedge/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise ThinWedge TUI handling of MCP image outputs.
///
    ...
    fn image_scenario_result(args: ImageScenarioArgs) -> Result<CallToolResult, McpError> {
        ...
        match args.scenario {
            ...
            ImageScenario::ImageOnlyOriginalDetail => {
                ...
                meta.insert(
                    "thinwedge/imageDetail".to_string(),
