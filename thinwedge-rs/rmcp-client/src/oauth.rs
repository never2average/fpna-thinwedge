...
//!
//! If the keyring is not available or fails, we fall back to THINWEDGE_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "ThinWedge MCP Credentials";
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_thinwedge_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    struct TempThinWedgeHome {
        _guard: MutexGuard<'static, ()>,
        _dir: tempfile::TempDir,
    }

    impl TempThinWedgeHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create THINWEDGE_HOME temp dir");
            unsafe {
                std::env::set_var("THINWEDGE_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempThinWedgeHome {
        fn drop(&mut self) {
            unsafe {
    ...
    fn load_oauth_tokens_reads_from_keyring_when_available() -> Result<()> {
        let _env = TempThinWedgeHome::new();
