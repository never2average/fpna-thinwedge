...
fn transform_linux_seccomp_request(
    thinwedge_linux_sandbox_exe: &std::path::Path,
    ...
    manager
        .transform(SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd: cwd.as_path(),
            thinwedge_linux_sandbox_exe: Some(thinwedge_linux_sandbox_exe),
            use_legacy_landlock: false,
            windows_sandbox_level: WindowsSandboxLevel::Disabled,
            windows_sandbox_private_desktop: false,
        })
...
fn transform_linux_seccomp_preserves_helper_path_in_arg0_when_available() {
    let thinwedge_linux_sandbox_exe = std::path::PathBuf::from("/tmp/thinwedge-linux-sandbox");
    let exec_request = transform_linux_seccomp_request(&thinwedge_linux_sandbox_exe);

    assert_eq!(
        exec_request.arg0,
        Some(thinwedge_linux_sandbox_exe.to_string_lossy().into_owned())
    );
}
