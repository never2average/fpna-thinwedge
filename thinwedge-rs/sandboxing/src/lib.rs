...
pub fn system_bwrap_warning(
    _permission_profile: &thinwedge_protocol::models::PermissionProfile,
) -> Option<String> {
    None
}

impl From<SandboxTransformError> for ThinWedgeErr {
    fn from(err: SandboxTransformError) -> Self {
        match err {
            error @ SandboxTransformError::InvalidCommandCwd { .. }
            | error @ SandboxTransformError::InvalidSandboxPolicyCwd { .. } => {
                ThinWedgeErr::InvalidRequest(error.to_string())
            }
            ...
            SandboxTransformError::Wsl1UnsupportedForBubblewrap => {
                ThinWedgeErr::UnsupportedOperation(crate::bwrap::WSL1_BWRAP_WARNING.to_string())
            }
            #[cfg(not(target_os = "macos"))]
            SandboxTransformError::SeatbeltUnavailable => ThinWedgeErr::UnsupportedOperation(
