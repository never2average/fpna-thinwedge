...
const MISSING_BWRAP_WARNING: &str = concat!(
    "ThinWedge could not find bubblewrap on PATH. ",
    "Install bubblewrap with your OS package manager. ",
    "See the sandbox prerequisites: ",
    "https://developers.openai.com/thinwedge/concepts/sandboxing#prerequisites. ",
    "ThinWedge will use the bundled bubblewrap in the meantime.",
);
const USER_NAMESPACE_WARNING: &str =
    "ThinWedge's Linux sandbox uses bubblewrap and needs access to create user namespaces.";
pub(crate) const WSL1_BWRAP_WARNING: &str = concat!(
    "ThinWedge's Linux sandbox uses bubblewrap, which is not supported on WSL1 ",
