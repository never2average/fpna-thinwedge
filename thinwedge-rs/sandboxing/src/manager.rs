...
use crate::landlock::THINWEDGE_LINUX_SANDBOX_ARG0;
...
pub struct SandboxTransformRequest<'a> {
    ...
    pub thinwedge_linux_sandbox_exe: Option<&'a Path>,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing thinwedge-linux-sandbox executable path")
            }
            #[cfg(target_os = "linux")]
    ...
    pub fn transform(
        ...
        let (argv, arg0_override) = match sandbox {
            ...
            SandboxType::LinuxSeccomp => {
                let exe = thinwedge_linux_sandbox_exe
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(THINWEDGE_LINUX_SANDBOX_ARG0) {
