...
use thinwedge_network_proxy::NetworkProxyConstraints;
use thinwedge_network_proxy::NetworkProxyState;
use thinwedge_network_proxy::build_config_state;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/thinwedge-unreadable");
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.thinwedge".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/thinwedge-readable");
    let unreadable = absolute_path("/tmp/thinwedge-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/thinwedge-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/thinwedge-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/thinwedge-proxy-use";
    let explicit_socket = "/tmp/thinwedge-browser-use";
    ...
    let network_proxy = NetworkProxy::builder()
        ...
        .managed_by_thinwedge(/*managed_by_thinwedge*/ false)
        .build()
        .await?;
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_thinwedge_subpaths() {
    ...
    // write to .thinwedge/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.thinwedge carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_thinwedge_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .thinwedge/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_thinwedge_canonical.join("config.toml");
    ...
    // Verify that writing a file to the folder containing .git and .thinwedge is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_thinwedge_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_thinwedge = repo_root.join(".thinwedge");
    let config_toml = dot_thinwedge.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_thinwedge.to_string_lossy().as_ref(),
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    let expected_dot_thinwedge = format!(
        "-DWRITABLE_ROOT_0_EXCLUDED_1={}",
        dot_thinwedge_canonical.to_string_lossy()
    );
    assert!(
        args.contains(&expected_dot_thinwedge),
        "missing {expected_dot_thinwedge}: {args:#?}"
    );
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .thinwedge/config.toml to
    ...
    dot_thinwedge_canonical: PathBuf,
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".thinwedge")).expect("create .thinwedge");
    fs::write(
        vulnerable_root.join(".thinwedge").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .thinwedge/config.toml");
    ...
    let dot_thinwedge_canonical = vulnerable_root_canonical.join(".thinwedge");
