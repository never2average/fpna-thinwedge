...
fn thinwedge_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_thinwedge-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> thinwedge_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<thinwedge_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let thinwedge_linux_sandbox_exe = Some(thinwedge_linux_sandbox_exe());
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(ThinWedgeErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(ThinWedgeErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<thinwedge_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> thinwedge_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(ThinWedgeErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let thinwedge_linux_sandbox_exe: Option<PathBuf> = Some(thinwedge_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_thinwedge_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_thinwedge = tmpdir.path().join(".thinwedge");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_thinwedge).expect("create .thinwedge");

    let git_target = dot_git.join("config");
    let thinwedge_target = dot_thinwedge.join("config.toml");
    ...
    let thinwedge_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", thinwedge_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".thinwedge write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(thinwedge_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_thinwedge_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-thinwedge");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_thinwedge = tmpdir.path().join(".thinwedge");
    symlink(&decoy, &dot_thinwedge).expect("create .thinwedge symlink");
    ...
    let thinwedge_output = expect_denied(
        ...
        .await,
        ".thinwedge symlink replacement should be denied",
    );
    assert_ne!(thinwedge_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_thinwedge_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    {
        Err(ThinWedgeErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
        result => panic!(".thinwedge symlink build failure should deny: {result:?}"),
    };
...
if grep -E '(^|[[:space:]])\.(git|thinwedge|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_thinwedge_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".thinwedge"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .thinwedge directory creation should be denied",
    );
    assert_ne!(mkdir_thinwedge_output.exit_code, 0);
    assert!(!subdir.join(".thinwedge").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = thinwedge_linux_sandbox_exe()
        .parent()
