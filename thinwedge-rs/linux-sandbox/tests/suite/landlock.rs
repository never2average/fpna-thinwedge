...
async fn run_cmd_result_with_permission_profile(
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_thinwedge-linux-sandbox");
    let thinwedge_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_thinwedge-linux-sandbox");
    let thinwedge_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = PathBuf::from(env!("CARGO_BIN_EXE_thinwedge-linux-sandbox"))
        .parent()
