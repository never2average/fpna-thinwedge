...
//! - sensitive subpaths such as `.git` and `.thinwedge` remain read-only even when
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // Missing `.thinwedge` remains protected so first-time project config
...
mod tests {
    ...
    #[test]
    fn skips_missing_project_root_metadata_carveouts_except_thinwedge() {
        let temp_dir = TempDir::new().expect("temp dir");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            args.args
                .windows(3)
                .any(|window| { window == ["--ro-bind", "/dev/null", dot_thinwedge.as_str()] })
        );
    }
    ...
    fn mounts_dev_before_writable_dev_binds() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/".to_string(),
                "/".to_string(),
                // Mask the default protected .thinwedge subpath under that writable
