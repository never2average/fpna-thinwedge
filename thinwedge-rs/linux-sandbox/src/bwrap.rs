...
//! - sensitive subpaths such as `.git`, `.agents`, and `.thinwedge` remain
...
use thinwedge_protocol::permissions::is_protected_metadata_name;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.thinwedge`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".thinwedge")
                {
                    return None;
                }
    ...
    for (search_root, globs) in patterns_by_search_root {
        for path in ripgrep_files(search_root.as_path(), &globs, max_depth)? {
            ...
            if expanded_paths.len() > MAX_UNREADABLE_GLOB_MATCHES {
                return Err(ThinWedgeErr::Fatal(format!(
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_thinwedge = temp_dir.path().join("real-thinwedge");
        let logical_thinwedge = logical_home.join(".thinwedge");
        let real_memories = real_thinwedge.join("memories");
        let logical_memories = logical_thinwedge.join("memories");
        ...
        std::os::unix::fs::symlink(&real_thinwedge, &logical_thinwedge)
            .expect("create symlinked thinwedge home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".thinwedge"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_thinwedge = path_to_string(&temp_dir.path().join(".thinwedge"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_thinwedge));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_thinwedge)));
        assert_eq!(
            protected_create_target_paths(&args),
            Vec::<PathBuf>::new(),
    ...
    fn mounts_dev_before_writable_dev_binds() {
        ...
        assert_eq!(
            ...
            vec![
                PathBuf::from("/.git"),
                PathBuf::from("/.agents"),
                PathBuf::from("/.thinwedge"),
                PathBuf::from("/dev/.git"),
                PathBuf::from("/dev/.agents"),
                PathBuf::from("/dev/.thinwedge"),
            ]
        );
        assert_eq!(
            args.args,
