# thinwedge-linux-sandbox

This crate is responsible for producing:

- a `thinwedge-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the ThinWedge CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `thinwedge-exec` CLI can check if its arg0 is `thinwedge-linux-sandbox` and, if so, execute as if it were `thinwedge-linux-sandbox`
  - this should also be true of the `thinwedge` multitool CLI

On Linux, ThinWedge prefers the first `bwrap` found on `PATH`
...
- WSL1 is not supported for bubblewrap sandboxing; ThinWedge rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.thinwedge`) are re-applied as read-only via `--ro-bind`.
