...
pub type StateDbHandle = Arc<thinwedge_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        thinwedge_state::StateRuntime::init(sqlite_home.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    thinwedge_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == thinwedge_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, thinwedge_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = thinwedge_state::state_db_path(config.sqlite_home());
    if !tokio::fs::try_exists(&state_path).await.unwrap_or(false) {
        thinwedge_state::record_fallback(
            "get_state_db",
            "db_unavailable",
    ...
    let runtime = match thinwedge_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    metrics: thinwedge_otel::MetricsClient,
    originator: &str,
) -> thinwedge_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == thinwedge_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<thinwedge_state::Anchor> {
    let cursor = cursor?;
    ...
    Some(thinwedge_state::Anchor { ts })
}
...
pub async fn list_thread_ids_db(
    context: Option<&thinwedge_state::StateRuntime>,
    thinwedge_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.thinwedge_home() != thinwedge_home {
        warn!(
            "state db thinwedge_home mismatch: expected {}, got {}",
            ctx.thinwedge_home().display(),
            thinwedge_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => thinwedge_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => thinwedge_state::SortKey::UpdatedAt,
            },
...
pub async fn list_threads_db(
...
) -> Option<thinwedge_state::ThreadsPage> {
    let ctx = context?;
    if ctx.thinwedge_home() != thinwedge_home {
        warn!(
            "state db thinwedge_home mismatch: expected {}, got {}",
    ...
    match ctx
        .list_threads(
            page_size,
            thinwedge_state::ThreadFilterOptions {
                ...
                sort_direction: match sort_direction {
                    SortDirection::Asc => thinwedge_state::SortDirection::Asc,
                    SortDirection::Desc => thinwedge_state::SortDirection::Desc,
                },
                search_term,
            },
        )
