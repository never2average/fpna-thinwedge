...
async fn insert_state_db_thread(
    ...
    let runtime = thinwedge_state::StateRuntime::init(home.to_path_buf(), TEST_PROVIDER.to_string())
        .await
...
async fn find_thread_path_repairs_missing_db_row_after_filesystem_fallback() {
    ...
    let _runtime = thinwedge_state::StateRuntime::init(home.to_path_buf(), TEST_PROVIDER.to_string())
        .await
        .expect("state db should initialize");
    _runtime
        .mark_backfill_complete(/*last_watermark*/ None)
...
async fn test_model_provider_filter_selects_only_matching_sessions() -> Result<()> {
    ...
    let thinwedge_id = Uuid::from_u128(1);
    ...
    let thinwedge_id_str = thinwedge_id.to_string();
    let none_id_str = none_id.to_string();
    let thinwedge_filter = provider_vec(&["thinwedge"]);
    let thinwedge_sessions = get_threads(
        home,
        /*page_size*/ 10,
        /*cursor*/ None,
        ThreadSortKey::CreatedAt,
        NO_SOURCE_FILTER,
        Some(thinwedge_filter.as_slice()),
        /*cwd_filters*/ None,
        "thinwedge",
    )
    .await?;
    assert_eq!(thinwedge_sessions.items.len(), 2);
    let thinwedge_ids: Vec<_> = thinwedge_sessions
        .items
        .iter()
        .filter_map(|item| item.thread_id.as_ref().map(ToString::to_string))
        .collect();
    assert!(thinwedge_ids.contains(&thinwedge_id_str));
    assert!(thinwedge_ids.contains(&none_id_str));
