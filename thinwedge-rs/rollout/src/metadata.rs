...
use thinwedge_state::BackfillState;
use thinwedge_state::BackfillStats;
use thinwedge_state::BackfillStatus;
use thinwedge_state::DB_ERROR_METRIC;
use thinwedge_state::DB_METRIC_BACKFILL;
use thinwedge_state::DB_METRIC_BACKFILL_DURATION_MS;
...
pub(crate) async fn backfill_sessions(
    ...
    let metric_client = thinwedge_otel::global();
    ...
    let sessions_root = config.thinwedge_home().join(SESSIONS_SUBDIR);
    let archived_root = config.thinwedge_home().join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(config.thinwedge_home(), &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(thinwedge_home: &Path, path: &Path) -> String {
    path.strip_prefix(thinwedge_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
