...
pub async fn append_thread_name(
    ...
    append_session_index_entry(thinwedge_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(thinwedge_home);
...
pub async fn find_thread_meta_by_name_str(
    ...
    while let Some(thread_id) = rx.recv().await {
        ...
        if let Some(path) =
            super::list::find_thread_path_by_id_str(thinwedge_home, &thread_id.to_string()).await?
...
fn session_index_path(thinwedge_home: &Path) -> PathBuf {
    thinwedge_home.join(SESSION_INDEX_FILE)
}
