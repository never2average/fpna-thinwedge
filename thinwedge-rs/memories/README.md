...
Runtime orchestration for Phase 1 and Phase 2 still lives in `thinwedge-core` under
`thinwedge-rs/core/src/memories/`.

## Crates

- `thinwedge-rs/memories/read` (`thinwedge-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `thinwedge-rs/memories/write` (`thinwedge-memories-write`) owns the write path:
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.thinwedge/memories/.git` by `thinwedge-git-utils`
