...
use thinwedge_rollout::INTERACTIVE_SESSION_SOURCES;
use thinwedge_rollout::should_persist_response_item_for_memories;
use thinwedge_secrets::redact_secrets;
use tracing::info;
use tracing::warn;
...
async fn claim_startup_jobs(
...
) -> Option<Vec<thinwedge_state::Stage1JobClaim>> {
    ...
    match state_db
        ...
        .claim_stage1_jobs_for_startup(
            context.thread_id(),
            thinwedge_state::Stage1StartupClaimParams {
...
async fn run_jobs(
    ...
    config: Arc<Config>,
    claimed_candidates: Vec<thinwedge_state::Stage1JobClaim>,
...
mod job {
    ...
    pub(crate) async fn run(
        context: &MemoryStartupContext,
        config: &Config,
        claim: thinwedge_state::Stage1JobClaim,
        stage_one_context: &StageOneRequestContext,
    ) -> JobResult {
    ...
    pub(super) fn serialize_filtered_rollout_response_items(
        items: &[RolloutItem],
    ) -> thinwedge_protocol::error::Result<String> {
        let filtered = items
            .iter()
        ...
        let serialized = serde_json::to_string(&filtered).map_err(|err| {
            ThinWedgeErr::InvalidRequest(format!("failed to serialize rollout memory: {err}"))
        })?;
...
mod tests {
    ...
    fn serializes_memory_rollout_redacts_secrets_before_prompt_upload() {
            ...
            job::serialize_filtered_rollout_response_items(&[RolloutItem::ResponseItem(
                ResponseItem::FunctionCallOutput {
                    ...
                    output: thinwedge_protocol::models::FunctionCallOutputPayload {
                        body: thinwedge_protocol::models::FunctionCallOutputBody::Text(format!(
                            r#"{{"token":"{}"}}"#,
                            concat!("sk-", "abcdefghijklmnopqrstuvwxyz123456")
                        )),
                        success: Some(true),
                    },
                },
        ...
        assert!(!serialized.contains(concat!("sk-", "abcdefghijklmnopqrstuvwxyz123456")));
