...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            thinwedge_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
                context.counter(
            ...
            thinwedge_state::Phase2JobClaimOutcome::SkippedRetryUnavailable => {
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[thinwedge_state::Stage1Output],
        reason: &'static str,
    ) -> bool {
        context.counter(MEMORY_PHASE_TWO_JOBS, /*inc*/ 1, &[("status", reason)]);
...
mod agent {
    ...
    pub(super) fn handle(
        ...
        claim: Claim,
        new_watermark: i64,
        selected_outputs: Vec<thinwedge_state::Stage1Output>,
        memory_root: thinwedge_utils_absolute_path::AbsolutePathBuf,
        agent: SpawnedConsolidationAgent,
        phase_two_e2e_timer: Option<thinwedge_otel::Timer>,
    ) {
        let Some(db) = context.state_db() else {
            return;
        };
    ...
    async fn loop_agent(
        ...
        token: String,
        thread_id: ThreadId,
        thread: &thinwedge_core::ThinWedgeThread,
    ) -> AgentStatus {
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[thinwedge_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()
