...
#[derive(Clone, Debug)]
pub struct ThinWedgeHttpClient {
    inner: reqwest::Client,
}

impl ThinWedgeHttpClient {
    pub fn new(inner: reqwest::Client) -> Self {
        Self { inner }
    }

    pub fn get<U>(&self, url: U) -> ThinWedgeRequestBuilder
    where
        U: IntoUrl,
    {
    ...
    pub fn post<U>(&self, url: U) -> ThinWedgeRequestBuilder
    where
        U: IntoUrl,
    {
    ...
    pub fn request<U>(&self, method: Method, url: U) -> ThinWedgeRequestBuilder
    where
        U: IntoUrl,
    {
        let url_str = url.as_str().to_string();
        ThinWedgeRequestBuilder::new(self.inner.request(method.clone(), url), method, url_str)
    }
}

#[must_use = "requests are not sent unless `send` is awaited"]
#[derive(Debug)]
pub struct ThinWedgeRequestBuilder {
    builder: reqwest::RequestBuilder,
    method: Method,
    url: String,
}

impl ThinWedgeRequestBuilder {
