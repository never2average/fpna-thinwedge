...
/// Adds the process-local ChatGPT Cloudflare cookie jar used by ThinWedge HTTP clients.
///
...
mod tests {
    ...
    fn rejects_plain_http_chatgpt_cookie_urls() {
        let store = ChatGptCloudflareCookieStore::default();
        let http_url = reqwest::Url::parse("http://chatgpt.com/backend-api/thinwedge/responses")
            .expect("URL should parse");
        let https_url = reqwest::Url::parse("https://chatgpt.com/backend-api/thinwedge/responses")
            .expect("URL should parse");
        let set_cookie = HeaderValue::from_static("_cfuvid=visitor; Path=/; Secure; HttpOnly");
    ...
    fn only_allows_https_urls() {
        ...
        let url = reqwest::Url::parse("wss://chatgpt.com/backend-api/thinwedge/responses").unwrap();
