...
/// MCP server capability indicating that ThinWedge should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "thinwedge/sandbox-state-meta";

pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "thinwedge.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "thinwedge.mcp.tools.fetch_uncached.duration_ms";
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) thinwedge_apps_tools_cache_context: Option<ThinWedgeAppsToolsCacheContext>,
}

impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        let total_start = Instant::now();
        if let Some(cache_context) = self.thinwedge_apps_tools_cache_context.as_ref()
            && let CachedThinWedgeAppsToolsLoad::Hit(tools) =
                load_cached_thinwedge_apps_tools(cache_context)
        {
            emit_duration(
        ...
        if self.thinwedge_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
pub(crate) async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .map(|tool| {
            ...
            let callable_name = normalize_thinwedge_apps_callable_name(
                server_name,
                &tool_def.name,
    ...
    if server_name == THINWEDGE_APPS_MCP_SERVER_NAME {
        return Ok(filter_disallowed_thinwedge_apps_tools(tools));
    }
    Ok(tools)
}
...
mod tests {
    ...
    #[test]
    fn thinwedge_apps_connector_metadata_is_preserved() {
        let mut tool = tool_with_connector_meta();
