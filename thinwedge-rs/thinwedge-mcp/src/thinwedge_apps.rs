...
pub fn thinwedge_apps_tools_cache_key(auth: Option<&ThinWedgeAuth>) -> ThinWedgeAppsToolsCacheKey {
...
pub(crate) struct ThinWedgeAppsToolsCacheContext {
    ...
    pub(crate) user_key: ThinWedgeAppsToolsCacheKey,
}
...
pub(crate) enum CachedThinWedgeAppsToolsLoad {
    Hit(Vec<ToolInfo>),
    Missing,
    Invalid,
}

pub(crate) fn normalize_thinwedge_apps_tool_title(
    server_name: &str,
...
pub(crate) fn normalize_thinwedge_apps_callable_name(
    server_name: &str,
    tool_name: &str,
...
pub(crate) fn normalize_thinwedge_apps_callable_namespace(
    server_name: &str,
    connector_name: Option<&str>,
) -> String {
    if server_name == THINWEDGE_APPS_MCP_SERVER_NAME
...
pub(crate) fn load_startup_cached_thinwedge_apps_tools_snapshot(
    server_name: &str,
    cache_context: Option<&ThinWedgeAppsToolsCacheContext>,
) -> Option<Vec<ToolInfo>> {
    if server_name != THINWEDGE_APPS_MCP_SERVER_NAME {
    ...
    match load_cached_thinwedge_apps_tools(cache_context) {
        CachedThinWedgeAppsToolsLoad::Hit(tools) => Some(tools),
        CachedThinWedgeAppsToolsLoad::Missing | CachedThinWedgeAppsToolsLoad::Invalid => None,
    }
}
...
pub(crate) fn load_cached_thinwedge_apps_tools(
    ...
    let bytes = match std::fs::read(cache_path) {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            return CachedThinWedgeAppsToolsLoad::Missing;
        }
        Err(_) => return CachedThinWedgeAppsToolsLoad::Invalid,
    };
    let cache: ThinWedgeAppsToolsDiskCache = match serde_json::from_slice(&bytes) {
        Ok(cache) => cache,
        Err(_) => return CachedThinWedgeAppsToolsLoad::Invalid,
    };
    if cache.schema_version != THINWEDGE_APPS_TOOLS_CACHE_SCHEMA_VERSION {
        return CachedThinWedgeAppsToolsLoad::Invalid;
    }
    CachedThinWedgeAppsToolsLoad::Hit(filter_disallowed_thinwedge_apps_tools(cache.tools))
}

pub(crate) fn write_cached_thinwedge_apps_tools(
    cache_context: &ThinWedgeAppsToolsCacheContext,
    tools: &[ToolInfo],
) {
    let cache_path = cache_context.cache_path();
    ...
    let tools = filter_disallowed_thinwedge_apps_tools(tools.to_vec());
    let Ok(bytes) = serde_json::to_vec_pretty(&ThinWedgeAppsToolsDiskCache {
...
struct ThinWedgeAppsToolsDiskCache {
    schema_version: u8,
    tools: Vec<ToolInfo>,
}
