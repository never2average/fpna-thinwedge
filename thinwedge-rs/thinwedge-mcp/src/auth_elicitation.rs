...
//! Session orchestration stays in `thinwedge-core`.

use serde::Serialize;
use thinwedge_protocol::mcp::CallToolResult;

pub const MCP_TOOL_THINWEDGE_APPS_META_KEY: &str = "_thinwedge_apps";
...
pub struct ThinWedgeAppsConnectorAuthFailure {
...
pub struct ThinWedgeAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct ThinWedgeAppsAuthElicitationPlan {
    pub auth_failure: ThinWedgeAppsConnectorAuthFailure,
    pub elicitation: ThinWedgeAppsAuthElicitation,
}

#[derive(Serialize)]
struct ThinWedgeAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<ThinWedgeAppsConnectorAuthFailure> {
    ...
    Some(ThinWedgeAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<ThinWedgeAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(ThinWedgeAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &ThinWedgeAppsConnectorAuthFailure,
) -> ThinWedgeAppsAuthElicitation {
    ThinWedgeAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_THINWEDGE_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: ThinWedgeAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("thinwedge_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &ThinWedgeAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in ThinWedge.",
            auth_failure.connector_name
        ),
        _ => format!(
