...
impl McpRuntimeContext {
    ...
    pub(crate) fn resolve_server_environment(
        &self,
        server_name: &str,
        config: &thinwedge_config::McpServerConfig,
        ...
        if config.is_local_environment() {
            return match config.transport {
                thinwedge_config::McpServerTransportConfig::Stdio { .. } => Err(format!(
                    "local stdio MCP server `{server_name}` requires a local environment"
                )),
                thinwedge_config::McpServerTransportConfig::StreamableHttp { .. } => Ok(None),
            };
        }
...
fn ensure_remote_stdio_cwd(
    ...
    let thinwedge_config::McpServerTransportConfig::Stdio { cwd, .. } = &config.transport else {
        return Ok(());
    };
...
mod tests {
    ...
    use thinwedge_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
