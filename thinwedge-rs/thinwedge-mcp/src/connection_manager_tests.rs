...
fn create_thinwedge_apps_tools_cache_context(
    thinwedge_home: PathBuf,
    account_id: Option<&str>,
    chatgpt_user_id: Option<&str>,
) -> ThinWedgeAppsToolsCacheContext {
    ThinWedgeAppsToolsCacheContext {
        thinwedge_home,
        user_key: ThinWedgeAppsToolsCacheKey {
...
fn tool_with_model_visible_input_schema_masks_file_params() {
    let mut tool = create_test_tool(THINWEDGE_APPS_MCP_SERVER_NAME, "upload").tool;
...
#[test]
fn thinwedge_apps_tools_cache_is_overwritten_by_last_write() {
    let thinwedge_home = tempdir().expect("tempdir");
    let cache_context = create_thinwedge_apps_tools_cache_context(
        thinwedge_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let tools_gateway_1 = vec![create_test_tool(THINWEDGE_APPS_MCP_SERVER_NAME, "one")];
...
#[test]
fn thinwedge_apps_tools_cache_is_scoped_per_user() {
    let thinwedge_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_thinwedge_apps_tools_cache_context(
        thinwedge_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_thinwedge_apps_tools_cache_context(
        thinwedge_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    let tools_user_1 = vec![create_test_tool(THINWEDGE_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(THINWEDGE_APPS_MCP_SERVER_NAME, "two")];

    write_cached_thinwedge_apps_tools(&cache_context_user_1, &tools_user_1);
    write_cached_thinwedge_apps_tools(&cache_context_user_2, &tools_user_2);

    let read_user_1 =
        read_cached_thinwedge_apps_tools(&cache_context_user_1).expect("cache entry for user one");
    let read_user_2 =
        read_cached_thinwedge_apps_tools(&cache_context_user_2).expect("cache entry for user two");
...
#[test]
fn thinwedge_apps_tools_cache_filters_disallowed_connectors() {
    let thinwedge_home = tempdir().expect("tempdir");
    let cache_context = create_thinwedge_apps_tools_cache_context(
        thinwedge_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn thinwedge_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let thinwedge_home = tempdir().expect("tempdir");
    let cache_context = create_thinwedge_apps_tools_cache_context(
        thinwedge_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": THINWEDGE_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(THINWEDGE_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");

    assert!(read_cached_thinwedge_apps_tools(&cache_context).is_none());
}

#[test]
fn thinwedge_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let thinwedge_home = tempdir().expect("tempdir");
    let cache_context = create_thinwedge_apps_tools_cache_context(
        thinwedge_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_thinwedge_apps_tools_loads_from_disk_cache() {
    let thinwedge_home = tempdir().expect("tempdir");
    let cache_context = create_thinwedge_apps_tools_cache_context(
        thinwedge_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let server_info = create_test_server_info("ThinWedge Apps");
    ...
    let startup_tools = load_startup_cached_thinwedge_apps_tools_snapshot(
        THINWEDGE_APPS_MCP_SERVER_NAME,
        Some(&cache_context),
    )
    ...
    assert_eq!(startup_tools[0].server_name, THINWEDGE_APPS_MCP_SERVER_NAME);
    assert_eq!(startup_tools[0].callable_name, "calendar_search");
    assert_eq!(cached_server_info, Some(server_info));
}

#[test]
fn startup_cached_thinwedge_apps_tools_loads_without_server_info_cache() {
    let thinwedge_home = tempdir().expect("tempdir");
    let cache_context = create_thinwedge_apps_tools_cache_context(
        thinwedge_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": THINWEDGE_APPS_TOOLS_CACHE_SCHEMA_VERSION,
        "tools": [create_test_tool(THINWEDGE_APPS_MCP_SERVER_NAME, "calendar_search")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");
...
#[test]
fn thinwedge_apps_server_info_cache_survives_legacy_tools_cache_write() {
    let thinwedge_home = tempdir().expect("tempdir");
    let cache_context = create_thinwedge_apps_tools_cache_context(
        thinwedge_home.path().to_path_buf(),
        Some("account-one"),
    ...
    assert_eq!(
        load_startup_cached_thinwedge_apps_server_info(
            THINWEDGE_APPS_MCP_SERVER_NAME,
            Some(&cache_context),
        ),
        Some(server_info)
    ...
    assert!(
        load_startup_cached_thinwedge_apps_tools_snapshot(
            THINWEDGE_APPS_MCP_SERVER_NAME,
            Some(&cache_context),
        )
        .is_none()
...
async fn list_all_tools_uses_cached_tool_info_snapshot_while_client_is_pending() {
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__thinwedge_apps", "calendar_create_event")
        })
        .expect("tool from startup cache");
    assert_eq!(tool.server_name, THINWEDGE_APPS_MCP_SERVER_NAME);
...
async fn list_available_server_infos_uses_cache_while_client_is_pending() {
    ...
    assert_eq!(
        server_infos.get(THINWEDGE_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}
...
async fn no_local_runtime_fails_local_stdio_but_keeps_local_http_server() {
    ...
    let manager = McpConnectionManager::new(
        ...
        thinwedge_home.path().to_path_buf(),
        ThinWedgeAppsToolsCacheKey {
            account_id: None,
            chatgpt_user_id: None,
            is_workspace_account: false,
        },
        /*host_owned_thinwedge_apps_enabled*/ false,
        /*prefix_mcp_tool_names*/ true,
...
fn mcp_init_error_display_prompts_for_github_pat() {
    ...
    let expected = format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = THINWEDGE_GITHUB_PERSONAL_ACCESS_TOKEN"
    );
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Run `thinwedge mcp login {server_name}`."
    );
