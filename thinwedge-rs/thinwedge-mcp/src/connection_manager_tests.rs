...
fn startup_cached_thinwedge_apps_tools_loads_from_disk_cache() {
    ...
    write_cached_thinwedge_apps_tools(&cache_context, &cached_tools);
...
async fn list_all_tools_uses_startup_snapshot_while_client_is_pending() {
    ...
    let tool = tools
        .get("mcp__thinwedge_apps__calendar_create_event")
...
fn elicitation_capability_enabled_for_custom_servers() {
    for server_name in [THINWEDGE_APPS_MCP_SERVER_NAME, "custom_mcp"] {
