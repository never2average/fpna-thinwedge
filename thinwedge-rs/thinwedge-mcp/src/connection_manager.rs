//! Aggregates MCP server connections for ThinWedge.
//!
...
use thinwedge_protocol::protocol::McpStartupFailure;
...
pub struct McpConnectionManager {
    ...
    host_owned_thinwedge_apps_enabled: bool,
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        thinwedge_apps_tools_cache_key: ThinWedgeAppsToolsCacheKey,
        host_owned_thinwedge_apps_enabled: bool,
        prefix_mcp_tool_names: bool,
        ...
        let thinwedge_apps_auth_provider = auth
            .filter(|auth| auth.uses_thinwedge_backend())
            .map(thinwedge_model_provider::auth_provider_from_auth);
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            let runtime_auth_provider =
                if server_name == THINWEDGE_APPS_MCP_SERVER_NAME && !uses_env_bearer_token {
                    thinwedge_apps_auth_provider.clone()
                } else {
                    None
                };
    ...
    pub fn is_host_owned_thinwedge_apps_server(&self, server_name: &str) -> bool {
        self.host_owned_thinwedge_apps_enabled && server_name == THINWEDGE_APPS_MCP_SERVER_NAME
    }
    ...
    /// Force-refresh thinwedge apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_thinwedge_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let managed_client = self
            .clients
            .get(THINWEDGE_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{THINWEDGE_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{THINWEDGE_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        write_cached_thinwedge_apps_tools_if_needed(
            THINWEDGE_APPS_MCP_SERVER_NAME,
            managed_client.thinwedge_apps_tools_cache_context.as_ref(),
