...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_thinwedge_uuid: Arc<Mutex<HashMap<RequestId, ThreadId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        Self {
            ...
            arg0_paths,
            thread_manager,
            running_requests_id_to_thinwedge_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_thinwedge_tool_call_param(),
                create_tool_for_thinwedge_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "thinwedge" => self.handle_tool_call_thinwedge(id, arguments).await,
            "thinwedge-reply" => {
                self.handle_tool_call_thinwedge_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_thinwedge(
        &self,
        id: RequestId,
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<ThinWedgeToolCallParam>(json_val) {
        ...
        task::spawn(async move {
            // Run the ThinWedge session and stream events back to the client.
            crate::thinwedge_tool_runner::run_thinwedge_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
    ...
    async fn handle_tool_call_thinwedge_session_reply(
        &self,
        ...
        let thread_id = match thinwedge_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let thinwedge = match self.thread_manager.get_thread(thread_id).await {
            ...
            Err(_) => {
                ...
                let result = crate::thinwedge_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_thinwedge_uuid.lock().await;
        ...
        // Obtain the ThinWedge thread from the server.
        let thinwedge_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to ThinWedge.
        if let Err(e) = thinwedge_arc
            ...
            .await
        {
            tracing::error!("Failed to submit interrupt to ThinWedge: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_thinwedge_uuid
            .lock()
            .await
            .remove(&request_id);
    }
