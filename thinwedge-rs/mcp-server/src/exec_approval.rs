...
pub struct ExecApprovalElicitRequestParams {
    ...
    // correlate the request with the thinwedge tool call.
    #[serde(rename = "threadId")]
    pub thread_id: ThreadId,
    pub thinwedge_elicitation: String,
    pub thinwedge_mcp_tool_call_id: String,
    pub thinwedge_event_id: String,
    pub thinwedge_call_id: String,
    pub thinwedge_command: Vec<String>,
    pub thinwedge_cwd: PathBuf,
    pub thinwedge_parsed_cmd: Vec<ParsedCommand>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    call_id: String,
    approval_id: String,
    thinwedge_parsed_cmd: Vec<ParsedCommand>,
    thread_id: ThreadId,
) {
    let escaped_command =
        shlex::try_join(command.iter().map(String::as_str)).unwrap_or_else(|_| command.join(" "));
    let message = format!(
        "Allow ThinWedge to run `{escaped_command}` in `{cwd}`?",
    ...
    let params = ExecApprovalElicitRequestParams {
        ...
        thinwedge_mcp_tool_call_id: tool_call_id.clone(),
        thinwedge_event_id: event_id.clone(),
        thinwedge_call_id: call_id,
    ...
    {
        let thinwedge = thinwedge.clone();
        let approval_id = approval_id.clone();
        let event_id = event_id.clone();
        tokio::spawn(async move {
            on_exec_approval_response(approval_id, event_id, on_response, thinwedge).await;
        });
    }
}
