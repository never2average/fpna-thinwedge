//! Asynchronous worker that executes a **ThinWedge** tool-call inside a spawned
...
/// Run a complete ThinWedge session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_thinwedge_tool_session(
    id: RequestId,
    initial_prompt: String,
    config: ThinWedgeConfig,
    ...
    // Use the original MCP request ID as the `sub_id` for the ThinWedge submission so that
    ...
    run_thinwedge_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_thinwedge_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_thinwedge_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<ThinWedgeThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
    ...
    loop {
        match thread.next_event().await {
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(thinwedge_event_to_notification(&event)) above has
            ...
            Err(e) => {
                let result = create_call_tool_result_with_thread_id(
                    thread_id,
                    format!("ThinWedge runtime error: {e}"),
                    Some(true),
                );
                outgoing.send_response(request_id.clone(), result).await;
                break;
