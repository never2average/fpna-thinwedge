...
impl McpProcess {
    pub async fn new(thinwedge_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(thinwedge_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = thinwedge_utils_cargo_bin::cargo_bin("thinwedge-mcp-server")
            .context("should find binary for thinwedge-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = thinwedge_login::default_client::originator().value;
    ...
    pub async fn send_thinwedge_tool_call(
        &mut self,
        params: ThinWedgeToolCallParam,
    ) -> anyhow::Result<i64> {
        let thinwedge_tool_call_params = CallToolRequestParams::new("thinwedge").with_arguments(
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(thinwedge_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "thinwedge/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = if notification.notification.method == "thinwedge/event" {
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `thinwedge-mcp-server` child process.
        //
