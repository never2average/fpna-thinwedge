...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(THINWEDGE_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a ThinWedge sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "thinwedge" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let thinwedge_request_id = mcp_process
        .send_thinwedge_tool_call(ThinWedgeToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            thinwedge_request_id.to_string(),
            params.thinwedge_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `thinwedge` tool call completes and that the file was created.
    let thinwedge_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(thinwedge_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(thinwedge_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    thinwedge_mcp_tool_call_id: String,
    thinwedge_event_id: String,
    thread_id: thinwedge_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow ThinWedge to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let thinwedge_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        thinwedge_elicitation: "exec-approval".to_string(),
        thinwedge_mcp_tool_call_id,
        thinwedge_event_id,
        thinwedge_command: command,
        thinwedge_cwd: workdir.to_path_buf(),
        thinwedge_call_id: "call1234".to_string(),
        thinwedge_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "thinwedge" tool request that will trigger the apply_patch command
    let thinwedge_request_id = mcp_process
        .send_thinwedge_tool_call(ThinWedgeToolCallParam {
    ...
    // Verify the original `thinwedge` tool call completes
    let thinwedge_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(thinwedge_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_thinwedge_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = thinwedge_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn thinwedge_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `thinwedge mcp` with a specific config.toml.
    let thinwedge_home = TempDir::new()?;
    create_config_toml(thinwedge_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(thinwedge_home.path()).await?;
    ...
    assert_eq!(
        ...
        json!({
            ...
            "structuredContent": {
                "threadId": thinwedge_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("thinwedge tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow ThinWedge to apply proposed code changes?".to_string());
    let params_json = serde_json::to_value(PatchApprovalElicitRequestParams {
        message: message_lines.join("\n"),
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        thinwedge_elicitation: "patch-approval".to_string(),
        thinwedge_mcp_tool_call_id,
        thinwedge_event_id,
        thinwedge_reason: reason,
        thinwedge_grant_root: grant_root,
        thinwedge_changes: changes,
        thinwedge_call_id: "call1234".to_string(),
    })?;
...
/// Create a ThinWedge config that uses the mock server as the model provider.
