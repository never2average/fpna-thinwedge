...
/// OAuth client settings used when ThinWedge launches an MCP OAuth flow.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
...
pub struct McpServerConfig {
    ...
    /// Experimental environment selector for where ThinWedge should start this MCP server.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub experimental_environment: Option<String>,

    /// When `false`, ThinWedge skips initializing this MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,

    /// When `true`, `thinwedge exec` exits with an error if this MCP server fails to initialize.
