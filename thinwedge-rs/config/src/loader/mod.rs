...
pub async fn load_config_layers_state(
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, thinwedge_home, overrides.clone()).await?;
    if !ignore_managed_requirements {
    ...
    // Add a layer for $THINWEDGE_HOME/config.toml so folder-derived resources such
    // as rules/ can still be discovered. When user config is ignored, preserve
    // the layer metadata without reading config.toml.
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, thinwedge_home);
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        // value for base_dir, so thinwedge_home is as good a value as any.
        let managed_config =
            resolve_relative_paths_in_config_toml(config.managed_config, thinwedge_home)?;
...
async fn load_project_layers(
    ...
    for dir in dirs {
        ...
        if !fs
            .get_metadata(&dot_thinwedge_abs, /*sandbox*/ None)
            .await
