...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/thinwedge/config.toml";
...
/// - profile   `${THINWEDGE_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.thinwedge/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.thinwedge/config.toml` (loaded but disabled when untrusted)
...
pub async fn load_config_layers_state(
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(thinwedge_home)?;
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, thinwedge_home, overrides.clone(), strict_config)
            .await?;
    if !ignore_managed_requirements {
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(thinwedge_home)?;
    let base_user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, thinwedge_home);
        ...
        && let Some(base_user_config) = base_user_layer.config.as_table()
    {
        ...
        if legacy_profile_is_selected || legacy_profile_table_exists {
            return Err(io::Error::new(
                ...
                format!(
                    "--profile `{active_user_profile}` cannot be used while {} contains legacy `profile = \"{active_user_profile}\"` or `[profiles.{active_user_profile}]` config; move those settings into {} and remove the legacy profile selector/table. See https://developers.openai.com/thinwedge/config-advanced#profiles for more information.",
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(thinwedge_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/thinwedge/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_thinwedge_system_dir() -> PathBuf {
...
fn windows_system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let requirements_toml_file = windows_thinwedge_system_dir().join("requirements.toml");
    AbsolutePathBuf::try_from(requirements_toml_file)
}

#[cfg(windows)]
fn windows_system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    let config_toml_file = windows_thinwedge_system_dir().join("config.toml");
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for ThinWedge to function correctly.
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".thinwedge"))
    }
}
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        dot_thinwedge_folder: dot_thinwedge_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_thinwedge_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    for dir in dirs {
        let dot_thinwedge_abs = dir.join(".thinwedge");
        let dot_thinwedge_uri = PathUri::from_abs_path(&dot_thinwedge_abs);
        if !fs
            .get_metadata(&dot_thinwedge_uri, /*sandbox*/ None)
            .await
...
/// from a file like `/etc/thinwedge/managed_config.toml` that has the same
