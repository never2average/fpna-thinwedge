...
#[cfg(unix)]
const THINWEDGE_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/thinwedge/managed_config.toml";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/thinwedge/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(thinwedge_home)),
    )?;
...
pub(super) fn managed_config_default_path(thinwedge_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = thinwedge_home;
        PathBuf::from(THINWEDGE_MANAGED_CONFIG_SYSTEM_PATH)
    }
