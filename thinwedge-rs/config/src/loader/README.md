# `thinwedge-config` loader

This module is the canonical place to **load and describe ThinWedge configuration layers** (user config, CLI/session overrides, cloud-managed config, managed config, and MDM-managed preferences) and to produce:
...
Exported from `thinwedge_config::loader`:

- `load_config_layers_state(fs, thinwedge_home, cwd_opt, cli_overrides, options, thread_config_loader) -> ConfigLayerStack`
- `ConfigLayerStack`
...
4. `Project` config (`.thinwedge/config.toml`)
5. `User` profile config, when present
6. `User` config (`config.toml`)
7. `EnterpriseManaged` cloud-managed config bundle layers
8. `System` config (`/etc/thinwedge/config.toml` or the Windows system config path)
