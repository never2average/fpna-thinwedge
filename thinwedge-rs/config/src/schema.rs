...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == thinwedge_features::Feature::Artifact {
            continue;
        }
        if feature.id == thinwedge_features::Feature::CodeMode {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<thinwedge_features::FeatureToml<
                    thinwedge_features::CodeModeConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == thinwedge_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<thinwedge_features::FeatureToml<
                    thinwedge_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == thinwedge_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
        ...
        if feature.id == thinwedge_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<thinwedge_features::FeatureToml<
                    thinwedge_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
