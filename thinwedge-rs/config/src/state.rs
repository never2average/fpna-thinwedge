...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("thinwedge-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, thinwedge_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    // Get the `.thinwedge/` folder associated with this config layer, if any.
    ...
    /// Returns the `.thinwedge/` folder that should be used for hook declarations.
    ///
...
pub struct ConfigLayerStack {
    ...
    /// the base `$THINWEDGE_HOME/config.toml` layer followed by the profile override
...
impl ConfigLayerStack {
    ...
    /// the base `$THINWEDGE_HOME/config.toml` layer because the active layer is the
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_thinwedge_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_thinwedge_folder: current_project_dot_thinwedge_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_thinwedge_folder {
                ...
                if previous == current_project_dot_thinwedge_folder
                    || !current_project_dot_thinwedge_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_thinwedge_folder = Some(current_project_dot_thinwedge_folder);
        }
    }
