...
fn github_repo_api_path() -> String {
    format!("/repos/{THINWEDGE_PLUGINS_OWNER}/{THINWEDGE_PLUGINS_REPO}")
}

fn github_repo_git_ref_path(branch: &str) -> String {
    format!("{}/git/ref/heads/{branch}", github_repo_api_path())
}

fn github_repo_zipball_path(sha: &str) -> String {
    format!("{}/zipball/{sha}", github_repo_api_path())
}
...
fn write_curated_plugin_sha(thinwedge_home: &Path) {
    write_file(
...
fn has_plugins_clone_dirs(thinwedge_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(thinwedge_home.join(".tmp")) else {
        return false;
    };
...
async fn mount_github_zipball(server: &MockServer, sha: &str, bytes: Vec<u8>) {
    Mock::given(method("GET"))
        .and(path(github_repo_zipball_path(sha)))
        .respond_with(
            ResponseTemplate::new(200)
...
fn assert_curated_gmail_repo(repo_path: &Path) {
    ...
    assert!(
        repo_path
            .join("plugins/gmail/.thinwedge-plugin/plugin.json")
            .is_file()
    );
}

#[test]
fn curated_plugins_repo_path_uses_thinwedge_home_tmp_dir() {
...
fn sync_openai_plugins_repo_via_git_succeeds_with_local_rewritten_remote() {
    ...
    let remote_repo = repo_root
        .path()
        .join("remotes")
        .join(THINWEDGE_PLUGINS_OWNER)
        .join(format!("{}.git", THINWEDGE_PLUGINS_REPO));
    ...
    std::fs::write(
        work_repo.join("plugins/gmail/.thinwedge-plugin/plugin.json"),
        r#"{"name":"gmail"}"#,
    )
    ...
    run_git(
        ...
        &[
            "-c",
            "user.name=ThinWedge Test",
            "-c",
            "user.email=thinwedge@example.com",
            "commit",
            "-m",
            "init",
        ],
    ...
    assert!(
        curated_plugins_repo_path(tmp.path())
            .join("plugins/linear/.thinwedge-plugin/plugin.json")
            .is_file()
    );
    assert_eq!(
        read_curated_plugins_sha(tmp.path()).as_deref(),
    ...
    assert!(incremental_sync_invocations.iter().any(|invocation| {
        invocation.starts_with(&format!("-C {} fetch ", curated_repo_path.display()))
            && invocation.contains(&format!(" {THINWEDGE_PLUGINS_GIT_URL} "))
...
if [ "$1" = "-C" ] && [ "$3" = "reset" ]; then
  mkdir -p "$2/plugins/linear/.thinwedge-plugin"
  printf '%s\n' '{{"name":"linear"}}' > "$2/plugins/linear/.thinwedge-plugin/plugin.json"
  exit 0
fi
if [ "$1" = "-C" ] && [ "$3" = "clean" ]; then
  exit 0
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.thinwedge-plugin/plugin.json"),
            options,
        )
