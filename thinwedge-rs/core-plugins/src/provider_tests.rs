...
use thinwedge_exec_server::FileSystemResult;
...
async fn plugin_root_resolution_uses_supplied_executor_file_system() {
    ...
    let manifest_path = plugin_root.join(".thinwedge-plugin/plugin.json");
...
async fn malformed_preferred_manifest_does_not_fall_through_to_alternate() {
    ...
    write_manifest(&plugin_root, ".thinwedge-plugin/plugin.json", "{not-json");
    write_manifest(
        &plugin_root,
        ".claude-plugin/plugin.json",
        MANIFEST_CONTENTS,
    );
    let expected_path = AbsolutePathBuf::from_absolute_path_checked(
