...
async fn returns_fallback_plugins_without_installed_apps() {
    ...
    let plugins = load_plugins_config(thinwedge_home.path(), thinwedge_home.path()).await;
...
async fn returns_microsoft_fallback_plugins() {
    ...
    install_marketplace_plugin(thinwedge_home.path(), curated_root.as_path(), "teams").await;
...
async fn omits_openai_curated_when_remote_enabled() {
    ...
    let bundled_marketplace_root = thinwedge_home
        .path()
...
"#
    ...
    install_marketplace_plugin(thinwedge_home.path(), curated_root.as_path(), "installed").await;
...
async fn omits_installed_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(thinwedge_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    install_marketplace_plugin(thinwedge_home.path(), curated_root.as_path(), "slack").await;
...
async fn does_not_reload_marketplace_per_plugin() {
  ...
  "interface": {{
    ...
    assert_eq!(
        logs.matches("gmail/.thinwedge-plugin/plugin.json").count(),
        4
    );
    assert_eq!(
        logs.matches("openai-developers/.thinwedge-plugin/plugin.json")
            .count(),
        4
    );
}
...
async fn does_not_expand_local_sales_apps() {
    ...
    let sales_marketplace_root = thinwedge_home
        .path()
...
"#,
    ...
    let mut plugins = load_plugins_config(thinwedge_home.path(), thinwedge_home.path()).await;
...
async fn install_marketplace_plugin(
    ...
    write_curated_plugin_sha_with(thinwedge_home, TEST_CURATED_PLUGIN_SHA);
