...
pub(crate) async fn load_plugins_config(thinwedge_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let thinwedge_home =
        AbsolutePathBuf::try_from(thinwedge_home).expect("thinwedge home should be absolute");
