...
pub(crate) fn maybe_start_remote_installed_plugin_bundle_sync(
    ...
    let key = RemoteInstalledPluginBundleSyncKey {
        plugin_cache_root: remote_plugin_cache_root(&thinwedge_home),
    };
    ...
    tokio::spawn(async move {
        let result =
            sync_remote_installed_plugin_bundles_once(thinwedge_home, &config, Some(&auth)).await;
        match result {
            Ok(outcome) => {
...
fn remote_plugin_cache_root(thinwedge_home: &Path) -> PathBuf {
    thinwedge_home.join(PLUGINS_CACHE_DIR)
}
...
mod tests {
    ...
    fn remote_installed_plugin_sync_in_flight_dedupes_by_cache_root() {
        ...
        let key = RemoteInstalledPluginBundleSyncKey {
            plugin_cache_root: remote_plugin_cache_root(thinwedge_home.path()),
        };
    ...
    fn stale_remote_plugin_cleanup_skips_cache_mutations_in_progress() {
        let thinwedge_home = tempfile::tempdir().expect("create thinwedge home");
        let cached_manifest = thinwedge_home
            .path()
            .join(PLUGINS_CACHE_DIR)
    ...
    fn stale_remote_plugin_cleanup_removes_old_shared_with_me_cache_and_keeps_canonical_cache() {
        ...
        std::fs::write(&cached_manifest, r#"{"name":"private-plugin"}"#)
            .expect("write cached plugin manifest");
        let canonical_cached_manifest = thinwedge_home
            .path()
            .join(PLUGINS_CACHE_DIR)
