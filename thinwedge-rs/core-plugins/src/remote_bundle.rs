...
pub async fn download_and_install_remote_plugin_bundle(
    ...
    tokio::task::spawn_blocking(move || {
        install_remote_plugin_bundle(thinwedge_home, bundle, bundle_bytes)
    })
    .await
    .map_err(|err| {
...
fn install_remote_plugin_bundle(
    ...
    let staging_root = thinwedge_home.join(REMOTE_PLUGIN_INSTALL_STAGING_DIR);
    ...
    let store = PluginStore::try_new(thinwedge_home)?;
    store
...
mod tests {
    ...
    fn find_extracted_plugin_root_uses_local_manifest_discovery() {
        ...
        std::fs::create_dir_all(extraction_root.path().join(".thinwedge-plugin"))
            .expect("create manifest dir");
        std::fs::write(
            extraction_root.path().join(".thinwedge-plugin/plugin.json"),
            r#"{"name":"linear"}"#,
        )
