...
// Keep this comfortably above a normal sync attempt so we do not race another ThinWedge process.
...
pub fn curated_plugins_repo_path(thinwedge_home: &Path) -> PathBuf {
    thinwedge_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}

pub fn read_curated_plugins_sha(thinwedge_home: &Path) -> Option<String> {
    read_sha_file(curated_plugins_sha_path(thinwedge_home).as_path())
}

fn curated_plugins_sha_path(thinwedge_home: &Path) -> PathBuf {
...
fn sync_thinwedge_plugins_repo_via_backup_archive(
    ...
    let repo_path = curated_plugins_repo_path(thinwedge_home);
    let sha_path = curated_plugins_sha_path(thinwedge_home);
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|err| format!("failed to create curated plugins sync runtime: {err}"))?;
...
pub fn has_local_curated_plugins_snapshot(thinwedge_home: &Path) -> bool {
    curated_plugins_repo_path(thinwedge_home)
        .join(".agents/plugins/marketplace.json")
        .is_file()
        && thinwedge_home.join(CURATED_PLUGINS_SHA_FILE).is_file()
}
