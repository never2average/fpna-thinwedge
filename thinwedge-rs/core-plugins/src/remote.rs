...
const THINWEDGE_PRODUCT_SKU: &str = "thinwedge";
    ...
    for source in sources {
        match source {
            RemoteMarketplaceSource::Global => {
                ...
                if let Some(thinwedge_home) = global_catalog_cache_path
                ...
                if let (Some(thinwedge_home), Some(directory_plugins)) =
...
pub async fn fetch_and_cache_global_remote_plugin_catalog(
    ...
    catalog_cache::write_cached_global_directory_plugins(thinwedge_home, config, auth, &plugins);
    Ok(())
}
...
pub fn has_cached_global_remote_plugin_catalog(
    ...
    catalog_cache::load_cached_global_directory_plugins(thinwedge_home, config, auth).is_some()
}
...
pub fn cached_global_remote_discoverable_plugins(
    ...
    catalog_cache::load_cached_global_directory_plugins(thinwedge_home, config, auth)
        .unwrap_or_default()
        .into_iter()
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(thinwedge_home.clone())
...
fn authenticated_request(
    ...
    Ok(request
        ...
        .header(OAI_PRODUCT_SKU_HEADER, THINWEDGE_PRODUCT_SKU))
}
