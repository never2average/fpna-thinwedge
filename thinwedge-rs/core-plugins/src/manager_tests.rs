...
use thinwedge_config::AppToolApproval;
...
use thinwedge_config::McpServerOAuthConfig;
use thinwedge_config::McpServerToolConfig;
...
fn write_auth_projection_plugin(thinwedge_home: &Path, name: &str, include_app: bool) {
...
async fn auth_projection_config(thinwedge_home: &Path) -> PluginsConfigInput {
...
"#
    .to_string();
    write_file(&thinwedge_home.join(CONFIG_TOML_FILE), &config_toml);
    load_config(thinwedge_home, thinwedge_home).await
}
...
async fn plugin_auth_projection_hides_apps_without_chatgpt_auth() {
    ...
    write_auth_projection_plugin(thinwedge_home.path(), "sample", /*include_app*/ true);
    write_auth_projection_plugin(thinwedge_home.path(), "docs", /*include_app*/ false);
    let config = auth_projection_config(thinwedge_home.path()).await;
...
async fn load_plugins_from_config(
    ...
    write_file(&thinwedge_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(thinwedge_home, thinwedge_home).await;
...
async fn load_config(thinwedge_home: &Path, cwd: &Path) -> PluginsConfigInput {
    load_plugins_config_input(thinwedge_home, cwd).await
}
...
fn remote_installed_plugin_in_marketplace(
    ...
    RemoteInstalledPlugin {
        ...
        name: name.to_string(),
        enabled: true,
        install_policy: thinwedge_app_server_protocol::PluginInstallPolicy::Available,
        auth_policy: thinwedge_app_server_protocol::PluginAuthPolicy::OnUse,
...
fn write_cached_plugin(thinwedge_home: &Path, marketplace_name: &str, plugin_name: &str) {
    write_plugin_with_version(
        &thinwedge_home
            .join("plugins/cache")
...
"#;
    ...
    let outcome =
        load_plugins_from_config(config_toml, thinwedge_home.path(), /*auth_mode*/ None).await;
...
"#,
    ...
    let config = load_config(thinwedge_home.path(), thinwedge_home.path()).await;
    let manager = PluginsManager::new(thinwedge_home.path().to_path_buf());
...
"#,
    ...
    write_cached_plugin(thinwedge_home.path(), "openai-curated", "linear");
    write_cached_plugin(thinwedge_home.path(), "openai-curated", "calendar");
    write_cached_plugin(thinwedge_home.path(), "openai-curated-remote", "linear");
    write_cached_plugin(
        thinwedge_home.path(),
...
async fn build_remote_installed_plugin_marketplaces_from_cache_uses_remote_metadata() {
    ...
    plugin.install_policy = thinwedge_app_server_protocol::PluginInstallPolicy::InstalledByDefault;
    plugin.auth_policy = thinwedge_app_server_protocol::PluginAuthPolicy::OnInstall;
    plugin.interface = Some(thinwedge_app_server_protocol::PluginInterface {
    ...
    assert_eq!(
        ...
        thinwedge_app_server_protocol::PluginInstallPolicy::InstalledByDefault
    );
    assert_eq!(
        plugin.auth_policy,
        thinwedge_app_server_protocol::PluginAuthPolicy::OnInstall
    );
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    let thinwedge_home = TempDir::new().unwrap();
    let plugin_a_root = thinwedge_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_root = thinwedge_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-b/local");

    write_file(
        &plugin_a_root.join(".thinwedge-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".thinwedge-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    let outcome =
        load_plugins_from_config(&config_toml, thinwedge_home.path(), Some(AuthMode::Chatgpt))
            .await;
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(thinwedge_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        disabled_skill_paths: HashSet::new(),
        has_enabled_skills: false,
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![thinwedge_home.path().join("skills-plugin/skills").abs()],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![thinwedge_home.path().join("disabled-plugin/skills").abs()],
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config(thinwedge_home.path(), thinwedge_home.path()).await;
    let outcome = PluginsManager::new(thinwedge_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn plugin_cache_ignores_unrelated_session_overrides() {
    ...
    let plugin_root = thinwedge_home
        ...
        .join("test/sample/local");
    write_plugin(
        thinwedge_home.path().join("plugins/cache/test").as_path(),
        "sample/local",
        "sample",
    );
    write_file(
  ...
  "mcpServers": {
    ...
    let user_file = thinwedge_home.path().join(CONFIG_TOML_FILE).abs();
...
"#,
    ...
    write_file(
        &repo_root.join(".thinwedge/config.toml"),
        r#"[[skills.config]]
...
"#,
    ...
    let featured_plugin_ids = manager
        .featured_plugin_ids_for_config(
            &config,
            Some(&ThinWedgeAuth::create_dummy_chatgpt_auth_for_testing()),
        )
        .await
        .unwrap();
...
#[tokio::test]
async fn featured_plugin_ids_for_config_defaults_query_param_to_thinwedge() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["thinwedge-plugin"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(featured_plugin_ids, vec!["thinwedge-plugin".to_string()]);
}
...
#[test]
fn curated_plugin_ids_from_config_keys_reads_latest_thinwedge_home_user_config() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    assert_eq!(
        configured_curated_plugin_ids_from_thinwedge_home(tmp.path())
            .into_iter()
...
async fn load_plugins_ignores_project_config_files() {
    let thinwedge_home = TempDir::new().unwrap();
    let project_root = thinwedge_home.path().join("project");
    let plugin_root = thinwedge_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_root.join(".thinwedge/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let outcome = load_plugins_from_layer_stack(
        ...
        &PluginStore::new(thinwedge_home.path().to_path_buf()),
        Some(Product::ThinWedge),
