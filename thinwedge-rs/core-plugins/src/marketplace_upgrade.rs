...
use thinwedge_config::types::MarketplaceConfig;
...
pub fn upgrade_configured_git_marketplaces(
    ...
    let install_root = marketplace_install_root(thinwedge_home);
    ...
    for marketplace in marketplaces {
        match upgrade_configured_git_marketplace(thinwedge_home, &install_root, &marketplace) {
...
fn marketplace_install_root(thinwedge_home: &Path) -> PathBuf {
...
fn upgrade_configured_git_marketplace(
    ...
    activate_marketplace_root(&destination, staged_dir, || {
        ensure_configured_git_marketplace_unchanged(thinwedge_home, marketplace)?;
        record_user_marketplace(thinwedge_home, &marketplace.name, &update).map_err(|err| {
            format!(
...
fn ensure_configured_git_marketplace_unchanged(
    ...
    let current = read_configured_git_marketplace(thinwedge_home, &expected.name)?;
    match current {
