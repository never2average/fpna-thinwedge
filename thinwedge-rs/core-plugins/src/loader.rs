...
use thinwedge_config::types::PluginMcpServerConfig;
use thinwedge_core_skills::SkillMetadata;
use thinwedge_core_skills::config_rules::SkillConfigRules;
use thinwedge_core_skills::config_rules::resolve_disabled_skill_paths;
use thinwedge_core_skills::config_rules::skill_config_rules_from_stack;
use thinwedge_core_skills::loader::SkillRoot;
use thinwedge_core_skills::loader::load_skills_from_roots;
use thinwedge_exec_server::LOCAL_FS;
use thinwedge_mcp::PluginMcpServerPlacement;
use thinwedge_mcp::parse_plugin_mcp_config;
use thinwedge_plugin::AppConnectorId;
use thinwedge_plugin::LoadedPlugin;
...
use thinwedge_plugin::PluginLoadOutcome;
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    let configured_non_curated_plugin_ids =
        non_curated_plugin_ids_from_config_keys(configured_plugins_from_thinwedge_home(
            thinwedge_home,
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let materialized =
            materialize_marketplace_plugin_source(thinwedge_home, &source).map_err(|err| {
...
fn configured_plugins_from_thinwedge_home(
    thinwedge_home: &Path,
...
pub fn configured_curated_plugin_ids_from_thinwedge_home(thinwedge_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_thinwedge_home(
        thinwedge_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".thinwedge-plugin/plugin.json"));
...
pub async fn installed_plugin_telemetry_metadata(
    ...
    let store = match PluginStore::try_new(thinwedge_home.to_path_buf()) {
        Ok(store) => store,
        Err(err) => {
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = thinwedge_home.join("plugins/.marketplace-plugin-source-staging");
