...
pub async fn add_marketplace(
    ...
    tokio::task::spawn_blocking(move || add_marketplace_sync(thinwedge_home.as_path(), request))
        .await
...
fn add_marketplace_sync(
    ...
    add_marketplace_sync_with_cloner(thinwedge_home, request, clone_git_source)
}
    ...
    F: Fn(&str, Option<&str>, &[String], &Path) -> Result<(), MarketplaceAddError>,
{
    ...
    if let Some(existing_root) =
        installed_marketplace_root_for_source(thinwedge_home, &install_root, &install_metadata)?
    {
        let marketplace_name = validate_marketplace_source_root(&existing_root)?;
        record_added_marketplace_entry(thinwedge_home, &marketplace_name, &install_metadata)?;
...
mod tests {
    ...
    fn write_marketplace_source(source: &Path, marker: &str) -> std::io::Result<()> {
        ...
        fs::create_dir_all(source.join("plugins/sample/.thinwedge-plugin"))?;
        fs::write(
  ...
  "plugins": [
    {
      ...
      "source": {
        ...
        fs::write(
            source.join("plugins/sample/.thinwedge-plugin/plugin.json"),
            r#"{"name":"sample"}"#,
        )?;
