...
pub fn apply_git_patch(req: &ApplyGitRequest) -> io::Result<ApplyGitResult> {
    ...
    if let Ok(cfg) = std::env::var("THINWEDGE_APPLY_GIT_CFG") {
...
mod tests {
    ...
    fn init_repo() -> tempfile::TempDir {
        ...
        let _ = run(root, &["git", "config", "user.email", "thinwedge@example.com"]);
        let _ = run(root, &["git", "config", "user.name", "ThinWedge"]);
        dir
    }
