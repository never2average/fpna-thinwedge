...
impl Client {
    ...
    pub fn from_auth(base_url: impl Into<String>, auth: &ThinWedgeAuth) -> Result<Self> {
        Ok(Self::new(base_url)?
            .with_user_agent(get_thinwedge_user_agent())
            .with_auth_provider(thinwedge_model_provider::auth_provider_from_auth(auth)))
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("thinwedge-cli"));
        }
    ...
    pub async fn get_rate_limits_many(&self) -> Result<Vec<RateLimitSnapshot>> {
        let url = match self.path_style {
            PathStyle::ThinWedgeApi => format!("{}/api/thinwedge/usage", self.base_url),
    ...
    pub async fn list_tasks(
        ...
        let url = match self.path_style {
            PathStyle::ThinWedgeApi => format!("{}/api/thinwedge/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::ThinWedgeApi => format!("{}/api/thinwedge/tasks/{}", self.base_url, task_id),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::ThinWedgeApi => format!(
                "{}/api/thinwedge/tasks/{}/turns/{}/sibling_turns",
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::ThinWedgeApi => format!("{}/api/thinwedge/tasks", self.base_url),
    ...
    fn send_add_credits_nudge_email_url(&self) -> String {
        match self.path_style {
            PathStyle::ThinWedgeApi => format!(
                "{}/api/thinwedge/accounts/send_add_credits_nudge_email",
                self.base_url
            ),
            PathStyle::ChatGptApi => {
                format!(
...
mod tests {
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![AdditionalRateLimitDetails {
                limit_name: "thinwedge_other".to_string(),
                metered_feature: "thinwedge_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
        ...
        assert_eq!(snapshots[0].limit_id.as_deref(), Some("thinwedge"));
    ...
    fn preferred_snapshot_selection_matches_get_rate_limits_behavior() {
        ...
        assert_eq!(preferred.limit_id.as_deref(), Some("thinwedge"));
    }
