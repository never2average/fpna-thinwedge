...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=THINWEDGE_BWRAP_SOURCE_DIR");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_ALLOW_CROSS");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_PATH");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_SYSROOT_DIR");
    println!("cargo:rerun-if-env-changed=THINWEDGE_SKIP_BWRAP_BUILD");
    ...
    if target_os != "linux" || env::var_os("THINWEDGE_SKIP_BWRAP_BUILD").is_some() {
        return;
    }
...
fn try_build_bwrap() -> Result<(), String> {
    ...
    std::fs::write(
        &config_h,
        r#"#pragma once
#define PACKAGE_STRING "bubblewrap built for ThinWedge"
"#,
    )
...
fn resolve_bwrap_source_dir(manifest_dir: &Path) -> Result<PathBuf, String> {
    if let Ok(path) = env::var("THINWEDGE_BWRAP_SOURCE_DIR") {
        ...
        return Err(format!(
            "THINWEDGE_BWRAP_SOURCE_DIR was set but does not exist: {}",
            src_dir.display()
        ));
    }
...
Set THINWEDGE_BWRAP_SOURCE_DIR to an existing checkout or vendor bubblewrap under thinwedge-rs/vendor.",
        vendor_dir.display()
    ))
}
