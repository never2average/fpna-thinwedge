...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::ThinWedgeCompactionEvent) {
        self.record_fact(AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(
            Box::new(event),
        )));
    }

    pub fn track_goal_event(&self, event: ThinWedgeGoalEvent) {
    ...
    pub fn track_turn_thinwedge_error(&self, fact: TurnThinWedgeErrorFact) {
        self.record_fact(AnalyticsFact::Custom(
            CustomAnalyticsFact::TurnThinWedgeError(Box::new(fact)),
        ));
    }
...
async fn send_track_events(
    ...
    let url = format!("{base_url}/thinwedge/analytics-events/events");
