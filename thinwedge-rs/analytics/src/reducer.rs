...
use crate::events::ThinWedgeCollabAgentToolCallEventParams;
use crate::events::ThinWedgeCollabAgentToolCallEventRequest;
...
use crate::events::ThinWedgeDynamicToolCallEventParams;
use crate::events::ThinWedgeDynamicToolCallEventRequest;
use crate::events::ThinWedgeFileChangeEventParams;
use crate::events::ThinWedgeFileChangeEventRequest;
use crate::events::ThinWedgeGoalEventRequest;
use crate::events::ThinWedgeHookRunEventRequest;
use crate::events::ThinWedgeImageGenerationEventParams;
use crate::events::ThinWedgeImageGenerationEventRequest;
use crate::events::ThinWedgeMcpToolCallEventParams;
use crate::events::ThinWedgeMcpToolCallEventRequest;
...
use crate::events::ThinWedgeTurnEventParams;
...
use crate::events::ThinWedgeWebSearchEventParams;
use crate::events::ThinWedgeWebSearchEventRequest;
...
use crate::events::thinwedge_goal_event_params;
...
use crate::facts::ThinWedgeGoalEvent;
...
use crate::facts::TurnThinWedgeError;
...
use thinwedge_app_server_protocol::CollabAgentStatus;
...
use thinwedge_app_server_protocol::NetworkPolicyRuleAction;
...
use thinwedge_app_server_protocol::WebSearchAction;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn compaction(input: &'a ThinWedgeCompactionEvent) -> Self {
        Self {
    ...
    fn goal(input: &'a ThinWedgeGoalEvent) -> Self {
        Self {
            event_name: "goal",
            thread_id: &input.thread_id,
    ...
    fn tool_item(
        notification: &'a thinwedge_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
struct CompletedTurnState {
    ...
    turn_error: Option<ThinWedgeErrorInfo>,
    completed_at: u64,
...
struct TurnState {
    ...
    thinwedge_error: Option<TurnThinWedgeError>,
...
impl AnalyticsReducer {
    pub(crate) async fn ingest(&mut self, input: AnalyticsFact, out: &mut Vec<TrackEventRequest>) {
        match input {
            ...
            AnalyticsFact::Custom(input) => match input {
                ...
                CustomAnalyticsFact::TurnThinWedgeError(input) => {
                    self.ingest_turn_thinwedge_error(*input);
                }
    ...
    fn ingest_guardian_review(
        ...
        out.push(TrackEventRequest::GuardianReview(Box::new(
            GuardianReviewEventRequest {
                event_type: "thinwedge_guardian_review",
    ...
    fn ingest_turn_thinwedge_error(&mut self, input: TurnThinWedgeErrorFact) {
        let TurnThinWedgeErrorFact {
            turn_id,
            thread_id,
            error,
        } = input;
        ...
        turn_state.thinwedge_error = Some(error);
    }
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = thinwedge_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(ThinWedgeAppMentionedEventRequest {
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = thinwedge_app_metadata(&tracking, app);
        out.push(TrackEventRequest::AppUsed(ThinWedgeAppUsedEventRequest {
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(ThinWedgeHookRunEventRequest {
            event_type: "thinwedge_hook_run",
            event_params: thinwedge_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_state_changed(
        ...
        let event = ThinWedgePluginEventRequest {
            ...
            ServerNotification::TurnCompleted(notification) => {
                ...
                turn_state.completed = Some(CompletedTurnState {
                    ...
                    turn_error: notification
                        .turn
                        .error
                        .and_then(|error| error.thinwedge_error_info),
    ...
    fn emit_thread_initialized(
        &mut self,
        connection_id: u64,
        thread: thinwedge_app_server_protocol::Thread,
        model: String,
        initialization_mode: ThreadInitializationMode,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    fn ingest_compaction(
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            ThinWedgeCompactionEventRequest {
                event_type: "thinwedge_compaction_event",
                event_params: thinwedge_compaction_event_params(
                    input,
    ...
    fn ingest_goal(&mut self, input: ThinWedgeGoalEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_metadata)) =
        ...
        out.push(TrackEventRequest::Goal(Box::new(
            ThinWedgeGoalEventRequest {
                event_type: "thinwedge_goal_event",
                event_params: thinwedge_goal_event_params(
                    input,
                    thread_metadata.session_id.clone(),
                    connection_state.app_server_client.clone(),
                    connection_state.runtime.clone(),
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: thinwedge_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    async fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        let turn_event = TrackEventRequest::TurnEvent(Box::new(ThinWedgeTurnEventRequest {
            event_type: "thinwedge_turn_event",
            event_params: thinwedge_turn_event_params(
                connection_state.app_server_client.clone(),
                connection_state.runtime.clone(),
                turn_id.to_string(),
                turn_state,
    ...
    match item {
        ThreadItem::CommandExecution {
            ...
            Some(TrackEventRequest::CommandExecution(
                ThinWedgeCommandExecutionEventRequest {
        ...
        ThreadItem::FileChange {
            ...
            Some(TrackEventRequest::FileChange(
                ThinWedgeFileChangeEventRequest {
                    event_type: "thinwedge_file_change_event",
                    event_params: ThinWedgeFileChangeEventParams {
                        base,
        ...
        ThreadItem::McpToolCall {
            ...
            Some(TrackEventRequest::McpToolCall(
                ThinWedgeMcpToolCallEventRequest {
                    event_type: "thinwedge_mcp_tool_call_event",
                    event_params: ThinWedgeMcpToolCallEventParams {
                        base,
        ...
        ThreadItem::DynamicToolCall {
            ...
            Some(TrackEventRequest::DynamicToolCall(
                ThinWedgeDynamicToolCallEventRequest {
                    event_type: "thinwedge_dynamic_tool_call_event",
                    event_params: ThinWedgeDynamicToolCallEventParams {
                        base,
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            Some(TrackEventRequest::CollabAgentToolCall(
                ThinWedgeCollabAgentToolCallEventRequest {
                    event_type: "thinwedge_collab_agent_tool_call_event",
                    event_params: ThinWedgeCollabAgentToolCallEventParams {
                        base,
        ...
        ThreadItem::WebSearch { id, query, action } => {
            ...
            Some(TrackEventRequest::WebSearch(
                ThinWedgeWebSearchEventRequest {
                    event_type: "thinwedge_web_search_event",
                    event_params: ThinWedgeWebSearchEventParams {
                        base,
        ...
        ThreadItem::ImageGeneration {
            ...
            Some(TrackEventRequest::ImageGeneration(
                ThinWedgeImageGenerationEventRequest {
                    event_type: "thinwedge_image_generation_event",
                    event_params: ThinWedgeImageGenerationEventParams {
                        base,
...
fn file_change_counts(
    changes: &[thinwedge_app_server_protocol::FileUpdateChange],
) -> FileChangeCounts {
...
fn thinwedge_turn_event_params(
    app_server_client: ThinWedgeAppServerClientMetadata,
    runtime: ThinWedgeRuntimeMetadata,
    turn_id: String,
    turn_state: &TurnState,
    thread_metadata: &ThreadMetadataState,
) -> ThinWedgeTurnEventParams {
    let (
        Some(thread_id),
    ...
    let thinwedge_error = turn_state.thinwedge_error.as_ref();
    ThinWedgeTurnEventParams {
        thread_id,
        session_id: thread_metadata.session_id.clone(),
        turn_id,
        ...
        thinwedge_error_kind: thinwedge_error.map(|error| error.kind),
        thinwedge_error_http_status_code: thinwedge_error.and_then(|error| error.http_status_code),
...
fn analytics_turn_status(status: thinwedge_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        thinwedge_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        thinwedge_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        thinwedge_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        thinwedge_app_server_protocol::TurnStatus::InProgress => None,
    }
}
