...
use crate::events::ThinWedgeAcceptedLineFingerprintsEventParams;
use crate::events::ThinWedgeAcceptedLineFingerprintsEventRequest;
...
use crate::events::ThinWedgeCommandExecutionEventParams;
use crate::events::ThinWedgeCommandExecutionEventRequest;
use crate::events::ThinWedgeCompactionEventRequest;
use crate::events::ThinWedgeHookRunEventRequest;
use crate::events::ThinWedgePluginEventRequest;
use crate::events::ThinWedgePluginUsedEventRequest;
use crate::events::ThinWedgeReviewEventParams;
use crate::events::ThinWedgeReviewEventRequest;
use crate::events::ThinWedgeRuntimeMetadata;
use crate::events::ThinWedgeToolItemEventBase;
...
use crate::facts::ThinWedgeErrKind;
...
use crate::facts::TurnThinWedgeErrorFact;
...
use std::sync::Arc;
use std::sync::Mutex;
use thinwedge_app_server_protocol::ApprovalsReviewer as AppServerApprovalsReviewer;
use thinwedge_app_server_protocol::AskForApproval as AppServerAskForApproval;
...
use thinwedge_app_server_protocol::DynamicToolCallStatus;
use thinwedge_app_server_protocol::GuardianApprovalReview;
...
use thinwedge_app_server_protocol::GuardianCommandSource as AppServerGuardianCommandSource;
...
use thinwedge_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification;
...
use thinwedge_app_server_protocol::NonSteerableTurnKind;
use thinwedge_app_server_protocol::PatchApplyStatus;
use thinwedge_app_server_protocol::PermissionsRequestApprovalParams;
use thinwedge_app_server_protocol::RequestId;
use thinwedge_app_server_protocol::RequestPermissionProfile;
use thinwedge_app_server_protocol::SandboxPolicy as AppServerSandboxPolicy;
use thinwedge_app_server_protocol::ServerNotification;
use thinwedge_app_server_protocol::ServerRequest;
use thinwedge_app_server_protocol::ServerResponse;
use thinwedge_app_server_protocol::SessionSource as AppServerSessionSource;
use thinwedge_app_server_protocol::SubAgentActivityKind;
...
use thinwedge_app_server_protocol::ThreadSource as AppServerThreadSource;
use thinwedge_app_server_protocol::ThreadStartResponse;
use thinwedge_app_server_protocol::ThreadStatus as AppServerThreadStatus;
use thinwedge_app_server_protocol::Turn;
use thinwedge_app_server_protocol::TurnCompletedNotification;
use thinwedge_app_server_protocol::TurnDiffUpdatedNotification;
use thinwedge_app_server_protocol::TurnError as AppServerTurnError;
use thinwedge_app_server_protocol::TurnStartParams;
use thinwedge_app_server_protocol::TurnStartedNotification;
use thinwedge_app_server_protocol::TurnStatus as AppServerTurnStatus;
...
use thinwedge_login::default_client::DEFAULT_ORIGINATOR;
...
use thinwedge_protocol::request_permissions::RequestPermissionsResponse as CoreRequestPermissionsResponse;
...
fn sample_app_server_client_metadata() -> ThinWedgeAppServerClientMetadata {
    ThinWedgeAppServerClientMetadata {
...
fn sample_runtime_metadata() -> ThinWedgeRuntimeMetadata {
    ThinWedgeRuntimeMetadata {
        thinwedge_rs_version: "0.1.0".to_string(),
...
fn sample_turn_start_response(turn_id: &str) -> ClientResponsePayload {
    ClientResponsePayload::TurnStart(thinwedge_app_server_protocol::TurnStartResponse {
        turn: Turn {
...
fn sample_turn_completed_notification(
    ...
    thinwedge_error_info: Option<thinwedge_app_server_protocol::ThinWedgeErrorInfo>,
) -> ServerNotification {
    ServerNotification::TurnCompleted(TurnCompletedNotification {
        ...
        turn: Turn {
            ...
            items: vec![],
            status,
            error: thinwedge_error_info.map(|thinwedge_error_info| AppServerTurnError {
                message: "turn failed".to_string(),
                thinwedge_error_info: Some(thinwedge_error_info),
            ...
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "thinwedge-web".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "thinwedge-web".to_string(),
    ...
    reducer
        .ingest(
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "thinwedge-tui".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "thinwedge-tui".to_string(),
                runtime: sample_runtime_metadata(),
                rpc_transport: AppServerRpcTransport::Stdio,
            },
            out,
...
fn sample_initialize_fact(connection_id: u64) -> AnalyticsFact {
    AnalyticsFact::Initialize {
        ...
        product_client_id: DEFAULT_ORIGINATOR.to_string(),
        runtime: ThinWedgeRuntimeMetadata {
            thinwedge_rs_version: "0.99.0".to_string(),
...
fn sample_guardian_review_completed(
    ...
    ServerNotification::ItemGuardianApprovalReviewCompleted(
        ItemGuardianApprovalReviewCompletedNotification {
            ...
            decision_source: thinwedge_app_server_protocol::AutoReviewDecisionSource::Agent,
...
fn normalize_path_for_skill_id_repo_scoped_uses_relative_path() {
    ...
    let skill_path = PathBuf::from("/repo/root/.thinwedge/skills/doc/SKILL.md");
    ...
    assert_eq!(path, ".thinwedge/skills/doc/SKILL.md");
}

#[test]
fn normalize_path_for_skill_id_user_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/Users/abc/.thinwedge/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_admin_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/etc/thinwedge/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_repo_root_not_in_skill_path_uses_absolute_path() {
    let repo_root = PathBuf::from("/repo/root");
    let skill_path = PathBuf::from("/other/path/.thinwedge/skills/doc/SKILL.md");
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(ThinWedgeAppMentionedEventRequest {
        event_type: "thinwedge_app_mentioned",
        event_params: thinwedge_app_metadata(
            &tracking,
            AppInvocation {
...
fn app_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppUsed(ThinWedgeAppUsedEventRequest {
        event_type: "thinwedge_app_used",
        event_params: thinwedge_app_metadata(
            &tracking,
            AppInvocation {
...
fn accepted_line_fingerprints_event_serializes_expected_shape() {
    let event = TrackEventRequest::AcceptedLineFingerprints(Box::new(
        ThinWedgeAcceptedLineFingerprintsEventRequest {
            event_type: "thinwedge_accepted_line_fingerprints",
            event_params: ThinWedgeAcceptedLineFingerprintsEventParams {
                event_type: "thinwedge.accepted_line_fingerprints",
                turn_id: "turn-1".to_string(),
                thread_id: "thread-1".to_string(),
                product_surface: Some("thinwedge".to_string()),
                model_slug: Some("gpt-5.1-thinwedge".to_string()),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": "turn-1",
                "thread_id": "thread-1",
                "product_surface": "thinwedge",
                "model_slug": "gpt-5.1-thinwedge",
                "completed_at": 1710000000,
...
fn compaction_event_serializes_expected_shape() {
    let event = TrackEventRequest::Compaction(Box::new(ThinWedgeCompactionEventRequest {
        event_type: "thinwedge_compaction_event",
        event_params: crate::events::thinwedge_compaction_event_params(
            ThinWedgeCompactionEvent {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                ...
                thinwedge_error_kind: None,
                thinwedge_error_http_status_code: None,
        ...
        json!({
            ...
            "event_params": {
                ...
                "app_server_client": {
                    ...
                    "client_name": "thinwedge-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
                },
                "runtime": {
                    "thinwedge_rs_version": "0.1.0",
                    "runtime_os": "macos",
                ...
                "strategy": "memento",
                "status": "completed",
                "thinwedge_error_kind": null,
                "thinwedge_error_http_status_code": null,
...
fn thread_initialized_event_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(ThreadInitializedEvent {
        event_type: "thinwedge_thread_initialized",
        event_params: ThreadInitializedEventParams {
            thread_id: "thread-0".to_string(),
            session_id: "session-thread-0".to_string(),
            app_server_client: ThinWedgeAppServerClientMetadata {
                product_client_id: DEFAULT_ORIGINATOR.to_string(),
                client_name: Some("thinwedge-tui".to_string()),
                client_version: Some("1.0.0".to_string()),
                rpc_transport: AppServerRpcTransport::Stdio,
...
fn command_execution_event_serializes_expected_shape() {
    let event = TrackEventRequest::CommandExecution(ThinWedgeCommandExecutionEventRequest {
        event_type: "thinwedge_command_execution_event",
        event_params: ThinWedgeCommandExecutionEventParams {
            base: ThinWedgeToolItemEventBase {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                item_id: "item-1".to_string(),
                app_server_client: ThinWedgeAppServerClientMetadata {
                    product_client_id: "thinwedge_tui".to_string(),
                    client_name: Some("thinwedge-tui".to_string()),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "item_id": "item-1",
                "app_server_client": {
                    "product_client_id": "thinwedge_tui",
                    "client_name": "thinwedge-tui",
...
fn review_event_serializes_expected_shape() {
    let event = TrackEventRequest::ReviewEvent(ThinWedgeReviewEventRequest {
        event_type: "thinwedge_review_event",
        event_params: ThinWedgeReviewEventParams {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            item_id: None,
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    assert_eq!(payload[0]["event_type"], "thinwedge_thread_initialized");
    ...
    assert_eq!(
        payload[0]["event_params"]["runtime"]["thinwedge_rs_version"],
        "0.99.0"
    );
...
async fn compaction_event_ingests_custom_fact() {
    ...
    let parent_thread_id =
        thinwedge_protocol::ThreadId::from_string("22222222-2222-2222-2222-222222222222")
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(Box::new(
                ThinWedgeCompactionEvent {
                    ...
                    thinwedge_error_kind: Some(ThinWedgeErrKind::ContextWindowExceeded),
                    thinwedge_error_http_status_code: None,
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "thinwedge_compaction_event");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-compact");
    assert_eq!(
        payload[0]["event_params"]["thinwedge_error_kind"],
        json!("context_window_exceeded")
    );
    assert_eq!(
        payload[0]["event_params"]["thinwedge_error_http_status_code"],
        json!(null)
    );
    assert_eq!(
        payload[0]["event_params"]["app_server_client"]["product_client_id"],
...
async fn guardian_review_event_ingests_custom_fact_with_optional_target_item() {
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::GuardianReview(Box::new(
                GuardianReviewEventParams {
                    ...
                    guardian_default_review_model_id: Some("thinwedge-auto-review".to_string()),
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "thinwedge_guardian_review");
    assert_eq!(
...
async fn item_lifecycle_notifications_publish_command_execution_event() {
    ...
    assert_eq!(
        ...
        "thinwedge_command_execution_event"
    );
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
...
async fn command_execution_approval_response_publishes_user_review_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "thinwedge_review_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-1");
    assert_eq!(payload[0]["event_params"]["item_id"], "item-1");
...
async fn guardian_completed_notification_publishes_review_event_with_thread_metadata() {
    ...
    assert_eq!(payload["event_type"], "thinwedge_review_event");
...
fn subagent_thread_started_review_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            ...
            forked_from_thread_id: None,
            product_client_id: "thinwedge-tui".to_string(),
            client_name: "thinwedge-tui".to_string(),
...
fn subagent_thread_started_thread_spawn_serializes_thread_lineage() {
    let parent_thread_id =
        thinwedge_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
            .expect("valid thread id");
    let forked_from_thread_id =
        thinwedge_protocol::ThreadId::from_string("22222222-2222-4222-8222-222222222222")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
fn subagent_thread_started_other_serializes_explicit_parent_thread_id() {
    let parent_thread_id =
        thinwedge_protocol::ThreadId::from_string("33333333-3333-4333-8333-333333333333")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
async fn subagent_events_use_inherited_connection_unless_turn_connection_is_explicit() {
    ...
    let mut events = Vec::new();
    let parent_thread_id =
        thinwedge_protocol::ThreadId::from_string("44444444-4444-4444-4444-444444444444")
            .expect("valid parent thread id");
    ...
    assert_eq!(params.runtime.thinwedge_rs_version, "0.1.0");
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(ThinWedgePluginUsedEventRequest {
        event_type: "thinwedge_plugin_used",
        event_params: thinwedge_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(ThinWedgePluginEventRequest {
        event_type: "thinwedge_plugin_installed",
        event_params: thinwedge_plugin_metadata(sample_plugin_metadata()),
    });
...
fn plugin_management_event_can_use_remote_plugin_id_override() {
    ...
    let event = TrackEventRequest::PluginInstalled(ThinWedgePluginEventRequest {
        event_type: "thinwedge_plugin_installed",
        event_params: thinwedge_plugin_metadata(plugin),
    });
...
fn hook_run_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::HookRun(ThinWedgeHookRunEventRequest {
        event_type: "thinwedge_hook_run",
        event_params: thinwedge_hook_run_metadata(
            &tracking,
            HookRunFact {
...
fn hook_run_metadata_maps_sources_and_statuses() {
    ...
    let system = serde_json::to_value(thinwedge_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let project = serde_json::to_value(thinwedge_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let cloud_requirements = serde_json::to_value(thinwedge_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
    ...
    let unknown = serde_json::to_value(thinwedge_hook_run_metadata(
        &tracking,
        HookRunFact {
...
fn hook_run_metadata_maps_stopped_status() {
    ...
    let stopped = serde_json::to_value(thinwedge_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
            hook_source: HookSource::User,
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    skill_scope: thinwedge_protocol::protocol::SkillScope::User,
                    skill_path,
                    plugin_id: None,
...
async fn reducer_includes_plugin_id_for_plugin_skill_invocations() {
    ...
    let skill_path =
        PathBuf::from("/Users/abc/.thinwedge/plugins/cache/test/sample/skills/doc/SKILL.md");
...
async fn reducer_ingests_hook_run_fact() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "thinwedge_hook_run");
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "thinwedge_app_mentioned");
    assert_eq!(payload[1]["event_type"], "thinwedge_app_used");
    assert_eq!(payload[2]["event_type"], "thinwedge_plugin_used");
}
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "thinwedge_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "plugin_name": "sample",
                "marketplace_name": "test",
...
fn turn_event_serializes_expected_shape() {
    let event = TrackEventRequest::TurnEvent(Box::new(ThinWedgeTurnEventRequest {
        event_type: "thinwedge_turn_event",
        event_params: crate::events::ThinWedgeTurnEventParams {
            thread_id: "thread-2".to_string(),
    ...
    let expected = serde_json::from_str::<serde_json::Value>(
        r#"{
            ...
            "event_params": {
                ...
                "app_server_client": {
                    "product_client_id": "thinwedge_cli_rs",
                    "client_name": "thinwedge-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
...
async fn accepted_turn_steer_emits_expected_event() {
    ...
    assert_eq!(payload["event_type"], json!("thinwedge_turn_steer_event"));
    ...
    assert_eq!(
        payload["event_params"]["runtime"]["thinwedge_rs_version"],
        json!("0.1.0")
    );
...
async fn turn_lifecycle_emits_turn_event() {
    ...
    assert_eq!(payload["event_type"], json!("thinwedge_turn_event"));
    assert_eq!(payload["event_params"]["thread_id"], json!("thread-2"));
    assert_eq!(
        payload["event_params"]["session_id"],
        json!("session-thread-2")
    ...
    assert_eq!(
        ...
        json!({
            "product_client_id": "thinwedge-tui",
            "client_name": "thinwedge-tui",
            "client_version": "1.0.0",
            "rpc_transport": "stdio",
...
async fn turn_lifecycle_emits_failed_turn_event() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::TurnThinWedgeError(Box::new(
                TurnThinWedgeErrorFact::from_thinwedge_err(
                    "thread-2".to_string(),
                    "turn-2".to_string(),
                    &ThinWedgeErr::InvalidRequest(
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(thinwedge_app_server_protocol::ThinWedgeErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
    ...
    assert_eq!(
        payload["event_params"]["thinwedge_error_kind"],
        json!("invalid_request")
    );
    assert_eq!(
        payload["event_params"]["thinwedge_error_http_status_code"],
        json!(null)
    );
}
...
async fn turn_lifecycle_emits_interrupted_turn_event_without_error() {
    ...
    assert_eq!(payload["event_params"]["thinwedge_error_kind"], json!(null));
}
