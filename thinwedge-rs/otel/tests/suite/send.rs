...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "thinwedge.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        match find_metric(&resource_metrics, "thinwedge.tool_latency").and_then(|metric| {
            match metric.data() {
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "thinwedge.alpha").expect("thinwedge.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "thinwedge.beta").expect("thinwedge.beta metric missing");
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("thinwedge.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("thinwedge.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;
