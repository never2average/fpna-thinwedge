...
pub use thinwedge_utils_string::sanitize_metric_tag_value;
...
/// Maps to API/auth `AuthMode` to avoid a circular dependency on thinwedge-core.
...
impl From<thinwedge_app_server_protocol::AuthMode> for TelemetryAuthMode {
    fn from(mode: thinwedge_app_server_protocol::AuthMode) -> Self {
        match mode {
            thinwedge_app_server_protocol::AuthMode::ApiKey
            | thinwedge_app_server_protocol::AuthMode::BedrockApiKey => Self::ApiKey,
            ...
            | thinwedge_app_server_protocol::AuthMode::PersonalAccessToken => Self::Chatgpt,
        }
    }
}
