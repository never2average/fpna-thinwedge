...
`thinwedge-otel` is the OpenTelemetry integration crate for ThinWedge. It provides:

- Provider wiring for log/trace/metric exporters (`thinwedge_otel::OtelProvider`
  and `thinwedge_otel::provider`).
- Session-scoped business event emission via `thinwedge_otel::SessionTelemetry`.
- Low-level metrics APIs via `thinwedge_otel::metrics`.
- Trace-context helpers via `thinwedge_otel::trace_context` and crate-root re-exports.
...
```rust
use thinwedge_otel::config::OtelExporter;
use thinwedge_otel::config::OtelHttpProtocol;
use thinwedge_otel::config::OtelSettings;
use thinwedge_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "thinwedge-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    thinwedge_home: std::path::PathBuf::from("/tmp"),
...
ThinWedge-specific session events. Rich session/business events should go through
...
```rust
use thinwedge_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("thinwedge.session_started", 1, &[("source", "tui")])?;
metrics.histogram("thinwedge.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("thinwedge.turns", 1, &[("model", "gpt-5.1")])?;
