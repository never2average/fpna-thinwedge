# thinwedge-network-proxy

`thinwedge-network-proxy` is ThinWedge's local network policy enforcement proxy. It runs:
...
`thinwedge-network-proxy` reads from ThinWedge's merged `config.toml` (via `thinwedge-core` config loading).
...
# CA cert/key are managed internally under $THINWEDGE_HOME/proxy/ (ca.pem + ca.key).
# When MITM is active, spawned commands receive CA bundle env vars pointing at
# immutable bundles under $THINWEDGE_HOME/proxy/ so common HTTPS clients trust the managed CA.
...
```bash
cargo run -p thinwedge-network-proxy --
```
...
`thinwedge-network-proxy` can be embedded as a library with a thin API:

```rust
use thinwedge_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `thinwedge-network-proxy` is embedded in managed ThinWedge runtime, policy decisions emit structured
OTEL-compatible events with `target=thinwedge_otel.network_proxy`.

Event name:

- `thinwedge.network_proxy.policy_decision`
...
This section documents the protections implemented by `thinwedge-network-proxy`, and the boundaries of
