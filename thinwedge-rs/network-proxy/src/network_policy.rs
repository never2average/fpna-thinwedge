...
const AUDIT_TARGET: &str = "thinwedge_otel.network_proxy";
const POLICY_DECISION_EVENT_NAME: &str = "thinwedge.network_proxy.policy_decision";
...
mod tests {
    ...
    const LEGACY_DOMAIN_POLICY_DECISION_EVENT_NAME: &str =
        "thinwedge.network_proxy.domain_policy_decision";
    const LEGACY_BLOCK_DECISION_EVENT_NAME: &str = "thinwedge.network_proxy.block_decision";
    ...
    async fn evaluate_host_policy_emits_domain_event_for_decider_allow_override() {
        ...
        assert!(event.target.starts_with("thinwedge_otel."));
        assert_eq!(
    ...
    async fn evaluate_host_policy_emits_metadata_fields() {
        ...
        assert_eq!(event.field("originator"), Some("thinwedge_cli_rs"));
        ...
        assert_eq!(event.field("model"), Some("gpt-5.3-thinwedge"));
        assert_eq!(event.field("slug"), Some("gpt-5.3-thinwedge"));
    }
