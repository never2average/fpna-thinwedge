...
impl NetworkProxyBuilder {
    ...
    pub fn managed_by_thinwedge(mut self, managed_by_thinwedge: bool) -> Self {
        self.managed_by_thinwedge = managed_by_thinwedge;
        self
    }
...
pub const PROXY_ACTIVE_ENV_KEY: &str = "THINWEDGE_NETWORK_PROXY_ACTIVE";
pub const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "THINWEDGE_NETWORK_ALLOW_LOCAL_BINDING";
...
#[cfg(target_os = "macos")]
pub const THINWEDGE_PROXY_GIT_SSH_COMMAND_MARKER: &str = "THINWEDGE_PROXY_GIT_SSH_COMMAND=1 ";
#[cfg(target_os = "macos")]
const THINWEDGE_PROXY_GIT_SSH_COMMAND_PREFIX: &str =
    "THINWEDGE_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x ";
#[cfg(target_os = "macos")]
const THINWEDGE_PROXY_GIT_SSH_COMMAND_SUFFIX: &str = " %h %p'";
...
#[cfg(target_os = "macos")]
fn is_thinwedge_proxy_git_ssh_command(command: &str) -> bool {
    command.starts_with(THINWEDGE_PROXY_GIT_SSH_COMMAND_PREFIX)
        && command.ends_with(THINWEDGE_PROXY_GIT_SSH_COMMAND_SUFFIX)
}
...
fn apply_proxy_env_overrides(
    ...
    if socks_enabled {
        ...
        // but refresh a previously injected ThinWedge fallback so it cannot point
        // at a stale proxy port after the proxy is restarted.
        match env.get(GIT_SSH_COMMAND_ENV_KEY) {
            Some(command) if !is_thinwedge_proxy_git_ssh_command(command) => {}
            _ => {
                env.insert(
                    GIT_SSH_COMMAND_ENV_KEY.to_string(),
                    thinwedge_proxy_git_ssh_command(socks_addr),
                );
            }
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn non_thinwedge_managed_proxy_builder_uses_configured_ports() {
    ...
    fn apply_proxy_env_overrides_sets_common_tool_vars() {
        ...
        assert_eq!(
            ...
            Some(
                &"THINWEDGE_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
                    .to_string()
            )
        );
    ...
    fn apply_proxy_env_overrides_sets_mitm_ca_trust_bundle_vars() {
        let mut env = HashMap::new();
        let mitm_ca_trust_bundle_path = Path::new("/tmp/thinwedge-proxy/ca-bundle.pem");
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn apply_proxy_env_overrides_refreshes_previous_thinwedge_proxy_git_ssh_command() {
        let mut env = HashMap::new();
        env.insert(
            GIT_SSH_COMMAND_ENV_KEY.to_string(),
            thinwedge_proxy_git_ssh_command(SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 8081)),
        );
        ...
        assert_eq!(
            env.get(GIT_SSH_COMMAND_ENV_KEY),
            Some(&thinwedge_proxy_git_ssh_command(SocketAddr::new(
