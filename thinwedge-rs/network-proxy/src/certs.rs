...
fn managed_ca_paths() -> Result<(PathBuf, PathBuf)> {
    let thinwedge_home =
        find_thinwedge_home().context("failed to resolve THINWEDGE_HOME for managed MITM CA")?;
    let proxy_dir = thinwedge_home.join(MANAGED_MITM_CA_DIR);
    Ok((
...
/// Returns whether `path` points at a current ThinWedge-generated MITM CA bundle.
