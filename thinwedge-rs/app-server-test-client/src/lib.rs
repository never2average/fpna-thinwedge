...
use thinwedge_app_server_protocol::SandboxPolicy;
...
/// Minimal launcher that initializes the ThinWedge app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "ThinWedge", version, about = "Bootstrap ThinWedge app-server", long_about = None)]
struct Cli {
    /// Path to the `thinwedge` CLI binary. When set, requests use stdio by
    /// spawning `thinwedge app-server` as a child process.
    #[arg(long, env = "THINWEDGE_BIN", global = true)]
    thinwedge_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--thinwedge-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "THINWEDGE_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `thinwedge` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `thinwedge app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/thinwedge-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `thinwedge app-server --listen`.
    ...
    /// Send a user message through the ThinWedge app-server.
    SendMessage {
        /// User message to send to ThinWedge.
        user_message: String,
    },
    ...
    /// Fetch the current account rate limits from the ThinWedge app-server.
    GetAccountRateLimits,
    /// List the available models from the ThinWedge app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the ThinWedge app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "THINWEDGE_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let thinwedge_bin = thinwedge_bin.unwrap_or_else(|| PathBuf::from("thinwedge"));
            serve(&thinwedge_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(thinwedge_bin, url)?;
...
fn resolve_endpoint(thinwedge_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if thinwedge_bin.is_some() && url.is_some() {
        bail!("--thinwedge-bin and --url are mutually exclusive");
    }
    if let Some(thinwedge_bin) = thinwedge_bin {
        return Ok(Endpoint::SpawnThinWedge(thinwedge_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if thinwedge_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --thinwedge-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(thinwedge_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(thinwedge_bin);
        if let Some(thinwedge_bin_parent) = thinwedge_bin.parent() {
            let mut path = OsString::from(thinwedge_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", thinwedge_bin.display()))?;
...
fn serve(thinwedge_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,thinwedge_=trace {}",
        shell_quote(&thinwedge_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started thinwedge app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnThinWedge(thinwedge_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut ThinWedgeClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = ThinWedgeClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = ThinWedgeClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (thinwedge_bin, url) {
        (Some(_), Some(_)) => bail!("--thinwedge-bin and --url are mutually exclusive"),
        (Some(thinwedge_bin), None) => {
            let server = BackgroundAppServer::spawn(&thinwedge_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve thinwedge-app-server-test-client binary path")?;
...
impl ThinWedgeClient {
    ...
    fn spawn_stdio(thinwedge_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        let thinwedge_bin_display = thinwedge_bin.display();
        let mut cmd = Command::new(thinwedge_bin);
        if let Some(thinwedge_bin_parent) = thinwedge_bin.parent() {
            let mut path = OsString::from(thinwedge_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut thinwedge_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{thinwedge_bin_display}` app-server"))?;

        let stdin = thinwedge_app_server
            .stdin
            .take()
            .context("thinwedge app-server stdin unavailable")?;
        let stdout = thinwedge_app_server
            .stdout
            .take()
            .context("thinwedge app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: thinwedge_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `thinwedge-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "thinwedge-toy-app-server".to_string(),
                    title: Some("ThinWedge Toy App Server".to_string()),
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to thinwedge app-server")?;
                    return Ok(());
                }
                bail!("thinwedge app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from thinwedge app-server")?;
                if bytes == 0 {
                    bail!("thinwedge app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = thinwedge_core::otel_init::build_provider(
            &config,
...
impl Drop for ThinWedgeClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[thinwedge app-server exited: {status}]");
            return;
        }
