...
pub enum Command {
    /// Submit a new ThinWedge Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a ThinWedge Cloud task.
    Status(StatusCommand),
    /// List ThinWedge Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a ThinWedge Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a ThinWedge Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in ThinWedge Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `thinwedge cloud` to browse).
    ...
    /// Git branch to run in ThinWedge Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// ThinWedge Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// ThinWedge Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// ThinWedge Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
