...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = thinwedge_login::default_client::USER_AGENT_SUFFIX.lock() {
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    set_user_agent_suffix("thinwedge_cloud_tasks_tui");
    let ua = thinwedge_login::default_client::get_thinwedge_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("thinwedge-cli")),
    );
    if let Some(am) = load_auth_manager(/*chatgpt_base_url*/ None).await
        && let Some(auth) = am.auth().await
        && auth.uses_thinwedge_backend()
    {
        headers.extend(thinwedge_model_provider::auth_provider_from_auth(&auth).to_auth_headers());
    }
    headers
}
...
pub fn task_url(base_url: &str, task_id: &str) -> String {
    ...
    if let Some(root) = normalized.strip_suffix("/backend-api") {
        return format!("{root}/thinwedge/tasks/{task_id}");
    }
    if let Some(root) = normalized.strip_suffix("/api/thinwedge") {
        return format!("{root}/thinwedge/tasks/{task_id}");
    }
    if normalized.ends_with("/thinwedge") {
        return format!("{normalized}/tasks/{task_id}");
    }
    format!("{normalized}/thinwedge/tasks/{task_id}")
}
