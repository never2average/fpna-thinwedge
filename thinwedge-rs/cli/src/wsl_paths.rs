...
pub use thinwedge_utils_path::is_wsl;
...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\thinwedge.zip").as_deref(),
            Some("/mnt/c/Temp/thinwedge.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/thinwedge.tgz").as_deref(),
            Some("/mnt/d/Work/thinwedge.tgz")
        );
        assert!(win_path_to_wsl("/home/user/thinwedge").is_none());
    }
