...
#[cfg(target_os = "macos")]
use thinwedge_core::spawn::THINWEDGE_SANDBOX_ENV_VAR;
...
async fn run_command_under_sandbox(
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Landlock => {
            ...
            let thinwedge_linux_sandbox_exe = config
                .thinwedge_linux_sandbox_exe
...
async fn load_debug_sandbox_config_with_thinwedge_home(
    ...
    let config = build_debug_sandbox_config(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            thinwedge_linux_sandbox_exe: thinwedge_linux_sandbox_exe.clone(),
            ..Default::default()
        },
...
async fn build_debug_sandbox_config(
    ...
    if let Some(thinwedge_home) = thinwedge_home {
        builder = builder
            ...
            .fallback_cwd(Some(thinwedge_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        write_permissions_profile_config(&thinwedge_home, &docs, &private)?;
