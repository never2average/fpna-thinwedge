//! Implements the `thinwedge doctor` diagnostic report.
//!
...
use thinwedge_api::ResponsesWebsocketClient;
...
use thinwedge_install_context::InstallMethod;
use thinwedge_install_context::StandalonePlatform;
use thinwedge_install_context::ThinWedgePackageLayout;
...
/// Options for building a local ThinWedge diagnostic report.
///
...
///
/// This is the CLI entry point for thinwedge doctor. It does not repair issues;
...
async fn build_report(
    ...
    match &config_result {
        Ok(config) => {
            let auth_manager =
                AuthManager::shared_from_config(config, /*enable_thinwedge_api_key_env*/ true)
                    .await;
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun thinwedge doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
    ...
    DoctorReport {
        ...
        overall_status,
        thinwedge_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load ThinWedge config")
}
...
/// JSON support report emitted by `thinwedge doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        thinwedge_version: report.thinwedge_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("THINWEDGE_MANAGED_BY_BUN").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "THINWEDGE_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = thinwedge_path_entries();
    let mut status = CheckStatus::Ok;
    let mut summary = "installation looks consistent".to_string();
    let mut remediation = None;

    if path_entries.len() > 1 {
        details.push(format!("PATH thinwedge entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH thinwedge #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary =
                    "npm install -g @openai/thinwedge would update a different install".to_string();
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update ThinWedge so the JS shim provides THINWEDGE_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("THINWEDGE_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("THINWEDGE_MANAGED_BY_NPM").is_none()
        && env::var_os("THINWEDGE_MANAGED_BY_BUN").is_none()
    {
        return false;
    }
...
fn describe_method_with_package_layout(
    method: &str,
    package_layout: Option<&ThinWedgePackageLayout>,
) -> String {
...
fn npm_global_root_check() -> NpmRootCheck {
    let Some(running_package_root) =
        env::var_os("THINWEDGE_MANAGED_PACKAGE_ROOT").map(PathBuf::from)
    else {
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("@openai").join("thinwedge");
...
fn thinwedge_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["thinwedge"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "thinwedge"]);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.thinwedge_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check = check
                    .remediation("Run thinwedge login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no ThinWedge credentials were found",
        )
        .details(details)
        .remediation("Run thinwedge login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run thinwedge login again."),
    }
}
...
fn stored_auth_mode(auth: &thinwedge_login::AuthDotJson) -> &'static str {
    match stored_auth_mode_value(auth) {
        thinwedge_app_server_protocol::AuthMode::ApiKey => "api_key",
        thinwedge_app_server_protocol::AuthMode::Chatgpt => "chatgpt",
        thinwedge_app_server_protocol::AuthMode::ChatgptAuthTokens => "chatgpt_auth_tokens",
        thinwedge_app_server_protocol::AuthMode::AgentIdentity => "agent_identity",
        thinwedge_app_server_protocol::AuthMode::PersonalAccessToken => "personal_access_token",
        thinwedge_app_server_protocol::AuthMode::BedrockApiKey => "bedrock_api_key",
    }
}

fn stored_auth_mode_value(auth: &AuthDotJson) -> thinwedge_app_server_protocol::AuthMode {
    if let Some(mode) = auth.auth_mode {
        return mode;
    }
    if auth.personal_access_token.is_some() {
        thinwedge_app_server_protocol::AuthMode::PersonalAccessToken
    } else if auth.bedrock_api_key.is_some() {
        thinwedge_app_server_protocol::AuthMode::BedrockApiKey
    } else if auth.openai_api_key.is_some() {
        thinwedge_app_server_protocol::AuthMode::ApiKey
    } else {
        thinwedge_app_server_protocol::AuthMode::Chatgpt
    }
}
    ...
    match stored_auth_mode_value(auth) {
        thinwedge_app_server_protocol::AuthMode::ApiKey => {
            ...
            let env_key_present = env_var_present(OPENAI_API_KEY_ENV_VAR)
                || env_var_present(THINWEDGE_API_KEY_ENV_VAR);
        ...
        thinwedge_app_server_protocol::AuthMode::ChatgptAuthTokens => {
            match auth.tokens.as_ref() {
                Some(tokens) => {
                    if tokens.access_token.trim().is_empty() {
        ...
        thinwedge_app_server_protocol::AuthMode::AgentIdentity => {
            if auth
                .agent_identity
                .as_deref()
...
fn network_check() -> DoctorCheck {
    ...
    for name in ["THINWEDGE_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
fn sandbox_check(config: &Config, arg0_paths: &Arg0DispatchPaths) -> DoctorCheck {
    ...
    push_path_detail(
        &mut details,
        "thinwedge-linux-sandbox helper",
        arg0_paths.thinwedge_linux_sandbox_exe.as_deref(),
    );
    push_path_detail(
        &mut details,
    ...
    if let Some(helper) = arg0_paths.thinwedge_linux_sandbox_exe.as_deref()
        && !helper.exists()
    {
        status = CheckStatus::Warning;
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "THINWEDGE_HOME", &config.thinwedge_home);
    ...
    for db in thinwedge_state::runtime_db_paths(&config.sqlite_home) {
    ...
    rollout_stats_details(&mut details, &config.thinwedge_home);
...
async fn sqlite_integrity_detail(
    ...
    match thinwedge_state::sqlite_integrity_check(path).await {
...
fn rollout_stats_details(details: &mut Vec<String>, thinwedge_home: &Path) {
    let active = collect_rollout_stats(&thinwedge_home.join("sessions"));
    let archived = collect_rollout_stats(&thinwedge_home.join("archived_sessions"));
...
fn auth_mode_name(auth: &ThinWedgeAuth) -> &'static str {
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match thinwedge_home {
        Ok(path) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Ok,
            "THINWEDGE_HOME was resolved without config",
        )
        .detail(format!("THINWEDGE_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "THINWEDGE_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(THINWEDGE_API_KEY_ENV_VAR) {
        return ProviderAuthReachabilityMode::ApiKey;
    }
    if env_var_present(THINWEDGE_ACCESS_TOKEN_ENV_VAR) {
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/@openai/thinwedge");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("@openai").join("thinwedge")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/@openai/thinwedge");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("@openai").join("thinwedge"),
            }
        );
    }
    ...
    fn startup_warning_counts_group_known_sources() {
        let warnings = vec![
            ...
            "[features].thinwedge_hooks is deprecated. Use [features].hooks instead.".to_string(),
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            thinwedge_self_exe: Some(PathBuf::from("/bin/thinwedge")),
            thinwedge_linux_sandbox_exe: Some(PathBuf::from("/bin/thinwedge-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/thinwedge-execve-wrapper")),
        };
        ...
        assert_eq!(overrides.thinwedge_self_exe, arg0_paths.thinwedge_self_exe);
        assert_eq!(
            overrides.thinwedge_linux_sandbox_exe,
            arg0_paths.thinwedge_linux_sandbox_exe
        );
        assert_eq!(
    ...
    fn redacted_json_report_structures_and_sanitizes_details() {
        let report = DoctorReport {
            ...
            thinwedge_version: "0.0.0".to_string(),
            checks: vec![
                DoctorCheck::new(
                    "system.environment",
                    "system",
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "THINWEDGE_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("THINWEDGE_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running ThinWedge."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running ThinWedge.".to_string())
        );
    }
    ...
    fn stored_auth_validation_rejects_missing_api_key() {
        let auth = AuthDotJson {
            auth_mode: Some(thinwedge_app_server_protocol::AuthMode::ApiKey),
            openai_api_key: None,
            tokens: None,
            last_refresh: None,
            agent_identity: None,
    ...
    fn stored_auth_validation_handles_personal_access_token() {
        ...
        auth.auth_mode = Some(thinwedge_app_server_protocol::AuthMode::PersonalAccessToken);
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-thinwedge-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-thinwedge-doctor-mcp\" is not resolvable",
            )
        }));
    }
