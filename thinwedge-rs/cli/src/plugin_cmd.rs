...
use thinwedge_core_plugins::ConfiguredMarketplace;
use thinwedge_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use thinwedge_core_plugins::PluginInstallOutcome;
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "thinwedge plugin add",
    after_help = "Examples:\n  thinwedge plugin add sample@debug\n  thinwedge plugin add sample --marketplace debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "thinwedge plugin list",
    after_help = "Examples:\n  thinwedge plugin list\n  thinwedge plugin list --marketplace debug\n  thinwedge plugin list --json\n  thinwedge plugin list --available --json"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "thinwedge plugin remove",
    after_help = "Examples:\n  thinwedge plugin remove sample@debug\n  thinwedge plugin remove sample --marketplace debug"
)]
        ...
        for (index, marketplace) in marketplaces.into_iter().enumerate() {
            ...
            for plugin in &marketplace.plugins {
                ...
                let path = match &plugin.source {
                    thinwedge_core_plugins::marketplace::MarketplacePluginSource::Local {
                        path,
                    } => path.as_path().display().to_string(),
                    thinwedge_core_plugins::marketplace::MarketplacePluginSource::Git {
                        url,
                        path,
                        ref_name,
                        sha,
...
impl JsonPluginListOutput {
    fn from_marketplaces(
        marketplaces: Vec<thinwedge_core_plugins::ConfiguredMarketplace>,
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, thinwedge_home, &root) {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
                    marketplace_name: configured_name.clone(),
