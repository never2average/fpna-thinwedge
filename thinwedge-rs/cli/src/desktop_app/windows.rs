...
const THINWEDGE_WINDOWS_INSTALLER_URL: &str =
    "https://get.microsoft.com/installer/download/9PLM9XGG6VKS?cid=website_cta_psi";
const THINWEDGE_MICROSOFT_STORE_WEB_URL: &str = "https://apps.microsoft.com/detail/9plm9xgg6vks";
...
pub async fn run_windows_app_open_or_install(
    ...
    eprintln!("ThinWedge Desktop not found; opening Windows installer...");
    let download_url = download_url_override
        .as_deref()
        .unwrap_or(THINWEDGE_WINDOWS_INSTALLER_URL);
...
async fn thinwedge_app_is_installed() -> anyhow::Result<bool> {
    let output = Command::new("powershell.exe")
        .arg("-NoProfile")
        .arg("-Command")
        .arg("Get-StartApps -Name 'ThinWedge' | Select-Object -First 1 -ExpandProperty AppID")
        .output()
        .await
...
mod tests {
    ...
    fn display_workspace_path_preserves_unc_prefix() {
        assert_eq!(
            display_workspace_path(Path::new(r"\\?\UNC\server\share\thinwedge")),
            r"\\server\share\thinwedge"
        );
    }
