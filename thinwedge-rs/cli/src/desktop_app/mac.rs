...
pub async fn run_mac_app_open_or_install(
    ...
    if let Some(app_path) = find_existing_thinwedge_app_path() {
        eprintln!(
            "Opening ThinWedge Desktop at {app_path}...",
            app_path = app_path.display()
        );
        open_thinwedge_app(&app_path, &workspace).await?;
        return Ok(());
    }
    ...
    let download_url = download_url_override.unwrap_or_else(|| {
        let default_url = if is_apple_silicon_mac() {
            THINWEDGE_DMG_URL_ARM64
        } else {
            THINWEDGE_DMG_URL_X64
        };
        default_url.to_string()
    });
    let installed_app = download_and_install_thinwedge_to_user_applications(&download_url)
        .await
    ...
    open_thinwedge_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_thinwedge_app_path() -> Option<PathBuf> {
    candidate_thinwedge_app_paths()
        .into_iter()
...
async fn open_thinwedge_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    eprintln!(
...
async fn download_and_install_thinwedge_to_user_applications(
    ...
    let dmg_path = tmp_root.join("ThinWedge.dmg");
...
async fn install_thinwedge_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        ...
        let dest_app = applications_dir.join("ThinWedge.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
...
fn find_thinwedge_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("ThinWedge.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
mod tests {
    ...
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tThinWedge\t/Volumes/ThinWedge\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/ThinWedge")
        );
    }

    #[test]
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tThinWedge Installer\t/Volumes/ThinWedge Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
