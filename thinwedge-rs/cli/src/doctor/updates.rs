//! Diagnoses whether ThinWedge update paths target the running installation.
//!
...
const GITHUB_LATEST_RELEASE_URL: &str =
    "https://api.github.com/repos/openai/thinwedge/releases/latest";
const HOMEBREW_CASK_API_URL: &str = "https://formulae.brew.sh/api/cask/thinwedge.json";
...
pub(super) fn updates_check(config: &Config) -> DoctorCheck {
    ...
    let version_file = config.thinwedge_home.join(VERSION_FILE_NAME);
...
fn update_action_label(context: &InstallContext) -> &'static str {
    match &context.method {
        InstallMethod::Npm => "npm install -g @openai/thinwedge",
        InstallMethod::Bun => "bun install -g @openai/thinwedge",
        InstallMethod::Brew => "brew upgrade --cask thinwedge",
...
mod tests {
    ...
    fn update_action_labels_install_contexts() {
        assert_eq!(
            ...
            "npm install -g @openai/thinwedge"
        );
        assert_eq!(
            update_action_label(&InstallContext {
