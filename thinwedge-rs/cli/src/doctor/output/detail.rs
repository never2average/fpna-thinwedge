...
fn install_details(parsed: &[ParsedDetail], options: HumanOutputOptions) -> Vec<HumanDetail> {
    ...
    let path_entries = numbered_values(parsed, "PATH thinwedge #");
...
fn state_details(parsed: &[ParsedDetail]) -> Vec<HumanDetail> {
    let mut out = Vec::new();
    push_row_if_present(&mut out, parsed, "THINWEDGE_HOME", "THINWEDGE_HOME");
...
fn display_label(label: &str) -> String {
    match label {
        "thinwedge-linux-sandbox helper" => "linux helper",
