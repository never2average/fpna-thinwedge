...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run thinwedge doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
fn header_suffix(report: &DoctorReport) -> String {
    let version = format!("v{}", report.thinwedge_version);
    report
        .checks
        .iter()
...
fn update_note(check: &DoctorCheck, report: &DoctorReport) -> Option<DoctorNote> {
    ...
    let mut parenthetical = format!("current {}", report.thinwedge_version);
...
mod tests {
    ...
    fn sample_report() -> DoctorReport {
        let checks = vec![
            ...
            .detail("terminal title project value: thinwedge"),
            DoctorCheck::new(
                "state.paths",
                "state",
                CheckStatus::Ok,
            ...
            .remediation("Run `thinwedge login`."),
            DoctorCheck::new(
                "updates.status",
                "updates",
                CheckStatus::Ok,
    ...
    fn render_human_report_includes_details_by_default_without_color() {
        ...
        let expected = format!(
            "\
ThinWedge Doctor v0.0.0

Notes
   ⚠ terminal     narrow terminal
   ✗ auth         token expired - Run `thinwedge login`.
─────────────────────────────────────────────────────────────
...
Environment
  ...
  ✓ git          git version 2.54.0
      ...
      repo detected            true
  ⚠ terminal     narrow terminal
  ✓ title        default · project thinwedge
      title source             default
      title items              activity, project-name
      project value            thinwedge
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `thinwedge login`.
-------------------------------------------------------------
...
Environment
  ...
  [!!] terminal     narrow terminal
  [ok] title        default | project thinwedge
...
",
    ...
    fn detail_value_colors_inline_statuses_and_low_signal_values() {
        let rendered = detail_value(
            "npm: no · commit unknown · integrity ok · ~/code/thinwedge/target/debug/thinwedge · <redacted>",
        ...
        assert!(rendered.contains("\u{1b}[38;5;117m~/code/thinwedge/target/debug/thinwedge"));
    ...
    fn redact_detail_preserves_env_var_names() {
        assert_eq!(
            redact_detail("auth env vars present: OPENAI_API_KEY, THINWEDGE_API_KEY"),
            "auth env vars present: OPENAI_API_KEY, THINWEDGE_API_KEY"
        );
    }
