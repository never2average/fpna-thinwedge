...
use thinwedge_state::ThreadStateAuditRow;
...
async fn thread_inventory_check_for_roots(
    ...
    let scan = scan_rollout_files(thinwedge_home).await;
    let state_db_path = thinwedge_state::state_db_path(sqlite_home);
    ...
    let rows = match thinwedge_state::read_thread_state_audit_rows(&state_db_path).await {
        Ok(rows) => rows,
        Err(err) => {
    ...
    parity_check_from_scan_and_rows(thinwedge_home, scan, rows, details)
}
...
fn missing_state_db_check(scan: RolloutScan, details: Vec<String>) -> DoctorCheck {
    ...
    if !scan.files.is_empty() {
        check = check
            .issue(
                ...
                .remedy("Start ThinWedge with no state DB present so startup backfill can create it from rollout files."),
        )
            .remediation(
                "Start ThinWedge with no state DB present so startup backfill can create it from rollout files.",
            );
    }
    if !scan.scan_errors.is_empty() || !scan.malformed_names.is_empty() || scan.reached_scan_cap {
        check = check.issue(
            ...
            .remedy("Check file permissions and unexpected files under THINWEDGE_HOME sessions."),
        );
    }
    check
}
            ...
            .filter_map(|row| {
                let expected_archived = rollout_by_key
                    ...
                    .or_else(|| {
                        row.rollout_path
                            .is_file()
                            .then(|| archived_from_rollout_path(thinwedge_home, &row.rollout_path))
                            .flatten()
                    })?;
...
async fn scan_rollout_files(thinwedge_home: &Path) -> RolloutScan {
    let mut scan = RolloutScan::default();
    scan_rollout_root(
        &thinwedge_home.join("sessions"),
        /*archived*/ false,
        &mut scan,
    )
    .await;
    scan_rollout_root(
        &thinwedge_home.join("archived_sessions"),
        /*archived*/ true,
        &mut scan,
    )
    .await;
...
async fn thread_id_from_rollout(path: &Path) -> RolloutThreadId {
    ...
    thinwedge_rollout::builder_from_items(items.as_slice(), path)
...
fn archived_from_rollout_path(thinwedge_home: &Path, path: &Path) -> Option<bool> {
    let key = path_key(path);
    if key.starts_with(path_key(&thinwedge_home.join("archived_sessions"))) {
        return Some(true);
    }
    if key.starts_with(path_key(&thinwedge_home.join("sessions"))) {
        return Some(false);
    }
    None
}
...
mod tests {
    ...
    async fn thread_inventory_check_warns_for_missing_stale_and_mismatched_rows() {
        ...
        assert!(check.issues.iter().all(|issue| {
            !issue
                .remedy
                .as_deref()
                .is_some_and(|remedy| remedy.starts_with("Restart ThinWedge"))
        }));
        assert!(
            check
                .details
    ...
    impl Fixture {
        async fn new() -> Self {
            let thinwedge_home = TempDir::new().expect("thinwedge home");
            let sqlite_home = TempDir::new().expect("sqlite home");
            let _runtime = thinwedge_state::StateRuntime::init(
        ...
        fn write_rollout(&self, archived: bool, timestamp: &str, thread_id: &str) -> PathBuf {
            let root = if archived {
                self.thinwedge_home.path().join("archived_sessions")
            } else {
                self.thinwedge_home.path().join("sessions/2025/01/02")
            };
            std::fs::create_dir_all(&root).expect("rollout dir");
            let path = root.join(format!("rollout-{timestamp}-{thread_id}.jsonl"));
            let rollout_line = RolloutLine {
                timestamp: timestamp.to_string(),
                item: RolloutItem::SessionMeta(thinwedge_protocol::protocol::SessionMetaLine {
                    meta: thinwedge_protocol::protocol::SessionMeta {
                        id: ThreadId::from_string(thread_id).expect("thread id"),
                        timestamp: timestamp.to_string(),
                        cwd: self.thinwedge_home.path().to_path_buf(),
        ...
        async fn insert_thread_row(&self, id: &str, rollout_path: &Path, archived: bool) {
            let state_db_path = thinwedge_state::state_db_path(self.sqlite_home.path());
...
INSERT INTO threads (
    ...
    archived_at
            ...
            .bind("cli")
            .bind("test-provider")
            .bind(self.thinwedge_home.path().display().to_string())
            .bind("test title")
            .bind("read-only")
            .bind("on-request")
