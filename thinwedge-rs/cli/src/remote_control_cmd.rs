...
use thinwedge_app_server_daemon::LifecycleOutput as AppServerLifecycleOutput;
use thinwedge_app_server_daemon::LifecycleStatus as AppServerLifecycleStatus;
use thinwedge_app_server_daemon::RemoteControlReadyOutput as AppServerRemoteControlReadyOutput;
use thinwedge_app_server_daemon::RemoteControlReadyStatus as AppServerRemoteControlReadyStatus;
use thinwedge_app_server_daemon::RemoteControlStartOutput as AppServerRemoteControlStartOutput;
...
pub(crate) async fn run(
    ...
    match command.subcommand {
        ...
        Some(RemoteControlSubcommand::Start) => {
            ...
            let output = thinwedge_app_server_daemon::ensure_remote_control_ready().await?;
        ...
        Some(RemoteControlSubcommand::Stop) => {
            ...
            let output = thinwedge_app_server_daemon::run(AppServerLifecycleCommand::Stop).await?;
...
async fn run_foreground_remote_control(
    ...
    let mut app_server_task = tokio::spawn(thinwedge_app_server::run_main_with_transport_options(
        arg0_paths,
...
async fn wait_for_foreground_remote_control_ready(
    ...
    thinwedge_app_server_daemon::enable_remote_control_on_socket(
        socket_path.as_path(),
...
fn daemon_app_server_human_lines(output: &AppServerRemoteControlStartOutput) -> Vec<String> {
    let (managed_thinwedge_path, managed_thinwedge_version) = daemon_app_server_identity(output);
    vec![
        "Daemon used app-server:".to_string(),
        format!("  path: {}", managed_thinwedge_path.display()),
        format!(
            "  version: {}",
            managed_thinwedge_version.unwrap_or("unknown")
        ),
    ]
}
...
fn daemon_app_server_identity(
    ...
    match output {
        AppServerRemoteControlStartOutput::Bootstrap(output) => (
            &output.managed_thinwedge_path,
            output.managed_thinwedge_version.as_deref(),
        ),
...
mod tests {
    ...
    fn daemon_ready_output(
        ...
        AppServerRemoteControlReadyOutput {
            daemon: AppServerRemoteControlStartOutput::Start(AppServerLifecycleOutput {
                ...
                backend: None,
                pid: Some(42),
                managed_thinwedge_path: PathBuf::from("/opt/thinwedge/bin/thinwedge"),
                managed_thinwedge_version: Some("1.0.0".to_string()),
    ...
    fn daemon_app_server_human_lines_include_path_and_version() {
        assert_eq!(
            ...
            vec![
                "Daemon used app-server:".to_string(),
                "  path: /opt/thinwedge/bin/thinwedge".to_string(),
        ...
        assert_eq!(
            ...
            json!({
                ...
                "daemon": {
                    "status": "started",
                    "pid": 42,
                    "managedThinwedgePath": "/opt/thinwedge/bin/thinwedge",
                    "managedThinwedgeVersion": "1.0.0",
