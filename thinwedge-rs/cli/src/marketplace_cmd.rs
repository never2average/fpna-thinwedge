...
use thinwedge_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use thinwedge_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use thinwedge_core_plugins::marketplace::marketplace_root_dir;
use thinwedge_core_plugins::marketplace_add::MarketplaceAddOutcome;
use thinwedge_core_plugins::marketplace_add::MarketplaceAddRequest;
use thinwedge_core_plugins::marketplace_add::add_marketplace;
use thinwedge_core_plugins::marketplace_remove::MarketplaceRemoveOutcome;
use thinwedge_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
enum MarketplaceSubcommand {
    ...
    /// List plugin marketplaces ThinWedge is currently considering and their roots.
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "thinwedge plugin marketplace add",
    after_help = "Examples:\n  thinwedge plugin marketplace add ./path/to/marketplace\n  thinwedge plugin marketplace add owner/repo --ref main\n  thinwedge plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(bin_name = "thinwedge plugin marketplace list")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "thinwedge plugin marketplace upgrade",
    after_help = "Examples:\n  thinwedge plugin marketplace upgrade\n  thinwedge plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "thinwedge plugin marketplace remove",
    after_help = "Example:\n  thinwedge plugin marketplace remove debug"
)]
...
async fn run_list(overrides: Vec<(String, toml::Value)>, args: ListMarketplaceArgs) -> Result<()> {
    ...
    let manager = PluginsManager::new(config.thinwedge_home.to_path_buf());
    ...
    if args.json {
        let marketplace_sources =
            configured_marketplace_sources_by_root(config.thinwedge_home.as_path(), &plugins_input);
        let output =
...
impl JsonMarketplaceListOutput {
    fn from_marketplaces(
        marketplaces: Vec<thinwedge_core_plugins::marketplace::Marketplace>,
...
fn configured_marketplace_sources_by_root(
    ...
    let default_install_root = marketplace_install_root(thinwedge_home);
    marketplaces
        .iter()
...
async fn run_upgrade(
    ...
    let config = Config::load_with_cli_overrides(overrides)
        ...
        .context("failed to load configuration")?;
    let thinwedge_home = find_thinwedge_home().context("failed to resolve THINWEDGE_HOME")?;
    let manager = PluginsManager::new(thinwedge_home.to_path_buf());
    let plugins_input = config.plugins_config_input();
    let outcome = manager
