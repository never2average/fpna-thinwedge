...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = thinwedge_command(thinwedge_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(thinwedge_home.path()).await?;
    ...
    let mut list_json_cmd = thinwedge_command(thinwedge_home.path())?;
    ...
    let mut get_cmd = thinwedge_command(thinwedge_home.path())?;
    ...
    assert!(stdout.contains("remove: thinwedge mcp remove docs"));

    let mut get_json_cmd = thinwedge_command(thinwedge_home.path())?;
    get_json_cmd
