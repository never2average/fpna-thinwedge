...
fn thinwedge_command(thinwedge_home: &Path) -> Result<assert_cmd::Command> {
    ...
    cmd.env("HOME", thinwedge_home);
    Ok(cmd)
}

fn thinwedge_command_in(thinwedge_home: &Path, current_dir: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = thinwedge_command(thinwedge_home)?;
...
fn write_plugins_enabled_config(thinwedge_home: &Path) -> Result<()> {
    std::fs::write(
...
fn setup_local_marketplace_with_implicit_system_roots() -> Result<(TempDir, TempDir, TempDir)> {
    let (thinwedge_home, source) = setup_local_marketplace()?;

    let bundled_root = thinwedge_home
        .path()
        .join(".tmp")
    ...
    let runtime_root = cache_home
        .path()
        .join("thinwedge-runtimes")
        .join("thinwedge-primary-runtime")
        .join("plugins")
    ...
    Ok((thinwedge_home, source, cache_home))
}
...
fn setup_custom_marketplace_under_implicit_system_root() -> Result<(TempDir, std::path::PathBuf)> {
    ...
    let custom_root = thinwedge_home
        .path()
        .join(".tmp")
        .join("bundled-marketplaces")
    ...
    Ok((thinwedge_home, custom_root))
}

fn remove_installed_plugin_config(thinwedge_home: &Path, plugin_key: &str) -> Result<()> {
...
async fn marketplace_list_json_includes_configured_git_marketplace_source() -> Result<()> {
    ...
    record_user_marketplace(thinwedge_home.path(), "debug", &update)?;
...
async fn marketplace_list_includes_root_when_plugins_are_filtered_out() -> Result<()> {
    let (thinwedge_home, source) = setup_local_marketplace_with_explicit_empty_products()?;
    let expected_row = marketplace_list_row("debug", source.path());
...
async fn marketplace_list_fails_when_configured_marketplace_snapshot_is_missing() -> Result<()> {
    let (thinwedge_home, source) = setup_configured_marketplace_without_manifest()?;
...
async fn marketplace_list_fails_when_configured_marketplace_name_is_invalid() -> Result<()> {
    let thinwedge_home = setup_configured_local_marketplace_with_invalid_name()?;
...
{
    let thinwedge_home = setup_configured_local_marketplace_with_missing_source()?;
...
async fn marketplace_list_fails_when_configured_marketplace_snapshot_is_malformed() -> Result<()> {
    let (thinwedge_home, source) = setup_configured_marketplace_with_malformed_manifest()?;
...
async fn plugin_list_available_requires_json() -> Result<()> {
    let (thinwedge_home, _source) = setup_local_marketplace()?;
...
async fn plugin_list_excludes_unconfigured_repo_local_marketplaces() -> Result<()> {
    let (thinwedge_home, source) = setup_unconfigured_local_marketplace()?;

    thinwedge_command_in(thinwedge_home.path(), source.path())?
...
async fn plugin_list_ignores_implicit_system_marketplace_roots_without_manifests() -> Result<()> {
    let (thinwedge_home, source, cache_home) =
...
async fn plugin_list_fails_for_custom_marketplace_under_system_root() -> Result<()> {
    let (thinwedge_home, custom_root) = setup_custom_marketplace_under_implicit_system_root()?;
...
async fn plugin_list_hides_version_for_cached_but_unconfigured_plugin() -> Result<()> {
    ...
    remove_installed_plugin_config(thinwedge_home.path(), "sample@debug")?;
...
async fn plugin_add_and_remove_updates_installed_plugin_config() -> Result<()> {
    ...
    let config = std::fs::read_to_string(thinwedge_home.path().join(CONFIG_TOML_FILE))?;
...
async fn plugin_add_reinstalls_from_configured_marketplace_snapshot() -> Result<()> {
    ...
    assert!(
        thinwedge_home
            .path()
            .join("plugins/cache/debug/sample/1.2.3/.thinwedge-plugin/plugin.json")
            .is_file()
    );
