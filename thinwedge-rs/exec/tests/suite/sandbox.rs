...
async fn spawn_command_under_sandbox(
    ...
    use thinwedge_core::spawn_command_under_linux_sandbox;
    use thinwedge_protocol::models::PermissionProfile;

    let thinwedge_linux_sandbox_exe = core_test_support::find_thinwedge_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_thinwedge_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .thinwedge && echo 'sandbox_mode = \"danger-full-access\"' > .thinwedge/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &policy,
    ...
    let dot_thinwedge_metadata = tokio::fs::symlink_metadata(&dot_thinwedge).await;
    if let Ok(metadata) = dot_thinwedge_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_thinwedge.display()
        );
    } else if let Err(err) = &dot_thinwedge_metadata {
        assert_eq!(
            err.kind(),
