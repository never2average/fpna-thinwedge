...
#[command(
    version,
    override_usage = "thinwedge exec [OPTIONS] [PROMPT]\n       thinwedge exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running ThinWedge outside a Git repository.
    ...
    /// Do not load `$THINWEDGE_HOME/config.toml`; auth still uses `THINWEDGE_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "thinwedge resume --last <prompt>" treats the positional as a prompt, not a session id.
