...
use thinwedge_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use thinwedge_core::check_execpolicy_for_warnings;
use thinwedge_core::config::Config;
use thinwedge_core::config::ConfigBuilder;
use thinwedge_core::config::ConfigOverrides;
use thinwedge_core::config::ConfigTomlLoadResult;
use thinwedge_core::config::find_thinwedge_home;
use thinwedge_core::config::load_config_toml_with_layer_stack;
use thinwedge_core::config::resolve_bootstrap_auth_keyring_backend_kind;
use thinwedge_core::config::resolve_oss_provider;
use thinwedge_core::config::resolve_profile_v2_config_path;
use thinwedge_core::find_thread_meta_by_name_str;
use thinwedge_core::format_exec_policy_error_with_source;
...
use thinwedge_otel::set_parent_from_context;
use thinwedge_otel::traceparent_context_from_env;
...
enum StdinPromptBehavior {
    ...
    /// `thinwedge exec` behavior for `thinwedge exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `thinwedge exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&thinwedge_home, profile_v2));
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        thinwedge_core::otel_init::build_provider(
            &config,
    ...
    thinwedge_core::otel_init::record_process_start(otel.as_ref(), "thinwedge_exec");
    thinwedge_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "thinwedge_exec");
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        feedback: ThinWedgeFeedback::new(),
        log_db: None,
        ...
        client_name: "thinwedge_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = thinwedge_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what ThinWedge
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            thinwedge_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("ThinWedge initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        thinwedge_app_server_protocol::TurnStatus::Failed
                            | thinwedge_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<thinwedge_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<thinwedge_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
