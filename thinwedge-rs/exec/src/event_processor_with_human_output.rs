    ...
    fn render_item_completed(&mut self, item: ThreadItem) {
        match item {
            ThreadItem::AgentMessage { text, .. } => {
                eprintln!(
                    "{}\n{}",
                    "thinwedge".style(self.italic).style(self.magenta),
                    text
                );
...
impl EventProcessor for EventProcessorWithHumanOutput {
    fn print_config_summary(
        ...
        eprintln!("ThinWedge ThinWedge v{VERSION} (research preview)\n--------");
        ...
        match notification {
            ...
            ServerNotification::TurnPlanUpdated(notification) => {
                ...
                for step in notification.plan {
                    match step.status {
                        thinwedge_app_server_protocol::TurnPlanStepStatus::Completed => {
                            eprintln!("  {} {}", "✓".style(self.green), step.step);
                        }
                        thinwedge_app_server_protocol::TurnPlanStepStatus::InProgress => {
                            eprintln!("  {} {}", "→".style(self.cyan), step.step);
                        }
                        thinwedge_app_server_protocol::TurnPlanStepStatus::Pending => {
                            eprintln!(
                                "  {} {}",
                                "•".style(self.dimmed),
            ...
            ServerNotification::TurnStarted(_) => ThinWedgeStatus::Running,
