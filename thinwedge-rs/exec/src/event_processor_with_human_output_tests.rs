...
fn distinct_missing_paths_do_not_match_after_canonicalization() {
    assert!(!paths_match_after_canonicalization(
        test_path_buf("/tmp/thinwedge-missing-left").as_path(),
        test_path_buf("/tmp/thinwedge-missing-right").as_path(),
    ));
}
...
fn turn_completed_recovers_final_message_from_turn_items() {
    ...
    assert_eq!(
        status,
        crate::event_processor::ThinWedgeStatus::InitiateShutdown
    );
