...
use thinwedge_aws_auth::AwsAuthError;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_thinwedge_error)?;
...
fn aws_auth_error_to_thinwedge_error(error: AwsAuthError) -> ThinWedgeErr {
    ThinWedgeErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer ThinWedge agent requests fail even though raw Responses requests work.
