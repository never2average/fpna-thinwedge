...
These are entities exit on the thinwedge backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `ThinWedge` core system.

0. `Model`
   - In our case, this is the Responses REST API
1. `ThinWedge`
   - The core engine of thinwedge
...
2. `Session`
   - The `ThinWedge`'s current configuration and state
   - `ThinWedge` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `ThinWedge` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `ThinWedge` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `ThinWedge`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `ThinWedge`, as `ThinWedge` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `ThinWedge` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `ThinWedge`)
...
- `Event`
  - These are messages sent on the `EQ` (`ThinWedge` -> UI)
...
The `response_id` returned from each turn matches the ThinWedge `response_id` stored in the API's `/responses` endpoint. It can be stored and used in future `Sessions` to resume threads of work.
...
sequenceDiagram
    ...
    box Daemon
    participant thinwedge as ThinWedge
    ...
    user->>thinwedge: Op::ConfigureSession
    thinwedge-->>session: create session
    thinwedge->>user: Event::SessionConfigured
