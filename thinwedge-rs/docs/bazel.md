# Bazel in thinwedge-rs

This repository uses Bazel to build the Rust workspace under `thinwedge-rs`.
...
- `rules_rs` imports third-party crates from `thinwedge-rs/Cargo.toml` and
  `thinwedge-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `thinwedge_rust_crate`, which wraps `rust_library`,
...
- Each crate in `thinwedge-rs/*/BUILD.bazel` typically uses `thinwedge_rust_crate` and
...
ThinWedge uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
...
| --- | --- | --- |
| Push to `main` in `openai/thinwedge` | Yes | Yes |
| `workflow_dispatch` in `openai/thinwedge` | Yes | Yes |
| Same-repository pull request in `openai/thinwedge` | Yes | Yes |
| Fork pull request into `openai/thinwedge` | No | No; local |
...
```bash
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/thinwedge \
  ./.github/scripts/run_bazel_with_buildbuddy.py \
  build --config=ci-linux //thinwedge-rs/cli:thinwedge
```

The wrapper selects the OpenAI host only inside GitHub Actions for a trusted
run in `openai/thinwedge`. A missing or malformed pull request event
...
2. Create a `BUILD.bazel` that calls `thinwedge_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `thinwedge_rust_crate` to configure it.
