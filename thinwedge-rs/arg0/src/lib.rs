...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "thinwedge-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";
const TOKIO_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current ThinWedge executable for child re-execs.
    ///
    ...
    /// instead of the real ThinWedge CLI.
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == THINWEDGE_LINUX_SANDBOX_ARG0 {
        // Safety: [`run_main`] never returns.
        thinwedge_linux_sandbox::run_main();
    } else if exe_name == APPLY_PATCH_ARG0 || exe_name == MISSPELLED_APPLY_PATCH_ARG0 {
        thinwedge_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    if argv1 == THINWEDGE_FS_HELPER_ARG1 {
        thinwedge_exec_server::run_fs_helper_main();
    }
    if argv1 == THINWEDGE_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                let cwd = match thinwedge_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                match runtime.block_on(thinwedge_apply_patch::apply_patch(
                    &patch_arg,
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    thinwedge_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
    ...
    let (path_entry_guard, updated_path_env_var) = prepare_path_env_var_with_aliases(
        ...
        std::env::var_os("PATH"),
        prepare_path_entry_for_thinwedge_aliases,
    );
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that ThinWedge will proceed successfully even if
...
/// While we want to deploy the ThinWedge CLI as a single executable for simplicity,
...
/// `thinwedge-linux-sandbox` we *directly* execute
/// [`thinwedge_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.thinwedge/.env` before creating any threads.
...
///     `thinwedge-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        thinwedge_self_exe: current_exe.clone(),
        thinwedge_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `thinwedge-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().thinwedge_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "THINWEDGE_";

/// Load env vars from ~/.thinwedge/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `THINWEDGE_`.
fn load_dotenv() {
    if let Ok(thinwedge_home) = find_thinwedge_home()
        && let Ok(iter) = dotenvy::from_path_iter(thinwedge_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `THINWEDGE_` keys.
...
///   with the hidden `--thinwedge-run-as-apply-patch` flag.
///
...
/// user to install a separate executable, simplifying the deployment of ThinWedge
/// CLI.
...
fn prepare_path_entry_for_thinwedge_aliases(
    existing_path: Option<OsString>,
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if thinwedge_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (thinwedge_home: {thinwedge_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a THINWEDGE_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = thinwedge_home.join("tmp").join("arg0");
    ...
    for filename in &[
        ...
        #[cfg(windows)]
        {
            ...
            std::fs::write(
                ...
                format!(
                    r#"@echo off
"{exe}" {THINWEDGE_CORE_APPLY_PATCH_ARG1} %*
"#,
                ),
            )?;
        }
    ...
    let paths = Arg0DispatchPaths {
        thinwedge_self_exe: std::env::current_exe().ok(),
        thinwedge_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(THINWEDGE_LINUX_SANDBOX_ARG0))
            }
...
mod tests {
    ...
    fn package_path_test_fixture() -> anyhow::Result<PackagePathTestFixture> {
        ...
        let path_dir = package_dir.join("thinwedge-path");
        ...
        let install_context = InstallContext {
            ...
            package_layout: Some(ThinWedgePackageLayout {
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_thinwedge_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("thinwedge-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                thinwedge_self_exe: Some(PathBuf::from("/usr/bin/thinwedge")),
                thinwedge_linux_sandbox_exe: Some(alias_path.clone()),
                main_execve_wrapper_exe: None,
            },
        );

        assert_eq!(
            linux_sandbox_exe_path(Some(&path_entry), Some(PathBuf::from("/usr/bin/thinwedge"))),
            Some(alias_path),
        );
        Ok(())
    }
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("thinwedge-helper-alias");
        ...
        super::build_runtime()?.block_on(run_main_with_arg0_guard(
            ...
            Some(PathBuf::from("/usr/bin/thinwedge")),
            |paths| async move {
