...
const PACKAGE_METADATA_FILENAME: &str = "thinwedge-package.json";
const PATH_DIRNAME: &str = "thinwedge-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "thinwedge-resources";
...
pub struct InstallContext {
    ...
    pub package_layout: Option<ThinWedgePackageLayout>,
}
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.thinwedge/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `thinwedge-resources/`, and `thinwedge-path/`.
    ...
    /// A ThinWedge binary launched through the npm-managed `thinwedge.js` shim.
    Npm,
    /// A ThinWedge binary launched through the bun-managed `thinwedge.js` shim.
    Bun,
    /// A ThinWedge binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled ThinWedge binaries, custom
    /// internal launchers, and tests that execute ThinWedge from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let thinwedge_home = thinwedge_utils_home_dir::find_thinwedge_home().ok();
        Self::from_exe_with_thinwedge_home(
            is_macos,
            current_exe,
            managed_by_npm,
            managed_by_bun,
    ...
    fn from_exe_with_thinwedge_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        managed_by_npm: bool,
        managed_by_bun: bool,
        ...
        let package_layout = current_exe.and_then(ThinWedgePackageLayout::from_exe);
        let method = if managed_by_npm {
            InstallMethod::Npm
        } else if managed_by_bun {
            InstallMethod::Bun
        } else if let Some(exe_path) = current_exe {
            install_method_from_exe(exe_path, thinwedge_home, package_layout.as_ref(), is_macos)
        } else {
            InstallMethod::Other
        };
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let managed_by_npm = std::env::var_os("THINWEDGE_MANAGED_BY_NPM").is_some();
            let managed_by_bun = std::env::var_os("THINWEDGE_MANAGED_BY_BUN").is_some();
            Self::from_exe(
                cfg!(target_os = "macos"),
...
fn install_method_from_exe(
    ...
    if let Some(standalone_method) =
        standalone_install_method(exe_path, thinwedge_home, package_layout)
    {
...
fn standalone_install_method(
    ...
    let canonical_thinwedge_home = canonical_absolute_path(thinwedge_home?)?;
    ...
    let releases_root = canonical_thinwedge_home
        .join("packages")
...
mod tests {
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let context = InstallContext::from_exe_with_thinwedge_home(
            ...
            /*thinwedge_home*/ Some(thinwedge_home.path()),
        );
        assert_eq!(
            context,
            InstallContext {
    ...
    fn detects_package_layout_independently_from_install_method() -> std::io::Result<()> {
        ...
        let package_layout = ThinWedgePackageLayout {
    ...
    fn standalone_package_layout_keeps_standalone_install_method() -> std::io::Result<()> {
        let thinwedge_home = tempfile::tempdir()?;
        let package_dir = thinwedge_home
            .path()
            .join("packages/standalone/releases/1.2.3-x86_64-unknown-linux-musl");
    ...
    #[test]
    fn standalone_package_rg_falls_back_when_thinwedge_path_is_missing() -> std::io::Result<()> {
        let package_dir = tempfile::tempdir()?;
        let bin_dir = package_dir.path().join(BIN_DIRNAME);
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
    ...
    fn npm_and_bun_take_precedence() {
        let npm_context = InstallContext::from_exe_with_thinwedge_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/thinwedge")),
            /*managed_by_npm*/ true,
            /*managed_by_bun*/ false,
            /*thinwedge_home*/ None,
        );
        ...
        let bun_context = InstallContext::from_exe_with_thinwedge_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/thinwedge")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ true,
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_thinwedge_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/thinwedge")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ false,
            /*thinwedge_home*/ None,
        );
