...
use thinwedge_backend_client::ConfigBundleResponse;
use thinwedge_backend_client::DeliveredTomlFragment;
use thinwedge_config::AbsolutePathBuf;
use thinwedge_config::CloudConfigFragment;
use thinwedge_config::CloudConfigTomlBundle;
use thinwedge_config::CloudRequirementsFragment;
use thinwedge_config::CloudRequirementsTomlBundle;
...
fn create_test_cache(thinwedge_home: &Path) -> CloudConfigBundleCache {
...
async fn get_bundle_ignores_invalid_cache_and_refetches() {
    let thinwedge_home = tempdir().expect("tempdir");
    let cache = create_test_cache(thinwedge_home.path());
    cache
        .save(
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(
            thinwedge_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            thinwedge_backend_client::DeliveredRequirementsToml {
