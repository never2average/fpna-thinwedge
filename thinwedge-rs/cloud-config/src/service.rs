...
use thinwedge_config::CloudConfigBundle;
...
fn cloud_config_eligible_auth(auth: &ThinWedgeAuth) -> bool {
    ...
    C: BundleClient + 'static,
{
    pub(crate) fn new(
        ...
        let thinwedge_home = AbsolutePathBuf::resolve_path_against_base(thinwedge_home, "/");
        Self {
            auth_manager,
            client,
            cache: CloudConfigBundleCache::new(thinwedge_home.clone()),
            thinwedge_home,
            timeout,
        }
    }
    ...
    pub(crate) async fn load_startup_bundle_with_timeout(
        ...
        let _timer = thinwedge_otel::start_global_timer(
            "thinwedge.cloud_config_bundle.fetch.duration_ms",
            &[],
        );
    ...
    async fn load_valid_cached_bundle(
        ...
        match self.cache.load(chatgpt_user_id, account_id).await {
            Ok(signed_payload) => {
                if let Err(err) = validate_bundle(&signed_payload.bundle, &self.thinwedge_home) {
                    tracing::warn!(
    ...
    async fn validate_and_cache_remote_bundle(
        ...
        if let Err(err) = validate_bundle(&bundle, &self.thinwedge_home) {
            emit_fetch_final_metric(
                trigger,
                "error",
                "invalid_bundle",
