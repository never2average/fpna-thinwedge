...
pub(super) async fn update_thread_metadata(
    ...
    thinwedge_rollout::state_db::reconcile_rollout(
...
async fn refresh_resolved_rollout_path(resolved: &mut ResolvedRolloutPath) {
    if let Some(path) = thinwedge_rollout::existing_rollout_path(resolved.path.as_path()).await {
        resolved.path = path;
    }
}
...
fn normalize_cwd(cwd: PathBuf) -> PathBuf {
    thinwedge_utils_path::normalize_for_path_comparison(cwd.as_path()).unwrap_or(cwd)
}
...
async fn apply_thread_git_info_to_rollout(
    ...
    session_meta.git = Some(GitInfo {
        commit_hash: sha.as_deref().map(thinwedge_git_utils::GitSha::new),
        branch: branch.clone(),
...
async fn apply_thread_name(
    ...
    append_thread_name(store.config.thinwedge_home.as_path(), thread_id, &name)
        .await
        .map_err(|err| ThreadStoreError::Internal {
...
fn rollout_path_is_archived(store: &LocalThreadStore, path: &Path) -> bool {
    path.starts_with(store.config.thinwedge_home.join(ARCHIVED_SESSIONS_SUBDIR))
}
...
mod tests {
    ...
    async fn update_thread_metadata_sets_name_on_active_rollout_and_indexes_name() {
        ...
        let latest_name = thinwedge_rollout::find_thread_name_by_id(home.path(), &thread_id)
            .await
    ...
    async fn update_thread_metadata_sets_git_info() {
            ...
            .update_thread_metadata(UpdateThreadMetadataParams {
                ...
                patch: ThreadMetadataPatch {
                    git_info: Some(GitInfoPatch {
                        ...
                        origin_url: Some(Some("https://github.com/openai/thinwedge".to_string())),
                    }),
                    ..Default::default()
                },
                include_archived: false,
        ...
        assert_eq!(
            ...
            Some("https://github.com/openai/thinwedge")
        );
    }
    ...
    async fn observed_metadata_normalizes_cwd_for_list_filters() {
        ...
        let normalized_cwd =
            thinwedge_utils_path::normalize_for_path_comparison(workspace.as_path())
