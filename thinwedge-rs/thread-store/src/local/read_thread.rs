...
use thinwedge_rollout::find_archived_thread_path_by_id_str;
use thinwedge_rollout::find_thread_name_by_id;
use thinwedge_rollout::find_thread_path_by_id_str;
use thinwedge_rollout::read_session_meta_line;
use thinwedge_rollout::read_thread_item_from_rollout;
...
async fn sqlite_rollout_path_can_load_history_for_thread(
    ...
    if thinwedge_rollout::existing_rollout_path(path)
        .await
        .is_none()
    {
        return false;
...
async fn resolve_requested_rollout_path(
    ...
    let path = if rollout_path.is_relative() {
        store.config.thinwedge_home.join(rollout_path)
    } else {
        rollout_path
    };
    ...
    let Some(path) = thinwedge_rollout::existing_rollout_path(path.as_path()).await else {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
async fn resolve_rollout_path(
    ...
    if let Ok(path) = live_writer::rollout_path(store, thread_id).await
        && thinwedge_rollout::existing_rollout_path(path.as_path())
            .await
            .is_some()
        && (include_archived
            || !rollout_path_is_archived(store.config.thinwedge_home.as_path(), &path))
    {
        return Ok(Some(path));
    }
...
async fn read_thread_from_rollout_path(
    ...
    let archived = rollout_path_is_archived(store.config.thinwedge_home.as_path(), path.as_path());
    ...
    thread.rollout_path = Some(thinwedge_rollout::plain_rollout_path(path.as_path()));
    ...
    if let Ok(Some(title)) =
        find_thread_name_by_id(store.config.thinwedge_home.as_path(), &thread.thread_id).await
    {
...
async fn load_history_items(
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<thinwedge_protocol::protocol::RolloutItem>> {
...
async fn stored_thread_from_sqlite_metadata(
    ...
    let name = match distinct_thread_metadata_title(&metadata) {
        ...
        None => find_thread_name_by_id(store.config.thinwedge_home.as_path(), &metadata.id)
            .await
            .ok()
            .flatten()
    ...
    let rollout_path = thinwedge_rollout::plain_rollout_path(metadata.rollout_path.as_path());
...
fn stored_thread_from_meta_line(
    ...
    let rollout_path = thinwedge_rollout::plain_rollout_path(path.as_path());
    StoredThread {
...
mod tests {
    ...
    async fn read_thread_returns_rollout_path_summary() {
        ...
        let relative_path = active_path
            ...
            .expect("path should be under thinwedge home")
            .to_path_buf();
    ...
    async fn read_thread_uses_legacy_thread_name_when_sqlite_title_is_missing() {
        ...
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        thinwedge_rollout::append_thread_name(home.path(), thread_id, "Legacy title")
            .await
