...
use thinwedge_rollout::find_thread_names_by_ids;
use thinwedge_rollout::first_rollout_content_match_snippet;
use thinwedge_rollout::parse_cursor;
use thinwedge_rollout::search_rollout_matches;
...
struct ThreadSearchItem {
    item: thinwedge_rollout::ThreadItem,
    snippet: String,
}
...
pub(super) async fn search_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => thinwedge_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => thinwedge_rollout::ThreadSortKey::UpdatedAt,
    };
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => thinwedge_rollout::SortDirection::Asc,
        SortDirection::Desc => thinwedge_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        thinwedge_home: store.config.thinwedge_home.clone(),
        sqlite_home: store.config.sqlite_home.clone(),
        cwd: store.config.thinwedge_home.clone(),
    ...
    loop {
        ...
        for item in page.items {
            let logical_path = thinwedge_rollout::plain_rollout_path(item.path.as_path());
...
) -> Option<thinwedge_rollout::Cursor> {
...
async fn set_thread_search_result_names(
    ...
    if names.len() < thread_ids.len()
        && let Ok(legacy_names) =
            find_thread_names_by_ids(store.config.thinwedge_home.as_path(), &thread_ids).await
    {
