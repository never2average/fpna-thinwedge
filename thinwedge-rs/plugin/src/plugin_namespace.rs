...
pub const PLUGIN_MANIFEST_PATH: &str = ".thinwedge-plugin/plugin.json";
...
/// plugin manifest (same `name` rules as full manifest loading in thinwedge-core).
...
mod tests {
    ...
    fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".thinwedge-plugin")).expect("mkdir manifest");
        fs::write(
