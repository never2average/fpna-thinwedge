//! Registry of model providers supported by ThinWedge.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so ThinWedge works out-of-the-box.
...
impl ModelProviderInfo {
    ...
    fn build_header_map(&self) -> ThinWedgeResult<HeaderMap> {
    ...
    pub fn api_key(&self) -> ThinWedgeResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
                let api_key = std::env::var(env_key)
                    ...
                    .ok_or_else(|| {
                        ThinWedgeErr::EnvVar(EnvVarError {
                            var: env_key.clone(),
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These THINWEDGE_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_thinwedge_oss_base_url = format!(
        "http://localhost:{thinwedge_oss_port}/v1",
        thinwedge_oss_port = std::env::var("THINWEDGE_OSS_PORT")
            .ok()
    ...
    let thinwedge_oss_base_url = std::env::var("THINWEDGE_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_thinwedge_oss_base_url);
    create_oss_provider_with_base_url(&thinwedge_oss_base_url, wire_api)
}
