...
//!
//! This crate wraps [`thinwedge_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](thinwedge_app_server::in_process::InProcessClientHandle),
...
pub use thinwedge_app_server::app_server_control_socket_path;
...
use thinwedge_app_server::in_process::LogDbLayer;
pub use thinwedge_app_server::in_process::StateDbHandle;
...
use thinwedge_app_server_protocol::Result as JsonRpcResult;
...
use thinwedge_config::NoopThreadConfigLoader;
use thinwedge_config::RemoteThreadConfigLoader;
use thinwedge_config::ThreadConfigLoader;
use thinwedge_config::config_toml::ConfigToml;
use thinwedge_core::config::Config;
pub use thinwedge_core::otel_init::build_provider as build_otel_provider;
use thinwedge_core::personality_migration::PersonalityMigrationStatus;
use thinwedge_core::personality_migration::maybe_migrate_personality;
...
/// module exists so clients can remove a direct `thinwedge-core` dependency
/// while legacy startup/config paths are migrated to RPCs.
pub mod legacy_core {
    pub use thinwedge_core::check_execpolicy_for_warnings;
    pub use thinwedge_core::format_exec_policy_error_with_source;

    pub mod config {
        pub use thinwedge_core::config::*;

        pub mod edit {
            pub use thinwedge_core::config::edit::*;
        }
    }
}
...
pub async fn migrate_personality_if_needed(
    ...
    let status = maybe_migrate_personality(thinwedge_home, config_toml, state_db).await?;
    match status {
...
pub struct InProcessClientStartArgs {
    ...
    pub feedback: ThinWedgeFeedback,
    ...
    /// Whether auth loading should honor the `THINWEDGE_API_KEY` environment variable.
    pub enable_thinwedge_api_key_env: bool,
...
impl InProcessClientStartArgs {
    ...
    fn into_runtime_start_args(self) -> InProcessStartArgs {
        ...
        InProcessStartArgs {
            ...
            enable_thinwedge_api_key_env: self.enable_thinwedge_api_key_env,
            initialize,
...
/// - [`thinwedge_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        ...
        let mut handle =
            thinwedge_app_server::in_process::start(args.into_runtime_start_args()).await?;
...
impl AppServerClient {
    pub fn thinwedge_home(&self, local_thinwedge_home: &AbsolutePathBuf) -> Option<AppServerPath> {
        match self {
            Self::InProcess(_) => Some(AppServerPath::from_app_server(
                local_thinwedge_home.display().to_string(),
            )),
            Self::Remote(client) => client.thinwedge_home().map(AppServerPath::from_app_server),
        }
    }
...
mod tests {
    ...
    use thinwedge_app_server_protocol::ServerNotification;
    use thinwedge_app_server_protocol::SessionSource as ApiSessionSource;
    ...
    use thinwedge_core::init_state_db;
    ...
    async fn build_test_config_for_thinwedge_home(thinwedge_home: &Path) -> Config {
        ...
        {
            Ok(config) => config,
            Err(_) => Config::load_default_with_cli_overrides_for_thinwedge_home(
    ...
    async fn start_test_client_with_capacity(
        ...
        let thinwedge_home = TempDir::new().expect("temp dir");
        let config = Arc::new(build_test_config_for_thinwedge_home(thinwedge_home.path()).await);
        ...
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            client_name: "thinwedge-app-server-client-test".to_string(),
        ...
        write_websocket_message(
            ...
            JSONRPCMessage::Response(JSONRPCResponse {
                ...
                result: serde_json::json!({
                    "userAgent": "thinwedge_cli_rs/9.8.7-test (Test OS; x86_64) rust",
                    "thinwedgeHome": "/server/.thinwedge",
                }),
            }),
        )
        .await;
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            thinwedge_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn agent_message_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::AgentMessageDelta(
            thinwedge_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: "thread".to_string(),
                turn_id: "turn".to_string(),
                item_id: "item".to_string(),
                delta: delta.to_string(),
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: thinwedge_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<thinwedge_app_server_protocol::ThreadReadResponse>(
    ...
    async fn forward_in_process_event_preserves_transcript_notifications_under_backpressure() {
        ...
        assert!(matches!(
            ...
            InProcessServerEvent::ServerNotification(ServerNotification::ItemCompleted(
                ...
                &notification.item,
                thinwedge_app_server_protocol::ThreadItem::AgentMessage { text, .. } if text == "hello"
            )
        ));
        assert!(matches!(
            &events[4],
            InProcessServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                notification
            )) if notification.turn.status == thinwedge_app_server_protocol::TurnStatus::Completed
        ));
    }
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        assert_eq!(client.thinwedge_home(), Some("/server/.thinwedge"));
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: thinwedge_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
    ...
    async fn remote_unix_socket_typed_request_roundtrip_works() {
        ...
        let socket_path =
            AbsolutePathBuf::from_absolute_path(socket_dir.path().join("thinwedge.sock"))
        ...
        for event in &remaining_events {
            match event {
                ...
                AppServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                    ...
                    == thinwedge_app_server_protocol::TurnStatus::Completed =>
                {
    ...
    fn event_requires_delivery_marks_transcript_and_terminal_events() {
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                thinwedge_app_server_protocol::ServerNotification::TurnCompleted(
                    thinwedge_app_server_protocol::TurnCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn: thinwedge_app_server_protocol::Turn {
                            id: "turn".to_string(),
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                thinwedge_app_server_protocol::ServerNotification::AgentMessageDelta(
                    thinwedge_app_server_protocol::AgentMessageDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                thinwedge_app_server_protocol::ServerNotification::ItemCompleted(
                    thinwedge_app_server_protocol::ItemCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        completed_at_ms: 0,
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                thinwedge_app_server_protocol::ServerNotification::ExternalAgentConfigImportCompleted(
                    thinwedge_app_server_protocol::ExternalAgentConfigImportCompletedNotification {},
                )
            )
        ));
        ...
        assert!(!event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                thinwedge_app_server_protocol::ServerNotification::CommandExecutionOutputDelta(
                    thinwedge_app_server_protocol::CommandExecutionOutputDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
    ...
    async fn runtime_start_args_use_remote_thread_config_loader_when_configured() {
        ...
        assert_eq!(
            err.code(),
            thinwedge_config::ThreadConfigLoadErrorCode::RequestFailed
        );
    }
