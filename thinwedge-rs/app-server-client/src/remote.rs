...
impl RemoteAppServerClient {
    ...
    pub fn thinwedge_home(&self) -> Option<&str> {
        self.thinwedge_home.as_deref()
    }
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        let (pending_events, server_version, thinwedge_home) = initialize_remote_connection(
            &mut stream,
            &endpoint,
            initialize_params,
            INITIALIZE_TIMEOUT,
    ...
    pub async fn shutdown(self) -> IoResult<()> {
        let Self {
            ...
            thinwedge_home: _thinwedge_home,
            worker_handle,
        } = self;
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
                ...
                Some(Ok(Message::Text(text))) => {
                    ...
                    match message {
                        JSONRPCMessage::Response(response) if response.id == initialize_request_id => {
                            ...
                            thinwedge_home = response
                                .result
                                .get("thinwedgeHome")
                                .and_then(serde_json::Value::as_str)
                                .filter(|thinwedge_home| !thinwedge_home.is_empty())
                                .map(str::to_string);
                            break Ok(());
                        }
    ...
    Ok((pending_events, server_version, thinwedge_home))
}
