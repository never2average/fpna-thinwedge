# thinwedge-app-server-daemon

> `thinwedge-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`thinwedge-app-server-daemon` backs the machine-readable `thinwedge app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for ThinWedge instances launched over SSH, including fresh developer
...
```sh
thinwedge app-server daemon start
thinwedge app-server daemon restart
thinwedge app-server daemon enable-remote-control
thinwedge app-server daemon disable-remote-control
thinwedge app-server daemon stop
thinwedge app-server daemon version
thinwedge app-server daemon bootstrap --remote-control
```
...
$HOME/.thinwedge/packages/standalone/current/thinwedge app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `THINWEDGE_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes ThinWedge is installed through `install.sh` and always launches
the standalone managed binary under `THINWEDGE_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `THINWEDGE_HOME/packages/standalone/current/thinwedge` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `THINWEDGE_HOME/packages/standalone/current/thinwedge` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `thinwedge remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `THINWEDGE_HOME`, so a concurrent
...
The daemon stores its local state under `THINWEDGE_HOME/app-server-daemon/`:
