...
#[cfg(unix)]
use crate::managed_install::resolved_managed_thinwedge_bin;
...
async fn update_once(
    ...
    let managed_thinwedge_bin =
        resolved_managed_thinwedge_bin(&daemon.managed_thinwedge_bin).await?;
    let managed_identity = executable_identity(&managed_thinwedge_bin).await?;
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_thinwedge_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_thinwedge_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_thinwedge_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed ThinWedge binary {}",
            managed_thinwedge_bin.display()
        )
    })
}
...
async fn install_latest_standalone() -> Result<()> {
    let script = reqwest::get("https://chatgpt.com/thinwedge/install.sh")
        .await
        .context("failed to fetch standalone ThinWedge updater")?
        .error_for_status()
        .context("standalone ThinWedge updater request failed")?
        .bytes()
        .await
        .context("failed to read standalone ThinWedge updater")?;
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone ThinWedge updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone ThinWedge updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone ThinWedge updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone ThinWedge updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone ThinWedge updater exited with status {status}")
    }
}
