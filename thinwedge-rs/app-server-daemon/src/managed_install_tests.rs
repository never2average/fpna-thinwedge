...
use super::parse_thinwedge_version;

#[test]
fn parses_thinwedge_cli_version_output() {
    assert_eq!(
        parse_thinwedge_version("thinwedge 1.2.3\n").expect("version"),
        "1.2.3"
    );
}

#[test]
fn rejects_malformed_thinwedge_cli_version_output() {
    assert!(parse_thinwedge_version("thinwedge\n").is_err());
}
