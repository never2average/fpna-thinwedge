...
use serde::Serialize;
#[cfg(unix)]
use thinwedge_app_server_transport::REMOTE_CONTROL_DISABLED_ENV_VAR;
use tokio::fs;
...
impl PidBackend {
    ...
    pub(crate) fn new_update_loop(thinwedge_bin: PathBuf, pid_file: PathBuf) -> Self {
        let lock_file = pid_file.with_extension("pid.lock");
        Self {
            thinwedge_bin,
            pid_file,
    ...
    pub(crate) async fn start(&self) -> Result<Option<u32>> {
        ...
        let mut command = Command::new(&self.thinwedge_bin);
