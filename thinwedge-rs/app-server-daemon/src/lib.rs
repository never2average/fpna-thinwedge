...
use managed_install::managed_thinwedge_bin;
#[cfg(unix)]
use managed_install::managed_thinwedge_version;
use serde::Serialize;
use settings::DaemonSettings;
use thinwedge_app_server_protocol::RemoteControlConnectionStatus;
use thinwedge_app_server_transport::app_server_control_socket_path;
...
pub struct LifecycleOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub pid: Option<u32>,
    pub managed_thinwedge_path: PathBuf,
    pub managed_thinwedge_version: Option<String>,
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "thinwedge app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
struct Daemon {
    ...
    managed_thinwedge_bin: PathBuf,
}

impl Daemon {
    fn from_environment() -> Result<Self> {
        let thinwedge_home = find_thinwedge_home().context("failed to resolve THINWEDGE_HOME")?;
        let socket_path = app_server_control_socket_path(thinwedge_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = thinwedge_home.as_path().join(STATE_DIR_NAME);
        Ok(Self {
            socket_path,
            ...
            managed_thinwedge_bin: managed_thinwedge_bin(thinwedge_home.as_path()),
        })
    }
    ...
    async fn start(&self) -> Result<LifecycleOutput> {
        ...
        self.ensure_managed_thinwedge_bin()?;
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by thinwedge app-server daemon"
            ));
        }
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_thinwedge_version(managed_thinwedge_bin).await?)
            } else {
                None
            };
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    backend.stop().await?;
                    let _ = self
                        .start_managed_backend_with_bin(&settings, managed_thinwedge_bin)
                        .await?;
        ...
        if should_reexec_updater(updater_refresh_mode, outcome) {
            crate::update_loop::reexec_managed_updater(managed_thinwedge_bin)?;
        }
    ...
    async fn append_daemon_app_server_context(&self, context: &mut String) {
        let managed_thinwedge_version = self
            .managed_thinwedge_version_best_effort()
            .await
            .unwrap_or_else(|| "unknown".to_string());
        context.push_str(&format!(
            "\n\nDaemon used app-server:\n  path: {}\n  version: {managed_thinwedge_version}",
            self.managed_thinwedge_bin.display()
        ));
    }
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        let info = self.wait_until_ready().await?;
        let managed_thinwedge_version = self.managed_thinwedge_version_best_effort().await;
        Ok(BootstrapOutput {
            status: BootstrapStatus::Bootstrapped,
            backend: BackendKind::Pid,
            auto_update_enabled: true,
            remote_control_enabled: settings.remote_control_enabled,
            managed_thinwedge_path: self.managed_thinwedge_bin.clone(),
            managed_thinwedge_version,
    ...
    async fn start_managed_backend(&self, settings: &DaemonSettings) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(settings, &self.managed_thinwedge_bin)
            .await
    }
    ...
    async fn start_managed_backend_with_bin(
        ...
        let backend =
            backend::pid_backend(self.backend_paths_with_bin(settings, managed_thinwedge_bin));
        backend.start().await
    }
    ...
    fn ensure_managed_thinwedge_bin(&self) -> Result<()> {
        if self.managed_thinwedge_bin.is_file() {
            return Ok(());
        }

        let managed_thinwedge_path = self.managed_thinwedge_bin.display();
        Err(anyhow!(
            "managed standalone ThinWedge install not found at {managed_thinwedge_path}\n\n\
             This command requires the standalone install managed by the ThinWedge installer, because \
             the daemon starts and updates app-server from that fixed path.\n\n\
             Install it with:\n  curl -fsSL https://chatgpt.com/thinwedge/install.sh | sh\n\n\
             Then rerun the command you just tried."
        ))
    }

    #[cfg(unix)]
    async fn managed_thinwedge_version_best_effort(&self) -> Option<String> {
        managed_thinwedge_version(&self.managed_thinwedge_bin)
            .await
            .ok()
    }
    ...
    fn backend_paths(&self, settings: &DaemonSettings) -> BackendPaths {
        self.backend_paths_with_bin(settings, &self.managed_thinwedge_bin)
    }
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            thinwedge_bin: managed_thinwedge_bin.to_path_buf(),
...
mod tests {
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        let lifecycle_output = LifecycleOutput {
            ...
            pid: None,
            managed_thinwedge_path: "thinwedge".into(),
            managed_thinwedge_version: Some("1.2.3".to_string()),
            socket_path: "thinwedge.sock".into(),
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                "backend": "pid",
                "managedThinWedgePath": "thinwedge",
                "managedThinWedgeVersion": "1.2.3",
                "socketPath": "thinwedge.sock",
                "cliVersion": "1.2.3",
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        let daemon = Daemon {
            ...
            managed_thinwedge_bin: temp_dir.path().join("missing-thinwedge"),
        };
        ...
        assert_eq!(
            ...
            format!(
                ...
                daemon.managed_thinwedge_bin.display(),
                stderr_log.display()
            )
        );
    }
