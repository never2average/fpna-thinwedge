...
# Run a suite of smoke tests against the Windows sandbox via the ThinWedge CLI
...
def _resolve_thinwedge_cmd() -> List[str]:
    """Resolve the ThinWedge CLI to invoke `thinwedge sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run ThinWedge.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "thinwedge.exe",
        ws_root / "target" / "release" / "thinwedge.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "thinwedge.exe",
            cargo_base / "release" / "thinwedge.exe",
        ])
    ...
    if shutil.which("thinwedge"):
        return ["thinwedge"]

    raise FileNotFoundError(
        "ThinWedge CLI not found. Build it first, e.g.\n"
        "  cargo build -p thinwedge-cli --release\n"
        "or for debug:\n"
        "  cargo build -p thinwedge-cli\n"
    )

THINWEDGE_CMD = _resolve_thinwedge_cmd()
print(THINWEDGE_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to thinwedge CLI overrides.
    ...
    argv = [*THINWEDGE_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"THINWEDGE_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\thinwedge_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.thinwedge artifacts) denied
    thinwedge_home = Path(os.environ["USERPROFILE"]) / ".thinwedge"
    cap_sid_target = thinwedge_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .thinwedge\\policy.json"], WS_ROOT)
    add("WS: .thinwedge cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .thinwedge policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > thinwedge_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://thinwedge-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
