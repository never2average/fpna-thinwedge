...
pub(super) fn ensure_thinwedge_app_runtime_bin_readable(
    ...
    // ThinWedge desktop copies bundled Windows binaries out of WindowsApps to this
    // fixed LocalAppData cache before launching thinwedge.exe.
    let runtime_bin_dir = local_app_data.join("OpenAI").join("ThinWedge").join("bin");
