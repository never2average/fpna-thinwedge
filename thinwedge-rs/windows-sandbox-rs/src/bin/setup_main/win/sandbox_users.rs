...
use thinwedge_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "ThinWedgeSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "ThinWedge sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(thinwedge_home);
...
pub(super) fn prepare_setup_marker(thinwedge_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(thinwedge_home).join("setup_marker.json");
