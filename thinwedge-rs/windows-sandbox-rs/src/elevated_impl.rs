...
mod windows_impl {
    ...
    /// Creates the sandbox user's ThinWedge home directory if it does not already exist.
    fn ensure_thinwedge_home_exists(p: &Path) -> Result<()> {
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let (psid_to_use, cap_sids) = match &policy {
            ...
            SandboxPolicy::WorkspaceWrite { .. } => {
                ...
                (
                    ...
                    vec![
                        caps.workspace,
                        crate::cap::workspace_cap_sid_for_cwd(thinwedge_home, cwd)?,
                    ],
                )
            }
            SandboxPolicy::DangerFullAccess | SandboxPolicy::ExternalSandbox { .. } => {
        ...
        (|| -> Result<CaptureResult> {
            let spawn_request = SpawnRequest {
                ...
                thinwedge_home: sandbox_base.clone(),
                real_thinwedge_home: thinwedge_home.to_path_buf(),
                cap_sids,
                timeout_ms,
                tty: false,
                stdin_open: false,
            ...
            let mut transport =
                spawn_runner_transport(thinwedge_home, cwd, &sandbox_creds, logs_base_dir)?;
