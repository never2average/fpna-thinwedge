...
pub fn current_log_file_path_for_thinwedge_home(thinwedge_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(thinwedge_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_thinwedge_home_uses_sandbox_dir() {
        let thinwedge_home = Path::new("thinwedge-home");

        assert_eq!(
            current_log_file_path_for_thinwedge_home(thinwedge_home),
            current_log_file_path(&thinwedge_home.join(".sandbox"))
        );
    }
}
