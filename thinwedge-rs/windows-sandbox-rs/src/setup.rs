...
pub const OFFLINE_USERNAME: &str = "ThinWedgeSandboxOffline";
pub const ONLINE_USERNAME: &str = "ThinWedgeSandboxOnline";
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;
const SETUP_EXE_FILENAME: &str = "thinwedge-windows-sandbox-setup.exe";
...
pub fn sandbox_dir(thinwedge_home: &Path) -> PathBuf {
    thinwedge_home.join(".sandbox")
}

pub fn sandbox_bin_dir(thinwedge_home: &Path) -> PathBuf {
    thinwedge_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(thinwedge_home: &Path) -> PathBuf {
    thinwedge_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(thinwedge_home: &Path) -> PathBuf {
    sandbox_dir(thinwedge_home).join("setup_marker.json")
}

pub fn sandbox_users_path(thinwedge_home: &Path) -> PathBuf {
    sandbox_secrets_dir(thinwedge_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, &permissions, env_map, thinwedge_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        thinwedge_home: request.thinwedge_home.to_path_buf(),
    ...
    let sbx_dir = sandbox_dir(request.thinwedge_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(request.thinwedge_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| request.thinwedge_home.to_path_buf());
    log_note(
        &format!(
    ...
    if !status.success() {
        ...
        return Err(report_helper_failure(
            request.thinwedge_home,
            cleared_report,
            status.code(),
        ));
    }
    if let Err(err) = clear_setup_error_report(request.thinwedge_home) {
        log_note(
...
fn gather_helper_read_roots(thinwedge_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(thinwedge_home);
...
fn gather_full_read_roots_for_permissions(
    ...
    let mut roots = gather_helper_read_roots(thinwedge_home);
    roots.extend(
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, thinwedge_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<thinwedge_otel::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
    #[serde(default)]
    refresh_only: bool,
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "THINWEDGE_NETWORK_ALLOW_LOCAL_BINDING";
...
fn report_helper_failure(
    ...
    match read_setup_error_report(thinwedge_home) {
...
fn verify_setup_completed(thinwedge_home: &Path) -> Result<()> {
    if sandbox_setup_is_complete(thinwedge_home) {
        Ok(())
    } else {
        Err(failure(
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(thinwedge_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
                ...
                Some(&sandbox_dir(thinwedge_home)),
            );
            false
        }
    };
    ...
    if !needs_elevation {
        ...
        verify_setup_completed(thinwedge_home)?;
        if let Err(err) = clear_setup_error_report(thinwedge_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: thinwedge_otel::global_statsig_metrics_settings(),
        mode: SetupMode::Full,
        refresh_only: false,
    };
    ...
    run_setup_exe(&payload, needs_elevation, request.thinwedge_home)
}

pub fn run_elevated_provisioning_setup(thinwedge_home: &Path, real_user: &str) -> Result<()> {
    let sbx_dir = sandbox_dir(thinwedge_home);
    std::fs::create_dir_all(&sbx_dir).map_err(|err| {
        failure(
            SetupErrorCode::OrchestratorSandboxDirCreateFailed,
            format!("failed to create sandbox dir {}: {err}", sbx_dir.display()),
    ...
    let payload = ElevationPayload {
        ...
        command_cwd: thinwedge_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
    ...
    run_setup_exe(&payload, /*needs_elevation*/ false, thinwedge_home)
}
...
fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.thinwedge_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, thinwedge_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to THINWEDGE_HOME or anything under THINWEDGE_HOME/.sandbox,
    // THINWEDGE_HOME/.sandbox-bin, or THINWEDGE_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let thinwedge_home_key = canonical_path_key(thinwedge_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(thinwedge_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(thinwedge_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(thinwedge_home));
    let secrets_dir_prefix = format!("{}/", secrets_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = canonical_path_key(root);
        key != thinwedge_home_key
            && key != sbx_dir_key
...
mod tests {
    ...
    fn setup_completion_requires_ready_artifacts() {
        let thinwedge_home = TempDir::new().expect("tempdir");
        let err = verify_setup_completed(thinwedge_home.path())
    ...
    fn report_helper_failure_uses_setup_error_report_when_clear_succeeded() {
        ...
        let thinwedge_home = tmp.path().join("thinwedge-home");
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("thinwedge.exe");
        let setup_exe = resources_dir.join("thinwedge-windows-sandbox-setup.exe");
        fs::write(&exe, b"thinwedge").expect("write exe");
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "THINWEDGE_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_thinwedge_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let thinwedge_home = user_profile.join("ThinWedgeHome");
        let documents = user_profile.join("Documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &thinwedge_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        ...
        let roots = gather_read_roots(&command_cwd, &permissions, &HashMap::new(), &thinwedge_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&thinwedge_home)).expect("canonical helper dir");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&thinwedge_home);
        fs::create_dir_all(&thinwedge_home).expect("create thinwedge home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_thinwedge_home =
            dunce::canonicalize(&thinwedge_home).expect("canonical thinwedge home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_thinwedge_home));
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_thinwedge = extra_write_root.join(".thinwedge");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_thinwedge).expect("create extra .thinwedge");
        let writable_roots = vec![
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_thinwedge).expect("canonical extra .thinwedge"),
                explicit_deny,
            ]
            .into_iter()
