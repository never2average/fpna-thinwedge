...
use thinwedge_windows_sandbox::LaunchDesktop;
...
use thinwedge_windows_sandbox::SandboxPolicy;
...
use thinwedge_windows_sandbox::create_readonly_token_with_caps_from;
use thinwedge_windows_sandbox::create_workspace_write_token_with_caps_from;
...
use thinwedge_windows_sandbox::parse_policy;
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, conpty) = thinwedge_windows_sandbox::spawn_conpty_process_as_user(
            h_token,
            &req.command,
            &effective_cwd,
            &req.env,
