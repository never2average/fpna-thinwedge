...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, policy, thinwedge_home);
...
fn run_setup_refresh_inner(
    ...
    log_note(
        ...
        Some(&sandbox_dir(request.thinwedge_home)),
    );
    let status = cmd
        .status()
        .map_err(|e| {
...
pub(crate) fn gather_read_roots(
    ...
    gather_legacy_full_read_roots(command_cwd, policy, thinwedge_home)
}
...
fn find_setup_exe() -> PathBuf {
    ...
    {
        let candidate = dir.join("thinwedge-windows-sandbox-setup.exe");
        if candidate.exists() {
            return candidate;
        }

        // Standalone installs keep Windows helper binaries under
        // `thinwedge-resources/` next to `thinwedge.exe`, so elevation needs to probe
        // that sibling folder before falling back to PATH.
        let resource_candidate = dir
            .join("thinwedge-resources")
            .join("thinwedge-windows-sandbox-setup.exe");
        if resource_candidate.exists() {
            return resource_candidate;
        }
    }
    PathBuf::from("thinwedge-windows-sandbox-setup.exe")
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, request.thinwedge_home);
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        gather_read_roots(request.command_cwd, request.policy, request.thinwedge_home)
    };
...
mod tests {
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        ...
        let roots = gather_read_roots(&command_cwd, &policy, &thinwedge_home);
        let expected =
    ...
    fn full_read_roots_preserve_legacy_platform_defaults() {
        ...
        let roots = gather_legacy_full_read_roots(&command_cwd, &policy, &thinwedge_home);
