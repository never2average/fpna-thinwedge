...
/// Contributes ThinWedge memory read-path prompt context and memory read tools.
...
pub(crate) struct MemoriesExtensionConfig {
    ...
    pub(crate) thinwedge_home: AbsolutePathBuf,
}
...
impl ContextContributor for MemoriesExtension {
    fn contribute<'a>(
        ...
        Box::pin(async move {
            ...
            build_memory_tool_developer_instructions(&config.thinwedge_home)
                .await
...
impl ToolContributor for MemoriesExtension {
    ...
    ) -> Vec<Arc<dyn thinwedge_extension_api::ToolExecutor<thinwedge_extension_api::ToolCall>>>
    {
        ...
        tools::memory_tools(
            LocalMemoriesBackend::from_thinwedge_home(&config.thinwedge_home),
