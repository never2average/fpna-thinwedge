    ...
    B: MemoriesBackend,
{
    ...
    fn spec(&self) -> ToolSpec {
        memory_function_tool::<SearchArgs, SearchMemoriesResponse>(
            SEARCH_TOOL_NAME,
            "Search ThinWedge memory files for substring matches, optionally normalizing separators or requiring all query substrings on the same line or within a line window.",
        )
    }
    ...
    B: MemoriesBackend,
{
    async fn handle_call(
        ...
        Box<dyn thinwedge_extension_api::ToolOutput>,
        thinwedge_extension_api::FunctionCallError,
    > {
