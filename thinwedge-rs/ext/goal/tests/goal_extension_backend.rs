...
use thinwedge_extension_api::ExtensionEventSink;
use thinwedge_extension_api::ExtensionRegistryBuilder;
use thinwedge_extension_api::FunctionCallError;
use thinwedge_extension_api::NoopTurnItemEmitter;
use thinwedge_extension_api::ThreadResumeInput;
use thinwedge_extension_api::ThreadStartInput;
use thinwedge_extension_api::ThreadStopInput;
use thinwedge_extension_api::ToolCall;
use thinwedge_extension_api::ToolCallOutcome;
use thinwedge_extension_api::ToolCallSource;
use thinwedge_extension_api::ToolExecutor;
use thinwedge_extension_api::ToolFinishInput;
use thinwedge_extension_api::ToolPayload;
use thinwedge_extension_api::TurnErrorInput;
use thinwedge_extension_api::TurnStartInput;
use thinwedge_extension_api::TurnStopInput;
use thinwedge_goal_extension::GoalObjectiveUpdate;
use thinwedge_goal_extension::GoalRuntimeHandle;
use thinwedge_goal_extension::GoalService;
use thinwedge_goal_extension::GoalSetRequest;
use thinwedge_goal_extension::GoalTokenBudgetUpdate;
use thinwedge_goal_extension::install_with_backend;
...
use thinwedge_protocol::protocol::TruncationPolicy;
...
async fn turn_error_usage_limit_accounts_progress_and_clears_accounting() -> anyhow::Result<()> {
    ...
    harness
        ...
        .await;
    harness
        .notify_turn_error("turn-1", ThinWedgeErrorInfo::UsageLimitExceeded)
        .await;
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(thinwedge_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(
        vec![
            CapturedGoalEvent {
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    harness
        .notify_turn_error("turn-1", ThinWedgeErrorInfo::Other)
        .await;
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(thinwedge_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
...
async fn installed_tools(
    runtime: Arc<thinwedge_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: thinwedge_extension_api::ExtensionRegistry<()>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish(&self, turn_id: &str, call_id: &str, tool_name: &str) {
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = thinwedge_extension_api::ToolName::plain(tool_name);
    ...
    async fn notify_turn_error(&self, turn_id: &str, error: ThinWedgeErrorInfo) {
        let turn_store = ExtensionData::new(turn_id);
        for contributor in self.registry.turn_lifecycle_contributors() {
            contributor
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall {
    ToolCall {
        ...
        tool_name: thinwedge_extension_api::ToolName::plain(tool_name),
        model: "gpt-test".to_string(),
        truncation_policy: TruncationPolicy::Bytes(1024),
        conversation_history: thinwedge_extension_api::ConversationHistory::default(),
...
async fn test_runtime() -> anyhow::Result<Arc<thinwedge_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    thinwedge_state::StateRuntime::init(tempdir.keep(), "test-provider".to_string()).await
}
...
async fn seed_thread_metadata(
    runtime: &thinwedge_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = thinwedge_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .thinwedge_home()
...
fn protocol_status(status: thinwedge_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
        thinwedge_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        thinwedge_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        thinwedge_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        thinwedge_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        thinwedge_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        thinwedge_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}
