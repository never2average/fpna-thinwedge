...
use thinwedge_extension_api::ThreadIdleInput;
...
use thinwedge_extension_api::TokenUsageContributor;
...
use thinwedge_extension_api::ToolLifecycleContributor;
use thinwedge_extension_api::ToolLifecycleFuture;
use thinwedge_extension_api::TurnAbortInput;
use thinwedge_extension_api::TurnErrorInput;
use thinwedge_extension_api::TurnLifecycleContributor;
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                thinwedge_protocol::config_types::ModeKind::Plan
            ) {
    ...
    fn on_turn_error<'a>(&'a self, input: TurnErrorInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            let reason = match input.error {
                ThinWedgeErrorInfo::UsageLimitExceeded => ActiveGoalStopReason::UsageLimit,
    ...
    C: Send + Sync + 'static,
{
    fn tools(
        ...
        _session_store: &ExtensionData,
        thread_store: &ExtensionData,
    ) -> Vec<Arc<dyn thinwedge_extension_api::ToolExecutor<thinwedge_extension_api::ToolCall>>>
    {
        let Some(runtime) = goal_runtime_handle(thread_store) else {
            return Vec::new();
        };
