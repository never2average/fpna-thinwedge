...
use thinwedge_extension_api::JsonToolOutput;
use thinwedge_extension_api::ToolCall;
use thinwedge_extension_api::ToolExecutor;
use thinwedge_extension_api::ToolName;
use thinwedge_extension_api::ToolOutput;
use thinwedge_extension_api::ToolSpec;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<thinwedge_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> thinwedge_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => thinwedge_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => thinwedge_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: thinwedge_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            thinwedge_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            thinwedge_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<thinwedge_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &thinwedge_state::StateRuntime,
    thread_id: ThreadId,
    goal: &thinwedge_state::ThreadGoal,
) {
...
fn protocol_status_from_state(status: thinwedge_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    ...
    match status {
        ...
        ThreadGoalStatus::Blocked => thinwedge_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => thinwedge_state::ThreadGoalStatus::UsageLimited,
