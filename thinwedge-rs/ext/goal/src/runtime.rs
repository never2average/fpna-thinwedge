...
struct GoalRuntimeInner {
    thread_id: ThreadId,
    state_dbs: Arc<thinwedge_state::StateRuntime>,
...
pub struct PreviousGoalSnapshot {
    pub goal_id: String,
    pub status: thinwedge_state::ThreadGoalStatus,
    pub objective: String,
}

impl From<&thinwedge_state::ThreadGoal> for PreviousGoalSnapshot {
    fn from(goal: &thinwedge_state::ThreadGoal) -> Self {
        Self {
...
impl GoalRuntimeHandle {
    ...
    pub async fn apply_external_goal_set(
        ...
        match goal.status {
            ...
            | thinwedge_state::ThreadGoalStatus::Blocked
            | thinwedge_state::ThreadGoalStatus::UsageLimited
    ...
    pub(crate) async fn stop_active_goal_for_turn(
        ...
        let (event_name, status) = match reason {
            ActiveGoalStopReason::TurnError => {
                ("turn-error", thinwedge_state::ThreadGoalStatus::Blocked)
            }
        ...
        let can_stop = active_goal.status == thinwedge_state::ThreadGoalStatus::Active
            || (active_goal.status == thinwedge_state::ThreadGoalStatus::BudgetLimited
                && status == thinwedge_state::ThreadGoalStatus::UsageLimited);
        if !can_stop {
            self.inner.accounting_state.clear_active_goal();
            return Ok(());
        }
    ...
    pub async fn restore_after_resume(&self) -> Result<(), String> {
        ...
        let goal = self
            ...
            .map_err(|err| err.to_string())?;
        match goal {
            Some(goal) if goal.status == thinwedge_state::ThreadGoalStatus::Active => {
                self.inner
                    .accounting_state
    ...
    async fn account_idle_goal_progress(
        ...
        Ok(match outcome {
            ...
            thinwedge_state::GoalAccountingOutcome::Unchanged(_) => {
    ...
    ) -> Result<Option<thinwedge_state::ThreadGoalStatus>, String> {
        let goal = self
            .inner
            .state_dbs
            .thread_goals()
