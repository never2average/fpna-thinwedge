...
use thinwedge_core_skills::HostLoadedSkills;
use thinwedge_core_skills::SKILLS_HOW_TO_USE_WITH_ABSOLUTE_PATHS;
use thinwedge_core_skills::SKILLS_INTRO_WITH_ABSOLUTE_PATHS;
use thinwedge_core_skills::SkillLoadOutcome;
use thinwedge_core_skills::SkillMetadata;
...
use thinwedge_protocol::capabilities::CapabilityRootLocation;
use thinwedge_protocol::capabilities::SelectedCapabilityRoot;
...
use thinwedge_skills_extension::SkillProviders;
use thinwedge_skills_extension::SkillsExtensionConfig;
use thinwedge_skills_extension::catalog::SkillAuthority;
use thinwedge_skills_extension::catalog::SkillCatalog;
use thinwedge_skills_extension::catalog::SkillCatalogEntry;
use thinwedge_skills_extension::catalog::SkillPackageId;
use thinwedge_skills_extension::catalog::SkillProviderError;
use thinwedge_skills_extension::catalog::SkillReadResult;
use thinwedge_skills_extension::catalog::SkillResourceId;
use thinwedge_skills_extension::catalog::SkillSearchResult;
use thinwedge_skills_extension::catalog::SkillSourceKind;
use thinwedge_skills_extension::install;
use thinwedge_skills_extension::install_with_providers;
use thinwedge_skills_extension::provider::SkillListQuery;
use thinwedge_skills_extension::provider::SkillProvider;
use thinwedge_skills_extension::provider::SkillProviderFuture;
use thinwedge_skills_extension::provider::SkillReadRequest;
use thinwedge_skills_extension::provider::SkillSearchRequest;
use thinwedge_utils_absolute_path::AbsolutePathBuf;

type TestResult = Result<(), Box<dyn std::error::Error>>;

static NEXT_THINWEDGE_HOME_ID: AtomicUsize = AtomicUsize::new(0);
const DEMO_SKILL_CONTENTS: &str =
    "---\nname: demo\ndescription: Demo skill.\n---\n# Demo\n\nUse the demo skill.\n";

#[tokio::test]
async fn installed_extension_uses_host_loaded_skills() -> TestResult {
    let thinwedge_home = test_thinwedge_home();
    let skill_path = thinwedge_home.join("skills").join("demo").join("SKILL.md");
    ...
    std::fs::remove_dir_all(thinwedge_home)?;
    Ok(())
}
...
fn test_thinwedge_home() -> PathBuf {
    let id = NEXT_THINWEDGE_HOME_ID.fetch_add(1, Ordering::Relaxed);
    std::env::temp_dir().join(format!(
        "thinwedge-skills-extension-test-{}-{id}",
        std::process::id(),
    ))
}
