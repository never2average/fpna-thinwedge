...
//! `EscalateRequest` over the inherited `THINWEDGE_ESCALATE_SOCKET`, and the server decides whether to
...
//! specified by the `THINWEDGE_ESCALATE_SOCKET` environment variable to _send_ escalation requests,
...
pub use thinwedge_protocol::approvals::EscalationPermissions;
pub use thinwedge_protocol::approvals::ResolvedPermissionProfile;
