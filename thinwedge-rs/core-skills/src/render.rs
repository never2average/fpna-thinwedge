...
pub const SKILL_DESCRIPTION_TRUNCATED_WARNING: &str = "Skill descriptions were shortened to fit the skills context budget. ThinWedge can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest.";
...
mod tests {
    ...
    fn budgeted_rendering_warns_when_average_description_truncation_exceeds_threshold() {
        ...
        assert_eq!(
            ...
            Some(
                "Skill descriptions were shortened to fit the skills context budget. ThinWedge can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest."
                    .to_string()
            )
        );
    }
    ...
    fn outcome_rendering_uses_marketplace_root_for_single_skill_plugin_versions() {
        let github_root =
            test_path_buf("/Users/xl/.thinwedge/plugins/cache/openai-curated/github/hash123/skills")
                .abs();
        let marketplace_root = test_path_buf("/Users/xl/.thinwedge/plugins/cache/openai-curated").abs();
