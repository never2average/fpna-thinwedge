...
async fn make_config(thinwedge_home: &TempDir) -> TestConfig {
    make_config_for_cwd(thinwedge_home, thinwedge_home.path().to_path_buf()).await
}
...
fn project_layers_for_cwd(cwd: &Path) -> Vec<ConfigLayerEntry> {
    ...
    layers
        .into_iter()
        .filter_map(|dir| {
            let dot_thinwedge = dir.join(REPO_ROOT_CONFIG_DIR_NAME);
            dot_thinwedge.is_dir().then(|| {
                ConfigLayerEntry::new(
                    ConfigLayerSource::Project {
                        dot_thinwedge_folder: dot_thinwedge.abs(),
                    },
...
async fn make_config_for_cwd(thinwedge_home: &TempDir, cwd: PathBuf) -> TestConfig {
    let user_config_path = thinwedge_home.path().join(CONFIG_TOML_FILE);
    let system_config_path = thinwedge_home.path().join("etc/thinwedge/config.toml");
    fs::create_dir_all(
        system_config_path
            .parent()
...
-> anyhow::Result<()> {
    ...
    let system_folder = tmp.path().join("etc/thinwedge");
    let home_folder = tmp.path().join("home");
    let user_folder = home_folder.join("thinwedge");
...
async fn skill_roots_from_layer_stack_includes_disabled_project_layers() -> anyhow::Result<()> {
    ...
    fs::create_dir_all(&dot_thinwedge)?;

    let user_file = user_folder.join("config.toml").abs();
    let project_dot_thinwedge = dot_thinwedge.abs();
    ...
    assert_eq!(
        got,
        vec![
            (SkillScope::Repo, dot_thinwedge.join("skills")),
            (SkillScope::User, user_folder.join("skills")),
            (
                SkillScope::User,
                home_folder.join(AGENTS_DIR_NAME).join(SKILLS_DIR_NAME)
...
async fn loads_skill_dependencies_metadata_from_yaml() {
    ...
    let skill_path = write_skill(&thinwedge_home, "demo", "dep-skill", "from json");
...
async fn loads_skill_interface_metadata_from_yaml() {
    let thinwedge_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&thinwedge_home, "demo", "ui-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_from_yaml() {
    let thinwedge_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&thinwedge_home, "demo", "policy-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn empty_skill_policy_defaults_to_allow_implicit_invocation() {
    let thinwedge_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&thinwedge_home, "demo", "policy-empty", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_products_from_yaml() {
    let thinwedge_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&thinwedge_home, "demo", "policy-products", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    assert_eq!(
        ...
        Some(SkillPolicy {
            allow_implicit_invocation: None,
            products: vec![Product::ThinWedge, Product::Chatgpt, Product::Atlas],
        })
    );
}
...
async fn loads_skills_via_symlinked_subdir_for_user_scope() {
    ...
    fs::create_dir_all(thinwedge_home.path().join("skills")).unwrap();
    symlink_dir(shared.path(), &thinwedge_home.path().join("skills/shared"));
...
async fn does_not_loop_on_symlink_cycle_for_user_scope() {
    ...
    // Create a cycle:
    //   $THINWEDGE_HOME/skills/cycle/loop -> $THINWEDGE_HOME/skills/cycle
    let cycle_dir = thinwedge_home.path().join("skills/cycle");
...
async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
    ...
    let cfg = make_config_for_cwd(&thinwedge_home, repo_dir.path().to_path_buf()).await;
    let outcome = load_skills_for_test(&cfg).await;
...
async fn system_scope_ignores_symlinked_subdir() {
    ...
    let system_root = thinwedge_home.path().join("skills/.system");
...
async fn respects_max_scan_depth_for_user_scope() {
    ...
    let skills_root = thinwedge_home.path().join("skills");
    let outcome = load_skills_from_roots([SkillRoot {
...
async fn skips_hidden_and_invalid() {
    let thinwedge_home = tempfile::tempdir().expect("tempdir");
    let hidden_dir = thinwedge_home.path().join("skills/.hidden");
    ...
    let invalid_dir = thinwedge_home.path().join("skills/invalid");
...
async fn enforces_length_limits() {
    ...
    write_skill(&thinwedge_home, "max-len", "max-len", &max_desc);
    let cfg = make_config(&thinwedge_home).await;
    ...
    write_skill(&thinwedge_home, "too-long", "too-long", &too_long_desc);
    let outcome = load_skills_for_test(&cfg).await;
    assert_eq!(outcome.skills.len(), 1);
    assert_eq!(outcome.errors.len(), 1);
    assert!(
...
#[tokio::test]
async fn loads_skills_from_agents_dir_without_thinwedge_dir() {
    let thinwedge_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
#[tokio::test]
async fn loads_skills_from_all_thinwedge_dirs_under_project_root() {
    let thinwedge_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
    ...
    let cfg = make_config_for_cwd(&thinwedge_home, nested_dir).await;
...
#[tokio::test]
async fn loads_skills_from_thinwedge_dir_when_not_git_repo() {
    let thinwedge_home = tempfile::tempdir().expect("tempdir");
    ...
    let cfg = make_config_for_cwd(&thinwedge_home, work_dir.path().to_path_buf()).await;
...
async fn keeps_duplicate_names_from_repo_and_user() {
    ...
    let user_skill_path = write_skill(&thinwedge_home, "user", "dupe-skill", "from user");
...
#[tokio::test]
async fn keeps_duplicate_names_from_nested_thinwedge_dirs() {
    let thinwedge_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
async fn repo_skills_search_does_not_escape_repo_root() {
    ...
    let cfg = make_config_for_cwd(&thinwedge_home, repo_dir).await;
...
async fn loads_skills_when_cwd_is_file_in_repo() {
    ...
    let cfg = make_config_for_cwd(&thinwedge_home, file_path).await;
...
async fn loads_skills_from_system_cache_when_present() {
    ...
    let skill_path = write_system_skill(&thinwedge_home, "system", "system-skill", "from system");
