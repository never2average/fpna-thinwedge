...
//!
//! The history is stored at `~/.thinwedge/history.jsonl` with **one JSON object per
...
impl HistoryConfig {
    pub fn new(thinwedge_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            thinwedge_home: thinwedge_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.thinwedge_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.thinwedge/history.jsonl` and ensure the parent directory exists.
