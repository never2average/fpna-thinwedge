...
- Serves as the wire-level layer consumed by `thinwedge-core`; higher layers handle auth refresh and business logic.
...
- **Compaction endpoint**
  - Input: `CompactionInput<'a>` (re-exported as `thinwedge_api::CompactionInput`):
    - `model: &str`.
...
- **Memory summarize endpoint**
  - Input: `MemorySummarizeInput` (re-exported as `thinwedge_api::MemorySummarizeInput`):
    - `model: String`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `thinwedge-api` and `thinwedge-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.
