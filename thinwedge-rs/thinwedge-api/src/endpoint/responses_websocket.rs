...
async fn connect_websocket(
    ...
    // If a ThinWedge-specific CA bundle is configured, build an explicit rustls connector so this
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-thinwedge-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-thinwedge-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
