...
impl<T: HttpTransport> RealtimeCallClient<T> {
    ...
    pub async fn create_with_session_architecture_and_headers(
        ...
        trace!(target: "thinwedge_api::realtime_websocket::wire", "realtime call request SDP: {sdp}");
...
mod tests {
    ...
    async fn extracts_call_id_from_forwarded_backend_location() {
        ...
        let client = RealtimeCallClient::new(
            transport.clone(),
            provider("https://chatgpt.com/backend-api/thinwedge"),
            Arc::new(DummyAuth),
        );
        ...
        assert_eq!(
            request.url,
            "https://chatgpt.com/backend-api/thinwedge/realtime/calls"
        );
        assert_eq!(
            request.body,
            Some(RequestBody::Raw(Bytes::from_static(b"v=offer\r\n")))
