...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, ThinWedgeErr::ServerOverloaded));
}
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let ThinWedgeErr::CyberPolicy { message } = err else {
        panic!("expected ThinWedgeErr::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let ThinWedgeErr::InvalidRequest(message) = err else {
        panic!("expected ThinWedgeErr::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("thinwedge_other"),
    );
    headers.insert(
        "x-thinwedge-other-limit-name",
        http::HeaderValue::from_static("thinwedge_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let ThinWedgeErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected ThinWedgeErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_ignores_unparseable_rate_limit_reached_type_headers() {
    ...
    for value in values {
        let mut headers = HeaderMap::new();
        headers.insert("x-thinwedge-rate-limit-reached-type", value);
        let body = serde_json::json!({
            "error": {
                "type": "usage_limit_reached",
                "plan_type": "pro",
...
fn map_api_error_extracts_identity_auth_details_from_headers() {
    ...
    let ThinWedgeErr::UnexpectedStatus(err) = err else {
        panic!("expected ThinWedgeErr::UnexpectedStatus, got {err:?}");
    };
