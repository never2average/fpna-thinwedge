...
pub fn map_api_error(err: ApiError) -> ThinWedgeErr {
    match err {
        ApiError::ContextWindowExceeded => ThinWedgeErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => ThinWedgeErr::QuotaExceeded,
        ApiError::UsageNotIncluded => ThinWedgeErr::UsageNotIncluded,
        ...
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return ThinWedgeErr::ServerOverloaded;
                }
                ...
                if status == http::StatusCode::BAD_REQUEST {
                            ...
                            == Some(CYBER_POLICY_ERROR_CODE)
                    {
                        ...
                        ThinWedgeErr::CyberPolicy { message }
                    } else if body_text
                        .contains("The image data you provided does not represent a valid image")
                    {
                        ThinWedgeErr::InvalidImageRequest()
                    } else {
                        ThinWedgeErr::InvalidRequest(body_text)
                    }
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return ThinWedgeErr::UsageLimitReached(UsageLimitReachedError {
                    ...
                    ThinWedgeErr::RetryLimit(RetryLimitReachedError {
                        status,
            ...
            TransportError::RetryLimit => ThinWedgeErr::RetryLimit(RetryLimitReachedError {
        ...
        ApiError::RateLimit(msg) => ThinWedgeErr::Stream(msg, None),
    }
}

const ACTIVE_LIMIT_HEADER: &str = "x-thinwedge-active-limit";
