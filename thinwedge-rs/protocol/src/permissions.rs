...
const PROTECTED_METADATA_THINWEDGE_PATH_NAME: &str = ".thinwedge";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_THINWEDGE_PATH_NAME,
];
...
pub fn is_protected_metadata_directory_name(name: &OsStr) -> bool {
    name == OsStr::new(PROTECTED_METADATA_AGENTS_PATH_NAME)
        || name == OsStr::new(PROTECTED_METADATA_THINWEDGE_PATH_NAME)
}
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer ThinWedge degrades to warn-and-ignore
    /// instead of failing to load. ThinWedge 0.112.0 rejected unknown values here,
...
const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "thinwedge-project-roots://";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_dot_thinwedge*/ false,
            ) {
    ...
    pub fn from_legacy_sandbox_policy_for_cwd(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_thinwedge*/ true,
                ) {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_dot_thinwedge*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(
                    &mut self.entries,
                    protected_path,
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            let protect_missing_dot_thinwedge = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| normalize_effective_absolute_path(cwd) == root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_thinwedge)
                    .into_iter()
            ...
            // Example: if `<root>/.thinwedge -> <root>/decoy`, bwrap must still see
            // `<root>/.thinwedge`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.thinwedge` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_thinwedge: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .thinwedge read-only to the agent by
    ...
    let top_level_thinwedge = writable_root.join(PROTECTED_METADATA_THINWEDGE_PATH_NAME);
    if protect_missing_dot_thinwedge || top_level_thinwedge.as_path().is_dir() {
        subpaths.push(top_level_thinwedge);
    }
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "THINWEDGE_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_proactively_protect_missing_dot_thinwedge() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let expected_dot_thinwedge = expected_root.join(".thinwedge");
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .contains(&expected_dot_thinwedge)
        );
    }
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry {
                    path: FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".thinwedge".into())),
                    },
                    access: FileSystemAccessMode::Read,
                },
            ])
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_thinwedge_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_thinwedge = expected_root.join(".thinwedge");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_dot_thinwedge.clone(),
                },
                access: FileSystemAccessMode::Write,
            },
        ]);
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".thinwedge".to_string()),
            "explicit .thinwedge rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_thinwedge_config = cwd.path().join(".thinwedge").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_thinwedge_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_thinwedge_config));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        expected_entries.extend(
            default_read_only_subpaths_for_writable_root(
                &expected_root,
                /*protect_missing_dot_thinwedge*/ true,
            )
            .into_iter()
            .map(|path| FileSystemSandboxEntry {
                path: FileSystemPath::Path { path },
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".thinwedge/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let thinwedge_dir = real_root.join(".thinwedge");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&thinwedge_dir).expect("create .thinwedge");
        ...
        let expected_thinwedge = link_root.join(".thinwedge");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_thinwedge = expected_root.join(".thinwedge");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-thinwedge");
        let dot_thinwedge = root.join(".thinwedge");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_thinwedge).expect("create .thinwedge symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_thinwedge = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".thinwedge"),
        )
        .expect("absolute .thinwedge symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let thinwedge_dir = real_tmpdir.join(".thinwedge");
