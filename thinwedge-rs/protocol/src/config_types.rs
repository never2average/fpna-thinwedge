...
/// Validated plain profile-v2 name used to select `$THINWEDGE_HOME/<name>.config.toml`.
...
/// Collaboration mode for a ThinWedge session.
...
mod tests {
    ...
    fn apply_mask_can_clear_optional_fields() {
        let mode = CollaborationMode {
            ...
            settings: Settings {
                model: "gpt-5.2-thinwedge".to_string(),
