...
pub type Result<T> = std::result::Result<T, ThinWedgeErr>;
...
pub enum ThinWedgeErr {
    ...
    #[error(
        "ThinWedge ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `ThinWedgeError::Spawn` variant.
    ...
    QuotaExceeded,
    #[error(
        "To use ThinWedge with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("thinwedge-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for ThinWedgeErr {
    fn from(_: CancelErr) -> Self {
        ThinWedgeErr::TurnAborted
    }
}
...
impl ThinWedgeErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            ThinWedgeErr::TurnAborted
            | ThinWedgeErr::Interrupted
            | ThinWedgeErr::EnvVar(_)
            | ThinWedgeErr::Fatal(_)
            | ThinWedgeErr::UsageNotIncluded
            | ThinWedgeErr::QuotaExceeded
            | ThinWedgeErr::InvalidImageRequest()
            | ThinWedgeErr::InvalidRequest(_)
            | ThinWedgeErr::RefreshTokenFailed(_)
            | ThinWedgeErr::UnsupportedOperation(_)
            | ThinWedgeErr::Sandbox(_)
            | ThinWedgeErr::LandlockSandboxExecutableNotProvided
            | ThinWedgeErr::RetryLimit(_)
            | ThinWedgeErr::ContextWindowExceeded
            | ThinWedgeErr::ThreadNotFound(_)
            | ThinWedgeErr::AgentLimitReached { .. }
            | ThinWedgeErr::Spawn
            | ThinWedgeErr::SessionConfiguredNotFirstEvent
            | ThinWedgeErr::UsageLimitReached(_)
            | ThinWedgeErr::ServerOverloaded
            | ThinWedgeErr::CyberPolicy { .. } => false,
            ThinWedgeErr::Stream(..)
            | ThinWedgeErr::Timeout
            | ThinWedgeErr::RequestTimeout
            | ThinWedgeErr::UnexpectedStatus(_)
            | ThinWedgeErr::ResponseStreamFailed(_)
            | ThinWedgeErr::ConnectionFailed(_)
            | ThinWedgeErr::InternalServerError
            | ThinWedgeErr::InternalAgentDied
            | ThinWedgeErr::Io(_)
            | ThinWedgeErr::Json(_)
            | ThinWedgeErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            ThinWedgeErr::LandlockRuleset(_) | ThinWedgeErr::LandlockPathFd(_) => false,
        }
    }

    /// Minimal shim so that existing `e.downcast_ref::<ThinWedgeErr>()` checks continue to compile
    ...
    pub fn to_thinwedge_protocol_error(&self) -> ThinWedgeErrorInfo {
        match self {
            ThinWedgeErr::ContextWindowExceeded => ThinWedgeErrorInfo::ContextWindowExceeded,
            ThinWedgeErr::UsageLimitReached(_)
            | ThinWedgeErr::QuotaExceeded
            | ThinWedgeErr::UsageNotIncluded => ThinWedgeErrorInfo::UsageLimitExceeded,
            ThinWedgeErr::ServerOverloaded => ThinWedgeErrorInfo::ServerOverloaded,
            ThinWedgeErr::CyberPolicy { .. } => ThinWedgeErrorInfo::CyberPolicy,
            ThinWedgeErr::RetryLimit(_) => ThinWedgeErrorInfo::ResponseTooManyFailedAttempts {
            ...
            ThinWedgeErr::SessionConfiguredNotFirstEvent
            | ThinWedgeErr::InternalServerError
            | ThinWedgeErr::InternalAgentDied => ThinWedgeErrorInfo::InternalServerError,
            ThinWedgeErr::UnsupportedOperation(_)
            | ThinWedgeErr::ThreadNotFound(_)
            | ThinWedgeErr::AgentLimitReached { .. } => ThinWedgeErrorInfo::BadRequest,
            ThinWedgeErr::Sandbox(_) => ThinWedgeErrorInfo::SandboxError,
            _ => ThinWedgeErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            thinwedge_error_info: Some(self.to_thinwedge_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            ThinWedgeErr::RetryLimit(err) => Some(err.status),
            ThinWedgeErr::UnexpectedStatus(err) => Some(err.status),
            ThinWedgeErr::ConnectionFailed(err) => err.source.status(),
            ThinWedgeErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("thinwedge")
        {
            return write!(
                f,
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/thinwedge/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            Some(PlanType::Known(
                KnownPlan::Team
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using ThinWedge (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
            Some(PlanType::Known(KnownPlan::Pro | KnownPlan::ProLite)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/thinwedge/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
pub fn get_error_message_ui(e: &ThinWedgeErr) -> String {
    let message = match e {
        ThinWedgeErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        ThinWedgeErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
