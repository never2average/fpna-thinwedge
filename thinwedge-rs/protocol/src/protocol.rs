//! Defines the protocol for a ThinWedge session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for ThinWedge:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::thinwedge_thread::ThinWedgeThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// This is a local-only operation handled by thinwedge-core; it does not
    /// involve the model.
    ...
    /// Request ThinWedge to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down thinwedge instance.
    Shutdown,
...
/// running the command proposed by ThinWedge.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.thinwedge`, `.git`, notably `.git/hooks`) under
...
/// ThinWedge errors that we expose to clients.
...
impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.thinwedge_error_info
            .as_ref()
            .is_none_or(ThinWedgeErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::ThinWedge),
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                thinwedge_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_thinwedge() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("thinwedge".to_string()).restriction_product(),
            Some(Product::ThinWedge)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::ThinWedge])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::ThinWedge]));
        assert!(
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_thinwedge.as_path())
        );
    }
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_thinwedge.to_path_buf(),
