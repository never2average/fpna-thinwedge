...
use thinwedge_utils_image::data_url_from_bytes;
...
/// Filesystem permissions for profiles where ThinWedge owns sandbox construction.
...
fn local_image_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!(
            "ThinWedge could not read the local image at `{}`: {}",
            path.display(),
            error
        ),
    }
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "ThinWedge cannot attach image at `{}`: unsupported image `{}`.",
            path.display(),
            mime
        ),
    }
...
fn convert_mcp_content_to_items(
    ...
    const THINWEDGE_IMAGE_DETAIL_META_KEY: &str = "thinwedge/imageDetail";
            ...
            Ok(McpContent::Image {
                ...
                FunctionCallOutputContentItem::InputImage {
                    ...
                    detail: meta
                        ...
                        .and_then(|meta| meta.get(THINWEDGE_IMAGE_DETAIL_META_KEY))
...
// (Moved event mapping logic into thinwedge-core to avoid coupling protocol to UI-facing events.)
...
mod tests {
    ...
    fn function_call_deserializes_optional_namespace() {
        let item: ResponseItem = serde_json::from_value(serde_json::json!({
            "type": "function_call",
            "name": "mcp__thinwedge_apps__gmail_get_recent_emails",
            "namespace": "mcp__thinwedge_apps__gmail",
            "arguments": "{\"top_k\":5}",
            "call_id": "call-1",
        }))
        ...
        assert_eq!(
            ...
            ResponseItem::FunctionCall {
                id: None,
                name: "mcp__thinwedge_apps__gmail_get_recent_emails".to_string(),
                namespace: Some("mcp__thinwedge_apps__gmail".to_string()),
        ...
        for i in 0..200 {
            exec_policy
                .add_prefix_rule(
                    ...
                    thinwedge_execpolicy::Decision::Allow,
                )
                .expect("add rule");
        }
    ...
    fn preserves_original_detail_metadata_on_mcp_images() -> Result<()> {
        let call_tool_result = CallToolResult {
            content: vec![serde_json::json!({
                ...
                "mimeType": "image/png",
                "_meta": {
                    "thinwedge/imageDetail": "original",
                },
            })],
            structured_content: None,
            is_error: Some(false),
    ...
    fn tool_search_output_roundtrips() -> Result<()> {
        let input = ResponseInputItem::ToolSearchOutput {
            ...
            tools: vec![serde_json::json!({
                "type": "function",
                "name": "mcp__thinwedge_apps__calendar_create_event",
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "ThinWedge cannot attach image at `{}`: unsupported image `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
