...
pub fn generate_json_with_experimental(out_dir: &Path, experimental_api: bool) -> Result<()> {
    ...
    write_pretty_json(
        out_dir.join("thinwedge_app_server_protocol.schemas.json"),
        &bundle,
    )?;
    let flat_v2_bundle = build_flat_v2_schema(&bundle)?;
    write_pretty_json(
        out_dir.join("thinwedge_app_server_protocol.v2.schemas.json"),
        &flat_v2_bundle,
    )?;
...
fn build_schema_bundle(schemas: Vec<GeneratedSchema>) -> Result<Value> {
    ...
    root.insert(
        "title".to_string(),
        Value::String("ThinWedgeAppServerProtocol".into()),
    );
...
fn build_flat_v2_schema(bundle: &Value) -> Result<Value> {
    ...
    let title = root
        ...
        .unwrap_or("ThinWedgeAppServerProtocol");
...
mod tests {
    ...
    fn stable_schema_filter_removes_mock_thread_start_field() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("thinwedge_schema_{}", Uuid::now_v7()));
    ...
    fn build_flat_v2_schema_keeps_shared_root_schemas_and_dependencies() -> Result<()> {
        let bundle = serde_json::json!({
            ...
            "title": "ThinWedgeAppServerProtocol",
            "type": "object",
            "definitions": {
                "ClientRequest": {
                    "oneOf": [
        ...
        assert_eq!(
            flat_bundle["title"],
            serde_json::json!("ThinWedgeAppServerProtocolV2")
        );
...
"#;
    ...
    fn generate_json_filters_experimental_fields_and_methods() -> Result<()> {
        ...
        let bundle_json =
            fs::read_to_string(output_dir.join("thinwedge_app_server_protocol.schemas.json"))?;
        ...
        let flat_v2_bundle_json =
            fs::read_to_string(output_dir.join("thinwedge_app_server_protocol.v2.schemas.json"))?;
        ...
        assert_eq!(
            flat_v2_bundle_json.contains("\"title\": \"ThinWedgeAppServerProtocolV2\""),
            true
        );
        let flat_v2_bundle =
            read_json_value(&output_dir.join("thinwedge_app_server_protocol.v2.schemas.json"))?;
