...
pub struct TurnStartParams {
    ...
    /// Optional metadata to enrich ThinWedge's ResponsesAPI turn metadata.
    ///
    /// Entries are flattened into the JSON string sent as
    /// `client_metadata["x-thinwedge-turn-metadata"]` on ResponsesAPI HTTP and websocket requests.
    ///
