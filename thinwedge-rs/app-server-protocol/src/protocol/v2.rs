...
use thinwedge_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use thinwedge_protocol::approvals::ExecPolicyAmendment as CoreExecPolicyAmendment;
use thinwedge_protocol::approvals::GuardianAssessmentAction as CoreGuardianAssessmentAction;
use thinwedge_protocol::approvals::GuardianAssessmentDecisionSource as CoreGuardianAssessmentDecisionSource;
use thinwedge_protocol::approvals::GuardianCommandSource as CoreGuardianCommandSource;
use thinwedge_protocol::approvals::NetworkApprovalContext as CoreNetworkApprovalContext;
use thinwedge_protocol::approvals::NetworkApprovalProtocol as CoreNetworkApprovalProtocol;
use thinwedge_protocol::approvals::NetworkPolicyAmendment as CoreNetworkPolicyAmendment;
use thinwedge_protocol::approvals::NetworkPolicyRuleAction as CoreNetworkPolicyRuleAction;
...
use thinwedge_protocol::config_types::CollaborationModeMask as CoreCollaborationModeMask;
...
use thinwedge_protocol::config_types::SandboxMode as CoreSandboxMode;
...
use thinwedge_protocol::items::AgentMessageContent as CoreAgentMessageContent;
use thinwedge_protocol::items::TurnItem as CoreTurnItem;
use thinwedge_protocol::mcp::CallToolResult as CoreMcpCallToolResult;
use thinwedge_protocol::mcp::Resource as McpResource;
pub use thinwedge_protocol::mcp::ResourceContent as McpResourceContent;
use thinwedge_protocol::mcp::ResourceTemplate as McpResourceTemplate;
use thinwedge_protocol::mcp::Tool as McpTool;
use thinwedge_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use thinwedge_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
use thinwedge_protocol::models::AdditionalPermissionProfile as CoreAdditionalPermissionProfile;
use thinwedge_protocol::models::FileSystemPermissions as CoreFileSystemPermissions;
use thinwedge_protocol::models::ManagedFileSystemPermissions as CoreManagedFileSystemPermissions;
...
use thinwedge_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use thinwedge_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use thinwedge_protocol::permissions::NetworkSandboxPolicy as CoreNetworkSandboxPolicy;
use thinwedge_protocol::plan_tool::PlanItemArg as CorePlanItemArg;
...
use thinwedge_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use thinwedge_protocol::protocol::GuardianRiskLevel as CoreGuardianRiskLevel;
use thinwedge_protocol::protocol::GuardianUserAuthorization as CoreGuardianUserAuthorization;
use thinwedge_protocol::protocol::HookEventName as CoreHookEventName;
use thinwedge_protocol::protocol::HookExecutionMode as CoreHookExecutionMode;
use thinwedge_protocol::protocol::HookHandlerType as CoreHookHandlerType;
use thinwedge_protocol::protocol::HookOutputEntry as CoreHookOutputEntry;
use thinwedge_protocol::protocol::HookOutputEntryKind as CoreHookOutputEntryKind;
use thinwedge_protocol::protocol::HookRunStatus as CoreHookRunStatus;
use thinwedge_protocol::protocol::HookRunSummary as CoreHookRunSummary;
use thinwedge_protocol::protocol::HookScope as CoreHookScope;
use thinwedge_protocol::protocol::HookSource as CoreHookSource;
use thinwedge_protocol::protocol::ModelRerouteReason as CoreModelRerouteReason;
use thinwedge_protocol::protocol::ModelVerification as CoreModelVerification;
use thinwedge_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use thinwedge_protocol::protocol::NonSteerableTurnKind as CoreNonSteerableTurnKind;
use thinwedge_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
use thinwedge_protocol::protocol::RateLimitReachedType as CoreRateLimitReachedType;
use thinwedge_protocol::protocol::RateLimitSnapshot as CoreRateLimitSnapshot;
use thinwedge_protocol::protocol::RateLimitWindow as CoreRateLimitWindow;
use thinwedge_protocol::protocol::RealtimeAudioFrame as CoreRealtimeAudioFrame;
...
use thinwedge_protocol::protocol::ReviewDecision as CoreReviewDecision;
use thinwedge_protocol::protocol::SessionSource as CoreSessionSource;
use thinwedge_protocol::protocol::SkillDependencies as CoreSkillDependencies;
use thinwedge_protocol::protocol::SkillInterface as CoreSkillInterface;
use thinwedge_protocol::protocol::SkillMetadata as CoreSkillMetadata;
...
/// This translation layer make sure that we expose thinwedge error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `thinwedgeErrorInfo` variant.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub enum ThinWedgeErrorInfo {
...
impl From<CoreThinWedgeErrorInfo> for ThinWedgeErrorInfo {
    fn from(value: CoreThinWedgeErrorInfo) -> Self {
        match value {
            ...
            CoreThinWedgeErrorInfo::HttpConnectionFailed { http_status_code } => {
                ThinWedgeErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreThinWedgeErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                ThinWedgeErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            ...
            CoreThinWedgeErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                ThinWedgeErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreThinWedgeErrorInfo::ActiveTurnNotSteerable { turn_kind } => {
                ThinWedgeErrorInfo::ActiveTurnNotSteerable {
                    turn_kind: turn_kind.into(),
                }
            }
            CoreThinWedgeErrorInfo::Other => ThinWedgeErrorInfo::Other,
        }
    }
}
...
v2_enum_from_core!(
    pub enum ReviewDelivery from thinwedge_protocol::protocol::ReviewDelivery {
        Inline, Detached
    }
);

v2_enum_from_core!(
    pub enum McpAuthStatus from thinwedge_protocol::protocol::McpAuthStatus {
        Unsupported,
        NotLoggedIn,
        BearerToken,
        OAuth
...
pub enum ConfigLayerSource {
    ...
    /// User config layer from $THINWEDGE_HOME/config.toml. This layer is special
    ...
    /// Path to a .thinwedge/ folder within a project. There could be multiple of
    /// these between `cwd` and the project/repo root.
    #[serde(rename_all = "camelCase")]
    #[ts(rename_all = "camelCase")]
    Project {
        dot_thinwedge_folder: AbsolutePathBuf,
    },
...
pub struct ExternalAgentConfigDetectParams {
    /// If true, include detection under the user's home (~/.claude, ~/.thinwedge, etc.).
    #[serde(default, skip_serializing_if = "std::ops::Not::not")]
...
pub enum PermissionProfile {
    /// ThinWedge owns sandbox construction for this profile.
    #[serde(rename_all = "camelCase")]
    #[ts(rename_all = "camelCase")]
    Managed {
...
impl SandboxPolicy {
    pub fn to_core(&self) -> thinwedge_protocol::protocol::SandboxPolicy {
        match self {
...
impl From<thinwedge_protocol::protocol::SandboxPolicy> for SandboxPolicy {
    fn from(value: thinwedge_protocol::protocol::SandboxPolicy) -> Self {
        match value {
            thinwedge_protocol::protocol::SandboxPolicy::DangerFullAccess => {
                SandboxPolicy::DangerFullAccess
            }
            thinwedge_protocol::protocol::SandboxPolicy::ReadOnly { network_access } => {
                SandboxPolicy::ReadOnly { network_access }
            }
            thinwedge_protocol::protocol::SandboxPolicy::ExternalSandbox { network_access } => {
...
pub struct GetAccountRateLimitsResponse {
    ...
    /// Multi-bucket view keyed by metered `limit_id` (for example, `thinwedge`).
...
pub struct ThreadStartParams {
    ...
    /// This is for internal use only (e.g. ThinWedge Cloud).
...
pub struct ThreadResumeParams {
    ...
    /// [UNSTABLE] FOR THINWEDGE CLOUD - DO NOT USE.
...
impl From<thinwedge_protocol::protocol::ThreadGoal> for ThreadGoal {
    fn from(value: thinwedge_protocol::protocol::ThreadGoal) -> Self {
        Self {
...
impl ThreadMemoryMode {
    ...
    pub fn to_core(self) -> thinwedge_protocol::protocol::ThreadMemoryMode {
        match self {
            Self::Enabled => thinwedge_protocol::protocol::ThreadMemoryMode::Enabled,
            Self::Disabled => thinwedge_protocol::protocol::ThreadMemoryMode::Disabled,
        }
    }
}
...
pub struct ThreadApproveGuardianDeniedActionParams {
    pub thread_id: String,
    /// Serialized `thinwedge_protocol::protocol::GuardianAssessmentEvent`.
    pub event: JsonValue,
}
...
pub struct Thread {
    ...
    /// Origin of the thread (CLI, VSCode, thinwedge exec, thinwedge app-server, etc.).
...
pub struct TurnError {
    pub message: String,
    pub thinwedge_error_info: Option<ThinWedgeErrorInfo>,
    #[serde(default)]
...
impl From<thinwedge_protocol::models::WebSearchAction> for WebSearchAction {
    fn from(value: thinwedge_protocol::models::WebSearchAction) -> Self {
        match value {
            ...
            thinwedge_protocol::models::WebSearchAction::Other => WebSearchAction::Other,
        }
    }
}
...
impl From<thinwedge_protocol::items::HookPromptFragment> for HookPromptFragment {
    fn from(value: thinwedge_protocol::items::HookPromptFragment) -> Self {
        Self {
            text: value.text,
...
impl McpServerElicitationAction {
    pub fn to_core(self) -> thinwedge_protocol::approvals::ElicitationAction {
        match self {
            Self::Accept => thinwedge_protocol::approvals::ElicitationAction::Accept,
            Self::Decline => thinwedge_protocol::approvals::ElicitationAction::Decline,
            Self::Cancel => thinwedge_protocol::approvals::ElicitationAction::Cancel,
        }
    }
}
...
pub struct McpServerElicitationRequestParams {
    pub thread_id: String,
    /// Active ThinWedge turn when this elicitation was observed, if app-server could correlate one.
    ///
...
impl From<DynamicToolCallOutputContentItem>
    for thinwedge_protocol::dynamic_tools::DynamicToolCallOutputContentItem
{
...
mod tests {
    ...
    use thinwedge_protocol::items::WebSearchItem;
    use thinwedge_protocol::models::WebSearchAction as CoreWebSearchAction;
    use thinwedge_protocol::protocol::NetworkAccess as CoreNetworkAccess;
    use thinwedge_protocol::user_input::UserInput as CoreUserInput;
    ...
    fn device_key_sign_params_round_trip_uses_accepted_payload_enum() {
        let params = DeviceKeySignParams {
            ...
            payload: DeviceKeySignPayload::RemoteControlClientConnection {
                ...
                target_path: "/api/thinwedge/remote/control/client".to_string(),
                account_user_id: "account-user-1".to_string(),
                client_id: "cli_123".to_string(),
        ...
        assert_eq!(
            ...
            json!({
                ...
                "payload": {
                    ...
                    "targetPath": "/api/thinwedge/remote/control/client",
                    "accountUserId": "account-user-1",
                    "clientId": "cli_123",
        ...
        let user_item = TurnItem::UserMessage(UserMessageItem {
            ...
            content: vec![
                ...
                CoreUserInput::Skill {
                    ...
                    path: PathBuf::from("/repo/.thinwedge/skills/skill-creator/SKILL.md"),
                },
    ...
    #[test]
    fn thinwedge_error_info_serializes_http_status_code_in_camel_case() {
        let value = ThinWedgeErrorInfo::ResponseTooManyFailedAttempts {
    ...
    #[test]
    fn thinwedge_error_info_serializes_cyber_policy_in_camel_case() {
        assert_eq!(
            serde_json::to_value(ThinWedgeErrorInfo::CyberPolicy).unwrap(),
            json!("cyberPolicy")
        );
    }

    #[test]
    fn thinwedge_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
        let value = ThinWedgeErrorInfo::ActiveTurnNotSteerable {
