...
/// Special argv[1] flag used when the ThinWedge executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `thinwedge-apply-patch` (to avoid forcing
/// `thinwedge-arg0` to depend on `thinwedge-core`), it remains part of the "thinwedge core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const THINWEDGE_CORE_APPLY_PATCH_ARG1: &str = "--thinwedge-run-as-apply-patch";
