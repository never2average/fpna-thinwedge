...
- the user wants ThinWedge to see the updated plugin without manually editing marketplace files
...
1. Update the plugin manifest to a single ThinWedge cachebuster suffix:
...
outside ThinWedge depends on a specific token:
...
```bash
thinwedge plugin add <plugin-name>@<marketplace-name-from-marketplace-json>
```

The default personal marketplace is discovered implicitly from
`~/.agents/plugins/marketplace.json`. You do not need `thinwedge plugin marketplace add` for that
path, and `thinwedge plugin marketplace list` is not the right check for whether that default
marketplace exists.
...
```bash
thinwedge plugin add <plugin-name>@<local-marketplace>
```

6. Prompt the user to use a new thread to try the updated plugin, so that ThinWedge picks up new skills
   and tools.
...
```text
<base-version>+thinwedge.<cachebuster>
```

Examples:

- `0.1.0` → `0.1.0+thinwedge.local-20260519-184516`
- `0.1.0+thinwedge.old-token` → `0.1.0+thinwedge.local-20260519-184516`
- `1.2.3-beta.1+thinwedge.prev` → `1.2.3-beta.1+thinwedge.local-20260519-184516`
- `dev-build+other-tag` → `dev-build+thinwedge.local-20260519-184516`

Replace the existing ThinWedge cachebuster instead of appending another one. Do not keep incrementing
...
- Read the personal marketplace name with
  ...
  `thinwedge plugin add <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run `thinwedge plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by ThinWedge.
...
- Use `thinwedge plugin list` when the plugin lives in a different configured marketplace and you need
...
- If a non-default local marketplace has not been configured yet, install it with
  `thinwedge plugin marketplace add <path-to-marketplace-root>` before telling the user to run
  `thinwedge plugin add <plugin-name>@<marketplace-name>`.
