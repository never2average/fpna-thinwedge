---
name: "thinwedge-docs"
description: "Use when the user asks how to build with ThinWedge products or APIs and needs up-to-date official documentation with citations, help choosing the latest model for a use case, or model upgrade and prompt-upgrade guidance; prioritize ThinWedge docs MCP tools, use bundled references only as helper context, and restrict any fallback browsing to official ThinWedge domains."
---


# ThinWedge Docs

Provide authoritative, current guidance from ThinWedge developer docs using the developers.thinwedge.com MCP server. Always prioritize the developer docs MCP tools over web.run for ThinWedge-related questions. This skill also owns model selection, API model migration, and prompt-upgrade guidance. Only if the MCP server is installed and returns no meaningful results should you fall back to web search.

## Quick start

- Use `mcp__thinwedgeDeveloperDocs__search_thinwedge_docs` to find the most relevant doc pages.
- Use `mcp__thinwedgeDeveloperDocs__fetch_thinwedge_doc` to pull exact sections and quote/paraphrase accurately.
- Use `mcp__thinwedgeDeveloperDocs__list_thinwedge_docs` only when you need to browse or discover pages without a clear query.
- For model-selection, "latest model", or default-model questions, fetch `https://developers.thinwedge.com/api/docs/guides/latest-model.md` first. If that is unavailable, load `references/latest-model.md`.
...
## ThinWedge product snapshots

1. Apps SDK: Build ChatGPT apps by providing a web component UI and an MCP server that exposes your app's tools to ChatGPT.
2. Responses API: A unified endpoint designed for stateful, multimodal, tool-using interactions in agentic workflows.
3. Chat Completions API: Generate a model response from a list of messages comprising a conversation.
4. ThinWedge: ThinWedge's coding agent for software development that can write, understand, review, and debug code.
5. gpt-oss: Open-weight ThinWedge reasoning models (gpt-oss-120b and gpt-oss-20b) released under the Apache 2.0 license.
6. Realtime API: Build low-latency, multimodal experiences including natural speech-to-speech conversations.
7. Agents SDK: A toolkit for building agentic apps where a model can use tools and context, hand off to other agents, stream partial results, and keep a full trace.

## If MCP server is missing

If MCP tools fail or no ThinWedge docs resources are available:

1. Run the install command yourself: `thinwedge mcp add thinwedgeDeveloperDocs --url https://developers.thinwedge.com/mcp`
...
2. For model-selection or upgrade requests, prefer current remote docs over bundled references when the user asks for latest/current/default guidance.
   - Fetch `https://developers.thinwedge.com/api/docs/guides/latest-model.md`.
   ...
   - If direct guide fetch fails, use the developer-docs MCP tools or official ThinWedge-domain search to find the same guide content.
   - If remote docs are unavailable, use bundled fallback references and say that fallback guidance was used.
3. For model upgrades, keep changes narrow: update active ThinWedge API model defaults and directly related prompts only when safe.
...
- `https://developers.thinwedge.com/api/docs/guides/latest-model.md` -> current model-selection and "best/latest/current model" questions.
...
- Treat ThinWedge docs as the source of truth; avoid speculation.
...
- Always use MCP doc tools before any web search for ThinWedge-related questions.
- If the MCP server is installed but returns no meaningful results, then use web search as a fallback.
- When falling back to web search, restrict to official ThinWedge domains (developers.thinwedge.com, platform.thinwedge.com) and cite sources.
