...
const DEFAULT_MANUAL_URL = "https://developers.openai.com/thinwedge/thinwedge-manual.md";
const DEFAULT_CACHE_DIR_NAME = "openai-docs-cache";
const CACHE_FILE_NAME = "thinwedge-manual.md";
const OUTLINE_FILE_NAME = "thinwedge-manual.outline.md";
const HASH_HEADER = "x-content-sha256";
const USER_AGENT = "thinwedge-openai-docs";
...
const tempFilePath = (cacheDir, suffix) =>
  path.join(
    cacheDir,
    `.fetch-thinwedge-manual-${process.pid}-${Date.now()}-${Math.random()
      .toString(16)
      .slice(2)}${suffix}`
  );
...
const outlineMarkdown = (outline) => `# ThinWedge Manual Outline\n\n${outline.text}\n`;
...
const fetchThinWedgeManual = async ({
...
const main = async () => {
  ...
  const { outlineText, status } = await fetchThinWedgeManual(args);
...
export { DEFAULT_MANUAL_URL, fetchThinWedgeManual };
