#!/usr/bin/env python3
"""
ThinWedge YAML Generator - Creates agents/thinwedge.yaml for a skill folder.

Usage:
    generate_thinwedge_yaml.py <skill_dir> [--name <skill_name>] [--interface key=value]
"""
...
def write_thinwedge_yaml(skill_dir, skill_name, raw_overrides):
    ...
    output_path = agents_dir / "thinwedge.yaml"
    output_path.write_text("\n".join(interface_lines) + "\n")
    print(f"[OK] Created agents/thinwedge.yaml")
    return output_path


def main():
    parser = argparse.ArgumentParser(
        description="Create agents/thinwedge.yaml for a skill directory.",
    )
    ...
    result = write_thinwedge_yaml(skill_dir, skill_name, args.interface)
    if result:
        sys.exit(0)
    sys.exit(1)
