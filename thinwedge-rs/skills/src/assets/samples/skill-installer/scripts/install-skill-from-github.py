#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $THINWEDGE_HOME/skills."""
...
def _thinwedge_home() -> str:
    return os.environ.get("THINWEDGE_HOME", os.path.expanduser("~/.thinwedge"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "thinwedge")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "thinwedge-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_thinwedge_home(), "skills")
