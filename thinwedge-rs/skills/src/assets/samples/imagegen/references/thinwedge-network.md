# ThinWedge network approvals / sandbox notes
...
- In `workspace-write`, network access still depends on your ThinWedge configuration (for example `[sandbox_workspace_write] network_access = true`).
...
Example `~/.thinwedge/config.toml` pattern:
