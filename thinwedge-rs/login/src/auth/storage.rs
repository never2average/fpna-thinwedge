...
pub(super) fn get_auth_file(thinwedge_home: &Path) -> PathBuf {
    thinwedge_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(thinwedge_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(thinwedge_home: PathBuf) -> Self {
        Self { thinwedge_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `THINWEDGE_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.thinwedge_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.thinwedge_home)
    }
}
...
fn compute_store_key(thinwedge_home: &Path) -> std::io::Result<String> {
...
// A global in-memory store for mapping thinwedge_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(thinwedge_home: PathBuf) -> Self {
        Self { thinwedge_home }
    }
