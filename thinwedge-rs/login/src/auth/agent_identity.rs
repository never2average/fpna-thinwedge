...
const PROD_AGENT_IDENTITY_AUTHAPI_BASE_URL: &str = "https://auth.example.invalid/api/accounts";
...
fn agent_identity_authapi_base_url() -> String {
    env::var(THINWEDGE_AGENT_IDENTITY_AUTHAPI_BASE_URL_ENV_VAR)
        .ok()
...
mod tests {
    ...
    fn agent_identity_authapi_base_url_prefers_env_value() {
        let _guard = EnvVarGuard::set(
            THINWEDGE_AGENT_IDENTITY_AUTHAPI_BASE_URL_ENV_VAR,
    ...
    fn agent_identity_authapi_base_url_uses_prod_authapi_by_default() {
        let _guard = EnvVarGuard::remove(THINWEDGE_AGENT_IDENTITY_AUTHAPI_BASE_URL_ENV_VAR);
        assert_eq!(
            agent_identity_authapi_base_url(),
