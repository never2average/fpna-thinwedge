...
use thinwedge_protocol::auth::KnownPlan as InternalKnownPlan;
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = ThinWedgeAuth::from_auth_dot_json(
        thinwedge_home.path(),
...
fn write_auth_file(params: AuthFileParams, thinwedge_home: &Path) -> std::io::Result<String> {
...
async fn load_auth_reads_agent_identity_from_env() {
    ...
    let ThinWedgeAuth::AgentIdentity(agent_identity) = auth else {
...
fn fake_agent_identity_jwt_with_plan_type(
    ...
    let payload = json!({
        "iss": "https://chatgpt.com/thinwedge-backend/agent-identity",
...
const TEST_AGENT_IDENTITY_RSA_PRIVATE_KEY_PEM: &[u8] = br#"PUBLIC_HISTORY_TEST_PRIVATE_KEY_REMOVED"#;
