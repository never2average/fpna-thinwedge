...
use thinwedge_app_server_protocol::AuthMode as ApiAuthMode;
...
impl ThinWedgeAuth {
    ...
    /// Returns `None` if ThinWedge backend auth does not expose an account id.
    ...
    /// Returns false if ThinWedge backend auth omits the FedRAMP claim.
    ...
    /// Returns `None` if ThinWedge backend auth does not expose an account email.
    ...
    /// Returns `None` if ThinWedge backend auth does not expose a ChatGPT user id.
...
impl ChatgptAuth {
    ...
    fn client(&self) -> &ThinWedgeHttpClient {
        &self.state.client
    }
}
...
/// Delete the auth.json file inside `thinwedge_home` if it exists. Returns `Ok(true)`
...
impl Debug for CachedAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CachedAuth")
            .field(
                "auth_mode",
                &self.auth.as_ref().map(ThinWedgeAuth::api_auth_mode),
            )
            .field(
...
impl UnauthorizedRecovery {
    fn new(manager: Arc<AuthManager>) -> Self {
        ...
        let expected_account_id = cached_auth.as_ref().and_then(ThinWedgeAuth::get_account_id);
        let mode = if manager.has_external_api_key_auth()
            || cached_auth
                .as_ref()
                .is_some_and(ThinWedgeAuth::is_external_chatgpt_tokens)
        {
...
/// hands out cloned `ThinWedgeAuth` values so the rest of the program has a
/// consistent snapshot.
///
...
/// `thinwedge_core::config::Config`, but this trait keeps `thinwedge-login` independent
/// from `thinwedge-core`.
pub trait AuthManagerConfig {
    /// Returns the ThinWedge home directory used for auth storage.
...
impl AuthManager {
    ...
    /// Create an AuthManager with a specific ThinWedgeAuth, for testing only.
    pub fn from_auth_for_testing(auth: ThinWedgeAuth) -> Arc<Self> {
    ...
    async fn reload_if_account_id_matches(
        ...
        let new_auth = self.load_auth_from_storage().await;
        let new_account_id = new_auth.as_ref().and_then(ThinWedgeAuth::get_account_id);
    ...
    fn auths_equal_for_refresh(a: Option<&ThinWedgeAuth>, b: Option<&ThinWedgeAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => match (a.api_auth_mode(), b.api_auth_mode()) {
                ...
                (ApiAuthMode::AgentIdentity, ApiAuthMode::AgentIdentity) => match (a, b) {
                    (ThinWedgeAuth::AgentIdentity(a), ThinWedgeAuth::AgentIdentity(b)) => {
                        a.record() == b.record()
                    }
                    _ => false,
                },
    ...
    fn auths_equal(a: Option<&ThinWedgeAuth>, b: Option<&ThinWedgeAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    fn set_cached_auth(&self, new_auth: Option<ThinWedgeAuth>) -> bool {
        if let Ok(mut guard) = self.inner.write() {
    ...
    pub fn thinwedge_api_key_env_enabled(&self) -> bool {
        self.enable_thinwedge_api_key_env
    }
    ...
    async fn resolve_external_api_key_auth(&self) -> Option<ThinWedgeAuth> {
        ...
        match external_auth.resolve().await {
            Ok(Some(tokens)) => Some(ThinWedgeAuth::from_api_key(&tokens.access_token)),
            Ok(None) => None,
            Err(err) => {
    ...
    pub async fn refresh_token(&self) -> Result<(), RefreshTokenError> {
        ...
        let expected_account_id = auth_before_reload
            .as_ref()
            .and_then(ThinWedgeAuth::get_account_id);
    ...
    async fn refresh_token_from_authority_impl(&self) -> Result<(), RefreshTokenError> {
        ...
        let result = match auth {
            ...
            ThinWedgeAuth::Chatgpt(chatgpt_auth) => {
    ...
    pub fn auth_mode(&self) -> Option<AuthMode> {
        ...
        self.auth_cached().as_ref().map(ThinWedgeAuth::auth_mode)
    }
