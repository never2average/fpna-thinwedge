//! Default ThinWedge HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! reqwest/`ThinWedgeHttpClient` construction.
//!
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(THINWEDGE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(THINWEDGE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "thinwedge-tui"
        || originator_value == "thinwedge_vscode"
        || originator_value.starts_with("ThinWedge ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "thinwedge_atlas" || originator_value == "thinwedge_chatgpt_desktop"
}

pub fn get_thinwedge_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized ThinWedge user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base ThinWedge user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default ThinWedge originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
pub fn create_client() -> ThinWedgeHttpClient {
    let inner = build_reqwest_client();
    ThinWedgeHttpClient::new(inner)
}

/// Builds the default reqwest client used for ordinary ThinWedge HTTP traffic.
///
/// This starts from the standard ThinWedge user agent, default headers, and sandbox-specific proxy
/// policy, then layers in shared custom CA handling from `THINWEDGE_CA_CERTIFICATE` /
...
/// Tries to build the default reqwest client used for ordinary ThinWedge HTTP traffic.
///
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_thinwedge_user_agent()) {
...
fn is_sandboxed() -> bool {
    std::env::var("THINWEDGE_SANDBOX").as_deref() == Ok("seatbelt")
}
