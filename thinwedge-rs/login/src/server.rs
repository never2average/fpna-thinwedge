                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], success_url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to ThinWedge failed.",
                            io::ErrorKind::Other,
...
fn is_missing_thinwedge_entitlement_error(
    ...
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_thinwedge_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_thinwedge_entitlement_error(error_code, error_description) {
        return "ThinWedge is not enabled for your workspace. Contact your workspace administrator to request access to ThinWedge.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_thinwedge_entitlement_error(code, error_description) {
            (
                "You do not have access to ThinWedge".to_string(),
                "This account is not currently authorized to use ThinWedge in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to ThinWedge.".to_string(),
                "Contact your workspace administrator to get access to ThinWedge, then return to ThinWedge and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to ThinWedge to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
mod tests {
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        let error_description = Some("missing_thinwedge_entitlement");
        assert!(is_missing_thinwedge_entitlement_error(
            "access_denied",
            error_description
        ));
        ...
        assert!(body.contains("You do not have access to ThinWedge"));
        assert!(body.contains("Contact your workspace administrator"));
        assert!(!body.contains("missing_thinwedge_entitlement"));
    }
}
