    ...
    if !resp.status().is_success() {
        ...
        if status == StatusCode::NOT_FOUND {
            return Err(io::Error::new(
                ...
                "device code login is not enabled for this ThinWedge server. Use the browser login or verify the server URL.",
            ));
        }
...
pub async fn request_device_code(opts: &ServerOptions) -> std::io::Result<DeviceCode> {
    ...
    Ok(DeviceCode {
        verification_url: format!("{base_url}/thinwedge/device"),
