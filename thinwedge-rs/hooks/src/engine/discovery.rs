...
use thinwedge_config::HookStateToml;
...
use thinwedge_protocol::protocol::HookTrustStatus;
...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project {
            ...
            .hooks_config_folder()
            .unwrap_or_else(|| dot_thinwedge_folder.clone())
...
mod tests {
        ...
        let source = RequirementSource::Composite {
            sources: vec![
                RequirementSource::SystemRequirementsToml {
                    file: test_path_buf("/etc/thinwedge/requirements.toml").abs(),
                },
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.thinwedge/config.toml").abs();
        let dot_thinwedge_folder = test_path_buf("/tmp/worktree/.thinwedge").abs();
