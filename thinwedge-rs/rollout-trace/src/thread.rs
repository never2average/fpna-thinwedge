...
use crate::protocol_event::thinwedge_turn_trace_event;
...
pub const THINWEDGE_ROLLOUT_TRACE_ROOT_ENV: &str = "THINWEDGE_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `THINWEDGE_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the ThinWedge
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(THINWEDGE_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed ThinWedge turn lifecycle events from protocol lifecycle events.
    pub fn record_thinwedge_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            thinwedge_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    pub fn record_tool_call_event(&self, thinwedge_turn_id: impl Into<ThinWedgeTurnId>, event: &EventMsg) {
        ...
        context.append_with_context_best_effort(thinwedge_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_thinwedge_turn_id: impl Into<ThinWedgeTurnId>,
    ...
    pub fn record_thinwedge_turn_started(&self, thinwedge_turn_id: impl Into<ThinWedgeTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let thinwedge_turn_id = thinwedge_turn_id.into();
        context.append_with_context_best_effort(
            thinwedge_turn_id.clone(),
            RawTraceEventPayload::ThinWedgeTurnStarted {
                thinwedge_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    pub fn start_code_cell_trace(
        ...
        let context = self.code_cell_trace_context(thinwedge_turn_id, runtime_cell_id);
    ...
    /// Builds reusable inference trace context for one ThinWedge turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        ...
        let event_context = RawTraceEventContext {
            ...
            thinwedge_turn_id: Some(thinwedge_turn_id),
        };
