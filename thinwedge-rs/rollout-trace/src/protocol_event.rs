//! Mapping from ThinWedge protocol events into raw rollout-trace events.
//!
...
//! `thinwedge-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `thinwedge-rollout-trace`.
//!
...
pub(crate) fn thinwedge_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<ThinWedgeTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let thinwedge_turn_id = event.turn_id.clone();
            Some(ThinWedgeTurnTraceEvent {
                context_turn_id: thinwedge_turn_id.clone(),
                payload: RawTraceEventPayload::ThinWedgeTurnStarted {
                    thinwedge_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(ThinWedgeTurnTraceEvent {
                context_turn_id: thinwedge_turn_id.clone(),
                payload: RawTraceEventPayload::ThinWedgeTurnEnded {
                    thinwedge_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a thinwedge_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a thinwedge_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a thinwedge_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a thinwedge_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a thinwedge_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a thinwedge_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a thinwedge_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a thinwedge_protocol::protocol::CollabCloseEndEvent),
}
