...
impl TraceReducer {
    ...
    pub(super) fn start_inference_call(
        ...
        let thinwedge_turn_id = started.thinwedge_turn_id.clone();
        let request_payload = started.request_payload.clone();
        let Some(turn) = self.rollout.thinwedge_turns.get(&thinwedge_turn_id) else {
            bail!(
                "inference start {inference_call_id} referenced unknown thinwedge turn {thinwedge_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "inference start {inference_call_id} used thread {thread_id}, \
                 but thinwedge turn {thinwedge_turn_id} belongs to {}",
                turn.thread_id
            );
        }
    ...
    /// If a call is still `Running`, ThinWedge stopped observing that provider stream
    ...
    pub(super) fn close_running_inference_calls_for_turn_end(
        ...
        for inference in self.rollout.inference_calls.values_mut() {
            if inference.thinwedge_turn_id == thinwedge_turn_id
