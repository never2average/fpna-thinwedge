...
impl TraceReducer {
    ...
    /// Starts a ThinWedge turn inside an existing thread.
    pub(super) fn start_thinwedge_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        self.rollout.thinwedge_turns.insert(
    ...
    /// Marks a ThinWedge turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_thinwedge_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.thinwedge_turns.get(&thinwedge_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "thinwedge turn end for {thinwedge_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.thinwedge_turns.get_mut(&thinwedge_turn_id) else {
            bail!("thinwedge turn end referenced unknown turn {thinwedge_turn_id}");
        };
