        ...
        match event.payload {
            ...
            RawTraceEventPayload::CodeCellStarted {
                ...
                let thread_id = self.code_cell_event_thread_id(
                    event.thread_id,
                    event.thinwedge_turn_id.as_deref(),
                    &runtime_cell_id,
                    "code cell start",
                )?;
                ...
                self.start_or_queue_code_cell(PendingCodeCellStart {
                    ...
                    thread_id,
                    thinwedge_turn_id: event.thinwedge_turn_id,
            ...
            RawTraceEventPayload::CompactionInstalled {
                ...
                let Some(thinwedge_turn_id) = event.thinwedge_turn_id else {
                    bail!(
                        "compaction installed event {compaction_id} did not include a thinwedge turn id"
                    );
                };
