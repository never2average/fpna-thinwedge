...
use thinwedge_protocol::protocol::CollabCloseBeginEvent;
...
pub(in crate::reducer) struct ObservedAgentResultEdge {
    ...
    pub(in crate::reducer) child_thinwedge_turn_id: String,
...
impl TraceReducer {
    ...
    pub(in crate::reducer) fn queue_agent_result_interaction_edge(
        ...
        let source = if let Some(source_item_id) = self.latest_assistant_message_item_for_turn(
            ...
            &observed.child_thinwedge_turn_id,
        ) {
        ...
        self.rollout
            ...
            .filter(|item| {
                item.thread_id == thread_id
                    && item.thinwedge_turn_id.as_deref() == Some(thinwedge_turn_id)
