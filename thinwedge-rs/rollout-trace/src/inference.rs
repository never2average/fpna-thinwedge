...
const INFERENCE_CALL_ID_HEADER: &str = "x-thinwedge-inference-call-id";
...
///
/// A ThinWedge turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one ThinWedge turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        thinwedge_turn_id: ThinWedgeTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                thinwedge_turn_id: attempt.context.thinwedge_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps thinwedge-core focused on
    ...
    /// Records a provider stream that ThinWedge intentionally stopped consuming.
    ///
...
/// ThinWedge received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        thinwedge_turn_id: Some(context.thinwedge_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        writer.append(RawTraceEventPayload::ThinWedgeTurnStarted {
        ...
        assert_eq!(inference.thinwedge_turn_id, "turn-1");
