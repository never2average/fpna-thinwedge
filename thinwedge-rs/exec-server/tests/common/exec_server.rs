...
pub(crate) struct ExecServerHarness {
    ...
    _helper_paths: TestThinWedgeHelperPaths,
    child: Child,
...
pub(crate) struct TestThinWedgeHelperPaths {
    pub(crate) thinwedge_exe: PathBuf,
    pub(crate) thinwedge_linux_sandbox_exe: Option<PathBuf>,
}

pub(crate) fn test_thinwedge_helper_paths() -> anyhow::Result<TestThinWedgeHelperPaths> {
    let (helper_binary, thinwedge_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestThinWedgeHelperPaths {
        thinwedge_exe: helper_binary,
...
{
    let helper_paths = test_thinwedge_helper_paths()?;
    let thinwedge_home = TempDir::new()?;
    let mut child = Command::new(&helper_paths.thinwedge_exe);
    child.args(["exec-server", "--listen", "ws://127.0.0.1:0"]);
    child.stdin(Stdio::null());
    child.stdout(Stdio::piped());
    child.stderr(Stdio::inherit());
    child.kill_on_drop(true);
    child.env("THINWEDGE_HOME", thinwedge_home.path());
    child.envs(env);
