...
#[path = "../src/proto/thinwedge.exec_server.relay.v1.rs"]
mod relay_proto;
...
use thinwedge_exec_server::RemoteEnvironmentConfig;
...
fn static_registry_auth_provider() -> thinwedge_api::SharedAuthProvider {
...
async fn multiplexed_remote_environment_routes_independent_virtual_streams() -> Result<()> {
    ...
    let (thinwedge_exe, thinwedge_linux_sandbox_exe) = common::current_test_binary_helper_paths()?;
    let runtime_paths = ExecServerRuntimePaths::new(thinwedge_exe, thinwedge_linux_sandbox_exe)?;
    let config = RemoteEnvironmentConfig::new(
        registry.uri(),
        ENVIRONMENT_ID.to_string(),
        static_registry_auth_provider(),
    )?;
    let remote_environment = tokio::spawn(thinwedge_exec_server::run_remote_environment(
        config,
        runtime_paths,
    ));
