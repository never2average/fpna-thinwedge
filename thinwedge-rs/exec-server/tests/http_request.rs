...
use thinwedge_exec_server::HttpRequestBodyDeltaNotification;
use thinwedge_exec_server::HttpRequestParams;
use thinwedge_exec_server::HttpRequestResponse;
...
async fn exec_server_http_request_buffers_response_body() -> anyhow::Result<()> {
        ...
        .send_request(
            ...
            serde_json::to_value(HttpRequestParams {
                ...
                headers: vec![HttpHeader {
                    name: "x-thinwedge-test".to_string(),
    ...
    assert_eq!(
        (
            ...
            captured.headers.get("x-thinwedge-test").map(String::as_str),
...
async fn wait_for_error_response(
    server: &mut ExecServerHarness,
    request_id: RequestId,
) -> anyhow::Result<thinwedge_app_server_protocol::JSONRPCErrorError> {
    let response = server
        .wait_for_event(|event| {
            matches!(
                event,
