# thinwedge-exec-server

`thinwedge-exec-server` is the library backing `thinwedge exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`thinwedge-utils-pty`.

It provides:

- a CLI entrypoint: `thinwedge exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `thinwedge` binary owns hidden helper dispatch for sandboxed
filesystem operations and `thinwedge-linux-sandbox`.

## Transport

The server speaks the shared `thinwedge-app-server-protocol` message envelope on
the wire.
...
It uses the standard ThinWedge ChatGPT sign-in state; run `thinwedge login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `THINWEDGE_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; ThinWedge then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `THINWEDGE_API_KEY`;
ThinWedge sends it as a bearer token on the registration request. For example:

```sh
THINWEDGE_API_KEY="$OPENAI_API_KEY" \
thinwedge exec-server \
  --remote ... \
...
`thinwedge.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/thinwedge.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `thinwedge-utils-pty`.
...
hidden helper process launched from the top-level `thinwedge` executable and
...
`thinwedge exec-server` command builds these paths from the `thinwedge` arg0 dispatch
state. `RemoteEnvironmentConfig::new(...)` also takes the auth provider that
remote registration should use; the CLI builds that provider from ThinWedge auth
