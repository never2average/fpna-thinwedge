...
use crate::environment_toml::environment_provider_from_thinwedge_home;
...
use thinwedge_shell_command::shell_detect::DetectedShell;

pub const THINWEDGE_EXEC_SERVER_URL_ENV_VAR: &str = "THINWEDGE_EXEC_SERVER_URL";

/// Owns the execution/filesystem environments available to the ThinWedge runtime.
///
/// `EnvironmentManager` is a shared registry for concrete environments. Its
/// default constructor preserves the legacy `THINWEDGE_EXEC_SERVER_URL` behavior
/// while configured construction accepts a provider-supplied snapshot.
///
/// Setting `THINWEDGE_EXEC_SERVER_URL=none` disables environment access by leaving
...
impl EnvironmentManager {
    ...
    /// Builds a manager from `THINWEDGE_HOME` and local runtime paths used when
    /// creating local filesystem helpers.
    ///
    /// If `THINWEDGE_HOME/environments.toml` is present, it defines the configured
    /// environments. Otherwise this preserves the legacy
    /// `THINWEDGE_EXEC_SERVER_URL` behavior.
    pub async fn from_thinwedge_home(
        thinwedge_home: impl AsRef<std::path::Path>,
        local_runtime_paths: Option<ExecServerRuntimePaths>,
    ) -> Result<Self, ExecServerError> {
        let provider = environment_provider_from_thinwedge_home(thinwedge_home.as_ref())?;
    ...
    /// reading user config files from `THINWEDGE_HOME`.
...
impl Environment {
    /// Builds an environment from the raw `THINWEDGE_EXEC_SERVER_URL` value.
    pub fn create(
        exec_server_url: Option<String>,
        local_runtime_paths: ExecServerRuntimePaths,
    ) -> Result<Self, ExecServerError> {
    ...
    /// Builds an environment from the raw `THINWEDGE_EXEC_SERVER_URL` value and
...
impl EnvironmentInfo {
    pub(crate) fn local() -> Self {
        Self {
            shell: thinwedge_shell_command::shell_detect::default_user_shell().into(),
        }
    }
}
...
mod tests {
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = thinwedge_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
            std::env::current_exe().expect("current exe").as_path(),
        )
        .expect("absolute current exe");
        let path = thinwedge_utils_path_uri::PathUri::from_abs_path(&path);
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            thinwedge_protocol::models::PermissionProfile::from_runtime_permissions(
                &thinwedge_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
