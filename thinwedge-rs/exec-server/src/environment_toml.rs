...
pub(crate) fn environment_provider_from_thinwedge_home(
    thinwedge_home: &Path,
) -> Result<Box<dyn EnvironmentProvider>, ExecServerError> {
    let path = thinwedge_home.join(ENVIRONMENTS_TOML_FILE);
...
mod tests {
        ...
        let provider = TomlEnvironmentProvider::new(EnvironmentsToml {
            ...
            environments: vec![
                ...
                EnvironmentToml {
                    ...
                    args: Some(vec![
                        "dev".to_string(),
                        "thinwedge exec-server --listen stdio".to_string(),
                    ]),
                    env: Some(HashMap::from([(
                        "THINWEDGE_LOG".to_string(),
                        "debug".to_string(),
                    )])),
                    ..Default::default()
                },
        ...
        let cases = [
            ...
            (
                EnvironmentToml {
                    id: "devbox".to_string(),
                    url: Some("ws://127.0.0.1:8765".to_string()),
                    program: Some("thinwedge".to_string()),
                    ..Default::default()
                },
    ...
    fn load_environments_toml_reads_root_environment_list() {
        ...
        let path = thinwedge_home.path().join(ENVIRONMENTS_TOML_FILE);
        std::fs::write(
            &path,
            r#"
default = "ssh-dev"
...
id = "ssh-dev"
program = "ssh"
args = ["dev", "thinwedge exec-server --listen stdio"]
cwd = "/tmp"
[environments.env]
THINWEDGE_LOG = "debug"
"#,
...
"#,
    ...
    #[tokio::test]
    async fn environment_provider_from_thinwedge_home_uses_present_environments_file() {
        let thinwedge_home = tempdir().expect("tempdir");
        std::fs::write(
            thinwedge_home.path().join(ENVIRONMENTS_TOML_FILE),
            r#"
default = "none"
include_local = false
"#,
        ...
        let provider = environment_provider_from_thinwedge_home(thinwedge_home.path())
    ...
    #[tokio::test]
    async fn environment_provider_from_thinwedge_home_falls_back_when_file_is_missing() {
        let thinwedge_home = tempdir().expect("tempdir");
