...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.thinwedge_self_exe;
        ...
        let command = SandboxCommand {
            ...
            args: vec![THINWEDGE_FS_HELPER_ARG1.to_string()],
            cwd: cwd.clone(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    ...
    for path in std::iter::once(runtime_paths.thinwedge_self_exe.as_path())
        .chain(runtime_paths.thinwedge_linux_sandbox_exe.as_deref())
    {
...
mod tests {
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/thinwedge".to_string()),
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let thinwedge_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(thinwedge_self_exe.clone(), Some(thinwedge_self_exe))
                .expect("runtime paths");
    ...
    fn helper_permissions_include_linux_sandbox_alias_parent() {
        ...
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let thinwedge_parent = AbsolutePathBuf::from_absolute_path(root.path().join("bin"))
            .expect("absolute thinwedge parent");
        ...
        assert!(policy.can_read_path_with_cwd(thinwedge_parent.as_path(), cwd.as_path()));
