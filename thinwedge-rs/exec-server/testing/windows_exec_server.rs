...
//!
//! Keeping this wrapper separate avoids depending on the full ThinWedge binary's
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let runtime_paths =
        ExecServerRuntimePaths::new(current_exe, /*thinwedge_linux_sandbox_exe*/ None)?;
    thinwedge_exec_server::run_main("ws://127.0.0.1:0", runtime_paths).await
}
