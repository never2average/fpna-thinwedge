...
- The legacy rule matcher lives in `thinwedge-execpolicy-legacy`.
...
- From the ThinWedge CLI, run `thinwedge execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
thinwedge execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
thinwedge execpolicy check \
...
```bash
cargo run -p thinwedge-execpolicy -- check --rules path/to/policy.rules git status
```
