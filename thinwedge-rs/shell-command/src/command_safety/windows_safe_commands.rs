...
mod tests {
    ...
    fn rejects_git_subcommand_options_with_side_effects() {
        let results: Vec<(&str, bool)> = [
            "git diff --output thinwedge_poc.txt",
            "git diff --ext-diff HEAD",
            "git log --textconv -1",
            "git show --output=thinwedge_poc.txt HEAD",
        ...
        assert_eq!(
            vec![
                ("git diff --output thinwedge_poc.txt", false),
                ("git diff --ext-diff HEAD", false),
                ("git log --textconv -1", false),
                ("git show --output=thinwedge_poc.txt HEAD", false),
    ...
    fn rejects_stop_parsing_git_forms() {
        assert!(!is_safe_command_windows(&vec_str(&[
            ...
            "-NoProfile",
            "-Command",
            "git log --% HEAD --output=thinwedge_poc.txt",
        ])));
    }
