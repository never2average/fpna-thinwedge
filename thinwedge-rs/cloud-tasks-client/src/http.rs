...
use thinwedge_backend_client as backend;
use thinwedge_backend_client::CodeTaskDetailsResponseExt;
use thinwedge_git_utils::ApplyGitRequest;
use thinwedge_git_utils::apply_git_patch;
    ...
    impl<'a> Tasks<'a> {
        ...
        pub(crate) async fn messages(&self, id: TaskId) -> Result<Vec<String>> {
            ...
            let url = match details_path(self.base_url, &id.0) {
                Some(url) => url,
                None => format!("{}/api/thinwedge/tasks/{}", self.base_url, id.0),
            };
        ...
        pub(crate) async fn create(
            ...
            if let Ok(diff) = std::env::var("THINWEDGE_STARTING_DIFF")
                && !diff.is_empty()
            {
                input_items.push(serde_json::json!({
