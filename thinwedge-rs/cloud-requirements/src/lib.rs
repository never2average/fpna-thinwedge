//! Cloud-hosted config requirements for ThinWedge.
//!
...
//! requirements cannot be loaded for those accounts, ThinWedge fails configuration loading rather than
...
const CLOUD_REQUIREMENTS_FETCH_ATTEMPT_METRIC: &str = "thinwedge.cloud_requirements.fetch_attempt";
const CLOUD_REQUIREMENTS_FETCH_FINAL_METRIC: &str = "thinwedge.cloud_requirements.fetch_final";
const CLOUD_REQUIREMENTS_LOAD_METRIC: &str = "thinwedge.cloud_requirements.load";
...
const CLOUD_REQUIREMENTS_CACHE_WRITE_HMAC_KEY: &[u8] =
    b"thinwedge-cloud-requirements-cache-v3-064f8542-75b4-494c-a294-97d3ce597271";
...
fn auth_identity(auth: &ThinWedgeAuth) -> (Option<String>, Option<String>) {
    (auth.get_chatgpt_user_id(), auth.get_account_id())
}

fn cloud_requirements_eligible_auth(auth: &ThinWedgeAuth) -> bool {
    let Some(plan_type) = auth.account_plan_type() else {
        return false;
    };
    auth.uses_thinwedge_backend()
...
impl CloudRequirementsService {
    fn new(
        ...
        Self {
            auth_manager,
            fetcher,
            cache_path: thinwedge_home.join(CLOUD_REQUIREMENTS_CACHE_FILENAME),
            timeout,
        }
    }
...
pub async fn cloud_requirements_loader_for_storage(
    ...
    .await;
    cloud_requirements_loader(auth_manager, chatgpt_base_url, thinwedge_home)
}
...
mod tests {
    ...
    async fn cloud_requirements_eligible_auth_allows_agent_identity_business_plan() {
        ...
        let auth = AgentIdentityAuth::load(record)
            .await
            .map(ThinWedgeAuth::AgentIdentity)
...
"#,
        ...
        assert_eq!(
            result,
            Some(ConfigRequirementsToml {
                apps: Some(thinwedge_config::AppsRequirementsToml {
                    apps: BTreeMap::from([(
                        "connector_5f3c8c41a1e54ad7a76272c89e2554fa".to_string(),
                        thinwedge_config::AppRequirementToml {
                            enabled: Some(false),
                        },
                    )]),
                }),
