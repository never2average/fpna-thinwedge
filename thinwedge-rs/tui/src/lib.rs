...
use crate::legacy_core::config::find_thinwedge_home;
    ...
    Fut: Future<Output = std::io::Result<InProcessAppServerClient>>,
{
    ...
    let client = start_client(InProcessClientStartArgs {
        ...
        environment_manager,
        config_warnings,
        session_source: thinwedge_protocol::protocol::SessionSource::Cli,
...
pub async fn run_main(
    ...
    let model_provider_override = if cli.oss {
        ...
        if let Some(provider) = resolved {
            ...
            let provider = oss_selection::select_oss_provider(&thinwedge_home).await?;
    ...
    // ThinWedge CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("thinwedge-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for thinwedge crates.
    let env_filter = || {
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: thinwedge_protocol::protocol::TokenUsage::default(),
                        thread_id: None,
                        thread_name: None,
        ...
        match tui_alternate_screen {
            ...
            AltScreenMode::Auto => {
                ...
                !matches!(
                    ...
                    Some(thinwedge_terminal_detection::Multiplexer::Zellij {})
                )
            }
        }
    }
...
mod tests {
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        ...
        let mut app_server =
            AppServerSession::new(thinwedge_app_server_client::AppServerClient::InProcess(
    ...
    fn build_turn_context(config: &Config, cwd: PathBuf) -> TurnContextItem {
        ...
        TurnContextItem {
            ...
            summary: config
                ...
                .unwrap_or(thinwedge_protocol::config_types::ReasoningSummary::Auto),
