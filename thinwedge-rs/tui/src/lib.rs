...
// The standalone `thinwedge-tui` binary prints a short help message before the
...
use thinwedge_app_server_client::InProcessClientStartArgs;
use thinwedge_app_server_client::RemoteAppServerClient;
use thinwedge_app_server_client::RemoteAppServerConnectArgs;
pub use thinwedge_app_server_client::RemoteAppServerEndpoint;
use thinwedge_app_server_protocol::Account as AppServerAccount;
...
pub(crate) use thinwedge_app_server_client::legacy_core;
...
const TUI_LOG_FILE_NAME: &str = "thinwedge-tui.log";
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = thinwedge_state::runtime_db_path_for_corruption_error(&err)
                .unwrap_or_else(|| thinwedge_state::state_db_path(config.sqlite_home.as_path()));
...
fn remove_legacy_tui_log_file(thinwedge_home: &Path) {
    ...
    let _ = std::fs::remove_file(thinwedge_home.join("log").join(TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let thinwedge_home =
                find_thinwedge_home().wrap_err("failed to resolve THINWEDGE_HOME")?;
            thinwedge_app_server_client::app_server_control_socket_path(&thinwedge_home)
...
#[cfg(unix)]
async fn maybe_probe_default_daemon_socket(thinwedge_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path =
        thinwedge_app_server_client::app_server_control_socket_path(thinwedge_home).ok()?;
...
#[cfg(not(unix))]
async fn maybe_probe_default_daemon_socket(_thinwedge_home: &Path) -> Option<AbsolutePathBuf> {
    None
}
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        thinwedge_feedback::ThinWedgeFeedback::new(),
        /*log_db*/ None,
        state_db,
        environment_manager,
    )
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = thinwedge_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let thinwedge_home = match find_thinwedge_home() {
        Ok(thinwedge_home) => thinwedge_home.to_path_buf(),
        Err(err) => {
            eprintln!("Error finding thinwedge home: {err}");
            std::process::exit(1);
        }
    };
    ...
    if let Some(profile_v2) = cli.config_profile_v2.as_ref() {
        let user_config_path = resolve_profile_v2_config_path(&thinwedge_home, profile_v2);
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        maybe_probe_default_daemon_socket(&thinwedge_home).await
    } else {
        None
    };
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        arg0_paths.thinwedge_self_exe.clone(),
    ...
    remove_legacy_tui_log_file(config.thinwedge_home.as_path());

    let otel_originator = originator().value;
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        thinwedge_app_server_client::build_otel_provider(
            &config,
    ...
    if let Some(metrics) = otel
        .as_ref()
        .and_then(thinwedge_otel::OtelProvider::metrics)
    {
        let _ = thinwedge_otel::record_process_start_once(metrics, otel_originator.as_str());
        let telemetry =
            thinwedge_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
        let _ = thinwedge_state::install_process_db_telemetry(telemetry);
    }
    ...
    match effective_toml.try_into() {
        Ok(config_toml) => {
            match thinwedge_app_server_client::migrate_personality_if_needed(
    ...
    let (tui_file_layer, _tui_file_log_guard) = if config_toml_log_dir_configured {
        ...
        let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("thinwedge_core=info,thinwedge_tui=info,thinwedge_rmcp_client=info")
        });
    ...
    let feedback = thinwedge_feedback::ThinWedgeFeedback::new();
...
async fn run_ratatui_app(
    ...
    let config = if should_show_onboarding {
        ...
        if show_login_screen && !uses_remote_workspace {
            cloud_config_bundle = cloud_config_bundle_loader_for_storage(
                initial_config.thinwedge_home.to_path_buf(),
                /*enable_thinwedge_api_key_env*/ false,
    ...
    let required_elevated_sandbox_needs_setup = windows_sandbox_level
        ...
        && config
            ...
            .source
            .is_some()
        && !crate::windows_sandbox::sandbox_setup_is_complete(config.thinwedge_home.as_path());
    #[cfg(target_os = "windows")]
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, thinwedge_config::TomlValue)>,
    loader_overrides: LoaderOverrides,
    strict_config: bool,
    cloud_config_bundle: CloudConfigBundleLoader,
...
mod tests {
    ...
    fn write_session_rollout(
        ...
        let rollout_path = thinwedge_home
            .join("sessions")
            .join(year)
            .join(month)
            .join(day)
        ...
        let session_meta = thinwedge_protocol::protocol::SessionMeta {
            id: thread_id,
            timestamp: meta_rfc3339.to_string(),
            cwd: cwd.to_path_buf(),
            originator: "thinwedge".to_string(),
            cli_version: "0.0.0".to_string(),
            source: thinwedge_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(thinwedge_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        let thinwedge_home = find_thinwedge_home().wrap_err("failed to resolve THINWEDGE_HOME")?;
        assert_eq!(
            resolve_remote_addr("unix://")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: thinwedge_app_server_client::app_server_control_socket_path(
                    &thinwedge_home
                )?,
            }
        );
    ...
    fn resolve_remote_addr_accepts_relative_socket_path() -> color_eyre::Result<()> {
        assert_eq!(
            resolve_remote_addr("unix://thinwedge.sock")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: AbsolutePathBuf::relative_to_current_dir("thinwedge.sock")?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_absolute_socket_path() -> color_eyre::Result<()> {
        let temp_dir = TempDir::new()?;
        let socket_path = temp_dir.path().join("thinwedge.sock");
        assert_eq!(
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        ...
        assert!(
            maybe_probe_default_daemon_socket(thinwedge_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let thinwedge_home = TempDir::new()?;
        let socket_path =
            thinwedge_app_server_client::app_server_control_socket_path(thinwedge_home.path())?;
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(thinwedge_home.path()).await,
            Some(socket_path)
        );
        Ok(())
    }
    ...
    {
        let socket_path = AbsolutePathBuf::relative_to_current_dir("thinwedge.sock")?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            thinwedge_app_server_client::AppServerClient::InProcess(
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        Box::pin(async {
            ...
            let state_runtime = thinwedge_state::StateRuntime::init(
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            thinwedge_state::state_db_path(occupied_sqlite_home.as_path()).as_path()
        );
        assert!(
            startup_error
                .detail()
    ...
    -> color_eyre::Result<()> {
        ...
        let logs_db_path = thinwedge_state::logs_db_path(&sqlite_home);
        ...
        assert!(
            thinwedge_state::sqlite_error_detail_is_corruption(startup_error.detail()),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let thinwedge_home = temp_dir.path().to_path_buf();
