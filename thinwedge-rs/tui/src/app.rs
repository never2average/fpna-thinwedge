...
use thinwedge_app_server_protocol::MergeStrategy;
...
use thinwedge_models_manager::model_presets::HIDE_GPT_5_1_THINWEDGE_MAX_MIGRATION_PROMPT_CONFIG;
use thinwedge_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
use thinwedge_protocol::protocol::FinalOutput;
...
fn command_execution_decision_to_review_decision(
    decision: thinwedge_app_server_protocol::CommandExecutionApprovalDecision,
) -> thinwedge_protocol::protocol::ReviewDecision {
    match decision {
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&thinwedge_protocol::protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&thinwedge_protocol::approvals::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[thinwedge_protocol::approvals::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&thinwedge_protocol::models::AdditionalPermissionProfile>,
) -> Vec<thinwedge_protocol::protocol::ReviewDecision> {
...
fn list_skills_response_to_core(response: SkillsListResponse) -> ListSkillsResponseEvent {
    ListSkillsResponseEvent {
        skills: response
            .data
            .into_iter()
            .map(|entry| thinwedge_protocol::protocol::SkillsListEntry {
                cwd: entry.cwd,
                skills: entry
                    .skills
                    .into_iter()
                    .map(|skill| thinwedge_protocol::protocol::SkillMetadata {
                        name: skill.name,
                        description: skill.description,
                        short_description: skill.short_description,
                        interface: skill.interface.map(|interface| {
                            thinwedge_protocol::protocol::SkillInterface {
                        ...
                        dependencies: skill.dependencies.map(|dependencies| {
                            thinwedge_protocol::protocol::SkillDependencies {
                                tools: dependencies
                                    .tools
                                    .into_iter()
                                    .map(|tool| thinwedge_protocol::protocol::SkillToolDependency {
                                        r#type: tool.r#type,
                                        value: tool.value,
                        ...
                        scope: match skill.scope {
                            thinwedge_app_server_protocol::SkillScope::User => {
                                thinwedge_protocol::protocol::SkillScope::User
                            }
                            thinwedge_app_server_protocol::SkillScope::Repo => {
                                thinwedge_protocol::protocol::SkillScope::Repo
                            }
                            thinwedge_app_server_protocol::SkillScope::System => {
                                thinwedge_protocol::protocol::SkillScope::System
                            }
                            thinwedge_app_server_protocol::SkillScope::Admin => {
                                thinwedge_protocol::protocol::SkillScope::Admin
                            }
                        },
                ...
                errors: entry
                    .errors
                    .into_iter()
                    .map(|error| thinwedge_protocol::protocol::SkillErrorInfo {
                        path: error.path,
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.thinwedge_error_info,
        Some(AppServerThinWedgeErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
impl App {
    ...
    pub async fn run(
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            thinwedge_login::default_client::originator().value,
        ...
        {
            session_telemetry.counter("thinwedge.status_line", /*inc*/ 1, &[]);
        }
...
See the ThinWedge keymap documentation for supported actions and examples."
            )
        })?;
        #[cfg(not(debug_assertions))]
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.thinwedge_home.clone();
