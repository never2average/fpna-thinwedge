...
use thinwedge_app_server_protocol::SandboxMode as AppServerSandboxMode;
...
use thinwedge_app_server_protocol::SkillErrorInfo;
...
use thinwedge_app_server_protocol::WriteStatus;
...
#[cfg(target_os = "windows")]
use thinwedge_protocol::permissions::FileSystemSandboxKind;
use thinwedge_rollout::StateDbHandle;
use thinwedge_terminal_detection::user_agent;
use thinwedge_utils_absolute_path::AbsolutePathBuf;
use thinwedge_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
use tokio::select;
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&thinwedge_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&thinwedge_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[thinwedge_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&thinwedge_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<thinwedge_app_server_protocol::CommandExecutionApprovalDecision> {
...
fn resume_hint_for_resumable_thread(
    ...
    thinwedge_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
fn archived_session_guidance(err: &color_eyre::eyre::Report) -> Option<String> {
    ...
    if !message.contains(" is archived. Run `thinwedge unarchive ") {
        return None;
    }
    let message = message
        .split_once(" (code ")
...
impl App {
    pub fn chatwidget_init_for_forked_or_resumed_thread(
        ...
        crate::chatwidget::ChatWidgetInit {
            ...
            has_thinwedge_backend_auth: self.chat_widget.has_thinwedge_backend_auth(),
    ...
    pub async fn run(
        ...
        let has_thinwedge_backend_auth = matches!(auth_mode, Some(TelemetryAuthMode::Chatgpt));
