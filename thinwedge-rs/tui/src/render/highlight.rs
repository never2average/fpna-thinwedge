...
//! | `THINWEDGE_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static THINWEDGE_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// Set the user-configured syntax theme override and thinwedge home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `thinwedge_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), thinwedge_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let thinwedge_home_set_ok = THINWEDGE_HOME.set(thinwedge_home.clone()).is_ok();
    ...
    if !override_set_ok || !thinwedge_home_set_ok {
...
pub(crate) fn validate_theme_name(
    ...
    let custom_theme_path_display = thinwedge_home
        ...
        .unwrap_or_else(|| format!("$THINWEDGE_HOME/themes/{name}.tmTheme"));
    ...
    if let Some(home) = thinwedge_home {
...
fn load_custom_theme(name: &str, thinwedge_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, thinwedge_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, thinwedge_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {THINWEDGE_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = thinwedge_home
...
fn build_default_theme() -> Theme {
    ...
    let thinwedge_home = THINWEDGE_HOME
        .get()
        .and_then(|thinwedge_home| thinwedge_home.as_deref());
    resolve_theme_with_override(name, thinwedge_home)
}
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = THINWEDGE_HOME.get()
...
pub(crate) fn resolve_theme_by_name(name: &str, thinwedge_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// `.tmTheme` file under `{THINWEDGE_HOME}/themes/`.
...
/// found in `{thinwedge_home}/themes/`.
pub(crate) fn list_available_themes(thinwedge_home: Option<&Path>) -> Vec<ThemeEntry> {
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*thinwedge_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*thinwedge_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*thinwedge_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*thinwedge_home*/ None).is_none());
    }
