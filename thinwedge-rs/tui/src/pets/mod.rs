...
//! managed THINWEDGE_HOME cache, while custom pets remain entirely user-owned data
//! under `$THINWEDGE_HOME/pets/<pet-id>/pet.json` or legacy avatar directories.
//!
...
pub(crate) const DEFAULT_PET_ID: &str = "thinwedge";
...
pub(crate) fn ensure_builtin_pack_for_pet(
    ...
    if let Some(pet) = catalog::builtin_pet(pet_id) {
        asset_pack::ensure_builtin_pet(thinwedge_home, pet)?;
    }
    Ok(())
}
