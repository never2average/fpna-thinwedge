...
/// has a sensible starting point without implying that ThinWedge is already the
...
pub(crate) fn build_pet_picker_params(
    ...
    let mut entries = available_pet_entries(thinwedge_home);
...
fn available_pet_entries(thinwedge_home: &Path) -> Vec<PetPickerEntry> {
    ...
    entries.extend(custom_pet_entries(thinwedge_home));
    entries
}

fn custom_pet_entries(thinwedge_home: &Path) -> Vec<PetPickerEntry> {
    let mut entries_by_selector = HashMap::new();
    for (directory_name, manifest_file) in [("avatars", "avatar.json"), ("pets", "pet.json")] {
        let Ok(children) = fs::read_dir(thinwedge_home.join(directory_name)) else {
            continue;
        };
        for child in children.flatten() {
            ...
            let Ok(pet) = Pet::load_with_thinwedge_home(
                &selector,
                /*thinwedge_home*/ Some(thinwedge_home),
            ) else {
                continue;
            };
...
mod tests {
    ...
    fn picker_lists_app_bundled_and_custom_pets() {
        ...
        write_pet(thinwedge_home.path(), "chefito", "Chefito");
    ...
    #[test]
    fn picker_preselects_thinwedge_without_marking_it_current_when_no_pet_is_configured() {
        let thinwedge_home = tempfile::tempdir().unwrap();
        let params = build_pet_picker_params(
            /*current_pet*/ None,
            thinwedge_home.path(),
        ...
        assert_eq!(params.initial_selected_idx, Some(2));
        assert_eq!(params.items[2].name, "ThinWedge");
    ...
    fn picker_imports_legacy_avatar_manifests() {
        let thinwedge_home = tempfile::tempdir().unwrap();
        write_legacy_avatar(thinwedge_home.path(), "legacy", "Legacy");
