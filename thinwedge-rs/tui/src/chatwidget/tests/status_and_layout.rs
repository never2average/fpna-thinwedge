...
async fn status_line_git_summary_items_render_values() {
    ...
    chat.status_line_git_summary = Some(StatusLineGitSummary {
        pull_request: Some(crate::branch_summary::StatusLinePullRequest {
            number: 20_252,
            url: "https://github.com/openai/thinwedge/pull/20252".to_string(),
        }),
...
async fn completed_plan_table_tail_skips_provisional_history_insert() {
    ...
    controller.push("| --- | --- |\n");
    controller.push("| Verify | ThinWedge |\n");
    assert!(
        controller.has_live_tail(),
...
async fn configured_pet_load_is_deferred_until_after_construction() {
    ...
    crate::pets::write_test_pack(&cfg.thinwedge_home);
...
async fn ambient_pet_stays_hidden_until_a_pet_is_selected() {
    ...
    crate::pets::write_test_pack(&chat.config.thinwedge_home);
    chat.set_tui_pet(Some("thinwedge".to_string()));
...
async fn interrupted_turn_clears_visible_running_hook() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            thinwedge_app_server_protocol::HookEventName::PreToolUse,
...
async fn completed_turn_clears_visible_running_hook() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            thinwedge_app_server_protocol::HookEventName::PostToolUse,
            /*status_message*/ None,
        ),
    );
    reveal_running_hooks(&mut chat);
...
async fn renamed_thread_footer_title_snapshot() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadNameUpdated(
            thinwedge_app_server_protocol::ThreadNameUpdatedNotification {
                thread_id: thread_id.to_string(),
...
fn goal_status_indicator_formats_statuses_and_budgets() {
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            thinwedge_app_server_protocol::ThreadGoalStatus::Blocked,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            thinwedge_app_server_protocol::ThreadGoalStatus::UsageLimited,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
...
async fn completed_hook_with_no_entries_stays_out_of_history() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            "post-tool-use:0:/tmp/hooks.json",
            thinwedge_app_server_protocol::HookEventName::PostToolUse,
            thinwedge_app_server_protocol::HookRunStatus::Completed,
            Vec::new(),
        ),
    );
...
async fn blocked_and_failed_hooks_render_feedback_and_errors() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            "pre-tool-use:0:/tmp/hooks.json",
            thinwedge_app_server_protocol::HookEventName::PreToolUse,
            thinwedge_app_server_protocol::HookRunStatus::Blocked,
            vec![thinwedge_app_server_protocol::HookOutputEntry {
                kind: thinwedge_app_server_protocol::HookOutputEntryKind::Feedback,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            thinwedge_app_server_protocol::HookEventName::PostToolUse,
            thinwedge_app_server_protocol::HookRunStatus::Failed,
            vec![thinwedge_app_server_protocol::HookOutputEntry {
                kind: thinwedge_app_server_protocol::HookOutputEntryKind::Error,
...
async fn completed_same_id_hook_output_survives_restart() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            hook_id,
            thinwedge_app_server_protocol::HookEventName::Stop,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            hook_id,
            thinwedge_app_server_protocol::HookEventName::Stop,
            thinwedge_app_server_protocol::HookRunStatus::Stopped,
            vec![thinwedge_app_server_protocol::HookOutputEntry {
                kind: thinwedge_app_server_protocol::HookOutputEntryKind::Stop,
...
async fn hook_completed_before_reveal_renders_completed_without_running_flash() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            thinwedge_app_server_protocol::HookEventName::SessionStart,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            thinwedge_app_server_protocol::HookRunStatus::Completed,
            vec![thinwedge_app_server_protocol::HookOutputEntry {
                kind: thinwedge_app_server_protocol::HookOutputEntryKind::Context,
...
fn hook_started_run(
    id: &str,
    event_name: thinwedge_app_server_protocol::HookEventName,
    status_message: Option<&str>,
) -> thinwedge_app_server_protocol::HookRunSummary {
    hook_run_summary(
        id,
        event_name,
        thinwedge_app_server_protocol::HookRunStatus::Running,
        status_message,
        Vec::new(),
    )
}
...
fn hook_completed_run(
    id: &str,
    event_name: thinwedge_app_server_protocol::HookEventName,
    status: thinwedge_app_server_protocol::HookRunStatus,
    entries: Vec<thinwedge_app_server_protocol::HookOutputEntry>,
) -> thinwedge_app_server_protocol::HookRunSummary {
    hook_run_summary(
...
fn hook_run_summary(
    ...
    thinwedge_app_server_protocol::HookRunSummary {
        id: id.to_string(),
        event_name,
        handler_type: thinwedge_app_server_protocol::HookHandlerType::Command,
        execution_mode: thinwedge_app_server_protocol::HookExecutionMode::Sync,
        scope: thinwedge_app_server_protocol::HookScope::Turn,
        source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
        source: thinwedge_app_server_protocol::HookSource::User,
        display_order: 0,
        status,
        ...
        started_at: 1,
        completed_at: (status != thinwedge_app_server_protocol::HookRunStatus::Running)
            .then_some(2),
        duration_ms: (status != thinwedge_app_server_protocol::HookRunStatus::Running).then_some(1),
        entries,
    }
}
