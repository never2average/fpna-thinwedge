...
async fn core_cyber_policy_error_renders_dedicated_notice() {
    ...
    chat.handle_thinwedge_event(Event {
        ...
        msg: EventMsg::Error(ErrorEvent {
            ...
            thinwedge_error_info: Some(ThinWedgeErrorInfo::CyberPolicy),
        }),
    });
...
async fn prefetch_rate_limits_is_gated_on_chatgpt_auth_provider() {
    ...
    chat.config.model_provider.requires_thinwedge_auth = false;
    assert!(!chat.should_prefetch_rate_limits());
...
async fn rate_limit_snapshots_keep_separate_entries_per_limit_id() {
    ...
    chat.on_rate_limit_snapshot(Some(RateLimitSnapshot {
        ...
        limit_name: Some("thinwedge".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 20.0,
            window_minutes: Some(300),
            resets_at: Some(100),
    ...
    let thinwedge = chat
        .rate_limit_snapshots_by_limit_id
        .get("thinwedge")
        .expect("thinwedge snapshot should exist");
    let other = chat
        .rate_limit_snapshots_by_limit_id
        .get("thinwedge_other")
...
#[tokio::test]
async fn rate_limit_switch_prompt_skips_non_thinwedge_limit() {
...
async fn fast_status_indicator_is_hidden_for_models_without_fast_support() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.3-thinwedge")).await;
    set_fast_mode_test_catalog(&mut chat);
    assert!(!get_available_model(&chat, "gpt-5.3-thinwedge").supports_fast_mode());
    chat.set_service_tier(Some(ServiceTier::Fast));
    set_chatgpt_auth(&mut chat);
    set_fast_mode_test_catalog(&mut chat);
    assert!(!get_available_model(&chat, "gpt-5.3-thinwedge").supports_fast_mode());
...
async fn stream_error_updates_status_indicator() {
    ...
    chat.handle_thinwedge_event(Event {
        ...
        msg: EventMsg::StreamError(StreamErrorEvent {
            ...
            thinwedge_error_info: Some(ThinWedgeErrorInfo::Other),
...
async fn status_line_model_with_reasoning_includes_fast_for_fast_capable_models() {
    ...
    assert_eq!(
        status_line_text(&chat),
        Some(format!(
            "gpt-5.3-thinwedge xhigh · Context 0% used · {test_cwd}"
        ))
    );
}
...
async fn status_line_model_with_reasoning_updates_on_mode_switch_without_manual_refresh() {
    ...
    assert_eq!(
        status_line_text(&chat),
        Some("gpt-5.3-thinwedge high".to_string())
    );
    ...
    assert_eq!(
        status_line_text(&chat),
        Some("gpt-5.3-thinwedge medium".to_string())
    );
...
async fn renamed_thread_footer_title_snapshot() {
    ...
    chat.handle_thinwedge_event(Event {
        id: "rename".to_string(),
        msg: EventMsg::ThreadNameUpdated(thinwedge_protocol::protocol::ThreadNameUpdatedEvent {
            thread_id,
        ...
        ServerNotification::ThreadGoalUpdated(
            thinwedge_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                goal: test_thread_goal(
                    thinwedge_app_server_protocol::ThreadGoalStatus::Active,
...
async fn thread_goal_update_for_other_thread_is_ignored() {
    ...
    let mut goal = test_thread_goal(
        thinwedge_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        /*token_budget*/ Some(50_000),
        /*tokens_used*/ 50_000,
    );
...
fn test_thread_goal(
    status: thinwedge_app_server_protocol::ThreadGoalStatus,
    token_budget: Option<i64>,
    tokens_used: i64,
) -> thinwedge_app_server_protocol::ThreadGoal {
    thinwedge_app_server_protocol::ThreadGoal {
        thread_id: "thread-1".to_string(),
    ...
    chat.handle_server_notification(
        ServerNotification::HookStarted(AppServerHookStartedNotification {
            ...
            run: AppServerHookRunSummary {
                ...
                source: thinwedge_app_server_protocol::HookSource::User,
                display_order: 0,
...
async fn pre_tool_use_hook_events_render_snapshot() {
    assert_hook_events_snapshot(
        thinwedge_protocol::protocol::HookEventName::PreToolUse,
...
async fn post_tool_use_hook_events_render_snapshot() {
    assert_hook_events_snapshot(
        thinwedge_protocol::protocol::HookEventName::PostToolUse,
...
async fn completed_hook_with_no_entries_stays_out_of_history() {
    ...
    chat.handle_thinwedge_event(Event {
        id: "hook-1".into(),
        msg: EventMsg::HookStarted(thinwedge_protocol::protocol::HookStartedEvent {
            turn_id: None,
            run: thinwedge_protocol::protocol::HookRunSummary {
                id: "post-tool-use:0:/tmp/hooks.json".to_string(),
                event_name: thinwedge_protocol::protocol::HookEventName::PostToolUse,
                handler_type: thinwedge_protocol::protocol::HookHandlerType::Command,
                execution_mode: thinwedge_protocol::protocol::HookExecutionMode::Sync,
                scope: thinwedge_protocol::protocol::HookScope::Turn,
                source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
                source: thinwedge_protocol::protocol::HookSource::User,
                display_order: 0,
                status: thinwedge_protocol::protocol::HookRunStatus::Running,
                status_message: None,
                started_at: 1,
                completed_at: None,
                duration_ms: None,
    ...
    chat.handle_thinwedge_event(Event {
        id: "hook-1".into(),
        msg: EventMsg::HookCompleted(thinwedge_protocol::protocol::HookCompletedEvent {
            turn_id: None,
            run: thinwedge_protocol::protocol::HookRunSummary {
                id: "post-tool-use:0:/tmp/hooks.json".to_string(),
                event_name: thinwedge_protocol::protocol::HookEventName::PostToolUse,
                ...
                source: thinwedge_protocol::protocol::HookSource::User,
                display_order: 0,
                status: thinwedge_protocol::protocol::HookRunStatus::Completed,
                status_message: None,
                started_at: 1,
                completed_at: Some(2),
                duration_ms: Some(1),
...
async fn quiet_hook_linger_starts_when_delayed_redraw_reveals_hook() {
    ...
    chat.handle_thinwedge_event(hook_started_event(
        "post-tool-use:0:/tmp/hooks.json",
        thinwedge_protocol::protocol::HookEventName::PostToolUse,
    ...
    chat.handle_thinwedge_event(hook_completed_event(
        "post-tool-use:0:/tmp/hooks.json",
        thinwedge_protocol::protocol::HookEventName::PostToolUse,
        thinwedge_protocol::protocol::HookRunStatus::Completed,
        Vec::new(),
    ));
...
async fn blocked_and_failed_hooks_render_feedback_and_errors() {
    ...
    chat.handle_thinwedge_event(hook_completed_event(
        "pre-tool-use:0:/tmp/hooks.json",
        thinwedge_protocol::protocol::HookEventName::PreToolUse,
        thinwedge_protocol::protocol::HookRunStatus::Blocked,
        vec![thinwedge_protocol::protocol::HookOutputEntry {
            kind: thinwedge_protocol::protocol::HookOutputEntryKind::Feedback,
    ...
    chat.handle_thinwedge_event(hook_completed_event(
        ...
        thinwedge_protocol::protocol::HookEventName::PostToolUse,
        thinwedge_protocol::protocol::HookRunStatus::Failed,
        vec![thinwedge_protocol::protocol::HookOutputEntry {
            kind: thinwedge_protocol::protocol::HookOutputEntryKind::Error,
...
async fn completed_same_id_hook_output_survives_restart() {
    ...
    chat.handle_thinwedge_event(hook_started_event(
        hook_id,
        thinwedge_protocol::protocol::HookEventName::Stop,
    ...
    chat.handle_thinwedge_event(hook_completed_event(
        hook_id,
        thinwedge_protocol::protocol::HookEventName::Stop,
        thinwedge_protocol::protocol::HookRunStatus::Stopped,
        vec![thinwedge_protocol::protocol::HookOutputEntry {
            kind: thinwedge_protocol::protocol::HookOutputEntryKind::Stop,
...
async fn hook_completed_before_reveal_renders_completed_without_running_flash() {
    ...
    chat.handle_thinwedge_event(hook_started_event(
        ...
        thinwedge_protocol::protocol::HookEventName::SessionStart,
    ...
    chat.handle_thinwedge_event(hook_completed_event(
        ...
        thinwedge_protocol::protocol::HookRunStatus::Completed,
        vec![thinwedge_protocol::protocol::HookOutputEntry {
            kind: thinwedge_protocol::protocol::HookOutputEntryKind::Context,
...
fn hook_started_event(
    id: &str,
    event_name: thinwedge_protocol::protocol::HookEventName,
    ...
    Event {
        ...
        msg: EventMsg::HookStarted(thinwedge_protocol::protocol::HookStartedEvent {
            ...
            run: hook_run_summary(
                id,
                event_name,
                thinwedge_protocol::protocol::HookRunStatus::Running,
                status_message,
                Vec::new(),
            ),
        }),
...
fn hook_completed_event(
    id: &str,
    event_name: thinwedge_protocol::protocol::HookEventName,
    status: thinwedge_protocol::protocol::HookRunStatus,
    entries: Vec<thinwedge_protocol::protocol::HookOutputEntry>,
) -> Event {
    Event {
        id: id.to_string(),
        msg: EventMsg::HookCompleted(thinwedge_protocol::protocol::HookCompletedEvent {
...
fn hook_run_summary(
    ...
    thinwedge_protocol::protocol::HookRunSummary {
        ...
        started_at: 1,
        completed_at: (status != thinwedge_protocol::protocol::HookRunStatus::Running).then_some(2),
        duration_ms: (status != thinwedge_protocol::protocol::HookRunStatus::Running).then_some(1),
        entries,
    }
}
