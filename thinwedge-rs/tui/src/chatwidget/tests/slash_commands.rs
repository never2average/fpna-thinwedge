...
async fn slash_compact_eagerly_queues_follow_up_before_turn_start() {
    ...
    match rx.try_recv() {
        Ok(AppEvent::ThinWedgeOp(Op::Compact)) => {}
...
async fn queued_slash_compact_dispatches_after_active_turn() {
    ...
    assert!(
        events
            .iter()
            .any(|event| matches!(event, AppEvent::ThinWedgeOp(Op::Compact))),
...
async fn queued_bare_rename_drains_next_input_after_name_update() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::ThinWedgeOp(Op::SetThreadName { name }) if name == "Queued rename"
        )),
...
async fn goal_slash_command_emits_attached_images() {
    ...
    let local_image = chat.config.thinwedge_home.join("goal-local.png");
...
async fn queued_goal_slash_command_preserves_large_paste() {
    ...
    chat.thread_id = Some(thread_id);
    handle_turn_started(&mut chat, "turn-1");
    let paste = "x".repeat(thinwedge_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
async fn interrupt_disambiguates_same_sized_goal_pastes() {
    ...
    chat.set_feature_enabled(Feature::Goals, /*enabled*/ true);
    handle_turn_started(&mut chat, "turn-1");
    let first = "a".repeat(thinwedge_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
async fn slash_rename_prefills_existing_thread_name() {
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::ThinWedgeOp(Op::SetThreadName { name })) if name == "Current project title"
    );
}
...
async fn usage_error_slash_command_is_available_from_local_recall() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.3-thinwedge")).await;
...
async fn usage_command_runs_with_backend_auth_from_widget_init() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual_with_auth(
        ...
        /*has_thinwedge_backend_auth*/ true,
    )
    .await;
    ...
    assert_matches!(rx.try_recv(), Ok(AppEvent::RefreshTokenActivity { .. }));
    assert!(!chat.has_chatgpt_account());
    assert!(chat.has_thinwedge_backend_auth());
}
...
async fn pending_token_activity_refresh_keeps_composer_visible_in_short_viewport() {
    ...
    assert!(
        term.backend()
            ...
            .screen()
            .contents()
            .contains("Ask ThinWedge to do anything")
    );
}
...
async fn copy_shortcut_can_be_remapped() {
    ...
    keymap_config.global.copy = Some(thinwedge_config::types::KeybindingsSpec::One(
        thinwedge_config::types::KeybindingSpec("ctrl-x".to_string()),
    ));
    let runtime_keymap =
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            thinwedge_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                turn_id: None,
                goal: thinwedge_app_server_protocol::ThreadGoal {
                    thread_id: "thread-1".to_string(),
                    objective: "finish the benchmark".to_string(),
                    status: thinwedge_app_server_protocol::ThreadGoalStatus::Active,
                    token_budget: None,
                    tokens_used: 0,
                    time_used_seconds: 0,
                    created_at: 1,
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let rollout_path = PathBuf::from("/tmp/thinwedge-test-rollout.jsonl");
...
async fn fast_slash_command_updates_and_persists_local_service_tier() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::ThinWedgeOp(Op::OverrideTurnContext {
...
async fn model_switch_recomputes_catalog_default_service_tier() {
    let (mut chat, _rx, mut op_rx) = make_chatwidget_manual(Some("gpt-5.3-thinwedge")).await;
    chat.thread_id = Some(ThreadId::new());
    set_chatgpt_auth(&mut chat);
    set_fast_mode_test_catalog(&mut chat);
    chat.set_feature_enabled(Feature::FastMode, /*enabled*/ true);
...
async fn compact_queues_user_messages_snapshot() {
    ...
    handle_error(
        ...
        Some(ThinWedgeErrorInfo::ActiveTurnNotSteerable {
