...
#[tokio::test]
async fn slash_power_user_requests_power_user_permissions() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;

    chat.dispatch_command(SlashCommand::PowerUser);

    assert_matches!(rx.try_recv(), Ok(AppEvent::EnablePowerUserPermissions));
}
...
async fn slash_init_skips_when_project_doc_exists() {
    ...
    match op_rx.try_recv() {
        ...
        other => panic!("expected no ThinWedge op to be sent, got {other:?}"),
    }
...
async fn fast_slash_command_updates_and_persists_local_service_tier() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(Some("gpt-5.3-thinwedge")).await;
    chat.set_feature_enabled(Feature::FastMode, /*enabled*/ true);
