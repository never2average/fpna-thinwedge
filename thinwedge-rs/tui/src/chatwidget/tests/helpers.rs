...
pub(super) async fn test_config() -> Config {
    ...
    let thinwedge_home = tempfile::Builder::new()
...
pub(super) async fn make_chatwidget_manual_with_auth(
    ...
    has_thinwedge_backend_auth: bool,
) -> (
    ChatWidget,
    tokio::sync::mpsc::UnboundedReceiver<AppEvent>,
    tokio::sync::mpsc::UnboundedReceiver<Op>,
    ...
    let mut widget =
        ChatWidget::new_with_op_target(common, super::ThinWedgeOpTarget::Direct(op_tx));
    widget.transcript.active_cell = None;
    widget.transcript.active_cell_revision = 0;
    widget.normal_placeholder_text = "Ask ThinWedge to do anything".to_string();
...
pub(crate) fn set_chatgpt_auth(chat: &mut ChatWidget) {
    ...
    chat.has_thinwedge_backend_auth = true;
...
fn token_usage_breakdown(usage: TokenUsage) -> thinwedge_app_server_protocol::TokenUsageBreakdown {
    thinwedge_app_server_protocol::TokenUsageBreakdown {
        ...
        Some(info) => {
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    thinwedge_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: thinwedge_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    thinwedge_error_info: Option<ThinWedgeErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            thinwedge_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_absolute_path(path: &str) -> AbsolutePathBuf {
    std::env::temp_dir()
        .join("thinwedge-plugin-menu-tests")
        .join(path)
        .abs()
}
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(thinwedge_app_server_protocol::HookEventName, usize)],
    apps: &[&str],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    thinwedge_app_server_protocol::PluginHookSummary {
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    handle_hook_completed(
        ...
        hook_run(
            ...
            status_message,
            vec![
                thinwedge_app_server_protocol::HookOutputEntry {
                    kind: thinwedge_app_server_protocol::HookOutputEntryKind::Warning,
...
fn hook_event_label(event_name: thinwedge_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        thinwedge_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        thinwedge_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        thinwedge_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        thinwedge_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        thinwedge_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        thinwedge_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        thinwedge_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        thinwedge_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        thinwedge_app_server_protocol::HookEventName::SubagentStop => "SubagentStop",
        thinwedge_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
