...
async fn unified_exec_wait_status_renders_command_in_single_details_row_snapshot() {
    ...
    begin_unified_exec_startup(
        ...
        "call-wait-ui",
        "proc-ui",
        "cargo test -p thinwedge-core -- --exact some::very::long::test::name",
    );
...
async fn apply_patch_full_flow_integration_like() {
    ...
    // 3) App forwards to widget.submit_op, which pushes onto thinwedge_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to thinwedge channel");
    match forwarded {
