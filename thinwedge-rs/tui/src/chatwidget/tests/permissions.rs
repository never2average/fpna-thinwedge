...
fn windows_sandbox_requirements_stack(
    ...
    let requirements_toml = thinwedge_config::ConfigRequirementsToml {
        windows: Some(thinwedge_config::WindowsRequirementsToml {
            allowed_sandbox_implementations: Some(allowed_sandbox_implementations),
        }),
        ..Default::default()
    };
    let mut requirements_with_sources = thinwedge_config::ConfigRequirementsWithSources::default();
    requirements_with_sources
        .merge_unset_fields(RequirementSource::Unknown, requirements_toml.clone());
    let requirements = thinwedge_config::ConfigRequirements::try_from(requirements_with_sources)
...
async fn startup_windows_sandbox_prompt_blocks_disallowed_unelevated_fallback() {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 120);
    assert!(
        popup.contains("Your organization requires the default ThinWedge agent sandbox"),
...
async fn permissions_selection_sends_approvals_reviewer_in_override_turn_context() {
    ...
    let op = std::iter::from_fn(|| rx.try_recv().ok())
        .find_map(|event| match event {
            AppEvent::ThinWedgeOp(op @ Op::OverrideTurnContext { .. }) => Some(op),
            _ => None,
        })
