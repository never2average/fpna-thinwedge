...
async fn guardian_denied_exec_renders_warning_and_denied_request() {
    ...
    chat.handle_thinwedge_event(Event {
        ...
        msg: EventMsg::GuardianWarning(WarningEvent {
            message: "Automatic approval review denied (risk: high): The planned action would transmit the full contents of a workspace source file (`core/src/thinwedge.rs`) to `https://example.com`, which is an external and untrusted endpoint.".into(),
        }),
    });
    chat.handle_thinwedge_event(Event {
        id: "guardian-assessment".into(),
