...
impl ChatWidget {
    ...
    /// Clears the terminal title ThinWedge most recently wrote, if any.
    ///
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::ThinWedgeVersion => Some(THINWEDGE_CLI_VERSION.to_string()),
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            StatusSurfacePreviewItem::AppName => return Some("thinwedge".to_string()),
            ...
            StatusSurfacePreviewItem::ThinWedgeVersion => StatusLineItem::ThinWedgeVersion,
    ...
    pub(super) fn terminal_title_value_for_item(
        ...
        match item {
            TerminalTitleItem::AppName => Some("thinwedge".to_string()),
            ...
            TerminalTitleItem::WeeklyLimit => self
                ...
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
            TerminalTitleItem::ThinWedgeVersion => self
                .status_line_value_for_item(StatusLineItem::ThinWedgeVersion)
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
...
fn five_hour_status_window(
    ...
    find_primary_thinwedge_window(snapshot, "5h")
...
fn weekly_status_window(
    ...
    find_thinwedge_window(snapshot, "weekly")
        .or_else(|| snapshot.secondary.as_ref().map(|window| (window, true)))
}

fn find_thinwedge_window<'a>(
...
fn find_primary_thinwedge_window<'a>(
    snapshot: &'a RateLimitSnapshotDisplay,
    label: &str,
) -> Option<(&'a RateLimitWindowDisplay, bool)> {
