...
pub(super) fn thinwedge_ml_dir(thinwedge_home: &Path) -> PathBuf {
    thinwedge_home.join(THINWEDGE_DATA_DIR)
}

pub(super) fn models_registry_path(thinwedge_home: &Path) -> PathBuf {
    thinwedge_ml_dir(thinwedge_home).join(MODELS_FILE_NAME)
}

pub(super) fn training_environments_registry_path(thinwedge_home: &Path) -> PathBuf {
    thinwedge_ml_dir(thinwedge_home).join(ENVIRONMENTS_FILE_NAME)
}

pub(super) fn jobs_dir(thinwedge_home: &Path) -> PathBuf {
    thinwedge_ml_dir(thinwedge_home).join(JOBS_DIR_NAME)
}

pub(super) fn evals_dir(thinwedge_home: &Path) -> PathBuf {
    thinwedge_ml_dir(thinwedge_home).join(EVALS_DIR_NAME)
}

pub(super) fn runtime_contexts_dir(thinwedge_home: &Path) -> PathBuf {
    thinwedge_ml_dir(thinwedge_home).join("runtime")
}

pub(super) async fn read_models_registry(
    thinwedge_home: &Path,
) -> Result<StatisticalModelsRegistry, String> {
    read_json_or_default(&models_registry_path(thinwedge_home)).await
}

pub(super) async fn read_training_environments_registry(
    thinwedge_home: &Path,
) -> Result<TrainingEnvironmentsRegistry, String> {
    read_json_or_default(&training_environments_registry_path(thinwedge_home)).await
}
...
pub(super) async fn read_job(thinwedge_home: &Path, job_id: &str) -> Result<StoredJob, String> {
    read_json_required(&jobs_dir(thinwedge_home).join(format!("{job_id}.json"))).await
}

pub(super) async fn read_eval(thinwedge_home: &Path, eval_id: &str) -> Result<StoredEval, String> {
    read_json_required(&evals_dir(thinwedge_home).join(format!("{eval_id}.json"))).await
}

pub(super) async fn read_jobs(thinwedge_home: &Path) -> Result<Vec<StoredJob>, String> {
    read_json_directory::<StoredJob>(&jobs_dir(thinwedge_home)).await
}

pub(super) async fn read_evals(thinwedge_home: &Path) -> Result<Vec<StoredEval>, String> {
    read_json_directory::<StoredEval>(&evals_dir(thinwedge_home)).await
}
...
pub(super) async fn get_visible_environment(
    ...
    let registry = read_training_environments_registry(thinwedge_home).await?;
    registry
        .environments
        .into_iter()
        .find(|environment| {
...
pub(super) async fn ensure_environment_visible(
    ...
    get_visible_environment(thinwedge_home, agent_role, environment_id)
        .await
        .map(|_| ())
}
