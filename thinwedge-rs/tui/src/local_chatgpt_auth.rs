...
mod tests {
    ...
    fn write_chatgpt_auth(thinwedge_home: &Path, plan_type: &str) {
    ...
    fn loads_local_chatgpt_auth_from_managed_auth() {
        ...
        write_chatgpt_auth(thinwedge_home.path(), "business");
    ...
    fn preserves_usage_based_plan_type_wire_name() {
        let thinwedge_home = TempDir::new().expect("tempdir");
        write_chatgpt_auth(thinwedge_home.path(), "self_serve_business_usage_based");
