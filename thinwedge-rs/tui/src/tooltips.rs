...
lazy_static! {
    static ref TOOLTIPS: Vec<&'static str> = RAW_TOOLTIPS
        ...
        .filter(|line| {
            ...
            if !IS_MACOS && !IS_WINDOWS && line.contains("thinwedge app") {
                return false;
            }
            true
        })
...
/// Pick a random tooltip to show to the user when starting ThinWedge.
...
pub(crate) mod announcement {
    ...
    impl TargetOs {
        const fn current() -> Self {
            if cfg!(target_os = "macos") {
                ...
                // ThinWedge currently publishes CLI builds for macOS, Windows, and Linux.
                Self::Linux
            }
        }
    }
    ...
    pub(crate) fn parse_announcement_tip_toml(
        ...
        for raw in announcements {
            ...
            if tip.version_matches(THINWEDGE_CLI_VERSION)
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for ThinWedge TUI.
...
[[announcements]]
content = "Welcome to ThinWedge! Check out the new onboarding flow."
