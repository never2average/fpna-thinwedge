//! The main ThinWedge TUI chat surface.
//!
...
use thinwedge_app_server_protocol::ErrorNotification;
use thinwedge_app_server_protocol::FileChangeRequestApprovalParams;
use thinwedge_app_server_protocol::GuardianApprovalReviewAction;
use thinwedge_app_server_protocol::ItemCompletedNotification;
use thinwedge_app_server_protocol::ItemStartedNotification;
use thinwedge_app_server_protocol::McpServerStartupState;
use thinwedge_app_server_protocol::McpServerStatusDetail;
use thinwedge_app_server_protocol::McpServerStatusUpdatedNotification;
...
use thinwedge_app_server_protocol::ThreadTokenUsage;
...
use thinwedge_app_server_protocol::TurnPlanStepStatus;
...
use thinwedge_config::types::Notifications;
...
use thinwedge_protocol::approvals::ElicitationRequestEvent;
...
use thinwedge_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use thinwedge_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
#[cfg(test)]
use thinwedge_protocol::protocol::AgentMessageDeltaEvent;
#[cfg(test)]
use thinwedge_protocol::protocol::AgentMessageEvent;
#[cfg(test)]
use thinwedge_protocol::protocol::AgentReasoningDeltaEvent;
#[cfg(test)]
use thinwedge_protocol::protocol::AgentReasoningEvent;
#[cfg(test)]
use thinwedge_protocol::protocol::AgentReasoningRawContentDeltaEvent;
#[cfg(test)]
...
use thinwedge_protocol::protocol::ExecCommandOutputDeltaEvent;
use thinwedge_protocol::protocol::ExecCommandSource;
#[cfg(test)]
use thinwedge_protocol::protocol::ExitedReviewModeEvent;
...
use thinwedge_protocol::protocol::ImageGenerationBeginEvent;
use thinwedge_protocol::protocol::ImageGenerationEndEvent;
use thinwedge_protocol::protocol::ListSkillsResponseEvent;
#[cfg(test)]
use thinwedge_protocol::protocol::McpListToolsResponseEvent;
#[cfg(test)]
use thinwedge_protocol::protocol::McpStartupCompleteEvent;
...
#[cfg(test)]
use thinwedge_protocol::protocol::ModelVerification as CoreModelVerification;
...
use thinwedge_protocol::protocol::SkillMetadata as ProtocolSkillMetadata;
#[cfg(test)]
...
#[cfg(test)]
use thinwedge_protocol::protocol::UndoStartedEvent;
...
use thinwedge_protocol::request_permissions::RequestPermissionsEvent;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) feedback: thinwedge_feedback::ThinWedgeFeedback,
...
#[cfg(test)]
fn core_rate_limit_error_kind(info: &CoreThinWedgeErrorInfo) -> Option<RateLimitErrorKind> {
    match info {
        CoreThinWedgeErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        CoreThinWedgeErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        CoreThinWedgeErrorInfo::ResponseTooManyFailedAttempts {
...
fn app_server_rate_limit_error_kind(info: &AppServerThinWedgeErrorInfo) -> Option<RateLimitErrorKind> {
    match info {
        ...
        AppServerThinWedgeErrorInfo::ResponseTooManyFailedAttempts {
            http_status_code: Some(429),
        } => Some(RateLimitErrorKind::Generic),
        _ => None,
    }
...
#[cfg(test)]
fn is_core_cyber_policy_error(info: &CoreThinWedgeErrorInfo) -> bool {
    matches!(info, CoreThinWedgeErrorInfo::CyberPolicy)
}

fn is_app_server_cyber_policy_error(info: &AppServerThinWedgeErrorInfo) -> bool {
    matches!(info, AppServerThinWedgeErrorInfo::CyberPolicy)
}
...
/// and by sending requests back to thinwedge-core.
///
...
pub(crate) struct ChatWidget {
    ...
    thinwedge_op_target: ThinWedgeOpTarget,
    ...
    thinwedge_rate_limit_reached_type: Option<RateLimitReachedType>,
    ...
    /// Tracks whether thinwedge-core currently considers an agent turn to be in progress.
    ///
    ...
    feedback: thinwedge_feedback::ThinWedgeFeedback,
    ...
    session_network_proxy: Option<thinwedge_protocol::protocol::SessionNetworkProxyRuntime>,
    // Shared latch so we only warn once about invalid status-line item IDs.
...
) -> thinwedge_protocol::protocol::SessionConfiguredEvent {
    thinwedge_protocol::protocol::SessionConfiguredEvent {
...
fn hook_output_entry_from_notification(
    entry: thinwedge_app_server_protocol::HookOutputEntry,
) -> thinwedge_protocol::protocol::HookOutputEntry {
    thinwedge_protocol::protocol::HookOutputEntry {
...
fn hook_run_summary_from_notification(
    run: thinwedge_app_server_protocol::HookRunSummary,
) -> thinwedge_protocol::protocol::HookRunSummary {
    thinwedge_protocol::protocol::HookRunSummary {
        id: run.id,
...
fn hook_started_event_from_notification(
    notification: thinwedge_app_server_protocol::HookStartedNotification,
) -> thinwedge_protocol::protocol::HookStartedEvent {
    thinwedge_protocol::protocol::HookStartedEvent {
...
fn hook_completed_event_from_notification(
    notification: thinwedge_app_server_protocol::HookCompletedNotification,
) -> thinwedge_protocol::protocol::HookCompletedEvent {
    thinwedge_protocol::protocol::HookCompletedEvent {
        turn_id: notification.turn_id,
        run: hook_run_summary_from_notification(notification.run),
    }
...
fn app_server_request_id_to_mcp_request_id(
    request_id: &thinwedge_app_server_protocol::RequestId,
) -> thinwedge_protocol::mcp::RequestId {
    match request_id {
        thinwedge_app_server_protocol::RequestId::String(value) => {
            thinwedge_protocol::mcp::RequestId::String(value.clone())
        }
        thinwedge_app_server_protocol::RequestId::Integer(value) => {
            thinwedge_protocol::mcp::RequestId::Integer(*value)
        }
    }
}
...
fn exec_approval_request_from_params(
    ...
    ExecApprovalRequestEvent {
        ...
        proposed_execpolicy_amendment: params
            ...
            .map(thinwedge_app_server_protocol::ExecPolicyAmendment::into_core),
        proposed_network_policy_amendments: params.proposed_network_policy_amendments.map(
            |amendments| {
                amendments
                    .into_iter()
                    .map(thinwedge_app_server_protocol::NetworkPolicyAmendment::into_core)
                    .collect()
            },
        ),
        available_decisions: params.available_decisions.map(|decisions| {
            decisions
                ...
                .map(|decision| match decision {
                    ...
                    thinwedge_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession => {
                        thinwedge_protocol::protocol::ReviewDecision::ApprovedForSession
                    }
                    thinwedge_app_server_protocol::CommandExecutionApprovalDecision::AcceptWithExecpolicyAmendment {
                        execpolicy_amendment,
                    } => thinwedge_protocol::protocol::ReviewDecision::ApprovedExecpolicyAmendment {
                        proposed_execpolicy_amendment: execpolicy_amendment.into_core(),
                    },
                    thinwedge_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                        network_policy_amendment,
                    } => thinwedge_protocol::protocol::ReviewDecision::NetworkPolicyAmendment {
                        network_policy_amendment: network_policy_amendment.into_core(),
                    },
                    thinwedge_app_server_protocol::CommandExecutionApprovalDecision::Decline => {
                        thinwedge_protocol::protocol::ReviewDecision::Denied
                    }
                    thinwedge_app_server_protocol::CommandExecutionApprovalDecision::Cancel => {
                        thinwedge_protocol::protocol::ReviewDecision::Abort
                    }
                })
                .collect()
        }),
        parsed_cmd: params
            ...
            .unwrap_or_default()
            .into_iter()
            .map(thinwedge_app_server_protocol::CommandAction::into_core)
            .collect(),
    }
}
...
fn request_permissions_from_params(
    params: thinwedge_app_server_protocol::PermissionsRequestApprovalParams,
...
fn request_user_input_from_params(params: ToolRequestUserInputParams) -> RequestUserInputEvent {
    RequestUserInputEvent {
        ...
        questions: params
            ...
            .into_iter()
            .map(
                |question| thinwedge_protocol::request_user_input::RequestUserInputQuestion {
                    id: question.id,
...
fn web_search_action_to_core(
    action: thinwedge_app_server_protocol::WebSearchAction,
) -> thinwedge_protocol::models::WebSearchAction {
    match action {
        thinwedge_app_server_protocol::WebSearchAction::Search { query, queries } => {
            thinwedge_protocol::models::WebSearchAction::Search { query, queries }
        }
        thinwedge_app_server_protocol::WebSearchAction::OpenPage { url } => {
            thinwedge_protocol::models::WebSearchAction::OpenPage { url }
        }
        thinwedge_app_server_protocol::WebSearchAction::FindInPage { url, pattern } => {
            thinwedge_protocol::models::WebSearchAction::FindInPage { url, pattern }
        }
        thinwedge_app_server_protocol::WebSearchAction::Other => {
            thinwedge_protocol::models::WebSearchAction::Other
        }
    }
}
...
impl ChatWidget {
    ...
    fn handle_app_server_steer_rejected_error(
        &mut self,
        thinwedge_error_info: &AppServerThinWedgeErrorInfo,
    ) -> bool {
        matches!(
            thinwedge_error_info,
            AppServerThinWedgeErrorInfo::ActiveTurnNotSteerable { .. }
        ) && self.enqueue_rejected_steer()
    }
    ...
    pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
        if let Some(mut snapshot) = snapshot {
            ...
            let is_thinwedge_limit = limit_id.eq_ignore_ascii_case("thinwedge");
            if is_thinwedge_limit
                && let Some(rate_limit_reached_type) = snapshot.rate_limit_reached_type
            {
                self.thinwedge_rate_limit_reached_type = Some(rate_limit_reached_type);
            }
            let warnings = if is_thinwedge_limit {
            ...
            let high_usage = is_thinwedge_limit
                && (snapshot
                    .secondary
                    .as_ref()
            ...
            self.thinwedge_rate_limit_reached_type = None;
        }
        self.refresh_status_line();
    }
    ...
    fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "ThinWedge is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        let rate_limit_reached_type = self.thinwedge_rate_limit_reached_type.map(|kind| {
        ...
        self.thinwedge_rate_limit_reached_type = rate_limit_reached_type;

        match rate_limit_reached_type {
            Some(RateLimitReachedType::WorkspaceOwnerCreditsDepleted) => {
                self.on_error(
                    "You're out of credits. Your workspace is out of credits. Add credits to continue using ThinWedge."
                        .to_string(),
                );
            }
            Some(RateLimitReachedType::WorkspaceOwnerUsageLimitReached) => {
                self.on_error(
                    "Usage limit reached. You've reached your usage limit. Increase your limits to continue using thinwedge."
                        .to_string(),
                );
            }
    ...
    fn handle_non_retry_error(
        &mut self,
        message: String,
        thinwedge_error_info: Option<AppServerThinWedgeErrorInfo>,
    ) {
        if thinwedge_error_info
            .as_ref()
            .is_some_and(|info| self.handle_app_server_steer_rejected_error(info))
        {
        } else if thinwedge_error_info
            .as_ref()
            .is_some_and(is_app_server_cyber_policy_error)
        {
            self.on_cyber_policy_error();
        } else if let Some(info) = thinwedge_error_info
            .as_ref()
    ...
    fn on_guardian_assessment(&mut self, ev: GuardianAssessmentEvent) {
        ...
        if ev.status == GuardianAssessmentStatus::TimedOut {
            let cell = if let Some(command) = guardian_command(&ev.action) {
                history_cell::new_approval_decision_cell(
                    command,
                    thinwedge_protocol::protocol::ReviewDecision::TimedOut,
                    history_cell::ApprovalDecisionActor::Guardian,
                )
            } else {
                match &ev.action {
                    ...
                    GuardianAssessmentAction::McpToolCall {
                        ...
                        "thinwedge could call MCP tool {server}.{tool_name}"
                    )),
                    GuardianAssessmentAction::NetworkAccess { target, .. } => {
                        history_cell::new_guardian_timed_out_action_request(format!(
                            "thinwedge could access {target}"
                        ))
                    }
                    GuardianAssessmentAction::RequestPermissions { reason, .. } => {
        ...
        let cell = if let Some(command) = guardian_command(&ev.action) {
            ...
            match &ev.action {
                ...
                GuardianAssessmentAction::McpToolCall {
                    ...
                    "thinwedge to call MCP tool {server}.{tool_name}"
                )),
                GuardianAssessmentAction::NetworkAccess { target, .. } => {
                    history_cell::new_guardian_denied_action_request(format!(
                        "thinwedge to access {target}"
                    ))
                }
                GuardianAssessmentAction::RequestPermissions { reason, .. } => {
                    history_cell::new_guardian_denied_action_request(permission_request_summary(
                        "thinwedge to request permissions",
                        reason,
                    ))
                }
                GuardianAssessmentAction::Command { .. } => unreachable!(),
    ...
    fn on_patch_apply_end(&mut self, event: thinwedge_protocol::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
            ...
            CollabAgentTool::SpawnAgent => {
                ...
                if !matches!(status, CollabAgentToolCallStatus::InProgress) {
                    ...
                    self.on_collab_event(multi_agents::spawn_end(
                        thinwedge_protocol::protocol::CollabAgentSpawnEndEvent {
                            call_id: id,
                            sender_thread_id,
            ...
            CollabAgentTool::SendInput => {
                    ...
                    && !matches!(status, CollabAgentToolCallStatus::InProgress)
                {
                    self.on_collab_event(multi_agents::interaction_end(
                        thinwedge_protocol::protocol::CollabAgentInteractionEndEvent {
                            call_id: id,
                            sender_thread_id,
                            receiver_thread_id,
                ...
                if let Some(receiver_thread_id) = first_receiver {
                    if matches!(status, CollabAgentToolCallStatus::InProgress) {
                        self.on_collab_event(multi_agents::resume_begin(
                            thinwedge_protocol::protocol::CollabResumeBeginEvent {
                                call_id: id,
                                sender_thread_id,
                                receiver_thread_id,
                                receiver_agent_nickname: first_receiver_metadata
                        ...
                        self.on_collab_event(multi_agents::resume_end(
                            thinwedge_protocol::protocol::CollabResumeEndEvent {
                                call_id: id,
                                sender_thread_id,
                                receiver_thread_id,
                                receiver_agent_nickname: first_receiver_metadata
            ...
            CollabAgentTool::Wait => {
                if matches!(status, CollabAgentToolCallStatus::InProgress) {
                    self.on_collab_event(multi_agents::waiting_begin(
                        thinwedge_protocol::protocol::CollabWaitingBeginEvent {
                            sender_thread_id,
                    ...
                    self.on_collab_event(multi_agents::waiting_end(
                        thinwedge_protocol::protocol::CollabWaitingEndEvent {
                            sender_thread_id,
                            call_id: id,
                            agent_statuses,
                            statuses,
            ...
            CollabAgentTool::CloseAgent => {
                    ...
                    && !matches!(status, CollabAgentToolCallStatus::InProgress)
                {
                    self.on_collab_event(multi_agents::close_end(
                        thinwedge_protocol::protocol::CollabCloseEndEvent {
                            call_id: id,
                            sender_thread_id,
                            receiver_thread_id,
                            receiver_agent_nickname: first_receiver_metadata
    ...
    pub(crate) fn handle_history_entry_response(
        &mut self,
        event: thinwedge_protocol::protocol::GetHistoryEntryResponseEvent,
    ) {
        let thinwedge_protocol::protocol::GetHistoryEntryResponseEvent {
            offset,
            log_id,
            entry,
        } = event;
    ...
    fn on_hook_started(&mut self, event: thinwedge_protocol::protocol::HookStartedEvent) {
        self.flush_answer_stream_with_separator();
    ...
    fn on_hook_completed(&mut self, event: thinwedge_protocol::protocol::HookCompletedEvent) {
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: thinwedge_protocol::protocol::PatchApplyEndEvent,
    ) {
    ...
    pub(crate) fn new_with_app_event(common: ChatWidgetInit) -> Self {
        Self::new_with_op_target(common, ThinWedgeOpTarget::AppEvent)
    }

    fn new_with_op_target(common: ChatWidgetInit, thinwedge_op_target: ThinWedgeOpTarget) -> Self {
        let ChatWidgetInit {
            config,
            frame_requester,
            app_event_tx,
        ...
        let mut widget = Self {
            ...
            thinwedge_rate_limit_reached_type: None,
    ...
    fn submit_user_message_with_history_and_shell_escape_policy(
        ...
        if let Some(apps) = self.connectors_for_mentions() {
            ...
            for app in app_mentions {
                let slug = thinwedge_connectors::metadata::connector_mention_slug(&app);
        ...
        match item {
            ThreadItem::UserMessage { id, content } => {
                let user_message = UserMessageItem {
                    ...
                    content: content
                        .into_iter()
                        .map(thinwedge_app_server_protocol::UserInput::into_core)
                        .collect(),
                };
            ...
            ThreadItem::AgentMessage {
                ...
                self.on_agent_message_item_completed(AgentMessageItem {
                    ...
                    memory_citation: memory_citation.map(|citation| {
                        thinwedge_protocol::memory_citation::MemoryCitation {
                            entries: citation
                                .entries
                                .into_iter()
                                .map(
                    ...
                    self.on_exec_command_end(ExecCommandEndEvent {
                        ...
                        status: match status {
                            thinwedge_app_server_protocol::CommandExecutionStatus::Completed => {
                                thinwedge_protocol::protocol::ExecCommandStatus::Completed
                            }
                            thinwedge_app_server_protocol::CommandExecutionStatus::Failed => {
                                thinwedge_protocol::protocol::ExecCommandStatus::Failed
                            }
                            thinwedge_app_server_protocol::CommandExecutionStatus::Declined => {
                                thinwedge_protocol::protocol::ExecCommandStatus::Declined
                            }
                            thinwedge_app_server_protocol::CommandExecutionStatus::InProgress => {
                                thinwedge_protocol::protocol::ExecCommandStatus::Failed
                            }
            ...
            ThreadItem::FileChange {
                ...
                if !matches!(
                    status,
                    thinwedge_app_server_protocol::PatchApplyStatus::InProgress
                ) {
                    self.on_patch_apply_end(thinwedge_protocol::protocol::PatchApplyEndEvent {
                        call_id: id,
                        turn_id: turn_id.clone(),
                        stdout: String::new(),
                        stderr: String::new(),
                        success: !matches!(
                            status,
                            thinwedge_app_server_protocol::PatchApplyStatus::Failed
                        ),
                        changes: file_update_changes_to_core(changes),
                        status: match status {
                            thinwedge_app_server_protocol::PatchApplyStatus::Completed => {
                                thinwedge_protocol::protocol::PatchApplyStatus::Completed
                            }
                            thinwedge_app_server_protocol::PatchApplyStatus::Failed => {
                                thinwedge_protocol::protocol::PatchApplyStatus::Failed
                            }
                            thinwedge_app_server_protocol::PatchApplyStatus::Declined => {
                                thinwedge_protocol::protocol::PatchApplyStatus::Declined
                            }
                            thinwedge_app_server_protocol::PatchApplyStatus::InProgress => {
                                thinwedge_protocol::protocol::PatchApplyStatus::Failed
                            }
            ...
            ThreadItem::McpToolCall {
                ...
                self.on_mcp_tool_call_end(thinwedge_protocol::protocol::McpToolCallEndEvent {
                    call_id: id,
                    invocation: thinwedge_protocol::protocol::McpInvocation {
                        server,
                        tool,
                    ...
                    result: match (result, error) {
                        ...
                        (Some(result), None) => {
                            let result = *result;
                            Ok(thinwedge_protocol::mcp::CallToolResult {
            ...
            ThreadItem::WebSearch { id, query, action } => {
                ...
                self.on_web_search_end(WebSearchEndEvent {
                    ...
                    action: action
                        ...
                        .unwrap_or(thinwedge_protocol::models::WebSearchAction::Other),
                });
            }
        ...
        match notification {
            ...
            ServerNotification::ThreadNameUpdated(notification) => {
                match ThreadId::from_string(&notification.thread_id) {
                    Ok(thread_id) => self.on_thread_name_updated(
                        thinwedge_protocol::protocol::ThreadNameUpdatedEvent {
                            thread_id,
            ...
            ServerNotification::CommandExecutionOutputDelta(notification) => {
                self.on_exec_command_output_delta(ExecCommandOutputDeltaEvent {
                    call_id: notification.item_id,
                    stream: thinwedge_protocol::protocol::ExecOutputStream::Stdout,
            ...
            ServerNotification::Error(notification) => {
                if notification.will_retry {
                    ...
                    self.handle_non_retry_error(
                        notification.error.message,
                        notification.error.thinwedge_error_info,
                    );
                }
            }
            ...
            ServerNotification::ThreadRealtimeStarted(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_started(
                        thinwedge_protocol::protocol::RealtimeConversationStartedEvent {
            ...
            ServerNotification::ThreadRealtimeOutputAudioDelta(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        thinwedge_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: thinwedge_protocol::protocol::RealtimeEvent::AudioOut(
            ...
            ServerNotification::ThreadRealtimeError(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_realtime(
                        thinwedge_protocol::protocol::RealtimeConversationRealtimeEvent {
                            payload: thinwedge_protocol::protocol::RealtimeEvent::Error(
            ...
            ServerNotification::ThreadRealtimeClosed(notification) => {
                if !from_replay {
                    self.on_realtime_conversation_closed(
                        thinwedge_protocol::protocol::RealtimeConversationClosedEvent {
    ...
    fn on_mcp_server_elicitation_request(
        ...
        params: thinwedge_app_server_protocol::McpServerElicitationRequestParams,
    ) {
        let request = thinwedge_protocol::approvals::ElicitationRequestEvent {
            turn_id: params.turn_id,
            server_name: params.server_name,
            id: request_id,
            request: match params.request {
                thinwedge_app_server_protocol::McpServerElicitationRequest::Form {
                    meta,
                    message,
                    requested_schema,
                } => thinwedge_protocol::approvals::ElicitationRequest::Form {
                    meta,
                    message,
                    requested_schema: serde_json::to_value(requested_schema)
                        .unwrap_or(serde_json::Value::Null),
                },
                thinwedge_app_server_protocol::McpServerElicitationRequest::Url {
                    meta,
                    message,
                    url,
                    elicitation_id,
                } => thinwedge_protocol::approvals::ElicitationRequest::Url {
                    meta,
                    message,
                    url,
                    elicitation_id,
            ...
            TurnStatus::Failed => {
                if let Some(error) = notification.turn.error {
                        ...
                        == Some(&(notification.turn.id.clone(), error.message.clone()))
                    {
                        ...
                        self.handle_non_retry_error(error.message, error.thinwedge_error_info);
                    }
                } else {
                    self.last_non_retry_error = None;
                    self.finalize_turn();
    ...
    fn on_guardian_review_notification(
        ...
        id: String,
        turn_id: String,
        review: thinwedge_app_server_protocol::GuardianApprovalReview,
        decision_source: Option<thinwedge_app_server_protocol::AutoReviewDecisionSource>,
        ...
        self.on_guardian_assessment(GuardianAssessmentEvent {
            ...
            status: match review.status {
                thinwedge_app_server_protocol::GuardianApprovalReviewStatus::InProgress => {
                    GuardianAssessmentStatus::InProgress
                }
                thinwedge_app_server_protocol::GuardianApprovalReviewStatus::Approved => {
                    GuardianAssessmentStatus::Approved
                }
                thinwedge_app_server_protocol::GuardianApprovalReviewStatus::Denied => {
                    GuardianAssessmentStatus::Denied
                }
                thinwedge_app_server_protocol::GuardianApprovalReviewStatus::TimedOut => {
                    GuardianAssessmentStatus::TimedOut
                }
                thinwedge_app_server_protocol::GuardianApprovalReviewStatus::Aborted => {
                    GuardianAssessmentStatus::Aborted
                }
            },
            risk_level: review.risk_level.map(|risk_level| match risk_level {
                thinwedge_app_server_protocol::GuardianRiskLevel::Low => {
                    thinwedge_protocol::protocol::GuardianRiskLevel::Low
                }
                thinwedge_app_server_protocol::GuardianRiskLevel::Medium => {
                    thinwedge_protocol::protocol::GuardianRiskLevel::Medium
                }
                thinwedge_app_server_protocol::GuardianRiskLevel::High => {
                    thinwedge_protocol::protocol::GuardianRiskLevel::High
                }
                thinwedge_app_server_protocol::GuardianRiskLevel::Critical => {
                    thinwedge_protocol::protocol::GuardianRiskLevel::Critical
                }
            }),
            user_authorization: review.user_authorization.map(|user_authorization| {
                match user_authorization {
                    thinwedge_app_server_protocol::GuardianUserAuthorization::Unknown => {
                        thinwedge_protocol::protocol::GuardianUserAuthorization::Unknown
                    }
                    thinwedge_app_server_protocol::GuardianUserAuthorization::Low => {
                        thinwedge_protocol::protocol::GuardianUserAuthorization::Low
                    }
                    thinwedge_app_server_protocol::GuardianUserAuthorization::Medium => {
                        thinwedge_protocol::protocol::GuardianUserAuthorization::Medium
                    }
                    thinwedge_app_server_protocol::GuardianUserAuthorization::High => {
                        thinwedge_protocol::protocol::GuardianUserAuthorization::High
                    }
                }
            }),
            rationale: review.rationale,
            decision_source: decision_source.map(|source| match source {
                thinwedge_app_server_protocol::AutoReviewDecisionSource::Agent => {
    ...
    #[cfg(test)]
    pub(crate) fn handle_thinwedge_event(&mut self, event: Event) {
        let Event { id, msg } = event;
        self.dispatch_event_msg(Some(id), msg, /*replay_kind*/ None);
    }

    #[cfg(test)]
    pub(crate) fn handle_thinwedge_event_replay(&mut self, event: Event) {
        let Event { msg, .. } = event;
    ...
    fn dispatch_event_msg(
        ...
        match msg {
            ...
            _ => {
                tracing::trace!("handle_thinwedge_event: {:?}", msg);
            }
        }
        ...
        match msg {
            ...
            EventMsg::Error(ErrorEvent {
                ...
                {
                    self.on_cyber_policy_error();
                } else if let Some(kind) = thinwedge_error_info
                    .as_ref()
            ...
            EventMsg::ItemCompleted(event) => {
                ...
                if let thinwedge_protocol::items::TurnItem::AgentMessage(item) = item {
    ...
    fn open_theme_picker(&mut self) {
        let thinwedge_home = crate::legacy_core::config::find_thinwedge_home().ok();
    ...
    fn prefetch_connectors_with_options(&mut self, force_refetch: bool) {
        ...
        tokio::spawn(async move {
            ...
            let should_schedule_force_refetch =
                !force_refetch && !accessible_result.thinwedge_apps_ready;
    ...
    fn open_workspace_owner_nudge_prompt(&mut self, credit_type: AddCreditsNudgeCreditType) {
        ...
        let (title, prompt) = match credit_type {
            ...
            AddCreditsNudgeCreditType::UsageLimit => (
                "Usage limit reached",
                "Request a limit increase from your owner to continue using thinwedge. Request increase?",
            ),
        };
    ...
    fn open_personality_popup_for_current_model(&mut self) {
        ...
        header.push(Line::from(
            "Choose a communication style for ThinWedge.".dim(),
        ));
    ...
    pub(crate) fn open_realtime_audio_popup(&mut self) {
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            subtitle: Some("Configure settings for ThinWedge.".to_string()),
            footer_hint: Some(standard_popup_hint_line()),
            items,
            ..Default::default()
        });
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("thinwedge-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "thinwedge-auto-fast" => 0,
            "thinwedge-auto-balanced" => 1,
            "thinwedge-auto-thorough" => 2,
            _ => 3,
        }
    }
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running thinwedge -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        let warn_for_model = preset.model.starts_with("gpt-5.1-thinwedge")
            || preset.model.starts_with("gpt-5.1-thinwedge-max")
                        ...
                        == WindowsSandboxLevel::Disabled
                    {
                        ...
                        if crate::legacy_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                            && crate::legacy_core::windows_sandbox::sandbox_setup_is_complete(
                                self.config.thinwedge_home.as_path(),
                            )
                        {
                            vec![Box::new(move |tx| {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match thinwedge_windows_sandbox::apply_world_writable_scan_and_denies(
            self.config.thinwedge_home.as_path(),
            cwd.as_path(),
            &env_map,
            &policy,
            Some(self.config.thinwedge_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn open_full_access_confirmation(
        ...
        let info_line = Line::from(vec![
            "When ThinWedge runs with full access, it can edit any file on your computer and run commands with network, without your approval. "
                .into(),
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, preset: ApprovalPreset) {
        ...
        self.session_telemetry.counter(
            "thinwedge.windows_sandbox.elevated_prompt_shown",
            /*inc*/ 1,
            &[],
        );
        ...
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    accept_otel.counter(
                        "thinwedge.windows_sandbox.elevated_prompt_accept",
                        /*inc*/ 1,
                        &[],
                    );
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    legacy_otel.counter(
                        "thinwedge.windows_sandbox.elevated_prompt_use_legacy",
                        /*inc*/ 1,
                        &[],
                    );
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "thinwedge.windows_sandbox.elevated_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
        ...
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new({
                    ...
                    move |tx| {
                        otel.counter(
                            "thinwedge.windows_sandbox.fallback_retry_elevated",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxElevatedSetup {
            ...
            SelectionItem {
                name: "Use ThinWedge with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
                    ...
                    move |tx| {
                        otel.counter(
                            "thinwedge.windows_sandbox.fallback_use_legacy",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxLegacySetup {
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "thinwedge.windows_sandbox.fallback_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
                    tx.send(AppEvent::Exit(ExitMode::ShutdownFirst));
    ...
    fn set_service_tier_selection(&mut self, service_tier: Option<ServiceTier>) {
        ...
        self.app_event_tx.send(AppEvent::ThinWedgeOp(
            AppCommand::override_turn_context(
                /*cwd*/ None,
                /*approval_policy*/ None,
                /*approvals_reviewer*/ None,
    ...
    pub(crate) fn set_collaboration_mask(&mut self, mut mask: CollaborationModeMask) {
            ...
            && (previous_model != next_model || previous_effort != next_effort)
        {
            ...
            if !next_model.starts_with("thinwedge-auto-") {
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd = crate::legacy_core::util::resume_command(Some(name), thread_id)
            .unwrap_or_else(|| format!("thinwedge resume {name}"));
    ...
    fn connectors_popup_params(
        ...
        for connector in connectors {
            ...
            let instructions = if connector.is_accessible {
                ...
                "Install this app in your browser, then reload ThinWedge."
            };
    ...
    /// Forward a command directly to thinwedge.
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.thinwedge_op_target {
            ThinWedgeOpTarget::Direct(thinwedge_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = thinwedge_op_tx.send(op.into_core()) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            ThinWedgeOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::ThinWedgeOp(op.into()));
            }
        }
        true
    }
...
impl Notification {
    ...
    fn user_input_request_summary(
        questions: &[thinwedge_protocol::request_user_input::RequestUserInputQuestion],
    ) -> Option<String> {
