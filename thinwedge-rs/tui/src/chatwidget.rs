...
use thinwedge_app_server_protocol::CreditsSnapshot;
...
use thinwedge_app_server_protocol::RateLimitReachedType;
...
use thinwedge_app_server_protocol::SkillMetadata as ProtocolSkillMetadata;
...
use thinwedge_config::ConstraintResult;
...
use thinwedge_protocol::approvals::GuardianAssessmentDecisionSource;
...
use thinwedge_utils_cli::resume_hint;
use thinwedge_utils_plugins::mention_syntax::PLUGIN_TEXT_MENTION_SIGIL;
use thinwedge_utils_plugins::mention_syntax::TOOL_MENTION_SIGIL;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) has_thinwedge_backend_auth: bool,
...
impl ChatWidget {
    ...
    pub(crate) fn open_feedback_consent(&mut self, category: crate::app_event::FeedbackCategory) {
        ...
        let include_windows_sandbox_log =
            thinwedge_windows_sandbox::current_log_file_path_for_thinwedge_home(
                ...
                .filter_map(|item| match item {
                    ...
                    UserInput::Mention { name, path } => {
                        ...
                        let mention = if let Some(plugin_id) = plugin_id {
                            ...
                            thinwedge_connectors::metadata::connector_mention_slug_from_name(name)
                        } else {
                            name.clone()
                        };
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.thinwedge_op_target {
            ThinWedgeOpTarget::Direct(thinwedge_op_tx) => {
                ...
                if let Err(e) = thinwedge_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            ThinWedgeOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::ThinWedgeOp(op));
            }
        }
        true
    }
