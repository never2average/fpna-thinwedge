...
use clap::Parser;
use thinwedge_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `thinwedge resume` subcommand.
    // These are not exposed as user flags on the base `thinwedge` command.
    #[clap(skip)]
    ...
    /// top-level `thinwedge resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `thinwedge fork` subcommand.
    // These are not exposed as user flags on the base `thinwedge` command.
    #[clap(skip)]
    ...
    /// top-level `thinwedge fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
