            ...
            ThreadItem::UserMessage { content, .. } => Some(TranscriptPreviewLine {
                ...
                text: content
                    ...
                    .filter_map(|input| match input {
                        thinwedge_app_server_protocol::UserInput::Text { text, .. } => {
                            Some(text.as_str())
                        }
                        _ => None,
                    })
...
impl PickerState {
    ...
    async fn persist_density(&self) -> Result<()> {
        ...
        ConfigEditsBuilder::new(&persistence.thinwedge_home)
...
mod tests {
    ...
    fn row_search_matches_metadata_fields() {
        ...
        let row = Row {
            ...
            created_at: None,
            updated_at: None,
            cwd: Some(PathBuf::from("/tmp/thinwedge-session-picker")),
    ...
    fn expanded_session_details_include_metadata() {
        ...
        let row = Row {
            ...
            cwd: Some(PathBuf::from("/Users/felipe.coury/code/thinwedge")),
            git_branch: Some(String::from("thinwedge/raw-scrollback-mode")),
        };
        ...
        assert!(rendered.contains("Branch:      thinwedge/raw-scrollback-mode"));
    ...
    fn footer_prioritizes_active_sort_timestamp() {
        ...
        assert_metadata_order(&updated[0], "⌁ tmp/thinwedge", " main");
        assert_metadata_order(&created[0], "⌁ tmp/thinwedge", " main");
    }
    ...
    fn footer_marks_missing_branch() {
        ...
        assert!(rendered.contains("⌁ /tmp/thinwedge"));
        assert!(rendered.contains(" no branch"));
        assert_metadata_order(&footer[0], "⌁ /tmp/thinwedge", " no branch");
    }
    ...
    fn footer_branch_expands_when_line_has_room() {
        ...
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some(branch),
            Some("~/code/thinwedge.etraut-animations-false-improvements/thinwedge-rs"),
            /*show_cwd*/ true,
            /*width*/ 140,
        );
    ...
    fn footer_cwd_truncates_to_responsive_column() {
        let cwd = "~/code/thinwedge.owner-extremely-long-worktree-name-that-needs-truncating/thinwedge-rs";
        ...
        assert!(footer.contains("⌁ ~/code/thinwedge."));
    ...
    fn footer_omits_cwd_when_hidden() {
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some("owner/branch"),
            Some("~/code/thinwedge.owner-worktree/thinwedge-rs"),
            /*show_cwd*/ false,
            /*width*/ 80,
        );
...
"#
    ...
    async fn ctrl_o_keeps_toggled_density_when_persistence_fails() {
        let tmp = tempdir().expect("tmpdir");
        let thinwedge_home_file = tmp.path().join("thinwedge-home-file");
        std::fs::write(&thinwedge_home_file, "not a directory").expect("write thinwedge home file");
        let loader = page_only_loader(|_| {});
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
            loader,
        ...
        state.view_persistence = Some(SessionPickerViewPersistence {
            thinwedge_home: thinwedge_home_file,
        });
    ...
    fn dense_snapshot_row() -> Row {
        Row {
            ...
            cwd: Some(PathBuf::from(
                "/Users/felipe.coury/code/thinwedge.fcoury-session-picker/thinwedge-rs",
            )),
            git_branch: Some(String::from("fcoury/session-picker")),
        }
    }
    ...
    fn expanded_session_snapshot() {
        ...
        let row = Row {
            ...
            created_at: parse_timestamp_str("2026-04-28T16:30:00Z"),
            updated_at: parse_timestamp_str("2026-04-28T17:45:00Z"),
            cwd: Some(PathBuf::from("/tmp/thinwedge")),
            git_branch: Some(String::from("fcoury/session-picker")),
        };
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
    ...
    fn thread_to_transcript_cells_renders_core_message_types() {
        ...
        let thread = Thread {
            ...
            git_info: None,
            name: None,
            turns: vec![thinwedge_app_server_protocol::Turn {
