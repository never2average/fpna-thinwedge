//! Transcript/history cells for the ThinWedge TUI.
//!
...
mod dynamic_tool_cell;
mod hook_cell;

pub(crate) use dynamic_tool_cell::new_dynamic_tool_call;
...
impl HistoryCell for UpdateAvailableHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let update_instruction = if let Some(update_action) = self.update_action {
            ...
            line![
                "See ",
                "https://github.com/never2average/fpna-thinwedge"
                    .cyan()
                    .underlined(),
                " for ThinWedge installation options."
            ]
        };
        ...
        let content = text![
            line![
                ...
                " ",
                format!("{THINWEDGE_CLI_VERSION} -> {}", self.latest_version).bold(),
            ],
            update_instruction,
            "",
...
pub fn new_approval_decision_cell(
    command: Vec<String>,
    decision: thinwedge_protocol::protocol::ReviewDecision,
        ...
        ApprovedExecpolicyAmendment {
            ...
            (
                ...
                vec![
                    actor.subject().into(),
                    "approved".bold(),
                    " ThinWedge to always run commands that start with ".into(),
                    snippet,
                ],
            )
        }
        ...
        NetworkPolicyAmendment {
            ...
            NetworkPolicyRuleAction::Allow => (
                ...
                vec![
                    actor.subject().into(),
                    "persisted".bold(),
                    " ThinWedge network access to ".into(),
        ...
        TimedOut => {
            ...
            (
                ...
                vec![
                    "Review ".into(),
                    "timed out".bold(),
                    " before ThinWedge could run ".into(),
                    snippet,
                ],
            )
        }
...
pub fn new_guardian_timed_out_patch_request(files: Vec<String>) -> Box<dyn HistoryCell> {
    let mut summary = vec![
        "Review ".into(),
        "timed out".bold(),
        " before ThinWedge could apply ".into(),
    ];
    if files.len() == 1 {
        summary.push("a patch touching ".into());
        summary.push(Span::from(files[0].clone()).dim());
    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for ThinWedge".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/permissions".into(),
                " - choose what ThinWedge is allowed to do".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
        ...
        && matches!(
            ...
            PermissionProfile::Disabled
                | PermissionProfile::Managed {
                    ...
                    network: thinwedge_protocol::protocol::NetworkSandboxPolicy::Enabled,
                }
        )
}
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // Title line rendered inside the box: ">_ ThinWedge (vX)"
...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<thinwedge_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<thinwedge_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(thinwedge_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`thinwedge-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
///   register it as an MCP server via `thinwedge mcp add`.
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, thinwedge_protocol::mcp::Tool>,
...
mod tests {
    ...
    async fn test_config() -> Config {
        let thinwedge_home = std::env::temp_dir();
    ...
    fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
        ...
        assert_eq!(
            rendered,
            vec![
                "✔ You approved ThinWedge to".to_string(),
    ...
    fn multiline_command_wraps_with_extra_indent_on_subsequent_lines() {
        ...
        let cmd = "set -o pipefail\ncargo test -p thinwedge-tui --quiet".to_string();
        let call_id = "c1".to_string();
        let mut cell = ExecCell::new(
            ExecCall {
                call_id: call_id.clone(),
