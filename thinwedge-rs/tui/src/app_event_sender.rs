...
impl AppEventSender {
    ...
    pub(crate) fn send(&self, event: AppEvent) {
        ...
        if !matches!(event, AppEvent::ThinWedgeOp(_)) {
    ...
    pub(crate) fn interrupt(&self) {
        self.send(AppEvent::ThinWedgeOp(AppCommand::interrupt()));
    }

    pub(crate) fn interrupt_and_restore_prompt_if_no_output(&self) {
        self.send(AppEvent::ThinWedgeOp(
            AppCommand::interrupt_and_restore_prompt_if_no_output(),
        ));
    }

    pub(crate) fn compact(&self) {
        self.send(AppEvent::ThinWedgeOp(AppCommand::compact()));
    }

    pub(crate) fn set_thread_name(&self, name: String) {
        self.send(AppEvent::ThinWedgeOp(AppCommand::set_thread_name(name)));
    }

    pub(crate) fn review(&self, target: ReviewTarget) {
        self.send(AppEvent::ThinWedgeOp(AppCommand::review(target)));
    }

    pub(crate) fn list_skills(&self, cwds: Vec<PathBuf>, force_reload: bool) {
        self.send(AppEvent::ThinWedgeOp(AppCommand::list_skills(
            cwds,
            force_reload,
        )));
    }
    ...
    pub(crate) fn user_input_answer(&self, id: String, response: ToolRequestUserInputResponse) {
        self.send(AppEvent::ThinWedgeOp(AppCommand::user_input_answer(
            id, response,
        )));
    }
