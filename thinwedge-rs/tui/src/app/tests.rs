...
use thinwedge_app_server_protocol::AdditionalPermissionProfile;
...
use thinwedge_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use thinwedge_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use thinwedge_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use thinwedge_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
...
use thinwedge_app_server_protocol::TokenUsageBreakdown;
...
use thinwedge_app_server_protocol::UserInput as AppServerUserInput;
...
use thinwedge_protocol::protocol::MAX_THREAD_GOAL_OBJECTIVE_CHARS;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let thinwedge_app_server_client::AppServerEvent::ServerNotification(
...
async fn handle_mcp_inventory_result_respects_origin_thread() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: thinwedge_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_thinwedge_backend_auth: false,
        model_catalog: app.model_catalog.clone(),
        feedback: thinwedge_feedback::ThinWedgeFeedback::new(),
        is_first_run: false,
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::ThinWedgeOp(Op::UserTurn { items, .. }) => {
                submitted_items = Some(items);
            }
            _ => {}
        }
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: thinwedge_app_server_protocol::CollabAgentTool::Wait,
                status: thinwedge_app_server_protocol::CollabAgentToolCallStatus::InProgress,
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemCompleted(
            thinwedge_app_server_protocol::ItemCompletedNotification {
                ...
                item: ThreadItem::CollabAgentToolCall {
                    id: "send-1".to_string(),
                    tool: thinwedge_app_server_protocol::CollabAgentTool::SendInput,
                    status: thinwedge_app_server_protocol::CollabAgentToolCallStatus::Failed,
                    sender_thread_id: ThreadId::new().to_string(),
                    receiver_thread_ids: vec![receiver_thread_id.to_string()],
                    ...
                    agents_states: HashMap::from([(
                        ...
                        thinwedge_app_server_protocol::CollabAgentState {
                            status: thinwedge_app_server_protocol::CollabAgentStatus::NotFound,
                            message: None,
                        },
                    )]),
                },
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let thinwedge_home = tempdir()?;
        app.config.thinwedge_home = thinwedge_home.path().to_path_buf().abs();
        app.config.sqlite_home = thinwedge_home.path().to_path_buf();
...
"#,
    ...
    let op = match app_event_rx.try_recv() {
        Ok(AppEvent::ThinWedgeOp(op)) => op,
        other => panic!("expected ThinWedgeOp event, got {other:?}"),
    };
    assert_eq!(
        op,
...
-> Result<()> {
    ...
    let thinwedge_home = tempdir()?;
    app.config.thinwedge_home = thinwedge_home.path().to_path_buf().abs();
    let config_toml_path = thinwedge_home.path().join("config.toml").abs();
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: thinwedge_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: thinwedge_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: thinwedge_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                source: thinwedge_app_server_protocol::SessionSource::Unknown,
                thread_source: None,
                agent_nickname: Some("Robie".to_string()),
                agent_role: Some("explorer".to_string()),
                git_info: None,
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            thinwedge_app_server_protocol::ServerRequestResolvedNotification {
...
async fn primary_thread_ignores_child_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        thinwedge_app_server_client::AppServerEvent::ServerNotification(
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            thinwedge_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        thinwedge_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
async fn set_thread_goal_draft_materializes_long_objective_and_confirms_before_paste() -> Result<()>
{
    ...
    let thinwedge_home = app_server
        .thinwedge_home_path(&app.chat_widget.config_ref().thinwedge_home)
        .expect("thinwedge home");
    assert!(goal_files::objective_file_path(&goal.objective, Some(&thinwedge_home)).is_some());
    assert_eq!(
    ...
    for path in [
        format!("/tmp/{suffix}"),
        format!("{thinwedge_home}/../other/{suffix}"),
        format!("{thinwedge_home}/other/{suffix}"),
    ] {
        assert!(!is_managed(&thinwedge_home, &path));
    }
    assert!(!is_managed(
        &AppServerPath::from_app_server("/tmp/thinwedge\\home"),
        &format!("/tmp/thinwedge/home/{suffix}")
    ));
    let unix_path = AppServerPath::from_app_server("/tmp/thinwedge\\").join("a");
    assert_eq!(unix_path.as_str(), "/tmp/thinwedge\\/a");
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        thinwedge_error_info: Some(AppServerThinWedgeErrorInfo::ActiveTurnNotSteerable {
...
fn session_start_error_surfaces_archived_guidance_without_rollout_path() {
    ...
    let target_session = SessionTarget {
        path: Some(std::path::PathBuf::from(
            "/Users/me/.thinwedge/archived_sessions/rollout.jsonl",
        )),
        thread_id,
    };
    let expected = format!(
        "session {thread_id} is archived. Run `thinwedge unarchive {thread_id}` to unarchive it first."
    );
...
async fn replace_chat_widget_reseeds_collab_agent_metadata_for_replay() {
    ...
    let replacement = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_thinwedge_backend_auth: app.chat_widget.has_thinwedge_backend_auth(),
        model_catalog: app.model_catalog.clone(),
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(
                ServerNotification::ItemStarted(
                    thinwedge_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: thinwedge_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                thinwedge_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
...
async fn queued_rollback_syncs_overlay_and_clears_deferred_history() {
    ...
    app.chat_widget.update_account_state(
        ...
        /*has_chatgpt_account*/ false, /*has_thinwedge_backend_auth*/ true,
    );
    app.chat_widget
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                thinwedge_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
            /*windows_sandbox_level*/ None,
...
async fn inactive_thread_settings_notification_updates_cached_collaboration_mode() {
    ...
    let notification = ThreadSettingsUpdatedNotification {
        ...
        thread_settings: ThreadSettings {
            ...
            approval_policy: AskForApproval::OnRequest,
            approvals_reviewer: thinwedge_app_server_protocol::ApprovalsReviewer::AutoReview,
            sandbox_policy: thinwedge_app_server_protocol::SandboxPolicy::ReadOnly {
                network_access: false,
            },
            active_permission_profile: Some(
                thinwedge_app_server_protocol::ActivePermissionProfile::read_only(),
            ),
            model: "gpt-plan".to_string(),
            model_provider: "openai".to_string(),
            service_tier: None,
...
async fn clear_only_ui_reset_allows_active_skill_warning_to_render_again() {
    ...
    let error = SkillErrorInfo {
        path: test_path_buf("/tmp/project/.thinwedge/skills/abc/SKILL.md"),
