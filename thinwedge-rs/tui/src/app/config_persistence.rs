...
impl App {
    ...
    pub(super) async fn apply_permission_profile_selection(
        ...
        self.sync_active_thread_permission_settings_to_cached_session()
            .await;
        self.app_event_tx
            .send(AppEvent::ThinWedgeOp(AppCommand::override_turn_context(
                /*cwd*/ None,
                approval_policy,
                approvals_reviewer,
    ...
    pub(super) async fn read_effective_config_after_overridden_write(
        ...
        match crate::config_update::read_effective_config(app_server.request_handle(), cwd).await {
            ...
            Err(err) => {
                ...
                self.chat_widget.add_error_message(format!(
                    "{setting} were saved, but ThinWedge could not refresh the effective config: {err}"
                ));
                None
            }
        }
    ...
    pub(super) fn reasoning_label_for(
        ...
        (!model.starts_with("thinwedge-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }
...
fn windows_sandbox_mode_from_effective_config(
    effective_config: &ConfigReadResponse,
) -> Option<thinwedge_config::types::WindowsSandboxModeToml> {
...
mod tests {
    ...
    -> Result<()> {
        let mut app = make_test_app().await;
        let thinwedge_home = tempdir()?;
        let required_policy = thinwedge_protocol::protocol::AskForApproval::Never;
        let cloud_config_bundle =
            thinwedge_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
...
"#
    ...
    -> Result<()> {
        ...
        let thinwedge_home = tempdir()?;
        app.config.thinwedge_home = thinwedge_home.path().to_path_buf().abs();
        std::fs::write(thinwedge_home.path().join("config.toml"), "[broken")?;
