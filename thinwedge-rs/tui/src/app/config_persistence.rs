...
impl App {
    ...
    pub(super) async fn update_feature_flags(&mut self, updates: Vec<(Feature, bool)>) {
        ...
        let mut builder = ConfigEditsBuilder::new(&self.config.thinwedge_home)
            .with_profile(self.active_profile.as_deref());
    ...
    pub(crate) fn token_usage(&self) -> thinwedge_protocol::protocol::TokenUsage {
...
mod tests {
    ...
    async fn refresh_in_memory_config_from_disk_loads_latest_apps_state() -> Result<()> {
        ...
        ConfigEditsBuilder::new(&app.config.thinwedge_home)
            .with_edits([
                ConfigEdit::SetPath {
