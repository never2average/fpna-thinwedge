...
#[cfg(test)]
use thinwedge_protocol::items::ContextCompactionItem;
#[cfg(test)]
use thinwedge_protocol::items::ImageGenerationItem;
#[cfg(test)]
...
#[cfg(test)]
use thinwedge_protocol::protocol::ExecCommandStatus;
#[cfg(test)]
...
#[cfg(test)]
use thinwedge_protocol::protocol::RealtimeConversationClosedEvent;
#[cfg(test)]
use thinwedge_protocol::protocol::RealtimeConversationRealtimeEvent;
#[cfg(test)]
use thinwedge_protocol::protocol::RealtimeConversationStartedEvent;
#[cfg(test)]
use thinwedge_protocol::protocol::RealtimeEvent;
#[cfg(test)]
use thinwedge_protocol::protocol::ThreadNameUpdatedEvent;
#[cfg(test)]
        ...
        ServerNotification::Error(notification) => Some((
            ...
            vec![Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    thinwedge_error_info: notification
                        .error
                        .thinwedge_error_info
                        .and_then(app_server_thinwedge_error_info_to_core),
                }),
            }],
        )),
        ...
        ServerNotification::ThreadRealtimeSdp(notification) => Some((
            ...
            vec![Event {
                ...
                msg: EventMsg::RealtimeConversationSdp(
                    thinwedge_protocol::protocol::RealtimeConversationSdpEvent {
...
fn token_usage_from_app_server(
    value: thinwedge_app_server_protocol::TokenUsageBreakdown,
) -> TokenUsage {
    TokenUsage {
...
) -> Option<thinwedge_protocol::models::WebSearchAction> {
    match action {
        thinwedge_app_server_protocol::WebSearchAction::Search { query, queries } => {
            Some(thinwedge_protocol::models::WebSearchAction::Search { query, queries })
        }
        thinwedge_app_server_protocol::WebSearchAction::OpenPage { url } => {
            Some(thinwedge_protocol::models::WebSearchAction::OpenPage { url })
        }
        thinwedge_app_server_protocol::WebSearchAction::FindInPage { url, pattern } => {
            Some(thinwedge_protocol::models::WebSearchAction::FindInPage { url, pattern })
        }
        thinwedge_app_server_protocol::WebSearchAction::Other => {
            Some(thinwedge_protocol::models::WebSearchAction::Other)
        }
    }
}

#[cfg(test)]
fn app_server_thinwedge_error_info_to_core(
    value: thinwedge_app_server_protocol::ThinWedgeErrorInfo,
) -> Option<thinwedge_protocol::protocol::ThinWedgeErrorInfo> {
...
mod tests {
    ...
    fn bridges_thread_snapshot_turns_for_resume_restore() {
        ...
        assert_eq!(
            ...
            Some(thinwedge_protocol::protocol::ThinWedgeErrorInfo::Other)
        );
            ...
            &Turn {
                ...
                items: vec![
                    ...
                    ThreadItem::WebSearch {
                        ...
                        action: Some(thinwedge_app_server_protocol::WebSearchAction::Other),
                    },
                    ThreadItem::ImageGeneration {
