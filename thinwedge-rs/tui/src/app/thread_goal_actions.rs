...
const EPHEMERAL_THREAD_GOAL_ERROR_MESSAGE: &str = concat!(
    ...
    "Run `thinwedge` to start a saved session, or `thinwedge resume` / `/resume` to reopen one.",
);
...
impl App {
    ...
    pub(super) async fn open_thread_goal_editor(
        ...
        let thinwedge_home = app_server.thinwedge_home_path(&self.config.thinwedge_home);
    ...
    pub(super) async fn set_thread_goal_draft(
        ...
        let (objective, output_dir) =
            match goal_files::materialize_goal_draft(app_server, thinwedge_home.as_ref(), draft)
                .await
            {
