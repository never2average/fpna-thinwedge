    ...
    pub(super) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::OpenWindowsSandboxFallbackPrompt { preset } => {
                self.session_telemetry.counter(
                    "thinwedge.windows_sandbox.fallback_prompt_shown",
                    /*inc*/ 1,
                    &[],
                );
                ...
                if let Some(started_at) = self.windows_sandbox.setup_started_at.take() {
                    self.session_telemetry.record_duration(
                        "thinwedge.windows_sandbox.elevated_setup_duration_ms",
                        started_at.elapsed(),
                        &[("result", "failure")],
                    );
                }
                        ...
                        let event = match result {
                            Ok(()) => {
                                session_telemetry.counter(
                                    "thinwedge.windows_sandbox.elevated_setup_success",
                                    /*inc*/ 1,
                                    &[],
                                );
            ...
            AppEvent::EnableWindowsSandboxForAgentMode { preset, mode } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let builder = ConfigEditsBuilder::new(&self.config.thinwedge_home)
                        ...
                        Ok(()) => {
                                ...
                                self.chat_widget.world_writable_warning_details()
                            {
                                ...
                                self.chat_widget.add_plain_history_lines(vec![
                                    ...
                                    Line::from(vec![
                                        "  ".into(),
                                        "ThinWedge can now safely edit files and execute commands in your computer"
                                            .dark_gray(),
                                    ]),
                                ]);
                            }
            ...
            AppEvent::PersistModelSelection { model, effort } => {
                let profile = self.active_profile.as_deref();
                match ConfigEditsBuilder::new(&self.config.thinwedge_home)
                    .with_profile(profile)
            ...
            AppEvent::PersistServiceTierSelection { service_tier } => {
                ...
                let mut edits = ConfigEditsBuilder::new(&self.config.thinwedge_home)
                    .with_profile(profile)
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.thinwedge_home.clone();
            ...
            AppEvent::StatusLineSetup { items } => {
                ...
                let apply_result = ConfigEditsBuilder::new(&self.config.thinwedge_home)
                    .with_edits([edit])
                    .apply()
                    .await;
                match apply_result {
