...
const DESKTOP_THREAD_OPENED_MESSAGE: &str = "Opened this session in ThinWedge Desktop.";
...
impl App {
    ...
    pub(super) fn open_desktop_thread(&mut self, thread_id: ThreadId) {
        let url = format!("thinwedge://threads/{thread_id}");
    ...
    pub(super) fn clear_ui_header_lines(&self, width: u16) -> Vec<Line<'static>> {
        self.clear_ui_header_lines_with_version(width, THINWEDGE_CLI_VERSION)
    }
...
fn desktop_thread_open_error_message(err: &str) -> String {
    format!(
        "Failed to open this session in ThinWedge Desktop: {err}. Install or launch ThinWedge Desktop and try again."
    )
}
...
fn open_desktop_thread_url(url: &str) -> Result<(), String> {
    ...
    let output = std::process::Command::new("powershell.exe")
        ...
        .arg(&script)
        .output()
        .map_err(|err| format!("failed to launch ThinWedge Desktop through PowerShell: {err}"))?;
    ...
    if stderr.is_empty() {
        Err(format!(
            "failed to launch ThinWedge Desktop through PowerShell with {}",
            output.status
        ))
    } else {
        Err(stderr)
...
$installLocation = (Get-AppxPackage -Name ThinWedge -ErrorAction SilentlyContinue).InstallLocation
if ([string]::IsNullOrWhiteSpace($installLocation)) {{
    Write-Error 'ThinWedge Desktop package is not installed'
    exit 1
}}

$appDir = Join-Path $installLocation 'app'
$exe = Join-Path $appDir 'ThinWedge.exe'
$app = Join-Path $appDir 'resources\app.asar'
if (-not (Test-Path $exe)) {{
    Write-Error "ThinWedge Desktop executable not found at $exe"
    exit 1
}}
if (-not (Test-Path $app)) {{
    Write-Error "ThinWedge Desktop app bundle not found at $app"
    exit 1
}}
...
fn open_desktop_thread_url(_url: &str) -> Result<(), String> {
    Err("ThinWedge Desktop is only available on macOS and Windows".to_string())
}
