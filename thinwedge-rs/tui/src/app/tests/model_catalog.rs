...
async fn accepted_model_migration_persists_target_default_reasoning_effort() {
    let thinwedge_home = tempdir().expect("temp thinwedge home");
...
async fn model_migration_prompt_shows_for_hidden_model() {
    ...
    let current = available_models
        .iter_mut()
        .find(|preset| preset.model == "gpt-5.3-thinwedge")
        .expect("gpt-5.3-thinwedge preset present");
    current.show_in_picker = false;
    let current = current.clone();
    assert!(
        !current.show_in_picker,
        "expected gpt-5.3-thinwedge to be hidden from picker for this test"
    );
