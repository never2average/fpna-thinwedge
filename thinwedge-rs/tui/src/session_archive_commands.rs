//! Shared implementation for `thinwedge archive`, `thinwedge delete`, and `thinwedge unarchive`.
//!
...
async fn start_app_server_for_archive_command(
    ...
    let thinwedge_home = find_thinwedge_home().wrap_err("failed to find ThinWedge home")?;
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        super::maybe_probe_default_daemon_socket(thinwedge_home.as_path()).await
    } else {
        None
    };
