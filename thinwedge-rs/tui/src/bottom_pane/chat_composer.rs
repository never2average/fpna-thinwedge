...
mod tests {
    ...
    fn status_line_hyperlink_marks_pr_number_cells() {
        ...
        let url = "https://github.com/openai/thinwedge/pull/20252";
        composer.set_status_line_enabled(/*enabled*/ true);
    ...
    fn default_unified_mention_popup_snapshot() {
        snapshot_composer_state(
            ...
            /*enhanced_keys_supported*/ false,
            |composer| {
                let features = thinwedge_features::Features::with_defaults();
                composer.set_mentions_v2_enabled(
                    features.enabled(thinwedge_features::Feature::MentionsV2),
                );
                composer.set_text_content("@sa".to_string(), Vec::new(), Vec::new());
                composer.set_plugin_mentions(Some(vec![PluginCapabilitySummary {
                    config_name: "sample@test".to_string(),
