...
mod tests {
    ...
    fn exec_request() -> ApprovalRequest {
        ApprovalRequest::Exec {
            thread_id: thinwedge_protocol::ThreadId::new(),
            thread_label: None,
            id: "1".to_string(),
    ...
    fn esc_with_skill_popup_does_not_interrupt_task() {
        ...
        while let Ok(ev) = rx.try_recv() {
            assert!(
                !matches!(ev, AppEvent::ThinWedgeOp(Op::Interrupt)),
    ...
    fn esc_interrupts_running_task_when_no_popup() {
        ...
        assert!(
            matches!(rx.try_recv(), Ok(AppEvent::ThinWedgeOp(Op::Interrupt))),
