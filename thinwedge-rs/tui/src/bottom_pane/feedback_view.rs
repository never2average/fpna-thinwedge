    ...
    match issue_url {
        Some(url) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            lines.extend([
                ...
                Line::from(vec![
                    "    ".into(),
                    format!("https://go/thinwedge-feedback/{thread_id}").bold(),
                ]),
            ]);
        }
        Some(url) => {
...
fn slack_feedback_url(_thread_id: &str) -> String {
    THINWEDGE_FEEDBACK_INTERNAL_URL.to_string()
}
...
mod tests {
    ...
    fn feedback_success_cell_matches_employee_bug_copy() {
        ...
        assert_eq!(
            rendered,
            "• Feedback uploaded. Please report this in #thinwedge-feedback:\n\n  http://go/thinwedge-feedback-internal\n\n  Share this and add some info about your problem:\n    https://go/thinwedge-feedback/thread-2"
        );
    }
