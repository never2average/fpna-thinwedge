...
use thinwedge_config::NetworkUnixSocketPermissionToml;
...
use thinwedge_config::format_config_layer_source;
...
mod tests {
    ...
    use thinwedge_config::ConstraintError;
    use thinwedge_config::FeatureRequirementsToml;
    use thinwedge_config::FilesystemConstraints;
    use thinwedge_config::HookEventsToml;
    use thinwedge_config::HookHandlerConfig;
    use thinwedge_config::ManagedHooksRequirementsToml;
    use thinwedge_config::MatcherGroup;
    ...
    use thinwedge_config::NetworkUnixSocketPermissionToml;
    use thinwedge_config::NetworkUnixSocketPermissionsToml;
    use thinwedge_config::RequirementSource;
    use thinwedge_config::ResidencyRequirement;
    use thinwedge_config::SandboxModeRequirement;
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\thinwedge\\config.toml")
        } else {
            absolute_path("/etc/thinwedge/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.thinwedge")
        } else {
            absolute_path("/repo/.thinwedge")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_thinwedge_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
                ...
                BTreeMap::from([(
                    ...
                    McpServerRequirement {
                        identity: McpServerIdentity::Command {
                            command: "thinwedge-mcp".to_string(),
                        },
                    },
                )]),
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.thinwedge\\config.toml")
        } else {
            absolute_path("/home/alice/.thinwedge/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let requirements_source = (RequirementSource::LegacyManagedConfigTomlFromMdm).to_string();
        assert!(rendered.contains(&format!(
            "experimental_network: unix_sockets={{/tmp/blocked.sock=deny, /tmp/thinwedge.sock=allow}} (source: {requirements_source})"
        )));
    }
...
"#;
        ...
        let mdm_base_dir = if cfg!(windows) {
            absolute_path("C:\\thinwedge")
        } else {
            absolute_path("/var/lib/thinwedge")
        };
