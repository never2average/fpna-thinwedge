...
mod tests {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let thinwedge_home = TempDir::new().expect("temp thinwedge home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = thinwedge_home.path().join(DEFAULT_AGENTS_MD_FILENAME);
        let config = test_config(&thinwedge_home, &cwd).await;
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let thinwedge_home = TempDir::new().expect("temp thinwedge home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = thinwedge_home.path().join(LOCAL_AGENTS_MD_FILENAME);
        let config = test_config(&thinwedge_home, &cwd).await;
