...
#[test]
#[ignore = "requires tmux and a locally built thinwedge binary; run with --ignored for manual resize smoke"]
fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let repo_root = thinwedge_utils_cargo_bin::repo_root()?;
    let thinwedge = thinwedge_binary(&repo_root)?;
    ...
    write_auth(thinwedge_home.path())?;

    let session_name = format!("thinwedge-resize-reflow-smoke-{}", std::process::id());
    ...
    let thinwedge_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!thinwedge_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&thinwedge_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&thinwedge_pane)?;
    let final_row =
...
fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("thinwedge-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&thinwedge_pane)?;
    let restored_row =
...
fn run_repeated_resize_smoke(terminal_resize_reflow_enabled: bool) -> Result<()> {
    ...
    let session_name = format!("thinwedge-resize-repeat-{suffix}-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn thinwedge_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = thinwedge_utils_cargo_bin::cargo_bin("thinwedge") {
        return Ok(path);
    }

    let fallback = repo_root.join("thinwedge-rs/target/debug/thinwedge");
    anyhow::ensure!(
        fallback.is_file(),
        "thinwedge binary is unavailable; run `cargo build -p thinwedge-cli` first"
    );
    Ok(fallback)
}
...
"#
    ...
    std::fs::write(thinwedge_home.join("config.toml"), config)?;
    Ok(())
}
