...
async fn resume_startup_does_not_consume_model_availability_nux_count() -> Result<()> {
    // run_thinwedge_cli() does not work on Windows due to PTY limitations.
    if cfg!(windows) {
        return Ok(());
    }
    ...
    let custom_catalog_path = thinwedge_home.path().join("catalog.json");
    std::fs::write(
        &custom_catalog_path,
...
"#
    ...
    std::fs::write(thinwedge_home.path().join("config.toml"), config_contents)?;

    let fixture_path =
        thinwedge_utils_cargo_bin::find_resource!("../core/tests/cli_responses_fixture.sse")?;
    let thinwedge = if let Ok(path) = thinwedge_utils_cargo_bin::cargo_bin("thinwedge") {
        path
    } else {
        ...
        if fallback.is_file() {
            ...
            eprintln!("skipping integration test because thinwedge binary is unavailable");
            return Ok(());
        }
    };

    let exec_output = std::process::Command::new(&thinwedge)
        .arg("exec")
    ...
    let spawned = thinwedge_utils_pty::spawn_pty_process(
        thinwedge.to_string_lossy().as_ref(),
        &args,
        &repo_root,
        &env,
        &None,
        thinwedge_utils_pty::TerminalSize::default(),
    )
    .await?;

    let mut output = Vec::new();
    let thinwedge_utils_pty::SpawnedProcess {
        session,
        stdout_rx,
        stderr_rx,
        exit_rx,
    } = spawned;
    let mut output_rx = thinwedge_utils_pty::combine_output_receivers(stdout_rx, stderr_rx);
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            session.terminate();
            anyhow::bail!("timed out waiting for thinwedge resume to exit");
        }
    };
    ...
    anyhow::ensure!(
        ...
        "unexpected exit code from thinwedge resume: {exit_code}; output: {output_text}",
    );

    let config_contents = std::fs::read_to_string(thinwedge_home.path().join("config.toml"))?;
