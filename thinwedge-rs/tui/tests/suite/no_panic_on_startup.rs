...
"#,
    ...
    std::fs::write(thinwedge_home.join("config.toml"), config_contents)?;

    let ThinWedgeCliOutput { exit_code, output } = run_thinwedge_cli(thinwedge_home, cwd).await?;
    assert_ne!(0, exit_code, "ThinWedge CLI should exit nonzero.");
    assert!(
        output.contains("ERROR: Failed to initialize thinwedge:"),
...
struct ThinWedgeCliOutput {
    exit_code: i32,
    output: String,
}

async fn run_thinwedge_cli(
    thinwedge_home: impl AsRef<Path>,
    cwd: impl AsRef<Path>,
) -> anyhow::Result<ThinWedgeCliOutput> {
    let thinwedge_cli = thinwedge_utils_cargo_bin::cargo_bin("thinwedge")?;
    let mut env = HashMap::new();
    env.insert(
        "THINWEDGE_HOME".to_string(),
        thinwedge_home.as_ref().display().to_string(),
    );

    let args = vec!["-c".to_string(), "analytics.enabled=false".to_string()];
    let spawned = thinwedge_utils_pty::spawn_pty_process(
        thinwedge_cli.to_string_lossy().as_ref(),
        &args,
        cwd.as_ref(),
        &env,
        &None,
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            session.terminate();
            anyhow::bail!("timed out waiting for thinwedge CLI to exit");
        }
    };
