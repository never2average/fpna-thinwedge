...
use thinwedge_core::test_support::auth_manager_from_auth_with_home;
...
fn remote_control_auth_manager() -> Arc<AuthManager> {
    auth_manager_from_auth(ThinWedgeAuth::create_dummy_chatgpt_auth_for_testing())
}

fn remote_control_auth_manager_with_home(thinwedge_home: &TempDir) -> Arc<AuthManager> {
...
async fn plain_start_resolves_persisted_remote_control_preference() {
    ...
    let thinwedge_home = TempDir::new().expect("temp dir should create");
...
async fn ephemeral_enable_preserves_durable_preference() {
    ...
    remote_handle.state_db = Some(remote_control_state_runtime(&thinwedge_home).await);
    remote_handle
        .desired_state_tx
...
async fn remote_control_transport_manages_virtual_clients_and_routes_messages() {
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Notification(
                    thinwedge_app_server_protocol::JSONRPCNotification {
    ...
    let followup_message =
        JSONRPCMessage::Notification(thinwedge_app_server_protocol::JSONRPCNotification {
            method: "initialized".to_string(),
            params: None,
        });
    send_client_event(
...
async fn remote_control_transport_reconnects_after_disconnect() {
    ...
    let (remote_task, remote_handle) = start_remote_control(
        ...
        Some(remote_control_state_runtime(&thinwedge_home).await),
        remote_control_auth_manager(),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
    ...
    send_client_event(
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Request(thinwedge_app_server_protocol::JSONRPCRequest {
                    id: thinwedge_app_server_protocol::RequestId::Integer(2),
                    method: "initialize".to_string(),
                    params: Some(json!({
                        "clientInfo": {
                            "name": "remote-test-client",
...
async fn remote_control_http_mode_enrolls_before_connecting() {
    ...
    assert_eq!(
        handshake_request.headers.get("x-thinwedge-server-id"),
        Some(&"srv_e_test".to_string())
    );
    assert_eq!(
        handshake_request.headers.get("x-thinwedge-name"),
    ...
    let writer = {
        let initialize_message =
            JSONRPCMessage::Request(thinwedge_app_server_protocol::JSONRPCRequest {
                id: thinwedge_app_server_protocol::RequestId::Integer(11),
                method: "initialize".to_string(),
                params: Some(json!({
                    "clientInfo": {
    ...
    writer
        .send(QueuedOutgoingMessage::new(OutgoingMessage::Response(
            crate::outgoing_message::OutgoingResponse {
                id: thinwedge_app_server_protocol::RequestId::Integer(11),
                result: json!({
                    "userAgent": "thinwedge-test-agent"
                }),
            },
        )))
        .await
...
async fn remote_control_http_mode_refreshes_persisted_enrollment_before_connecting() {
    ...
    let (remote_task, _remote_handle) = start_remote_control(
        ...
        Some(state_db.clone()),
        remote_control_auth_manager_with_home(&thinwedge_home),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
        RemoteControlStartupMode::EnabledEphemeral,
...
async fn remote_control_http_mode_reenrolls_after_explicit_missing_server_404() {
    ...
    assert_eq!(
        websocket_request.headers.get("x-thinwedge-server-id"),
