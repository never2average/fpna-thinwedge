...
use thinwedge_app_server_protocol::FsGetMetadataParams;
use thinwedge_app_server_protocol::FsReadDirectoryParams;
use thinwedge_app_server_protocol::FsReadFileParams;
use thinwedge_app_server_protocol::FsRemoveParams;
use thinwedge_app_server_protocol::FsUnwatchParams;
use thinwedge_app_server_protocol::FsWatchParams;
...
use thinwedge_app_server_protocol::ProcessKillParams;
use thinwedge_app_server_protocol::ProcessResizePtyParams;
use thinwedge_app_server_protocol::ProcessSpawnParams;
use thinwedge_app_server_protocol::ProcessWriteStdinParams;
use thinwedge_app_server_protocol::RemoteControlClientsListParams;
use thinwedge_app_server_protocol::RemoteControlClientsRevokeParams;
use thinwedge_app_server_protocol::RemoteControlPairingStartParams;
use thinwedge_app_server_protocol::RemoteControlPairingStatusParams;
...
pub const DEFAULT_CLIENT_NAME: &str = "thinwedge-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "THINWEDGE_APP_SERVER_DISABLE_MANAGED_CONFIG";
...
impl TestAppServer {
    ...
    pub async fn new(thinwedge_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(thinwedge_home, &[], &[DISABLE_PLUGIN_STARTUP_TASKS_ARG]).await
    }

    pub async fn new_without_managed_config(thinwedge_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(
            thinwedge_home,
    ...
    pub async fn new_without_managed_config_with_env(
        ...
        Self::new_with_env(thinwedge_home, &all_env_overrides).await
    }

    pub async fn new_with_plugin_startup_tasks(thinwedge_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(thinwedge_home, &[], &[]).await
    }
    ...
    pub async fn new_with_env_and_plugin_startup_tasks(
        ...
        Self::new_with_env_and_args(thinwedge_home, env_overrides, &[]).await
    }

    pub async fn new_with_args(thinwedge_home: &Path, args: &[&str]) -> anyhow::Result<Self> {
        let mut all_args = vec![DISABLE_PLUGIN_STARTUP_TASKS_ARG];
        all_args.extend_from_slice(args);
        Self::new_with_env_and_args(thinwedge_home, &[], &all_args).await
    }
    ...
    async fn new_with_env_and_args(
        ...
        let program = thinwedge_utils_cargo_bin::cargo_bin("thinwedge-app-server")
            .context("should find binary for thinwedge-app-server")?;
        Self::new_with_program_env_and_args(thinwedge_home, &program, env_overrides, args).await
    }
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.current_dir(thinwedge_home);
        ...
        cmd.env(
            ...
            thinwedge_home.join("managed_config.toml"),
        );
        cmd.env_remove(THINWEDGE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("thinwedge-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: thinwedge_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `thinwedge-app-server` child process.
        //
