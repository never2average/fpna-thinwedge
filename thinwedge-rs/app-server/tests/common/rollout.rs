...
/// Create a minimal rollout file under `THINWEDGE_HOME/sessions/YYYY/MM/DD/`.
///
...
pub fn create_fake_rollout_with_token_usage(
    ...
    let file_path = rollout_path(thinwedge_home, filename_ts, &thread_id);
    let line = json!({
...
fn create_fake_rollout_with_source_and_parent_thread_id(
    ...
    let file_path = rollout_path(thinwedge_home, filename_ts, &uuid_str);
    let dir = file_path
        .parent()
