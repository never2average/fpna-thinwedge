...
use thinwedge_app_server_protocol::McpResourceContent;
...
const TEST_RESOURCE_URI: &str = "test://thinwedge/resource";
const TEST_BLOB_RESOURCE_URI: &str = "test://thinwedge/resource.bin";
...
async fn orchestrator_skill_can_read_referenced_resource_without_an_executor() -> Result<()> {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "warnings": ["Orchestrator skill discovery stopped after 2 resource pages: failed to list orchestrator skill resources: resources/list failed for `thinwedge_apps`: Mcp error: -32603: simulated later-page failure"],
        })
    );
...
async fn start_resource_apps_mcp_server() -> Result<(String, JoinHandle<()>)> {
    ...
    let router = Router::new().nest_service("/api/thinwedge/ps/mcp", mcp_service);
