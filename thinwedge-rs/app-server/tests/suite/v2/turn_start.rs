...
use thinwedge_app_server_protocol::TextElement;
...
use thinwedge_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
...
const TEST_ORIGINATOR: &str = "thinwedge_vscode";
...
async fn run_local_image_turn(detail: Option<ImageDetail>) -> Result<Vec<Value>> {
    // Two ThinWedge turns hit the mock model (session start + turn/start).
    let responses = vec![
    ...
    let image_path = thinwedge_home.path().join("image.png");
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("ThinWedge VS Code Extension".to_string()),
...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    write_models_cache(thinwedge_home.path())?;
    let cache_path = thinwedge_home.path().join("models_cache.json");
    ...
    write_test_skill(thinwedge_home.path(), "alpha-skill")?;
    write_test_skill(thinwedge_home.path(), "beta-skill")?;

    let isolated_home = thinwedge_home.path().to_string_lossy();
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: thinwedge_home.path().to_path_buf().try_into()?,
            }]),
            ..Default::default()
        })
        .await?;
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three ThinWedge turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ...
    assert_eq!(
        started.turn.status,
        thinwedge_app_server_protocol::TurnStatus::InProgress
    );
    assert_eq!(started.turn.id, turn.id);
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("exp-thinwedge-personality".to_string()),
            ..Default::default()
        })
        .await?;
    let thread_resp: JSONRPCResponse = timeout(
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let thinwedge_home = tmp.path().to_path_buf();
    ...
    let mut mcp = TestAppServer::new(thinwedge_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new(&thinwedge_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn turn_start_permission_profile_rebinds_runtime_workspace_roots_between_turns() -> Result<()>
{
    ...
    let old_root = thinwedge_utils_absolute_path::AbsolutePathBuf::from_absolute_path(old_root)?;
    let new_root = thinwedge_utils_absolute_path::AbsolutePathBuf::from_absolute_path(new_root)?;
...
async fn turn_start_resolves_sticky_thread_local_environment_and_turn_overrides() -> Result<()> {
    ...
    std::fs::write(
        thinwedge_home.join("environments.toml"),
        r#"
[[environments]]
id = "remote"
url = "ws://127.0.0.1:1"
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![thinwedge_app_server_protocol::FileUpdateChange {
...
async fn turn_start_streams_apply_patch_change_updates_v2() -> Result<()> {
    ...
    write_models_cache(&thinwedge_home)?;
    let cache_path = thinwedge_home.join("models_cache.json");
    let mut cache: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&cache_path)?)?;
    let models = cache["models"]
        .as_array_mut()
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    std::fs::write(
        thinwedge_home.path().join("custom-role.toml"),
...
async fn turn_start_with_elevated_override_does_not_persist_project_trust() -> Result<()> {
    ...
    let config_toml = std::fs::read_to_string(thinwedge_home.path().join("config.toml"))?;
...
fn write_test_skill(thinwedge_home: &Path, name: &str) -> std::io::Result<()> {
    let skill_dir = thinwedge_home.join("skills").join(name);
