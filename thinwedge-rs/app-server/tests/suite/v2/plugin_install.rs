...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    mount_remote_plugin_install_after_cache_write(
        &server,
        REMOTE_PLUGIN_ID,
        installed_path.join(".thinwedge-plugin/plugin.json"),
    )
    .await;
...
#[tokio::test]
async fn plugin_install_rejects_when_workspace_thinwedge_plugins_disabled() -> Result<()> {
    let thinwedge_home = TempDir::new()?;
    ...
    assert!(
        err.error
            .message
            .contains("ThinWedge plugins are disabled for this workspace")
    );
    Ok(())
}
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                thinwedge_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
...
async fn plugin_install_tracks_analytics_event() -> Result<()> {
    ...
    let thinwedge_home = TempDir::new()?;
    write_analytics_config(thinwedge_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        thinwedge_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_analytics_config(thinwedge_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        thinwedge_home.join("config.toml"),
