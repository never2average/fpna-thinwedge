...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(thinwedge_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let thinwedge_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&thinwedge_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "thinwedge-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(thinwedge_home.path().join(".tmp"))?;
    std::fs::write(
        thinwedge_home
            .path()
...
[hooks.state."demo-plugin@thinwedge-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(
        ...
        vec![
            thinwedge_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@thinwedge-curated:hooks/hooks.json:pre_tool_use:0:0".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            thinwedge_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@thinwedge-curated:hooks/hooks.json:pre_tool_use:0:1".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            thinwedge_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@thinwedge-curated:hooks/hooks.json:session_start:0:0".to_string(),
...
fn write_plugins_enabled_config(thinwedge_home: &TempDir) -> Result<()> {
    std::fs::write(
        thinwedge_home.path().join("config.toml"),
        r#"[features]
plugins = true
