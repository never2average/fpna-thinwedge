...
-> Result<()> {
    ...
    let mut mcp =
        TestAppServer::new_with_env(thinwedge_home.path(), &[("HOME", Some(home_dir.as_str()))])
            .await?;
...
{
    let thinwedge_home = TempDir::new()?;
    let marketplace_root = thinwedge_home.path().join("marketplace");
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(thinwedge_home.path().join(".claude"))?;
    ...
    std::fs::write(
        thinwedge_home.path().join(".claude").join("settings.json"),
...
async fn external_agent_config_import_creates_session_rollouts() -> Result<()> {
    ...
    let project_root = thinwedge_home.path().join("repo");
    let recent_timestamp = chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, true);
    let session_dir = thinwedge_home.path().join(".claude/projects/repo");
...
async fn external_agent_config_import_does_not_initialize_required_mcp() -> Result<()> {
    ...
    let thinwedge_home = TempDir::new()?;
    create_config_toml(thinwedge_home.path(), &server.uri())?;
    let mut config = std::fs::read_to_string(thinwedge_home.path().join("config.toml"))?;
    config.push_str(
        r#"
...
-> Result<()> {
    ...
    let project_config_dir = project_root.join(".thinwedge");
...
async fn external_agent_config_import_rejects_undetected_session_paths() -> Result<()> {
    ...
    let undetected_session_path = thinwedge_home.path().join("outside.jsonl");
    std::fs::create_dir_all(&project_root)?;
    std::fs::create_dir_all(&session_dir)?;
