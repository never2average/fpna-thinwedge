...
use thinwedge_app_server_protocol::CommandExecOutputDeltaNotification;
use thinwedge_app_server_protocol::CommandExecOutputStream;
use thinwedge_app_server_protocol::CommandExecParams;
use thinwedge_app_server_protocol::CommandExecResizeParams;
use thinwedge_app_server_protocol::CommandExecResponse;
use thinwedge_app_server_protocol::CommandExecTerminalSize;
...
use thinwedge_app_server_protocol::FileSystemSpecialPath;
...
async fn command_exec_without_streams_can_be_terminated() -> Result<()> {
    ...
    create_config_toml(thinwedge_home.path(), &server.uri(), "never")?;
...
async fn command_exec_env_overrides_merge_with_server_environment_and_support_unset() -> Result<()>
{
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "/bin/sh".to_string(),
                "-lc".to_string(),
                "printf '%s|%s|%s|%s' \"$COMMAND_EXEC_BASELINE\" \"$COMMAND_EXEC_EXTRA\" \"${RUST_LOG-unset}\" \"$THINWEDGE_HOME\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
    ...
    assert_eq!(
        ...
        CommandExecResponse {
            exit_code: 0,
            stdout: format!("request|added|unset|{}", thinwedge_home.path().display()),
            stderr: String::new(),
        }
    );
...
async fn command_exec_permission_profile_project_roots_use_command_cwd() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let thinwedge_home = TempDir::new()?;
    let command_dir = thinwedge_home.path().join("command-cwd");
    ...
    assert!(
        !thinwedge_home.path().join("parent.txt").exists(),
...
-> Result<()> {
    ...
    create_config_toml(thinwedge_home.path(), &server.uri(), "never")?;
    let marker = format!(
        "thinwedge-command-exec-marker-{}",
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)?
            .as_nanos()
    );

    let (mut process, bind_addr) = spawn_websocket_server(thinwedge_home.path()).await?;
