    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "sh".to_string(),
                "-lc".to_string(),
                "printf '%s' \"${THINWEDGE_NETWORK_PROXY_ACTIVE-unset}\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
...
async fn command_exec_permission_profile_does_not_reuse_default_network_proxy() -> Result<()> {
    ...
    let thinwedge_home = TempDir::new()?;
    create_config_toml(thinwedge_home.path(), &server.uri(), "never")?;
    insert_networked_permission_profile_config(thinwedge_home.path(), Some("networked"))?;
    let mut mcp = TestAppServer::new(thinwedge_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
"#
    ...
    insert_command_exec_config(thinwedge_home, &inserted_config)?;
    Ok(())
}

fn insert_command_exec_config(thinwedge_home: &Path, inserted_config: &str) -> Result<()> {
    let config_path = thinwedge_home.join("config.toml");
