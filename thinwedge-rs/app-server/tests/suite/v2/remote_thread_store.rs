...
//! the temporary `thinwedge_home` must not contain rollout session files or sqlite
...
use thinwedge_app_server::in_process::InProcessClientHandle;
...
use thinwedge_thread_store::CreateThreadParams as StoreCreateThreadParams;
...
async fn thread_delete_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    // Plugin startup warmups may create `.tmp` under thinwedge_home. Disable them
    // here so this regression stays focused on thread persistence artifacts.
    create_config_toml_with_thread_store(thinwedge_home.path(), &server.uri(), &store_id)?;

    let thread_store = InMemoryThreadStore::for_id(store_id.clone());
    let _in_memory_store = InMemoryThreadStoreId { store_id };

    let mut client = start_in_process_server(thinwedge_home.path()).await?;
    ...
    assert_no_local_persistence_artifacts(thinwedge_home.path())?;
...
async fn start_in_process_server(thinwedge_home: &Path) -> Result<InProcessClientHandle> {
    let loader_overrides = LoaderOverrides::without_managed_config_for_tests();
    let config = Arc::new(
        ConfigBuilder::default()
            .thinwedge_home(thinwedge_home.to_path_buf())
            .fallback_cwd(Some(thinwedge_home.to_path_buf()))
            .loader_overrides(loader_overrides.clone())
            .build()
            .await?,
    );
...
fn assert_no_local_persistence_artifacts(thinwedge_home: &Path) -> Result<()> {
    ...
    // the isolated test thinwedge_home.
    assert!(
        !thinwedge_home.join("sessions").exists(),
        "non-local thread persistence should not create local rollout sessions"
    );
    assert!(
        !thinwedge_home.join("archived_sessions").exists(),
        "non-local thread persistence should not create archived rollout sessions"
    );
    assert!(
        !thinwedge_state::state_db_path(thinwedge_home).exists(),
        "non-local thread persistence should not create local thread sqlite"
    );

    let sqlite_artifacts = std::fs::read_dir(thinwedge_home)?
    ...
    let mut entries = thinwedge_home_entries(thinwedge_home)?;
    // Bazel test runs may initialize shell snapshot storage under thinwedge_home.
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in thinwedge_home"
    );

    Ok(())
}

fn thinwedge_home_entries(thinwedge_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(thinwedge_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;
