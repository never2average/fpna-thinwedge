...
use thinwedge_protocol::protocol::SessionSource as ProtocolSessionSource;
...
use thinwedge_thread_store::AppendThreadItemsParams;
...
async fn thread_turns_list_reads_store_history_without_rollout_path() -> Result<()> {
    let thinwedge_home = TempDir::new()?;
    let thread_id =
        thinwedge_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000123")?;
    let store_id = Uuid::new_v4().to_string();
    create_config_toml_with_thread_store(thinwedge_home.path(), &store_id)?;
    ...
    let client = in_process::start(InProcessStartArgs {
        ...
        thread_config_loader: Arc::new(thinwedge_config::NoopThreadConfigLoader),
        ...
        initialize: InitializeParams {
            client_info: ClientInfo {
                name: "thinwedge-app-server-tests".to_string(),
                title: None,
...
async fn thread_read_loaded_include_turns_reads_store_history_without_rollout_path() -> Result<()> {
    ...
    let thread_id = thinwedge_protocol::ThreadId::from_string(&thread.id)?;
    store
...
async fn thread_list_includes_store_thread_without_rollout_path() -> Result<()> {
    let thinwedge_home = TempDir::new()?;
    let thread_id =
        thinwedge_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000124")?;
    let store_id = Uuid::new_v4().to_string();
    create_config_toml_with_thread_store(thinwedge_home.path(), &store_id)?;
    let store = InMemoryThreadStore::for_id(store_id.clone());
    let _in_memory_store = InMemoryThreadStoreId { store_id };
...
async fn thread_resume_initial_turns_page_matches_requested_turns_list_page() -> Result<()> {
    ...
    assert_eq!(
        initial_turns_page,
        Some(thinwedge_app_server_protocol::TurnsPage::from(
            expected_page
        ))
    );
...
) -> anyhow::Result<thinwedge_app_server_protocol::Turn> {
    let read_id = mcp
        .send_thread_turns_list_request(ThreadTurnsListParams {
            thread_id: thread_id.to_string(),
            cursor: None,
...
fn turn_user_texts(turns: &[thinwedge_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
fn turn_agent_texts(turns: &[thinwedge_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
async fn seed_pathless_store_thread(
    ...
    thread_id: thinwedge_protocol::ThreadId,
) -> Result<()> {
    store
...
fn create_config_toml(thinwedge_home: &Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = thinwedge_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
