...
async fn review_start_exec_approval_item_id_matches_command_execution_item() -> Result<()> {
    ...
    let thinwedge_home = TempDir::new()?;
    create_config_toml_with_approval_policy(thinwedge_home.path(), &server.uri(), "untrusted")?;
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": thinwedge_protocol::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
...
fn create_config_toml(thinwedge_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    create_config_toml_with_approval_policy(thinwedge_home, server_uri, "never")
}
