...
async fn thread_start_creates_thread_and_emits_started() -> Result<()> {
    ...
    create_config_toml_without_approval_policy(thinwedge_home.path(), &server.uri())?;
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let thinwedge_home = TempDir::new()?;
    create_config_toml_without_approval_policy(thinwedge_home.path(), &server.uri())?;
    let global_agents_path = thinwedge_home.path().join("AGENTS.md");
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let thinwedge_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(thinwedge_home.path(), &server.uri())?;
...
"#,
    ...
    set_project_trust_level(thinwedge_home.path(), workspace.path(), TrustLevel::Trusted)?;
    let config_before = std::fs::read_to_string(thinwedge_home.path().join("config.toml"))?;
    ...
    let config_after = std::fs::read_to_string(thinwedge_home.path().join("config.toml"))?;
    assert_eq!(config_after, config_before);
