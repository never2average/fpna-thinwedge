...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_thinwedge_home = AbsolutePathBuf::try_from(thinwedge_home.path().canonicalize()?)?;
    ...
    let message = timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            name: "thinwedge_vscode".to_string(),
    ...
    let InitializeResponse {
        user_agent,
        thinwedge_home: response_thinwedge_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("thinwedge_vscode/"));
    assert_eq!(response_thinwedge_home, expected_thinwedge_home);
...
async fn initialize_probe_does_not_override_originator() -> Result<()> {
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "thinwedge_app_server_daemon".to_string(),
            title: Some("ThinWedge App Server Daemon".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
    ...
    assert!(user_agent.starts_with("thinwedge_cli_rs/"));
    Ok(())
}

#[tokio::test]
async fn initialize_thinwedge_backend_does_not_override_originator() -> Result<()> {
    let responses = Vec::new();
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let thinwedge_home = TempDir::new()?;
    create_config_toml(thinwedge_home.path(), &server.uri(), "never")?;
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "thinwedge-backend".to_string(),
            title: Some("ThinWedge Backend".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_with_env(
        ...
        &[(
            ...
            Some("thinwedge_originator_via_env_var"),
        )],
    )
    .await?;
    ...
    assert!(user_agent.starts_with("thinwedge_originator_via_env_var/"));
    assert_eq!(response_thinwedge_home, expected_thinwedge_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_with_env(
        thinwedge_home.path(),
        &[("THINWEDGE_INTERNAL_ORIGINATOR_OVERRIDE", None)],
    )
    .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let thinwedge_home = TempDir::new()?;
    let notify_file = thinwedge_home.path().join("notify.json");
    let notify_capture = cargo_bin("thinwedge-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(thinwedge_home, server_uri, approval_policy, "")
}
