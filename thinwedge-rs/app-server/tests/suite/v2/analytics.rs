...
fn set_metrics_exporter(config: &mut thinwedge_core::config::Config) {
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = thinwedge_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
...
pub(crate) async fn wait_for_goal_event(
    ...
    wait_for_matching_analytics_event(server, read_timeout, |event| {
        event["event_type"] == "thinwedge_goal_event"
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        ...
        .ok_or_else(|| anyhow::anyhow!("thinwedge_thread_initialized event should be present"))
}
