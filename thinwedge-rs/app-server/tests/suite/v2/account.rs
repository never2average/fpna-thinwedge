...
const LOGIN_ISSUER_ENV_VAR: &str = "THINWEDGE_APP_SERVER_LOGIN_ISSUER";
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(thinwedge_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<thinwedge_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(thinwedge_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            client_user_message_id: None,
            input: vec![thinwedge_app_server_protocol::UserInput::Text {
...
async fn login_account_chatgpt_device_code_succeeds_and_notifies() -> Result<()> {
    ...
    assert_eq!(verification_url, format!("{issuer}/thinwedge/device"));
