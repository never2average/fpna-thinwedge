...
async fn memory_reset_clears_memory_files_and_rows_preserves_threads() -> Result<()> {
    ...
    let thread_id = seed_stage1_output(&state_db, thinwedge_home.path()).await?;
...
async fn seed_stage1_output(state_db: &Arc<StateRuntime>, thinwedge_home: &Path) -> Result<ThreadId> {
    ...
    let mut builder = ThreadMetadataBuilder::new(
        thread_id,
        thinwedge_home.join("sessions").join("test.jsonl"),
        now,
        SessionSource::Cli,
    );
...
fn create_config_toml(thinwedge_home: &Path) -> std::io::Result<()> {
