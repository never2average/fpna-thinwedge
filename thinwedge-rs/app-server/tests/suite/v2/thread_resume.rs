...
async fn thread_goal_get_rejects_unmaterialized_thread() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_without_managed_config(thinwedge_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn thread_resume_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    assert_basic_thread_initialized_event(event, &thread.id, "gpt-5.3-thinwedge", "resumed");
    Ok(())
}
...
async fn thread_resume_by_path_uses_remote_thread_store_error() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let thinwedge_home = TempDir::new()?;
    create_config_toml_with_remote_thread_store(thinwedge_home.path(), &server.uri())?;
...
async fn thread_resume_without_overrides_does_not_change_updated_at_or_mtime() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let thinwedge_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(thinwedge_home.path(), &server.uri())?;
...
async fn thread_resume_keeps_in_flight_turn_streaming() -> Result<()> {
    ...
    let mut primary = McpProcess::new(thinwedge_home.path()).await?;
    ...
    let mut secondary = McpProcess::new(thinwedge_home.path()).await?;
...
"#;
    ...
    let mut primary = McpProcess::new(&thinwedge_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, primary.initialize()).await??;
...
async fn start_materialized_thread_and_restart(
    ...
    let mut first_mcp = McpProcess::new(thinwedge_home).await?;
    ...
    let mut second_mcp = McpProcess::new(thinwedge_home).await?;
...
fn setup_rollout_fixture(thinwedge_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
