...
use thinwedge_protocol::protocol::SessionSource as RolloutSessionSource;
...
use thinwedge_rollout::append_rollout_item_to_path;
use thinwedge_rollout::read_session_meta_line;
...
const THINWEDGE_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are ThinWedge, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn thread_resume_rejects_unmaterialized_thread() -> Result<()> {
    ...
    create_config_toml(thinwedge_home.path(), &server.uri())?;
...
async fn thread_goal_get_rejects_unmaterialized_thread() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_without_managed_config(thinwedge_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn thread_resume_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    let thinwedge_home = TempDir::new()?;
    create_config_toml_with_chatgpt_base_url(thinwedge_home.path(), &server.uri(), &server.uri())?;
    mount_analytics_capture(&server, thinwedge_home.path()).await?;
...
fn set_thread_source_on_fake_rollout(
    ...
    let path = rollout_path(thinwedge_home, filename_ts, thread_id);
...
async fn thread_resume_redacts_payloads_for_chatgpt_remote_clients() -> Result<()> {
    for client_name in [
        "thinwedge_chatgpt_android_remote",
        "thinwedge_chatgpt_ios_remote",
    ] {
...
fn append_resume_redaction_history(
    ...
    let rollout_file_path = rollout_path(thinwedge_home, filename_ts, conversation_id);
...
async fn thread_resume_rejects_archived_session_by_id() -> Result<()> {
    ...
    let active_rollout_path = rollout_path(thinwedge_home.path(), filename_ts, &conversation_id);
    let archived_dir = thinwedge_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
    ...
    assert!(
        message.contains(&format!("session {conversation_id} is archived"))
            && message.contains(&format!(
                "thinwedge unarchive {conversation_id}` to unarchive it first"
            )),
...
async fn thread_goal_set_edits_objective_without_resetting_usage() -> Result<()> {
    ...
    let state_db =
        StateRuntime::init(thinwedge_home.path().to_path_buf(), "mock_provider".into()).await?;
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            thinwedge_state::GoalAccountingMode::ActiveOnly,
...
async fn thread_goal_lifecycle_emits_analytics_and_clear_deletes_goal() -> Result<()> {
    ...
    .await??;
    let get: thinwedge_app_server_protocol::ThreadGoalGetResponse = to_response(get_resp)?;
...
async fn thread_resume_token_usage_replay_ignores_stale_interrupted_tail_turn() -> Result<()> {
    ...
    let rollout_file_path = rollout_path(thinwedge_home.path(), filename_ts, &conversation_id);
    let persisted_rollout = std::fs::read_to_string(&rollout_file_path)?;
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let thinwedge_home = TempDir::new()?;
    let config_toml = thinwedge_home.path().join("config.toml");
    std::fs::write(
        &config_toml,
        format!(
            r#"
model = "gpt-5.3-thinwedge"
...
"#,
    ...
    let repo_path = thinwedge_home.path().join("repo");
    ...
    let rollout_path = rollout_path(thinwedge_home.path(), "2025-01-05T12-00-00", &thread_id);
...
async fn thread_resume_defers_updated_at_until_turn_start() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let thinwedge_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(thinwedge_home.path(), &server.uri()).await?;
    ...
    let resume_id = mcp
        .send_thread_resume_request(ThreadResumeParams {
            ...
            cwd: Some(thinwedge_home.path().to_string_lossy().to_string()),
            ..Default::default()
        })
        .await?;
    let resume_resp: JSONRPCResponse = timeout(
...
async fn thread_resume_keeps_in_flight_turn_streaming() -> Result<()> {
    ...
    let mut primary = TestAppServer::new(thinwedge_home.path()).await?;
    ...
    let mut secondary = TestAppServer::new(thinwedge_home.path()).await?;
...
async fn thread_resume_replays_pending_file_change_request_approval() -> Result<()> {
    let tmp = TempDir::new()?;
    let thinwedge_home = tmp.path().join("thinwedge_home");
    std::fs::create_dir(&thinwedge_home)?;
...
"#;
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    create_config_toml(&thinwedge_home, &server.uri())?;

    let mut primary = TestAppServer::new(&thinwedge_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, primary.initialize()).await??;
    ...
    let expected_file_change = ThreadItem::FileChange {
        ...
        changes: vec![thinwedge_app_server_protocol::FileUpdateChange {
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        rollout_file_path,
        updated_at,
    } = start_materialized_thread_and_restart(thinwedge_home.path(), "materialize").await?;
...
async fn thread_resume_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let thinwedge_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(thinwedge_home.path(), &server.uri()).await?;
    create_config_toml_with_required_broken_mcp(thinwedge_home.path(), &server.uri())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(thinwedge_home.path(), "seed history").await?;
...
async fn start_materialized_thread_and_restart(
    ...
    let mut first_mcp = TestAppServer::new(thinwedge_home).await?;
    ...
    let mut second_mcp = TestAppServer::new(thinwedge_home).await?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    let start_id = primary
        .send_thread_start_request(ThreadStartParams {
            model: Some("gpt-5.3-thinwedge".to_string()),
            ..Default::default()
        })
        .await?;
    let start_resp: JSONRPCResponse = timeout(
    ...
    assert!(
        instructions_text.contains(THINWEDGE_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
fn create_config_toml(thinwedge_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
...
command = "thinwedge-definitely-not-a-real-binary"
required = true
"#
        ),
    )
...
async fn setup_rollout_fixture(thinwedge_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(thinwedge_home, server_uri)?;
