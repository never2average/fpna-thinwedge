...
async fn attestation_generate_round_trip_adds_header_to_responses_websocket_handshake() -> Result<()>
{
    ...
    let initialized = timeout(
        ...
        mcp.initialize_with_capabilities(
            ClientInfo {
                name: "thinwedge_desktop".to_string(),
                title: Some("ThinWedge Desktop".to_string()),
...
fn create_chatgpt_websocket_config(thinwedge_home: &Path, server_uri: &str) -> std::io::Result<()> {
    std::fs::write(
