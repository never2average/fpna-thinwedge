...
async fn get_account_rate_limits_returns_snapshot() -> Result<()> {
    ...
    write_chatgpt_base_url(thinwedge_home.path(), &server_url)?;
        ...
        "additional_rate_limits": [
            {
                ...
                "metered_feature": "thinwedge_other",
                "rate_limit": {
                    "allowed": true,
                    "limit_reached": false,
                    "primary_window": {
...
async fn send_add_credits_nudge_email_posts_expected_body() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path("/api/thinwedge/accounts/send_add_credits_nudge_email"))
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
...
fn write_chatgpt_base_url(thinwedge_home: &Path, base_url: &str) -> std::io::Result<()> {
