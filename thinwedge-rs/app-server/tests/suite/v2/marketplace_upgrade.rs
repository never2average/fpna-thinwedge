...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    run_git(root, &["init"])?;
    run_git(root, &["config", "user.email", "thinwedge@example.com"])?;
    run_git(root, &["config", "user.name", "ThinWedge Tests"])?;
...
fn disable_plugin_startup_tasks(thinwedge_home: &Path) -> Result<()> {
...
fn marketplace_install_root(thinwedge_home: &Path) -> std::path::PathBuf {
