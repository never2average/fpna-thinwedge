...
use thinwedge_rollout::append_thread_name;
...
async fn thread_fork_creates_new_thread_and_emits_started() -> Result<()> {
    ...
    let original_path = thinwedge_home
        .path()
        .join("sessions")
        .join("2025")
        .join("01")
...
async fn thread_fork_inherits_explicit_source_name_from_session_index() -> Result<()> {
    ...
    append_thread_name(thinwedge_home.path(), source_thread_id, source_name).await?;
...
#[tokio::test]
async fn pathless_ephemeral_thread_rejects_thinwedge_home_path_after_reload() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let thinwedge_home = TempDir::new()?;
    create_config_toml(thinwedge_home.path(), &server.uri())?;
    ...
    let resume_id = app_server
        .send_thread_resume_request(ThreadResumeParams {
            ...
            path: Some(thinwedge_home_path.clone()),
            ..Default::default()
        })
        .await?;
