...
use thinwedge_app_server::AppServerWebsocketAuthSettings;
use thinwedge_app_server::PluginStartupTasks;
use thinwedge_app_server::RemoteControlStartupMode;
use thinwedge_app_server::run_main_with_transport_options;
use thinwedge_app_server_protocol::JSONRPCError;
use thinwedge_app_server_protocol::JSONRPCResponse;
use thinwedge_app_server_protocol::RemoteControlClient;
use thinwedge_app_server_protocol::RemoteControlClientsListOrder;
use thinwedge_app_server_protocol::RemoteControlClientsListParams;
use thinwedge_app_server_protocol::RemoteControlClientsListResponse;
use thinwedge_app_server_protocol::RemoteControlClientsRevokeParams;
use thinwedge_app_server_protocol::RemoteControlClientsRevokeResponse;
use thinwedge_app_server_protocol::RemoteControlConnectionStatus;
use thinwedge_app_server_protocol::RemoteControlDisableResponse;
use thinwedge_app_server_protocol::RemoteControlEnableResponse;
use thinwedge_app_server_protocol::RemoteControlPairingStartParams;
use thinwedge_app_server_protocol::RemoteControlPairingStartResponse;
use thinwedge_app_server_protocol::RemoteControlPairingStatusParams;
use thinwedge_app_server_protocol::RemoteControlPairingStatusResponse;
use thinwedge_app_server_protocol::RemoteControlStatusChangedNotification;
use thinwedge_app_server_protocol::RemoteControlStatusReadResponse;
...
async fn managed_requirements_reject_all_remote_control_rpcs() -> Result<()> {
    ...
    std::fs::write(
        thinwedge_home.path().join("requirements.toml"),
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = thinwedge_home.path().join("managed_config.toml");
    let socket_path = thinwedge_home.path().join("app-server.sock");
    let transport =
        AppServerTransport::from_listen_url(&format!("unix://{}", socket_path.display()))?;
    let _thinwedge_home_guard =
        EnvVarGuard::set("THINWEDGE_HOME", thinwedge_home.path().as_os_str());

    let result = timeout(
        STARTUP_TIMEOUT,
        run_main_with_transport_options(
            Arg0DispatchPaths {
                thinwedge_self_exe: Some(std::env::current_exe()?),
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let thinwedge_home = TempDir::new()?;
    let listener = configured_remote_control_listener(thinwedge_home.path()).await?;
    ...
    let _app_server =
        TestAppServer::new_with_args(thinwedge_home.path(), &["--listen", "off"]).await?;
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let thinwedge_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(thinwedge_home.path()).await?;
    let mut mcp = TestAppServer::new(thinwedge_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let thinwedge_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(thinwedge_home.path()).await?;
    let mut mcp = TestAppServer::new(thinwedge_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let thinwedge_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(thinwedge_home.path()).await?;
    let mut mcp = TestAppServer::new(thinwedge_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let thinwedge_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(thinwedge_home.path()).await?;
    let mut mcp = TestAppServer::new(thinwedge_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
impl ClientManagementRemoteControlBackend {
    async fn start(thinwedge_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(thinwedge_home).await?;
