...
use thinwedge_app_server_protocol::ConfigEdit;
use thinwedge_app_server_protocol::HookEventName;
use thinwedge_app_server_protocol::HookHandlerType;
use thinwedge_app_server_protocol::HookMetadata;
use thinwedge_app_server_protocol::HookSource;
use thinwedge_app_server_protocol::HookTrustStatus;
...
fn command_hook_hash(
    ...
    let Ok(value) = thinwedge_config::TomlValue::try_from(identity) else {
        unreachable!("normalized hook identity should serialize to TOML");
    };
    thinwedge_config::version_for_toml(&value)
}

fn write_user_hook_config(thinwedge_home: &std::path::Path) -> Result<()> {
    std::fs::write(
...
fn write_plugin_hook_config(thinwedge_home: &std::path::Path, hooks_json: &str) -> Result<()> {
    let plugin_root = thinwedge_home.join("plugins/cache/test/demo/local");
...
fn write_project_hook_config(dot_thinwedge_folder: &std::path::Path, command: &str) -> Result<()> {
    std::fs::create_dir_all(dot_thinwedge_folder)?;
    std::fs::write(
        dot_thinwedge_folder.join("config.toml"),
        format!(
            r#"[features]
hooks = true
...
async fn hooks_list_shows_discovered_hook() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_user_hook_config(thinwedge_home.path())?;
...
async fn hooks_list_warms_plugin_capabilities_for_thread_start() -> Result<()> {
  ...
  "hooks": {
    ...
    let plugin_mcp_path = thinwedge_home
        .path()
...
async fn hooks_list_shows_plugin_hook_load_warnings() -> Result<()> {
    let thinwedge_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_plugin_hook_config(thinwedge_home.path(), "{ not-json")?;
...
"#,
    ...
    std::fs::create_dir_all(workspace.path().join(".thinwedge"))?;
    std::fs::write(
        workspace.path().join(".thinwedge/config.toml"),
        r#"[features]
hooks = true
...
"#,
    ...
    set_project_trust_level(thinwedge_home.path(), workspace.path(), TrustLevel::Trusted)?;
    ...
    let project_config_path =
        AbsolutePathBuf::try_from(workspace.path().join(".thinwedge/config.toml"))?;
    assert_eq!(
        data,
        vec![
            HooksListEntry {
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    write_project_hook_config(&repo_root.join(".thinwedge"), "echo root hook")?;
    write_project_hook_config(&worktree_root.join(".thinwedge"), "echo worktree hook")?;
    set_project_trust_level(thinwedge_home.path(), &repo_root, TrustLevel::Trusted)?;
    ...
    let repo_config_path =
        AbsolutePathBuf::from_absolute_path(repo_root.join(".thinwedge/config.toml"))?;
    ...
    .await??;
    let _: thinwedge_app_server_protocol::ConfigWriteResponse = to_response(response)?;
...
async fn config_batch_write_updates_hook_trust_for_loaded_session() -> Result<()> {
    ...
    let thinwedge_home = TempDir::new()?;
    let hook_script_path = thinwedge_home.path().join("user_prompt_submit_hook.py");
    let hook_log_path = thinwedge_home
        .path()
...
"#,
    ...
    let hook_list_id = mcp
        .send_hooks_list_request(HooksListParams {
            cwds: vec![thinwedge_home.path().to_path_buf()],
        })
        .await?;
    let response: JSONRPCResponse = timeout(
        DEFAULT_TIMEOUT,
