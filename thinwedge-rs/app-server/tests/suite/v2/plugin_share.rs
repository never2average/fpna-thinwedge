...
async fn plugin_share_save_uploads_local_plugin() -> Result<()> {
    ...
    write_corrupt_plugin_share_local_path_mapping(thinwedge_home.path())?;
...
async fn plugin_share_checkout_adds_personal_marketplace_entry() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareCheckoutResponse {
            remote_plugin_id: "plugins_123".to_string(),
            plugin_id: "demo-plugin@thinwedge-curated".to_string(),
    ...
    let request_id = mcp
        .send_plugin_list_request(PluginListParams {
            ...
            marketplace_kinds: Some(vec![
                thinwedge_app_server_protocol::PluginListMarketplaceKind::Local,
            ]),
        })
        .await?;
    let response: JSONRPCResponse = timeout(
    ...
    assert_eq!(response.marketplaces[0].name, "thinwedge-curated");
...
async fn plugin_share_update_targets_updates_share_targets() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareUpdateTargetsResponse {
            ...
            discoverability: thinwedge_app_server_protocol::PluginShareDiscoverability::Unlisted,
        }
    );
    Ok(())
}
...
async fn plugin_share_delete_removes_created_workspace_plugin() -> Result<()> {
    ...
    let local_plugin_path = AbsolutePathBuf::try_from(thinwedge_home.path().join("local-plugin"))?;
...
fn write_remote_plugin_config(thinwedge_home: &Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
...
fn write_test_plugin(root: &Path, plugin_name: &str) -> std::io::Result<PathBuf> {
    ...
    write_file(
        &plugin_path.join(".thinwedge-plugin/plugin.json"),
...
fn write_corrupt_plugin_share_local_path_mapping(thinwedge_home: &Path) -> std::io::Result<()> {
    write_file(
