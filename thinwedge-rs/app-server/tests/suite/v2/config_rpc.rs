...
use thinwedge_app_server_protocol::ApprovalsReviewer;
use thinwedge_app_server_protocol::AppsConfig;
use thinwedge_app_server_protocol::AppsDefaultConfig;
...
use thinwedge_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use thinwedge_app_server_protocol::ToolsV2;
...
fn write_config(thinwedge_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let thinwedge_home_path = thinwedge_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(thinwedge_home_path.join("config.toml"))?;
...
"#,
    ...
    assert_eq!(desktop.get("selected-avatar-id"), Some(&json!("thinwedge")));
    assert_eq!(
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let thinwedge_home = TempDir::new()?;
    write_config(&thinwedge_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_thinwedge_folder: project_config
        }
    );
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let thinwedge_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path =
        AbsolutePathBuf::resolve_path_against_base("config.toml", thinwedge_home);
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let thinwedge_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(thinwedge_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
fn assert_layers_user_then_optional_system(
    layers: &[thinwedge_app_server_protocol::ConfigLayer],
