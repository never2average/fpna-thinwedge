...
fn write_cached_remote_plugin_with_skill(
    ...
    let plugin_root = thinwedge_home.join("plugins/cache/openai-curated-remote/linear/local");
    std::fs::create_dir_all(plugin_root.join(".thinwedge-plugin"))?;
    std::fs::write(
        plugin_root.join(".thinwedge-plugin/plugin.json"),
        r#"{"name":"linear"}"#,
...
async fn skills_list_excludes_plugin_skills_when_workspace_thinwedge_plugins_disabled() -> Result<()>
{
    ...
    let server = MockServer::start().await;
    write_skill(&thinwedge_home, "home-skill")?;
    ...
    assert!(
        ...
        "plugin skills should be hidden when workspace ThinWedge plugins are disabled"
    );
    Ok(())
}
...
async fn skills_list_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_skill(&thinwedge_home, "home-skill")?;
    let repo_skill_dir = cwd.path().join(".thinwedge/skills/repo-skill");
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(thinwedge_home.path().join(&relative_cwd))?;
...
async fn skills_list_uses_cached_result_until_force_reload() -> Result<()> {
    ...
    let skill_dir = cwd.path().join(".thinwedge/skills/late-extra-skill");
    std::fs::create_dir_all(&skill_dir)?;
    std::fs::write(
        skill_dir.join("SKILL.md"),
...
async fn skills_changed_notification_is_emitted_after_skill_change() -> Result<()> {
    ...
    write_skill(&thinwedge_home, "demo")?;

    let mut mcp = TestAppServer::new_with_env(
        thinwedge_home.path(),
        &[(THINWEDGE_EXEC_SERVER_URL_ENV_VAR, None)],
    )
    .await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
