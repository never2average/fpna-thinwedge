...
const SERVER_MODEL: &str = "gpt-5.3-thinwedge";
...
async fn cyber_policy_response_emits_typed_error_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        error,
        ErrorNotification {
            error: thinwedge_app_server_protocol::TurnError {
    ...
    loop {
        ...
        match notification.method.as_str() {
            "error" => {
                ...
                if payload.error.thinwedge_error_info == Some(ThinWedgeErrorInfo::CyberPolicy) {
                    error = Some(payload);
                }
            }
            "model/rerouted" => {
