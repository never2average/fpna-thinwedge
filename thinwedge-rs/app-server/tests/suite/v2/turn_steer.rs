...
async fn turn_steer_requires_active_turn() -> Result<()> {
    ...
    mount_analytics_capture(&server, &thinwedge_home).await?;

    let mut mcp = TestAppServer::new_without_managed_config(&thinwedge_home).await?;
    ...
    let event =
        wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "thinwedge_turn_steer_event")
            .await?;
