...
async fn turn_steer_requires_active_turn() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_without_managed_config(&thinwedge_home).await?;
    ...
    let event =
        wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "thinwedge_turn_steer_event").await?;
