...
const REMOTE_CONTROL_SUBSCRIBE_CURSOR_HEADER: &str = "x-thinwedge-subscribe-cursor";
...
fn build_remote_control_websocket_request(
    ...
    set_remote_control_header(headers, "x-thinwedge-server-id", &enrollment.server_id)?;
...
pub(crate) async fn load_remote_control_auth(
    ...
    Ok(RemoteControlConnectionAuth {
        auth_provider: thinwedge_model_provider::auth_provider_from_auth(&auth),
