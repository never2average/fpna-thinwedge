...
use thinwedge_app_server_protocol::OverriddenMetadata;
...
use thinwedge_core::config::deserialize_config_toml_with_base;
use thinwedge_core::config::edit::ConfigEdit;
use thinwedge_core::config::edit::ConfigEditsBuilder;
use thinwedge_core::config::validate_feature_requirements_for_config_toml;
use thinwedge_core::path_utils;
use thinwedge_core::path_utils::SymlinkWritePaths;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let allowed_path =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, self.thinwedge_home());
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.thinwedge_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .thinwedge/ folders because there is no cwd/project root
