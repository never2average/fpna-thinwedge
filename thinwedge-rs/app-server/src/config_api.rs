...
use thinwedge_app_server_protocol::ConfigReadResponse;
use thinwedge_app_server_protocol::ConfigRequirements;
use thinwedge_app_server_protocol::ConfigRequirementsReadResponse;
use thinwedge_app_server_protocol::ConfigValueWriteParams;
use thinwedge_app_server_protocol::ConfigWriteErrorCode;
use thinwedge_app_server_protocol::ConfigWriteResponse;
use thinwedge_app_server_protocol::ConfiguredHookHandler;
use thinwedge_app_server_protocol::ConfiguredHookMatcherGroup;
use thinwedge_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use thinwedge_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use thinwedge_app_server_protocol::JSONRPCErrorError;
use thinwedge_app_server_protocol::ManagedHooksRequirements;
use thinwedge_app_server_protocol::NetworkDomainPermission;
use thinwedge_app_server_protocol::NetworkRequirements;
use thinwedge_app_server_protocol::NetworkUnixSocketPermission;
...
use thinwedge_config::HookHandlerConfig as CoreHookHandlerConfig;
use thinwedge_config::ManagedHooksRequirementsToml;
use thinwedge_config::MatcherGroup as CoreMatcherGroup;
use thinwedge_config::ResidencyRequirement as CoreResidencyRequirement;
use thinwedge_config::SandboxModeRequirement as CoreSandboxModeRequirement;
...
use thinwedge_core_plugins::toggles::collect_plugin_enabled_candidates;
use thinwedge_features::canonical_feature_for_key;
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(thinwedge_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(thinwedge_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
...
) -> thinwedge_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => thinwedge_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: thinwedge_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(thinwedge_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(thinwedge_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(thinwedge_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: thinwedge_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        thinwedge_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        thinwedge_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}
...
mod tests {
    ...
    use thinwedge_config::NetworkDomainPermissionToml as CoreNetworkDomainPermissionToml;
    use thinwedge_config::NetworkDomainPermissionsToml as CoreNetworkDomainPermissionsToml;
    use thinwedge_config::NetworkRequirementsToml as CoreNetworkRequirementsToml;
    use thinwedge_config::NetworkUnixSocketPermissionToml as CoreNetworkUnixSocketPermissionToml;
    use thinwedge_config::NetworkUnixSocketPermissionsToml as CoreNetworkUnixSocketPermissionsToml;
    ...
    fn map_requirements_toml_to_api_converts_core_enums() {
        ...
        assert_eq!(
            ...
            Some(vec![
                thinwedge_app_server_protocol::AskForApproval::Never,
                thinwedge_app_server_protocol::AskForApproval::OnRequest,
            ])
        );
        assert_eq!(
            mapped.allowed_approvals_reviewers,
            Some(vec![
                thinwedge_app_server_protocol::ApprovalsReviewer::User,
                thinwedge_app_server_protocol::ApprovalsReviewer::AutoReview,
            ])
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(thinwedge_app_server_protocol::ResidencyRequirement::Us),
        );
        assert_eq!(
            mapped.network,
    ...
    async fn apply_runtime_feature_enablement_keeps_cli_overrides_above_config_and_runtime() {
        ...
        let mut config = thinwedge_core::config::ConfigBuilder::default()
    ...
    async fn batch_write_reloads_user_config_when_requested() {
        let thinwedge_home = TempDir::new().expect("create temp dir");
        let user_config_path = thinwedge_home.path().join("config.toml");
        ...
        let analytics_config = Arc::new(
            thinwedge_core::config::ConfigBuilder::default()
                .build()
                .await
                .expect("load analytics config"),
        );
        let auth_manager = AuthManager::from_auth_for_testing(ThinWedgeAuth::from_api_key("test"));
        ...
        let response = config_api
            .batch_write(ConfigBatchWriteParams {
                edits: vec![thinwedge_app_server_protocol::ConfigEdit {
                    key_path: "model".to_string(),
                    value: json!("gpt-5"),
                    merge_strategy: thinwedge_app_server_protocol::MergeStrategy::Replace,
                }],
        ...
        assert_eq!(
            response,
            ConfigWriteResponse {
                status: thinwedge_app_server_protocol::WriteStatus::Ok,
                version: response.version.clone(),
                file_path: thinwedge_utils_absolute_path::AbsolutePathBuf::try_from(
