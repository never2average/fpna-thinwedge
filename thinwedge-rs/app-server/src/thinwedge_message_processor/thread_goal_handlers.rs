                    ...
                    if let Some(goal) = goal.as_ref().filter(|goal| {
                        ...
                        state_db
                            .replace_thread_goal(
                                thread_id,
                                objective,
                                status.unwrap_or(thinwedge_state::ThreadGoalStatus::Active),
...
fn thread_goal_status_to_state(status: ThreadGoalStatus) -> thinwedge_state::ThreadGoalStatus {
    match status {
...
fn thread_goal_status_from_state(status: thinwedge_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(super) fn api_thread_goal_from_state(goal: thinwedge_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
