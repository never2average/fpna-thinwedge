...
fn fixture_paths() -> (TempDir, PathBuf, PathBuf) {
    ...
    (root, external_agent_home, thinwedge_home)
}
...
fn service_for_paths(
    ...
    ExternalAgentConfigService::new_for_test(thinwedge_home, external_agent_home)
}
...
async fn detect_home_lists_config_skills_and_agents_md() {
    ...
    let items = service_for_paths(external_agent_home.clone(), thinwedge_home.clone())
    ...
    let expected = vec![
        ...
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                thinwedge_home.join("AGENTS.md").display()
            ),
            cwd: None,
            details: None,
        },
...
async fn detect_home_lists_recent_sessions() {
    let (root, external_agent_home, thinwedge_home) = fixture_paths();
...
async fn detect_repo_still_reports_non_plugin_items_when_home_config_is_invalid() {
    ...
    let items = service_for_paths(root.path().join(EXTERNAL_AGENT_DIR), thinwedge_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
        ...
        vec![
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".thinwedge").join("hooks.json").display()
                ),
                cwd: Some(repo_root.clone()),
                details: Some(MigrationDetails {
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".thinwedge").join("agents").display()
                ),
                cwd: Some(repo_root),
                details: Some(MigrationDetails {
...
"#,
    ...
    let _supported_mcp_config: std::collections::HashMap<
        String,
        thinwedge_config::types::McpServerConfig,
    > = mcp_servers
        .try_into()
        .expect("migrated MCP config should be supported");

    let hooks: JsonValue = serde_json::from_str(
        &fs::read_to_string(repo_root.join(".thinwedge").join("hooks.json")).expect("read hooks"),
    )
    .expect("parse hooks");
    let _supported_hooks: thinwedge_config::HooksFile =
...
Research with ThinWedge carefully."""
"#,
    )
...
async fn import_home_migrates_supported_config_fields_skills_and_agents_md() {
    ...
    fs::write(
            external_agent_home.join("settings.json"),
            format!(r#"{{"model":"{SOURCE_EXTERNAL_AGENT_NAME}","permissions":{{"ask":["git push"]}},"env":{{"FOO":"bar","CI":false,"MAX_RETRIES":3,"MY_TEAM":"thinwedge","IGNORED":null,"LIST":["a","b"],"MAP":{{"x":1}}}},"sandbox":{{"enabled":true,"network":{{"allowLocalBinding":true}}}}}}"#),
        )
        .expect("write settings");
    fs::write(
        external_agent_home
    ...
    assert_eq!(
        fs::read_to_string(thinwedge_home.join("AGENTS.md")).expect("read agents"),
        "ThinWedge guidance"
    );
...
async fn import_home_skips_empty_config_migration() {
    ...
    assert!(!thinwedge_home.join("config.toml").exists());
}
...
async fn import_local_plugins_returns_completed_status() {
    ...
    fs::create_dir_all(plugin_root.join(".thinwedge-plugin")).expect("create plugin manifest dir");
    fs::create_dir_all(&thinwedge_home).expect("create thinwedge home");
    ...
    fs::write(
        plugin_root.join(".thinwedge-plugin").join("plugin.json"),
        r#"{"name":"cloudflare","version":"0.1.0"}"#,
    )
    .expect("write plugin manifest");

    let outcome = service_for_paths(external_agent_home, thinwedge_home.clone())
        .import(vec![ExternalAgentConfigMigrationItem {
    ...
    let config = fs::read_to_string(thinwedge_home.join("config.toml")).expect("read config");
...
async fn detect_home_skips_config_when_target_already_has_supported_fields() {
    ...
    let items = service_for_paths(external_agent_home, thinwedge_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            cwds: None,
        })
...
#[tokio::test]
async fn import_repo_hooks_preserves_disabled_thinwedge_hooks_feature() {
    let root = TempDir::new().expect("create tempdir");
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create external agent dir");
...
#[tokio::test]
async fn detect_repo_skips_plugins_that_are_already_configured_in_thinwedge() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let thinwedge_home = root.path().join(".thinwedge");
    let repo_root = root.path().join("repo");
...
#[tokio::test]
async fn detect_repo_skips_plugins_that_are_disabled_in_thinwedge() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let thinwedge_home = root.path().join(".thinwedge");
    let repo_root = root.path().join("repo");
...
#[tokio::test]
async fn detect_repo_skips_plugins_without_explicit_enabled_in_thinwedge() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let thinwedge_home = root.path().join(".thinwedge");
    let repo_root = root.path().join("repo");
...
async fn import_plugins_requires_details() {
    ...
    let err = service_for_paths(external_agent_home, thinwedge_home)
...
#[tokio::test]
async fn detect_repo_does_not_skip_plugins_only_configured_in_project_thinwedge() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let thinwedge_home = root.path().join(".thinwedge");
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
    fs::create_dir_all(repo_root.join(".thinwedge")).expect("create repo thinwedge dir");
    fs::create_dir_all(&thinwedge_home).expect("create thinwedge home");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
        r#"{
...
"#,
    ...
    .expect("write project thinwedge config");
...
async fn import_plugins_requires_source_marketplace_details() {
    ...
    let outcome = service_for_paths(external_agent_home, thinwedge_home)
        .import_plugins(
            /*cwd*/ None,
...
fn import_skills_returns_only_new_skill_directory_count() {
    ...
    let copied_count = service_for_paths(external_agent_home, thinwedge_home)
