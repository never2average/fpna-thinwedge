...
use thinwedge_core_plugins::marketplace_add::is_local_marketplace_source;
use thinwedge_external_agent_migration::build_mcp_config_from_external;
use thinwedge_external_agent_migration::count_missing_commands;
use thinwedge_external_agent_migration::count_missing_subagents;
use thinwedge_external_agent_migration::hook_migration_event_names;
use thinwedge_external_agent_migration::import_commands;
use thinwedge_external_agent_migration::import_hooks;
use thinwedge_external_agent_migration::import_subagents;
use thinwedge_external_agent_migration::missing_command_names;
...
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "thinwedge.external_agent_config.import";
...
impl ExternalAgentConfigService {
    pub(crate) fn new(thinwedge_home: PathBuf) -> Self {
    ...
    #[cfg(test)]
    fn new_for_test(thinwedge_home: PathBuf, external_agent_home: PathBuf) -> Self {
        Self {
            thinwedge_home,
            external_agent_home,
        }
    ...
    async fn detect_migrations(
        ...
        let target_hooks = repo_root.map_or_else(
            || self.thinwedge_home.join("hooks.json"),
            |repo_root| repo_root.join(".thinwedge").join("hooks.json"),
        );
        ...
        let target_subagents = repo_root.map_or_else(
            || self.thinwedge_home.join("agents"),
            |repo_root| repo_root.join(".thinwedge").join("agents"),
        );
        ...
        let target_agents_md = repo_root.map_or_else(
            || self.thinwedge_home.join("AGENTS.md"),
        ...
        if let Some(settings) = settings.as_ref() {
            match ConfigBuilder::default()
                ...
                .fallback_cwd(Some(self.thinwedge_home.clone()))
                .build()
                .await
            {
                Ok(config) => {
                    ...
                    let configured_marketplace_plugins = configured_marketplace_plugins(
                        &config,
                        &PluginsManager::new(self.thinwedge_home.clone()),
                    )?;
        ...
        if repo_root.is_none() {
            let sessions = detect_recent_sessions(&self.external_agent_home, &self.thinwedge_home)?;
    ...
    pub(crate) async fn import_plugins(
        ...
        let plugins_manager = PluginsManager::new(self.thinwedge_home.clone());
    ...
    fn import_subagents(&self, cwd: Option<&Path>) -> io::Result<usize> {
        let (source_agents, target_agents) = if let Some(repo_root) = find_repo_root(cwd)? {
            (
                ...
                repo_root.join(".thinwedge").join("agents"),
            )
        } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
            return Ok(0);
        } else {
        ...
        let (source_external_agent_dir, target_hooks) =
            if let Some(repo_root) = find_repo_root(cwd)? {
                (
                    ...
                    repo_root.join(".thinwedge").join("hooks.json"),
                )
            } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
                return Ok(());
            } else {
...
fn rewrite_external_agent_terms(content: &str) -> String {
    ...
    for from in [
        ...
        rewritten = replace_case_insensitive_with_boundaries(&rewritten, from, "ThinWedge");
    }
    rewritten
}
