...
fn hooks_to_info(hooks: &[thinwedge_hooks::HookListEntry]) -> Vec<HookMetadata> {
    hooks
        .iter()
                ...
                async move {
                    let (cwd_abs, config_layer_stack) = match self.resolve_cwd_config(&cwd).await {
                        ...
                        Err(message) => {
                            ...
                            return (
                                index,
                                thinwedge_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                    ...
                    let effective_skill_roots = if workspace_thinwedge_plugins_enabled {
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(thinwedge_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
                        errors: vec![thinwedge_app_server_protocol::HookErrorInfo {
                            path: error_path,
            ...
            let plugin_hooks = if plugins_enabled {
                ...
                thinwedge_core_plugins::PluginHookLoadOutcome {
                ...
                thinwedge_core_plugins::PluginHookLoadOutcome::default()
            };
            let hooks = thinwedge_hooks::list_hooks(thinwedge_hooks::HooksConfig {
