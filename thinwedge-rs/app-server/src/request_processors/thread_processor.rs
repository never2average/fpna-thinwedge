...
use thinwedge_app_server_protocol::SelectedCapabilityRoot;
...
impl ThreadRequestProcessor {
    ...
    async fn thread_start_task(
            ...
            && (requested_permissions_trust_project || effective_permissions_trust_project)
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                thinwedge_core::config::set_project_trust_level(
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                ...
                let instruction_sources = thinwedge_thread.instruction_sources().await;
                ...
                thread.thread_source = thinwedge_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                self.thread_goal_processor
                    .emit_resume_goal_snapshot_and_continue(thread_id, thinwedge_thread.as_ref())
                    .await;
            }
            Err(err) => {
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(thinwedge_core::util::normalize_thread_name);
        let history_items = source_thread
            .history
            .as_ref()
            .map(|history| history.items.clone())
...
) -> Result<thinwedge_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    build_thread_turns_page_response(
        items,
        loaded_status,
        has_live_running_thread,
...
pub(super) fn core_thread_write_error(operation: &str, err: ThinWedgeErr) -> JSONRPCErrorError {
    match err {
        ThinWedgeErr::ThreadNotFound(thread_id) => {
            invalid_request(format!("thread not found: {thread_id}"))
        }
        ThinWedgeErr::InvalidRequest(message) => invalid_request(message),
        ThinWedgeErr::UnsupportedOperation(message) => method_not_found(message),
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<thinwedge_protocol::protocol::ThreadSource>,
