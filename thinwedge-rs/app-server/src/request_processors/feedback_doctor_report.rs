...
//! the configured ThinWedge executable as a subprocess, accepts only valid JSON from
//! `thinwedge doctor --json`, derives a small set of Sentry tags, and otherwise
...
use thinwedge_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `thinwedge-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `thinwedge doctor --json` and returns a best-effort feedback attachment.
///
/// Failure to spawn ThinWedge, finish before the timeout, or parse JSON means the
