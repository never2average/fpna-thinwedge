...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "THINWEDGE_APP_SERVER_LOGIN_ISSUER";
...
impl AccountRequestProcessor {
    ...
    async fn maybe_refresh_remote_installed_plugins_cache_for_current_config(
        ...
        thread_manager
            .plugins_manager()
            .set_auth_mode(auth.as_ref().map(ThinWedgeAuth::api_auth_mode));
    ...
    async fn login_v2(
        ...
        match params {
            ...
            LoginAccountParams::Chatgpt {
                thinwedge_streamlined_login,
            } => {
                self.login_chatgpt_v2(request_id, thinwedge_streamlined_login)
                    .await;
            }
    ...
    async fn login_chatgpt_v2(
        ...
        let result = self
            .login_chatgpt_response(thinwedge_streamlined_login)
            .await;
        self.outgoing.send_result(request_id, result).await;
    }
    ...
    async fn login_chatgpt_response(
        ...
        let opts = self
            .login_chatgpt_common(thinwedge_streamlined_login)
            .await?;
    ...
    async fn login_chatgpt_device_code_response(
        ...
        let opts = self
            .login_chatgpt_common(/*thinwedge_streamlined_login*/ false)
            .await?;
    ...
    async fn get_account_token_usage_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "thinwedge account authentication required to read token usage",
            ));
        };
    ...
    async fn send_add_credits_nudge_email_inner(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "thinwedge account authentication required to notify workspace owner",
            ));
        };
    ...
    > {
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "thinwedge account authentication required to read rate limits",
            ));
        };
        ...
        let snapshots = client.get_rate_limits_many().await.map_err(|err| {
            internal_error(format!("failed to fetch thinwedge rate limits: {err}"))
        })?;
        if snapshots.is_empty() {
            return Err(internal_error(
                "failed to fetch thinwedge rate limits: no snapshots returned",
            ));
        }
...
mod tests {
    ...
    use thinwedge_backend_client::TokenUsageProfileDailyBucket;
    use thinwedge_backend_client::TokenUsageProfileStats;
