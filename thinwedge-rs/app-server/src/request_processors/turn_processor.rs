use super::*;
use thinwedge_protocol::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use thinwedge_protocol::protocol::AdditionalContextKind as CoreAdditionalContextKind;
...
struct ThreadSettingsBuildParams {
    ...
    sandbox_policy: Option<thinwedge_app_server_protocol::SandboxPolicy>,
...
impl TurnRequestProcessor {
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<thinwedge_protocol::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        let approval_policy =
            approval_policy.map(thinwedge_app_server_protocol::AskForApproval::to_core);
        let approvals_reviewer =
            approvals_reviewer.map(thinwedge_app_server_protocol::ApprovalsReviewer::to_core);
        ...
        if has_any_overrides {
            thread
                .preview_thread_settings_overrides(ThinWedgeThreadSettingsOverrides {
        ...
        Ok(thinwedge_protocol::protocol::ThreadSettingsOverrides {
            environments,
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != thinwedge_protocol::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
