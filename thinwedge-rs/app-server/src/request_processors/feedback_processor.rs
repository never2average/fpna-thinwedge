    ...
    async fn upload_feedback_response(
        ...
        if include_logs {
            ...
            if let Some(sandbox_log_attachment) =
                windows_sandbox_log_attachment(&self.config.thinwedge_home)
...
#[cfg(target_os = "windows")]
fn windows_sandbox_log_attachment(thinwedge_home: &Path) -> Option<FeedbackAttachmentPath> {
    let sandbox_log_path =
        thinwedge_windows_sandbox::current_log_file_path_for_thinwedge_home(thinwedge_home);
    sandbox_log_path
        .is_file()
...
fn windows_sandbox_log_attachment(_thinwedge_home: &Path) -> Option<FeedbackAttachmentPath> {
    None
}
...
mod tests {
    ...
    fn windows_sandbox_log_attachment_uses_current_log() {
        let thinwedge_home = tempfile::tempdir().expect("create tempdir");
        let sandbox_dir = thinwedge_windows_sandbox::sandbox_dir(thinwedge_home.path());
        std::fs::create_dir_all(&sandbox_dir).expect("create sandbox dir");
        let sandbox_log_path = thinwedge_windows_sandbox::current_log_file_path_for_thinwedge_home(
            thinwedge_home.path(),
        );
        std::fs::write(&sandbox_log_path, "sandbox log").expect("write sandbox log");

        let attachment = windows_sandbox_log_attachment(thinwedge_home.path())
