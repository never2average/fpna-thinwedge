...
use thinwedge_external_agent_sessions::CompletedExternalAgentSessionImport;
...
use thinwedge_external_agent_sessions::PendingSessionImport;
use thinwedge_external_agent_sessions::prepare_validated_session_import;
use thinwedge_external_agent_sessions::record_completed_session_imports;
...
impl ExternalAgentSessionImporter {
    ...
    pub(super) async fn import_sessions(&self, sessions: Vec<ExternalAgentSessionMigration>) {
        ...
        if let Err(err) = record_completed_session_imports(&self.thinwedge_home, completed_imports)
        {
            tracing::warn!(
                error = %err,
    ...
    async fn prepare_session_import(
        ...
        tokio::task::spawn_blocking(move || {
            prepare_validated_session_import(&thinwedge_home, session)
        })
        .await
