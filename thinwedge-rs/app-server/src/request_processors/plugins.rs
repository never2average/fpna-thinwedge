...
use thinwedge_app_server_protocol::PluginSharePrincipalRole;
use thinwedge_app_server_protocol::PluginShareTargetRole;
use thinwedge_config::types::McpServerConfig;
use thinwedge_core_plugins::OPENAI_CURATED_MARKETPLACE_NAME;
use thinwedge_core_plugins::PluginListBackgroundTaskOptions;
use thinwedge_core_plugins::remote::REMOTE_GLOBAL_MARKETPLACE_NAME;
use thinwedge_core_plugins::remote::REMOTE_WORKSPACE_MARKETPLACE_NAME;
use thinwedge_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_MARKETPLACE_NAME;
use thinwedge_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_PRIVATE_MARKETPLACE_NAME;
use thinwedge_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_UNLISTED_MARKETPLACE_NAME;
use thinwedge_core_plugins::remote::RemoteAppTemplateUnavailableReason;
use thinwedge_core_plugins::remote::is_valid_remote_plugin_id;
use thinwedge_core_plugins::remote::validate_remote_plugin_id;
use thinwedge_mcp::McpOAuthLoginSupport;
use thinwedge_mcp::oauth_login_support;
use thinwedge_mcp::should_retry_without_scopes;
use thinwedge_rmcp_client::perform_oauth_login_silent;
...
fn load_shared_plugin_ids_by_local_path(
    ...
    thinwedge_core_plugins::remote::load_plugin_share_remote_ids_by_local_path(
...
fn convert_configured_marketplace_plugin_to_plugin_summary(
    plugin: thinwedge_core_plugins::ConfiguredMarketplacePlugin,
    shared_plugin_ids_by_local_path: &std::collections::BTreeMap<AbsolutePathBuf, String>,
) -> PluginSummary {
...
) -> thinwedge_core_plugins::remote::RemotePluginShareDiscoverability {
    match discoverability {
        PluginShareDiscoverability::Listed => {
            thinwedge_core_plugins::remote::RemotePluginShareDiscoverability::Listed
        }
        PluginShareDiscoverability::Unlisted => {
            thinwedge_core_plugins::remote::RemotePluginShareDiscoverability::Unlisted
        }
        PluginShareDiscoverability::Private => {
            thinwedge_core_plugins::remote::RemotePluginShareDiscoverability::Private
        }
    }
}
...
) -> thinwedge_core_plugins::remote::RemotePluginShareUpdateDiscoverability {
    match discoverability {
        PluginShareUpdateDiscoverability::Unlisted => {
            thinwedge_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Unlisted
        }
        PluginShareUpdateDiscoverability::Private => {
            thinwedge_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Private
        }
    }
}
...
) -> thinwedge_core_plugins::remote::RemotePluginShareTargetRole {
    match role {
        PluginShareTargetRole::Reader => {
            thinwedge_core_plugins::remote::RemotePluginShareTargetRole::Reader
        }
        PluginShareTargetRole::Editor => {
            thinwedge_core_plugins::remote::RemotePluginShareTargetRole::Editor
        }
    }
}

fn plugin_share_principal_role_from_remote(
    role: thinwedge_core_plugins::remote::RemotePluginSharePrincipalRole,
) -> PluginSharePrincipalRole {
    match role {
        thinwedge_core_plugins::remote::RemotePluginSharePrincipalRole::Reader => {
            PluginSharePrincipalRole::Reader
        }
        thinwedge_core_plugins::remote::RemotePluginSharePrincipalRole::Editor => {
            PluginSharePrincipalRole::Editor
        }
        thinwedge_core_plugins::remote::RemotePluginSharePrincipalRole::Owner => {
...
) -> Vec<thinwedge_core_plugins::remote::RemotePluginShareTarget> {
    targets
        .into_iter()
        .map(
            |target| thinwedge_core_plugins::remote::RemotePluginShareTarget {
                principal_type: match target.principal_type {
                    PluginSharePrincipalType::User => {
                        thinwedge_core_plugins::remote::RemotePluginSharePrincipalType::User
                    }
                    PluginSharePrincipalType::Group => {
                        thinwedge_core_plugins::remote::RemotePluginSharePrincipalType::Group
                    }
                    PluginSharePrincipalType::Workspace => {
                        thinwedge_core_plugins::remote::RemotePluginSharePrincipalType::Workspace
                    }
                },
...
fn plugin_share_principal_from_remote(
    principal: thinwedge_core_plugins::remote::RemotePluginSharePrincipal,
...
impl PluginRequestProcessor {
    ...
    async fn plugin_list_response(
        ...
        let refresh_global_remote_catalog_cache = include_global_remote
            && thinwedge_core_plugins::remote::has_cached_global_remote_plugin_catalog(
                config.thinwedge_home.as_path(),
            ...
            match tokio::task::spawn_blocking(move || {
                ...
                Ok::<
                    (
                        ...
                        Vec<thinwedge_app_server_protocol::MarketplaceLoadErrorInfo>,
                    ),
                    MarketplaceError,
                >((
                    outcome
        ...
        if include_vertical && !config.features.enabled(Feature::RemotePlugin) {
            match thinwedge_core_plugins::remote::fetch_openai_curated_remote_collection_marketplace(
                &remote_plugin_service_config,
                auth.as_ref(),
            )
            .await
        ...
        if !remote_sources.is_empty() {
            match thinwedge_core_plugins::remote::fetch_remote_marketplaces(
                &remote_plugin_service_config,
                auth.as_ref(),
                &remote_sources,
                /*global_catalog_cache_path*/ Some(config.thinwedge_home.as_path()),
            )
            .await
            {
    ...
    async fn load_local_installed_and_suggested_plugins(
        &self,
        plugins_manager: Arc<thinwedge_core_plugins::PluginsManager>,
        config: &Config,
        plugins_input: &thinwedge_core_plugins::PluginsConfigInput,
    ...
    async fn plugin_read_response(
        ...
        let plugin = match read_source {
            Ok(marketplace_path) => {
                ...
                let share_context = match share_context {
                    Some(context) => {
                        ...
                        match thinwedge_core_plugins::remote::fetch_remote_plugin_share_context(
                            &remote_plugin_service_config,
                            auth.as_ref(),
                ...
                PluginDetail {
                    ...
                    hooks: outcome
                        ...
                        .hooks
                        .into_iter()
                        .map(|hook| thinwedge_app_server_protocol::PluginHookSummary {
                            key: hook.key,
            ...
            Err(remote_marketplace_name) => {
                ...
                let remote_detail = thinwedge_core_plugins::remote::fetch_remote_plugin_detail(
                    &remote_plugin_service_config,
                    auth.as_ref(),
                    &remote_marketplace_name,
                    &plugin_name,
                ...
                let plugin_apps = remote_detail
                    ...
                    .iter()
                    .cloned()
                    .map(thinwedge_plugin::AppConnectorId)
                    .collect::<Vec<_>>();
    ...
    async fn plugin_skill_read_response(
        ...
        let remote_skill_detail = thinwedge_core_plugins::remote::fetch_remote_plugin_skill_detail(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_marketplace_name,
            &remote_plugin_id,
    ...
    async fn plugin_share_save_response(
        ...
        let access_policy = thinwedge_core_plugins::remote::RemotePluginShareAccessPolicy {
        ...
        let result = thinwedge_core_plugins::remote::save_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.thinwedge_home.as_path(),
            &plugin_path,
    ...
    async fn plugin_share_update_targets_response(
        ...
        let result = thinwedge_core_plugins::remote::update_remote_plugin_share_targets(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_plugin_id,
    ...
    async fn plugin_share_list_response(
        ...
        let data = thinwedge_core_plugins::remote::list_remote_plugin_shares(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.thinwedge_home.as_path(),
        )
    ...
    async fn plugin_share_checkout_response(
        ...
        let result = thinwedge_core_plugins::remote::checkout_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.thinwedge_home.as_path(),
            &remote_plugin_id,
    ...
    async fn plugin_share_delete_response(
        ...
        thinwedge_core_plugins::remote::delete_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.thinwedge_home.as_path(),
            &remote_plugin_id,
    ...
    async fn load_plugin_share_config_and_auth(
        &self,
    ) -> Result<(Config, Option<ThinWedgeAuth>), JSONRPCErrorError> {
        let config = self.load_latest_config(/*fallback_cwd*/ None).await?;
        if !config.features.enabled(Feature::Plugins) {
    ...
    async fn plugin_install_response(
        ...
        {
            return Err(invalid_request(
                "ThinWedge plugins are disabled for this workspace",
            ));
        }
        ...
        let apps_needing_auth = self
            .plugin_apps_needing_auth_for_install(
                &config,
                auth.as_ref().is_some_and(ThinWedgeAuth::is_chatgpt_auth),
    ...
    async fn remote_plugin_install_response(
        ...
        let remote_detail =
            thinwedge_core_plugins::remote::fetch_remote_plugin_detail_with_download_urls(
                &remote_plugin_service_config,
                auth.as_ref(),
                &remote_marketplace_name,
                &remote_plugin_id,
        ...
        let _remote_plugin_cache_mutation =
            thinwedge_core_plugins::remote::mark_remote_plugin_cache_mutation_in_flight(
                config.thinwedge_home.as_path(),
        ...
        let install_result = thinwedge_core_plugins::remote::install_remote_plugin(
            &remote_plugin_service_config,
            auth.as_ref(),
            &actual_remote_marketplace_name,
            &remote_plugin_id,
        ...
        let is_chatgpt_auth = auth.as_ref().is_some_and(ThinWedgeAuth::is_chatgpt_auth);
    ...
    async fn plugin_apps_needing_auth_for_install(
        ...
        plugin_id: &str,
        plugin_apps: &[thinwedge_plugin::AppConnectorId],
    ) -> Vec<AppSummary> {
        ...
        let (accessible_connectors, thinwedge_apps_ready) = match accessible_connectors_result {
            Ok(status) => (status.connectors, status.thinwedge_apps_ready),
            Err(err) => {
                warn!(
                    plugin = plugin_id,
        ...
        if !thinwedge_apps_ready {
            warn!(
                plugin = plugin_id,
                "thinwedge_apps MCP not ready after plugin install; skipping appsNeedingAuth check"
            );
        }
    ...
    async fn plugin_uninstall_response(
        ...
        if thinwedge_plugin::PluginId::parse(&plugin_id).is_err()
    ...
    async fn remote_plugin_uninstall_response(
        ...
        let uninstall_result = thinwedge_core_plugins::remote::uninstall_remote_plugin(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.thinwedge_home.to_path_buf(),
            &plugin_id,
...
fn plugin_app_category_by_id_from_value(value: &serde_json::Value) -> HashMap<String, String> {
    thinwedge_core_plugins::loader::plugin_app_metadata_from_value(value)
        .into_iter()
...
fn remote_plugin_share_discoverability_to_info(
    discoverability: thinwedge_core_plugins::remote::RemotePluginShareDiscoverability,
...
fn remote_plugin_bundle_install_error_to_jsonrpc(
    err: thinwedge_core_plugins::remote_bundle::RemotePluginBundleInstallError,
) -> JSONRPCErrorError {
