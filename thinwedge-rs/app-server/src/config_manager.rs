...
impl ConfigManager {
    ...
    pub(crate) fn thinwedge_home(&self) -> &Path {
    ...
    fn current_thread_config_loader(&self) -> Arc<dyn ThreadConfigLoader> {
        self.thread_config_loader
            ...
            .unwrap_or_else(|_| Arc::new(thinwedge_config::NoopThreadConfigLoader))
    }
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.thinwedge_self_exe = self.arg0_paths.thinwedge_self_exe.clone();
        config.thinwedge_linux_sandbox_exe = self.arg0_paths.thinwedge_linux_sandbox_exe.clone();
    ...
    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(thinwedge_home: PathBuf) -> Self {
        Self::new_for_tests(
            thinwedge_home,
            Vec::new(),
