...
use thinwedge_app_server_protocol::HookMigration;
use thinwedge_app_server_protocol::JSONRPCErrorError;
use thinwedge_app_server_protocol::McpServerMigration;
use thinwedge_app_server_protocol::MigrationDetails;
use thinwedge_app_server_protocol::PluginsMigration;
use thinwedge_app_server_protocol::SubagentMigration;
use thinwedge_external_agent_sessions::ExternalAgentSessionMigration as CoreSessionMigration;
use thinwedge_external_agent_sessions::PendingSessionImport;
use thinwedge_external_agent_sessions::PrepareSessionImportsError;
...
impl ExternalAgentConfigApi {
    pub(crate) fn new(thinwedge_home: PathBuf) -> Self {
        Self {
            migration_service: ExternalAgentConfigService::new(thinwedge_home.clone()),
            thinwedge_home,
        }
    }
                ...
                .map(|migration_item| ExternalAgentConfigMigrationItem {
                    ...
                    details: migration_item.details.map(|details| MigrationDetails {
                        ...
                        sessions: details
                            .sessions
                            .into_iter()
                            .map(|session| thinwedge_app_server_protocol::SessionMigration {
                                path: session.path,
                                cwd: session.cwd,
                                title: session.title,
                            })
    ...
    pub(crate) fn prepare_pending_session_imports(
        ...
        prepare_pending_session_imports(&self.thinwedge_home, sessions, detected_sessions).map_err(
            |err| match err {
