...
async fn build_refresh_config(
    ...
    let mcp_servers = thinwedge_mcp::configured_mcp_servers(&mcp_config);
...
mod tests {
    ...
    use thinwedge_config::ThreadConfigContext;
    use thinwedge_config::ThreadConfigLoadError;
    use thinwedge_config::ThreadConfigLoadErrorCode;
    ...
    use thinwedge_extension_api::NoopExtensionEventSink;
    ...
    async fn refresh_config_uses_latest_auth_keyring_backend() -> anyhow::Result<()> {
        ...
        std::fs::write(
            temp_dir.path().join(thinwedge_config::CONFIG_TOML_FILE),
    ...
    async fn refresh_test_state() -> anyhow::Result<(
        ...
        let auth_manager = AuthManager::from_auth_for_testing(ThinWedgeAuth::from_api_key("dummy"));
            ...
            ThreadManager::new(
                ...
                thread_extensions(
                    ...
                    ThreadExtensionDependencies {
                        ...
                        goal_service: Arc::new(thinwedge_goal_extension::GoalService::new()),
                        executor_skill_provider: Arc::clone(&executor_skill_provider),
                        thread_store: Arc::clone(&thread_store),
                    },
                ),
                Arc::new(ThinWedgeHomeUserInstructionsProvider::new(
                    good_config.thinwedge_home.clone(),
                )),
    ...
    impl ThreadConfigLoader for CountingThreadConfigLoader {
        fn load(
            &self,
            context: ThreadConfigContext,
        ) -> thinwedge_config::ThreadConfigLoaderFuture<'_, Vec<ThreadConfigSource>> {
