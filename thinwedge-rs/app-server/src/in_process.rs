...
//!
//! # Relationship to `thinwedge-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `thinwedge-app-server-client`,
...
pub use thinwedge_rollout::StateDbHandle;
pub use thinwedge_state::log_db::LogDbLayer;
use tokio::sync::mpsc;
...
/// through `thinwedge-app-server-client`, which adds worker-task buffering,
...
pub struct InProcessClientHandle {
    ...
    #[cfg(test)]
    _test_thinwedge_home: Option<tempfile::TempDir>,
}
...
async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    let installation_id = resolve_installation_id(&args.config.thinwedge_home).await?;
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(thinwedge_home.path()).await);
        let state_db = thinwedge_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "thinwedge-in-process-test".to_string(),
                    title: None,
        ...
        client._test_thinwedge_home = Some(thinwedge_home);
        client
    }
