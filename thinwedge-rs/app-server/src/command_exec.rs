...
use thinwedge_app_server_protocol::CommandExecResizeResponse;
...
use thinwedge_app_server_protocol::CommandExecTerminateResponse;
use thinwedge_app_server_protocol::CommandExecWriteParams;
use thinwedge_app_server_protocol::CommandExecWriteResponse;
...
use thinwedge_utils_pty::ProcessHandle;
use thinwedge_utils_pty::SpawnedProcess;
use thinwedge_utils_pty::TerminalSize;
...
impl CommandExecManager {
    pub(crate) async fn start(
        ...
        let spawned = if tty {
            thinwedge_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
...
mod tests {
    ...
    async fn windows_sandbox_non_streaming_exec_uses_execution_path() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: thinwedge_app_server_protocol::RequestId::Integer(99),
        };
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: thinwedge_app_server_protocol::RequestId::Integer(100),
        };
        let cwd = AbsolutePathBuf::current_dir().expect("current dir");
    ...
    async fn windows_sandbox_process_ids_reject_write_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: thinwedge_app_server_protocol::RequestId::Integer(1),
        };
    ...
    async fn windows_sandbox_process_ids_reject_terminate_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: thinwedge_app_server_protocol::RequestId::Integer(2),
        };
        let process_id = ConnectionProcessId {
            connection_id: request_id.connection_id,
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: thinwedge_app_server_protocol::RequestId::Integer(3),
        };
