...
use thinwedge_extension_api::AgentSpawnFuture;
use thinwedge_extension_api::AgentSpawner;
...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn thinwedge_skills_extension::SkillProvider>,
...
where
    S: AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = ThinWedgeErr> + 'static,
{
    ...
    if let Some(state_db) = state_db {
        thinwedge_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            thinwedge_otel::global(),
            thread_manager,
            goal_service,
            |config: &Config| config.features.enabled(thinwedge_features::Feature::Goals),
        );
    }
    thinwedge_guardian::install(&mut builder, guardian_agent_spawner);
    thinwedge_memories_extension::install(&mut builder, thinwedge_otel::global());
    thinwedge_mcp_extension::install(&mut builder);
    thinwedge_web_search_extension::install(&mut builder, auth_manager.clone());
    thinwedge_image_generation_extension::install(&mut builder, auth_manager);
    let mut skill_providers = thinwedge_skills_extension::SkillProviders::new()
        .with_executor_provider(executor_skill_provider);
    if ORCHESTRATOR_SKILLS_ENABLED {
        skill_providers = skill_providers.with_orchestrator_provider(Arc::new(
            thinwedge_skills_extension::OrchestratorSkillProvider::new(),
        ));
    }
    thinwedge_skills_extension::install_with_providers(
        &mut builder,
        skill_providers,
        |config: &Config| thinwedge_skills_extension::SkillsExtensionConfig {
...
) -> impl AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = ThinWedgeErr> {
    move |forked_from_thread_id: ThreadId,
          options: StartThreadOptions|
          -> AgentSpawnFuture<'static, NewThread, ThinWedgeErr> {
        let thread_manager = thread_manager.clone();
        Box::pin(async move {
            let thread_manager = thread_manager.upgrade().ok_or_else(|| {
                ThinWedgeErr::UnsupportedOperation("thread manager dropped".to_string())
            })?;
            thread_manager
...
mod tests {
    ...
    use thinwedge_protocol::protocol::ThreadGoal as CoreThreadGoal;
