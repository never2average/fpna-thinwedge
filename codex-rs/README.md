...
ThinWedge's `statisticalmodels.*` and `trainingenvironments.*` tool state is local-filesystem based in this build. The TUI reads and writes JSON under `CODEX_HOME/thinwedge/ml/`, including `statisticalmodels.json`, `trainingenvironments.json`, `jobs/*.json`, `evals/*.json`, runtime context files under `runtime/*.json`, and environment session caches under `environments/*.json`. ThinWedge also exposes direct API-backed cost namespaces for the built-in roles:
...
Runpod session state is cached at `CODEX_HOME/thinwedge/ml/environments/<environmentId>.json`.
...
- Required environment:
  - an active Runpod session cache at `CODEX_HOME/thinwedge/ml/environments/<environmentId>.json`
...
```json
{
  ...
  "tools": {
    "submitTraining": {
      "command": "bash /abs/path/to/codex-rs/scripts/thinwedge-ml/train.sh",
      "workingDirectory": "/abs/path/to/model-pricing-repo"
    },
    "submitBatchInference": {
      "command": "bash /abs/path/to/codex-rs/scripts/thinwedge-ml/batch_infer.sh",
      "workingDirectory": "/abs/path/to/model-pricing-repo"
    }
  }
}
...
```json
{
  ...
  "tools": {
    "launch": {
      "command": "bash /abs/path/to/codex-rs/scripts/thinwedge-ml/launch.sh",
      "workingDirectory": "/abs/path/to/env-pricing-repo"
    },
    "attach": {
      "command": "bash /abs/path/to/codex-rs/scripts/thinwedge-ml/attach.sh",
      "workingDirectory": "/abs/path/to/env-pricing-repo"
    },
    "stop": {
      "command": "bash /abs/path/to/codex-rs/scripts/thinwedge-ml/stop.sh",
      "workingDirectory": "/abs/path/to/env-pricing-repo"
    }
  }
}
