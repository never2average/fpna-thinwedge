...
pub(crate) async fn handle_dynamic_tool_call(
    ...
    let result = match (params.namespace.as_deref(), params.tool.as_str()) {
        (Some("statisticalmodels"), "list") => {
            list_models(codex_home, role, params.arguments).await
        }
        (Some("statisticalmodels"), "get") => get_model(codex_home, role, params.arguments).await,
        (Some("statisticalmodels"), "submitJob") => {
            submit_job(codex_home, role, params.arguments).await
        }
        (Some("statisticalmodels"), "listJobs") => list_jobs(codex_home, params.arguments).await,
        (Some("statisticalmodels"), "getJob") => get_job(codex_home, params.arguments).await,
        (Some("statisticalmodels"), "listEvals") => list_evals(codex_home, params.arguments).await,
        (Some("statisticalmodels"), "getEval") => get_eval(codex_home, params.arguments).await,
        (Some("trainingenvironments"), "list") => {
            list_training_environments(codex_home, role, params.arguments).await
        }
        (Some("trainingenvironments"), "get") => {
            get_training_environment(codex_home, role, params.arguments).await
        }
        ...
        (Some("trainingenvironments"), "attach") => {
            attach_training_environment(codex_home, role, params.arguments).await
        }
...
async fn list_models(
    ...
    let mut models = read_models_registry(codex_home).await?.models;
    models.retain(|model| {
...
async fn get_model(
    ...
    let registry = read_models_registry(codex_home).await?;
    let model = registry
        .models
        .into_iter()
        .find(|model| {
...
async fn submit_job(
    ...
    let models = read_models_registry(codex_home).await?;
    let model = models
        .models
        .into_iter()
        .find(|model| {
    ...
    write_job(codex_home, &job).await?;
...
async fn list_jobs(codex_home: &Path, arguments: JsonValue) -> Result<String, String> {
    let args: ListJobsArgs = parse_arguments(arguments)?;
    let mut jobs = read_jobs(codex_home).await?;
    jobs.retain(|job| {
        args.model_id
            .as_deref()
...
async fn get_job(codex_home: &Path, arguments: JsonValue) -> Result<String, String> {
    let args: GetJobArgs = parse_arguments(arguments)?;
    pretty_json(&read_job(codex_home, &args.job_id).await?)
}

async fn list_evals(codex_home: &Path, arguments: JsonValue) -> Result<String, String> {
    let args: ListEvalsArgs = parse_arguments(arguments)?;
    let mut evals = read_evals(codex_home).await?;
    evals.retain(|eval| {
        args.model_id
            .as_deref()
...
async fn get_eval(codex_home: &Path, arguments: JsonValue) -> Result<String, String> {
    let args: GetEvalArgs = parse_arguments(arguments)?;
    pretty_json(&read_eval(codex_home, &args.eval_id).await?)
}
...
async fn list_training_environments(
    ...
    let mut environments = read_training_environments_registry(codex_home)
        .await?
        .environments;
...
async fn get_training_environment(
    ...
    let environment = get_visible_environment(codex_home, agent_role, &args.environment_id).await?;
...
async fn set_training_environment_status(
    ...
    let args: EnvironmentByIdArgs = parse_arguments(arguments)?;
    let mut registry = read_training_environments_registry(codex_home).await?;
    ...
    write_training_environments_registry(codex_home, &registry).await?;
    pretty_json(&json!({
