...
export THINWEDGE_CODEX_HOME="$CODEX_HOME"
...
"$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/seed-fixtures.sh" "$CODEX_HOME" "$workspace_root" >/dev/null

model_repo_root="$workspace_root/model-pricing-repo"
context_path="$CODEX_HOME/thinwedge/ml/runtime/smoke-context.json"
...
python3 - "$CODEX_HOME/thinwedge/ml/statisticalmodels.json" "$CODEX_HOME/thinwedge/ml/trainingenvironments.json" "$context_path" <<'PY'
import json
import pathlib
import sys
...
if [[ "$live_runpod" -eq 0 ]]; then
python3 - "$CODEX_HOME" "$THINWEDGE_RUNPOD_MOCK_DIR" <<'PY'
import json
import pathlib
import sys
...
env_path = codex_home / "thinwedge" / "ml" / "environments" / "env-pricing.json"
evals_dir = codex_home / "thinwedge" / "ml" / "evals"
...
train_artifact_path = codex_home / "thinwedge" / "ml" / "artifacts" / "training" / "job-train-smoke.json"
batch_artifact_path = codex_home / "thinwedge" / "ml" / "artifacts" / "batch-inference" / "job-batch-smoke.json"
