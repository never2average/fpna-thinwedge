#!/usr/bin/env python3
"""Install ThinWedge native binaries (Rust CLI plus ripgrep helpers)."""
...
DEFAULT_WORKFLOW_URL = "https://github.com/never2average/fpna-thinwedge/actions/runs/17952349351"
...
class BinaryComponent:
    artifact_prefix: str  # matches the artifact filename prefix (e.g. thinwedge-<target>.zst)
...
BINARY_COMPONENTS = {
    "thinwedge": BinaryComponent(
        artifact_prefix="thinwedge",
        dest_dir="thinwedge",
        binary_basename="thinwedge",
    ),
    "thinwedge-responses-api-proxy": BinaryComponent(
    ...
    "thinwedge-windows-sandbox-setup": BinaryComponent(
        ...
        dest_dir="thinwedge",
        binary_basename="thinwedge-windows-sandbox-setup",
        targets=WINDOWS_TARGETS,
    ),
    "thinwedge-command-runner": BinaryComponent(
        artifact_prefix="thinwedge-command-runner",
        dest_dir="thinwedge",
        binary_basename="thinwedge-command-runner",
        targets=WINDOWS_TARGETS,
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install native ThinWedge binaries.")
    parser.add_argument(
        "--workflow-url",
        help=(
...
def main() -> int:
    ...
    thinwedge_cli_root = (args.root or THINWEDGE_CLI_ROOT).resolve()
...
def _download_artifacts(workflow_id: str, dest_dir: Path) -> None:
    cmd = [
        ...
        str(dest_dir),
        "--repo",
        os.environ.get("GITHUB_REPOSITORY", "never2average/fpna-thinwedge"),
        workflow_id,
    ]
