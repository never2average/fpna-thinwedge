...
THINWEDGE_CLI_ROOT = SCRIPT_DIR.parent
REPO_ROOT = THINWEDGE_CLI_ROOT.parent
RESPONSES_API_PROXY_NPM_ROOT = REPO_ROOT / "thinwedge-rs" / "responses-api-proxy" / "npm"
THINWEDGE_SDK_ROOT = REPO_ROOT / "sdk" / "typescript"
...
THINWEDGE_PLATFORM_PACKAGES: dict[str, dict[str, str]] = {
    "thinwedge-linux-x64": {
...
PACKAGE_EXPANSIONS: dict[str, list[str]] = {
    "thinwedge": ["thinwedge", *THINWEDGE_PLATFORM_PACKAGES],
}
...
PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    ...
    "thinwedge-win32-arm64": ["thinwedge", "rg", "thinwedge-windows-sandbox-setup", "thinwedge-command-runner"],
...
PACKAGE_TARGET_FILTERS: dict[str, str] = {
    ...
    for package_name, package_config in THINWEDGE_PLATFORM_PACKAGES.items()
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or stage the ThinWedge CLI npm package.")
    parser.add_argument(
        "--package",
        choices=PACKAGE_CHOICES,
        default="thinwedge",
        help="Which npm package to stage (default: thinwedge).",
    )
    parser.add_argument(
        "--version",
        ...
        if release_version:
            ...
            if package == "thinwedge":
                print(
                    ...
                    "Verify the CLI:\n"
                    f"    node {staging_dir_str}/bin/thinwedge.js --version\n"
                    f"    node {staging_dir_str}/bin/thinwedge.js --help\n\n"
                )
            elif package == "thinwedge-responses-api-proxy":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
                    "Verify the responses API proxy:\n"
                    f"    node {staging_dir_str}/bin/thinwedge-responses-api-proxy.js --help\n\n"
                )
            elif package in THINWEDGE_PLATFORM_PACKAGES:
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="thinwedge-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    ...
    if package == "thinwedge":
        ...
        shutil.copy2(THINWEDGE_CLI_ROOT / "bin" / "thinwedge.js", bin_dir / "thinwedge.js")
        shutil.copy2(THINWEDGE_CLI_ROOT / "bin" / "thinwedge.js", bin_dir / "thinwedge.js")
        ...
        package_json_path = THINWEDGE_CLI_ROOT / "package.json"
    elif package in THINWEDGE_PLATFORM_PACKAGES:
        platform_package = THINWEDGE_PLATFORM_PACKAGES[package]
        ...
        with open(THINWEDGE_CLI_ROOT / "package.json", "r", encoding="utf-8") as fh:
            thinwedge_package_json = json.load(fh)
        ...
        package_manager = thinwedge_package_json.get("packageManager")
        if isinstance(package_manager, str):
            package_json["packageManager"] = package_manager
    elif package == "thinwedge-responses-api-proxy":
        bin_dir = staging_dir / "bin"
        bin_dir.mkdir(parents=True, exist_ok=True)
        launcher_src = RESPONSES_API_PROXY_NPM_ROOT / "bin" / "thinwedge-responses-api-proxy.js"
        shutil.copy2(launcher_src, bin_dir / "thinwedge-responses-api-proxy.js")
    ...
    elif package == "thinwedge-sdk":
        package_json_path = THINWEDGE_SDK_ROOT / "package.json"
        stage_thinwedge_sdk_sources(staging_dir)
    else:
...
def stage_thinwedge_sdk_sources(staging_dir: Path) -> None:
    package_root = THINWEDGE_SDK_ROOT
    ...
    if not dist_src.exists():
        raise RuntimeError("thinwedge-sdk build did not produce a dist directory.")
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="thinwedge-npm-pack-") as pack_dir_str:
