...
- `thinwedge_tui::streaming::commit_tick`
...
Run ThinWedge with chunking traces enabled:

```bash
RUST_LOG='thinwedge_tui::streaming::commit_tick=trace,thinwedge_tui=info,thinwedge_core=info,thinwedge_rmcp_client=info' \
  just thinwedge --enable=responses_websockets
```
...
1. Record the current size of `~/.thinwedge/log/thinwedge-tui.log` as a start offset.
