...
- OpenAI Codex documents version-controlled folders using a lower-risk automation preset based on workspace writes and on-request approvals. It also separates filesystem sandboxing from approval policy, with `sandbox_mode`, `approval_policy`, `sandbox_workspace_write.network_access`, and extra writable roots as explicit configuration surfaces. Sources: https://developers.openai.com/codex/agent-approvals-security, https://developers.openai.com/codex/config-reference.
- Codex command rules use exact command-prefix matching and evaluate the strictest matching decision. ThinWedge already mirrors this through `thinwedge-execpolicy`, so the implementation should extend existing prefix-rule policy rather than inventing a parallel shell classifier. Source: https://developers.openai.com/codex/rules.
