...
- A version-controlled-folder agent pattern uses workspace writes plus on-request approvals, and separates filesystem sandboxing from approval policy with explicit configuration surfaces.
- Command-prefix rules with deny precedence are a useful pattern for auditable shell policy. ThinWedge already mirrors this through `thinwedge-execpolicy`, so the implementation should extend existing prefix-rule policy rather than inventing a parallel shell classifier.
