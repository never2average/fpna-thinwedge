...
This document explains the design decision behind ThinWedge's alternate screen handling, particularly in terminal multiplexers like Zellij. This addresses a fundamental conflict between fullscreen TUI behavior and terminal scrollback history preservation.

## The Problem

### Fullscreen TUI Benefits

ThinWedge's TUI uses the terminal's **alternate screen buffer** to provide a clean fullscreen experience. This approach:
...
When using ThinWedge's TUI in Zellij, users cannot scroll back through the conversation history because:
...
ThinWedge implements a **pragmatic workaround** with three modes, controlled by `tui.alternate_screen` in `config.toml`:
...
```rust
let terminal_info = thinwedge_core::terminal::terminal_info();
!matches!(terminal_info.multiplexer, Some(Multiplexer::Zellij { .. }))
```

This detection happens in the helper function `determine_alt_screen_mode()` in `thinwedge-rs/tui/src/lib.rs`.

### Configuration Schema

The `AltScreenMode` enum is defined in `thinwedge-rs/protocol/src/config_types.rs` and serializes to lowercase TOML:
...
ThinWedge's transcript pager (opened with Ctrl+T) provides an alternative way to review conversation history, even in fullscreen mode. However, this is not as seamless as natural scrollback.
...
If you encounter issues with terminal state after running ThinWedge, you can restore your terminal with:
