...
- `thinwedge-rs/tui/src/bottom_pane/chat_composer.rs`

Paste-burst detector:

- `thinwedge-rs/tui/src/bottom_pane/paste_burst.rs`
...
- **Persistent history** (cross-session, fetched from `~/.thinwedge/history.jsonl`): text-only. It
...
(`thinwedge-rs/tui/src/bottom_pane/chat_composer.rs`). The default config preserves current chat
behavior.
...
`thinwedge-rs/tui/src/bottom_pane/slash_commands.rs` and reused by both the composer and the command
