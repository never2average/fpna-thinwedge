...
work_root="$(mktemp -d "${temp_root%/}/thinwedge-zsh-release.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

source_root="${work_root}/zsh"
package_root="${work_root}/thinwedge-zsh"
...
#!/usr/bin/env bash
set -euo pipefail
: "${THINWEDGE_WRAPPER_LOG:?missing THINWEDGE_WRAPPER_LOG}"
printf '%s\n' "$@" > "$THINWEDGE_WRAPPER_LOG"
file="$1"
shift
if [[ "$#" -eq 0 ]]; then
  exec "$file"
...
THINWEDGE_WRAPPER_LOG="$wrapper_log_path" \
...
(cd "$work_root" && tar -czf "${workspace}/${archive_path}" thinwedge-zsh)
