...
DEFAULT_CARGO_TOML = ROOT / "thinwedge-rs" / "Cargo.toml"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "thinwedge-rs/Cargo.toml [workspace.lints.clippy]."
        )
    )
    parser.add_argument(
        "--cargo-toml",
...
def find_workspace_lints_example_manifest() -> str | None:
    for cargo_toml in sorted((ROOT / "thinwedge-rs").glob("**/Cargo.toml")):
