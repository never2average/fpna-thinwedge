...
"""Verify that thinwedge-rs Cargo manifests follow workspace manifest policy.

Checks:
- Crates inherit `[workspace.package]` metadata.
- Crates opt into `[lints] workspace = true`.
- Crate names follow the thinwedge-rs directory naming conventions.
...
CARGO_RS_ROOT = ROOT / "thinwedge-rs"
WORKSPACE_PACKAGE_FIELDS = ("version", "edition", "license")
TOP_LEVEL_NAME_EXCEPTIONS = {
    "windows-sandbox-rs": "thinwedge-windows-sandbox",
}
UTILITY_NAME_EXCEPTIONS = {
    "path-utils": "thinwedge-utils-path",
}
...
def main() -> int:
    ...
    print(
        "Cargo manifests under thinwedge-rs must inherit workspace package metadata, "
    ...
    print(
        "Cargo only applies `thinwedge-rs/Cargo.toml` `[workspace.lints.clippy]` "
    ...
    print()
    print(
        "Package-name checks apply to `thinwedge-rs/<crate>/Cargo.toml` and "
        "`thinwedge-rs/utils/<crate>/Cargo.toml`."
    )
    print(
...
def expected_package_name(path: Path) -> str | None:
    ...
    if len(parts) == 2 and parts[1] == "Cargo.toml":
        ...
        return TOP_LEVEL_NAME_EXCEPTIONS.get(
            directory,
            directory if directory.startswith("thinwedge-") else f"thinwedge-{directory}",
        )
    if len(parts) == 3 and parts[0] == "utils" and parts[2] == "Cargo.toml":
        directory = parts[1]
        return UTILITY_NAME_EXCEPTIONS.get(directory, f"thinwedge-utils-{directory}")
    return None
