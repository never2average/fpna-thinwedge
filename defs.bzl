...
def thinwedge_rust_crate(
    ...
    if manifest_relpath.startswith("thinwedge-rs/"):
        manifest_relpath = manifest_relpath[len("thinwedge-rs/"):]
    ...
    if lib_srcs:
        ...
        rust_test(
            ...
            # tests under gnullvm.
            # Bazel has emitted both `thinwedge-rs/<crate>/...` and
            # `../thinwedge-rs/<crate>/...` paths for `file!()`. Strip either
            # prefix so the workspace-root launcher sees Cargo-like metadata
            # such as `tui/src/...`.
            rustc_flags = rustc_flags_extra + WINDOWS_RUSTC_LINK_FLAGS + [
                "--remap-path-prefix=../thinwedge-rs=",
                "--remap-path-prefix=thinwedge-rs=",
            ],
            rustc_env = rustc_env,
        ...
        workspace_root_test(
            ...
            workspace_root_marker = "//thinwedge-rs/utils/cargo-bin:repo_root.marker",
            tags = test_tags,
            **unit_test_kwargs
        )
    ...
    for test in native.glob(["tests/*.rs"], allow_empty = True):
        ...
        rust_test(
            ...
            # `INSTA_WORKSPACE_ROOT="thinwedge-rs"` is tuned for unit tests that
