...
`openai-thinwedge` is a public beta. Install it with
`pip install openai-thinwedge`; public APIs may change before `1.0`. While beta
...
```python
from openai_thinwedge import Sandbox
...
When `sandbox=` is omitted, ThinWedge uses its configured default. A turn
...
- installation is incomplete and the pinned `openai-thinwedge-cli-bin` dependency is missing
- local `thinwedge_bin` override points to a missing file
- a custom local ThinWedge executable does not support the SDK operation being used
