# OpenAI ThinWedge Python SDK (Beta) - API Reference

Public surface of `openai_thinwedge` for ThinWedge workflows.
...
from openai_thinwedge.types import (
    Account,
...
- Version: `openai_thinwedge.__version__`
- Requires Python >= 3.10
- Public ThinWedge protocol value and event types live in `openai_thinwedge.types`

## ThinWedge (sync)

```python
ThinWedge(config: ThinWedgeConfig | None = None)
```
...
```python
AsyncThinWedge(config: ThinWedgeConfig | None = None)
```
...
When `sandbox=` is omitted, ThinWedge uses its configured default. A sandbox
...
- one `ThinWedge` instance can stream multiple active turns concurrently
...
- `stream()` and `run()` consume only notifications for their own turn ID
- one `AsyncThinWedge` instance can stream multiple active turns concurrently
...
The SDK wrappers return and accept public ThinWedge protocol models wherever possible:
