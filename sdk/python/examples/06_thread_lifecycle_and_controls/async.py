...
async def main() -> None:
    async with AsyncThinWedge(config=runtime_config()) as thinwedge:
        ...
        reopened = await thinwedge.thread_resume(thread.id)
        listing_active = await thinwedge.thread_list(limit=20, archived=False)
        reading = await reopened.read(include_turns=True)

        _ = await reopened.set_name("sdk-lifecycle-demo")
        _ = await thinwedge.thread_archive(reopened.id)
        listing_archived = await thinwedge.thread_list(limit=20, archived=True)
        unarchived = await thinwedge.thread_unarchive(reopened.id)

        resumed = await thinwedge.thread_resume(
            unarchived.id,
            model="gpt-5.4",
        ...
        forked = await thinwedge.thread_fork(unarchived.id, model="gpt-5.4")
        forked_result = await (
