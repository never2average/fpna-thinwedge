...
- `sync.py` (public sync surface: `ThinWedge`)
- `async.py` (public async surface: `AsyncThinWedge`)

All examples intentionally use only public SDK exports from `openai_thinwedge`
and `openai_thinwedge.types`.
...
```bash
python -m pip install openai-thinwedge
```

The SDK installs its pinned `openai-thinwedge-cli-bin` runtime dependency.
