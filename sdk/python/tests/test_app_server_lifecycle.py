...
from openai_thinwedge import AsyncThinWedge, ThinWedge
...
def test_thread_list_filters_archived_threads(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with ThinWedge(config=harness.app_server_config()) as thinwedge:
            active_thread = thinwedge.thread_start()
            archived_thread = thinwedge.thread_start()
            active_thread.run("keep this listed")
            archived_thread.run("archive this")
            thinwedge.thread_archive(archived_thread.id)
            active_list = thinwedge.thread_list(archived=False)
            archived_list = thinwedge.thread_list(archived=True)
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncThinWedge(config=harness.app_server_config()) as thinwedge:
                ...
                resumed = await thinwedge.thread_resume(thread.id)
                forked = await thinwedge.thread_fork(thread.id)
                archive_response = await thinwedge.thread_archive(thread.id)
                unarchived = await thinwedge.thread_unarchive(thread.id)
...
def test_thread_fork_returns_distinct_thread(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with ThinWedge(config=harness.app_server_config()) as thinwedge:
            ...
            forked = thinwedge.thread_fork(thread.id)
...
def test_archive_unarchive_round_trip_uses_materialized_rollout(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with ThinWedge(config=harness.app_server_config()) as thinwedge:
            ...
            archived = thinwedge.thread_archive(thread.id)
            unarchived = thinwedge.thread_unarchive(thread.id)
