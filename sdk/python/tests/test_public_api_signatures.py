...
import openai_thinwedge.types as public_types
from openai_thinwedge import (
    ApprovalMode,
    AsyncThinWedge,
    AsyncThread,
    AsyncTurnHandle,
...
from openai_thinwedge._initialize_metadata import validate_initialize_metadata
from openai_thinwedge.types import InitializeResponse
...
def test_root_exports_thinwedge_config() -> None:
    """The root package should expose the process configuration object."""
    assert ThinWedgeConfig.__name__ == "ThinWedgeConfig"
...
def test_package_and_default_client_versions_follow_project_version() -> None:
    ...
    assert openai_thinwedge.__version__ == pyproject["project"]["version"]
    assert ThinWedgeConfig().client_version == openai_thinwedge.__version__
...
def test_curated_public_api_has_builtin_help_documentation() -> None:
    ...
    documented = {
        "module": openai_thinwedge,
        "ThinWedge": ThinWedge,
        "AsyncThinWedge": AsyncThinWedge,
        "ThinWedgeConfig": ThinWedgeConfig,
        "Thread": Thread,
        ...
        "Sandbox": Sandbox,
        "thread_start": ThinWedge.thread_start,
        "thread_resume": ThinWedge.thread_resume,
...
def test_package_includes_py_typed_marker() -> None:
    ...
    marker = resources.files("openai_thinwedge").joinpath("py.typed")
...
def test_package_root_exports_only_public_api() -> None:
    ...
    assert openai_thinwedge.__all__ == EXPECTED_ROOT_EXPORTS
    assert {name: hasattr(openai_thinwedge, name) for name in EXPECTED_ROOT_EXPORTS} == dict.fromkeys(
        EXPECTED_ROOT_EXPORTS, True
    )
    assert {
        "ThinWedgeClient": hasattr(openai_thinwedge, "ThinWedgeClient"),
        "AsyncThinWedgeClient": hasattr(openai_thinwedge, "AsyncThinWedgeClient"),
        "InitializeResponse": hasattr(openai_thinwedge, "InitializeResponse"),
        "ThreadStartParams": hasattr(openai_thinwedge, "ThreadStartParams"),
        "TurnStartParams": hasattr(openai_thinwedge, "TurnStartParams"),
        "TurnCompletedNotification": hasattr(openai_thinwedge, "TurnCompletedNotification"),
        "TurnStatus": hasattr(openai_thinwedge, "TurnStatus"),
    } == {
        "ThinWedgeClient": False,
        "AsyncThinWedgeClient": False,
...
def test_package_star_import_matches_public_api() -> None:
    ...
    exec("from openai_thinwedge import *", namespace)
...
def test_types_module_exports_curated_public_types() -> None:
    """The public type module should expose ThinWedge protocol models."""
...
def test_types_star_import_matches_public_types() -> None:
    ...
    namespace: dict[str, object] = {}
    exec("from openai_thinwedge.types import *", namespace)
...
def test_examples_use_public_import_surfaces() -> None:
    ...
    private_import_markers = [
        "openai_thinwedge.api",
        "openai_thinwedge.client",
        "openai_thinwedge.generated",
        "openai_thinwedge.models",
        "openai_thinwedge.retry",
    ]
