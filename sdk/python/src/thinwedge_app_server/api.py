    ...
    async def _ensure_initialized(self) -> None:
        ...
        async with self._init_lock:
            ...
            try:
                ...
                self._init = ThinWedge._validate_initialize(payload)
...
class AsyncThread:
    ...
    async def read(self, *, include_turns: bool = False) -> ThreadReadResponse:
        await self._thinwedge._ensure_initialized()
        return await self._thinwedge._client.thread_read(
...
class AsyncTurnHandle:
    ...
    async def stream(self) -> AsyncIterator[Notification]:
        await self._thinwedge._ensure_initialized()
        # TODO: replace this client-wide experimental guard with per-turn event demux.
        self._thinwedge._client.acquire_turn_consumer(self.id)
        try:
            while True:
                event = await self._thinwedge._client.next_notification()
                yield event
                if (
                    event.method == "turn/completed"
                    and isinstance(event.payload, TurnCompletedNotification)
            ...
            self._thinwedge._client.release_turn_consumer(self.id)
