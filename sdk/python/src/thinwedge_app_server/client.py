...
def _installed_thinwedge_path() -> Path:
    try:
        ...
        raise FileNotFoundError(
            ...
            f"with its {RUNTIME_PKG_NAME} dependency, or set AppServerConfig.thinwedge_bin "
            "explicitly."
        ) from exc
...
def resolve_thinwedge_bin(config: "AppServerConfig", ops: ThinWedgeBinResolverOps) -> Path:
    if config.thinwedge_bin is not None:
        thinwedge_bin = Path(config.thinwedge_bin)
        if not ops.path_exists(thinwedge_bin):
            raise FileNotFoundError(
                f"ThinWedge binary not found at {thinwedge_bin}. Set AppServerConfig.thinwedge_bin "
...
def _resolve_thinwedge_bin(config: "AppServerConfig") -> Path:
