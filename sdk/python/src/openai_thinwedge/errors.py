...
class ThinWedgeError(Exception):
    """Base exception for SDK errors."""


class JsonRpcError(ThinWedgeError):
...
class TransportClosedError(ThinWedgeError):
    """Raised when the ThinWedge transport closes unexpectedly."""


class ThinWedgeRpcError(JsonRpcError):
    """Base typed error for JSON-RPC failures."""


class ParseError(ThinWedgeRpcError):
    """Raised when a request or response cannot be parsed."""


class InvalidRequestError(ThinWedgeRpcError):
    """Raised when the runtime rejects the request shape."""


class MethodNotFoundError(ThinWedgeRpcError):
    """Raised when the requested operation is unavailable."""


class InvalidParamsError(ThinWedgeRpcError):
    """Raised when an operation receives invalid parameters."""


class InternalRpcError(ThinWedgeRpcError):
    """Raised when the runtime reports an internal RPC failure."""


class ServerBusyError(ThinWedgeRpcError):
...
def _is_server_overloaded(data: Any) -> bool:
    ...
    if isinstance(data, dict):
        direct = data.get("thinwedge_error_info") or data.get("thinwedgeErrorInfo") or data.get("errorInfo")
...
def map_jsonrpc_error(code: int, message: str, data: Any = None) -> JsonRpcError:
    ...
    if -32099 <= code <= -32000:
        ...
        if _contains_retry_limit_text(message):
            return RetryLimitExceededError(code, message, data)
        return ThinWedgeRpcError(code, message, data)
