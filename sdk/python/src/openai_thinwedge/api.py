...
from .async_client import AsyncThinWedgeClient
from .client import ThinWedgeClient, ThinWedgeConfig
...
class ThinWedge:
    """Synchronous client for creating threads and running ThinWedge turns.

    The client starts its runtime connection during construction. Use it as a
    context manager so resources are closed promptly.
    """

    def __init__(self, config: ThinWedgeConfig | None = None) -> None:
        self._client = ThinWedgeClient(config=config)
        try:
            self._client.start()
    ...
    def login_api_key(self, api_key: str) -> None:
        """Authenticate ThinWedge with an API key."""
    ...
    def account(self, *, refresh_token: bool = False) -> GetAccountResponse:
        """Read the current ThinWedge account state."""
        return self._client.account_read(GetAccountParams(refresh_token=refresh_token))

    def logout(self) -> None:
        """Clear the current ThinWedge account session."""
        self._client.account_logout()

    # BEGIN GENERATED: ThinWedge.flat_methods
    def thread_start(
        self,
        *,
    ...
    # END GENERATED: ThinWedge.flat_methods

    def models(self, *, include_hidden: bool = False) -> ModelListResponse:
        """List available models reported by ThinWedge."""
        return self._client.model_list(include_hidden=include_hidden)


class AsyncThinWedge:
    """Async mirror of :class:`ThinWedge`.

    Prefer ``async with AsyncThinWedge()`` so initialization and shutdown are
    explicit and paired. The async client initializes lazily on context entry
    or first awaited API use.
    """

    def __init__(self, config: ThinWedgeConfig | None = None) -> None:
        self._client = AsyncThinWedgeClient(config=config)
        self._init: InitializeResponse | None = None
        self._initialized = False
        self._init_lock = asyncio.Lock()

    async def __aenter__(self) -> "AsyncThinWedge":
    ...
    def metadata(self) -> InitializeResponse:
        if self._init is None:
            raise RuntimeError(
                "AsyncThinWedge is not initialized yet. Prefer `async with AsyncThinWedge()`; "
    ...
    # BEGIN GENERATED: AsyncThinWedge.flat_methods
    ...
    # END GENERATED: AsyncThinWedge.flat_methods
...
class AsyncThread:
    ...
    async def read(self, *, include_turns: bool = False) -> ThreadReadResponse:
        """Read this thread, optionally including its turn history."""
        await self._thinwedge._ensure_initialized()
        return await self._thinwedge._client.thread_read(self.id, include_turns=include_turns)

    async def set_name(self, name: str) -> ThreadSetNameResponse:
        await self._thinwedge._ensure_initialized()
        return await self._thinwedge._client.thread_set_name(self.id, name)

    async def compact(self) -> ThreadCompactStartResponse:
        await self._thinwedge._ensure_initialized()
        return await self._thinwedge._client.thread_compact(self.id)
...
class AsyncTurnHandle:
    ...
    async def steer(self, input: RunInput) -> TurnSteerResponse:
        """Send additional input to this active turn."""
        await self._thinwedge._ensure_initialized()
        return await self._thinwedge._client.turn_steer(
            self.thread_id,
            self.id,
            _to_wire_input(_normalize_run_input(input)),
        )
    ...
    async def interrupt(self) -> TurnInterruptResponse:
        """Request interruption of this active turn."""
        await self._thinwedge._ensure_initialized()
        return await self._thinwedge._client.turn_interrupt(self.thread_id, self.id)

    async def stream(self) -> AsyncIterator[Notification]:
        """Yield only notifications routed to this async turn handle."""
        await self._thinwedge._ensure_initialized()
        self._thinwedge._client.register_turn_notifications(self.id)
        try:
            while True:
                event = await self._thinwedge._client.next_turn_notification(self.id)
                yield event
                if (
                    event.method == "turn/completed"
                    and isinstance(event.payload, TurnCompletedNotification)
            ...
            self._thinwedge._client.unregister_turn_notifications(self.id)
