...
from .errors import ThinWedgeError, InvalidRequestError, TransportClosedError
...
def _installed_thinwedge_path() -> Path:
    try:
        from thinwedge_cli_bin import bundled_thinwedge_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned ThinWedge runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set ThinWedgeConfig.thinwedge_bin "
            "explicitly."
        ) from exc

    return bundled_thinwedge_path()


def _installed_thinwedge_path_dirs() -> tuple[Path, ...]:
    try:
        from thinwedge_cli_bin import bundled_path_dir
...
class ThinWedgeBinResolverOps:
    installed_thinwedge_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_thinwedge_bin_resolver_ops() -> ThinWedgeBinResolverOps:
    return ThinWedgeBinResolverOps(
        installed_thinwedge_path=_installed_thinwedge_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_thinwedge_bin(config: "ThinWedgeConfig", ops: ThinWedgeBinResolverOps) -> Path:
    if config.thinwedge_bin is not None:
        thinwedge_bin = Path(config.thinwedge_bin)
        if not ops.path_exists(thinwedge_bin):
            raise FileNotFoundError(
                f"ThinWedge binary not found at {thinwedge_bin}. Set ThinWedgeConfig.thinwedge_bin "
                "to a valid binary path."
            )
        return thinwedge_bin

    return ops.installed_thinwedge_path()


def _resolve_thinwedge_bin(config: "ThinWedgeConfig") -> Path:
    return resolve_thinwedge_bin(config, _default_thinwedge_bin_resolver_ops())


@dataclass(slots=True)
class ThinWedgeConfig:
    """Configuration for launching and identifying the local ThinWedge runtime.

    Most callers can use ``ThinWedge()`` without configuration. Set ``thinwedge_bin``
    only when intentionally using a specific local ThinWedge executable.
    """

    thinwedge_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "thinwedge_python_sdk"
    client_title: str = "ThinWedge Python SDK"
    client_version: str = SDK_VERSION
    experimental_api: bool = True


class ThinWedgeClient:
    """Synchronous typed JSON-RPC client for `thinwedge app-server` over stdio."""

    def __init__(
        self,
        config: ThinWedgeConfig | None = None,
        approval_handler: ApprovalHandler | None = None,
    ) -> None:
        self.config = config or ThinWedgeConfig()
    ...
    def __enter__(self) -> "ThinWedgeClient":
        self.start()
        return self
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            thinwedge_bin = _resolve_thinwedge_bin(self.config)
            if self.config.thinwedge_bin is None:
                path_dirs = _installed_thinwedge_path_dirs()
            args = [str(thinwedge_bin)]
    ...
    def request(
        ...
        *,
        ...
        if not isinstance(result, dict):
            raise ThinWedgeError(f"{method} response must be a JSON object")
    ...
    def _write_message(self, payload: JsonObject) -> None:
        if self._proc is None or self._proc.stdin is None:
            raise TransportClosedError("ThinWedge process is not running")
        with self._lock:
    ...
    def _read_message(self) -> dict[str, JsonValue]:
        ...
        if not line:
            raise TransportClosedError(
                f"ThinWedge process closed stdout. stderr_tail={self._stderr_tail()[:2000]}"
            )

        try:
            message = json.loads(line)
        except json.JSONDecodeError as exc:
            raise ThinWedgeError(f"Invalid JSON-RPC line: {line!r}") from exc

        if not isinstance(message, dict):
            raise ThinWedgeError(f"Invalid JSON-RPC payload: {message!r}")
        return message


def default_thinwedge_home() -> str:
    return str(Path.home() / ".thinwedge")
