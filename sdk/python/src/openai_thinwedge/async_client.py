...
class AsyncThinWedgeClient:
    """Async wrapper around ThinWedgeClient using thread offloading."""

    def __init__(self, config: ThinWedgeConfig | None = None) -> None:
        """Create the wrapped sync client that owns the transport process."""
        self._sync = ThinWedgeClient(config=config)

    async def __aenter__(self) -> "AsyncThinWedgeClient":
        """Start the ThinWedge process when entering an async context."""
        await self.start()
        return self

    async def __aexit__(self, _exc_type, _exc, _tb) -> None:
        """Close the ThinWedge process when leaving an async context."""
        await self.close()
    ...
    async def initialize(self) -> InitializeResponse:
        """Initialize the ThinWedge session."""
        ...
        try:
            ...
            def cleanup_cancelled_start(
                ...
                threading.Thread(
                    ...
                    name="thinwedge-goal-start-cleanup",
                    daemon=True,
                ).start()
