...
SDK_DISTRIBUTION_NAME = "openai-thinwedge"
RUNTIME_DISTRIBUTION_NAME = "openai-thinwedge-cli-bin"
RUNTIME_PACKAGE_ROOT = Path("src") / "thinwedge_cli_bin"
THINWEDGE_PACKAGE_METADATA = "thinwedge-package.json"
...
def schema_bundle_path(schema_dir: Path) -> Path:
    ...
    return schema_dir / "thinwedge_app_server_protocol.v2.schemas.json"
...
def pinned_runtime_version() -> str:
    ...
    return normalize_thinwedge_version(pins[0])


def pinned_runtime_thinwedge_path() -> Path:
    """Return the bundled ThinWedge binary from the installed pinned runtime wheel."""
    ...
    normalized_installed_version = normalize_thinwedge_version(installed_version)
    ...
    try:
        from thinwedge_cli_bin import bundled_thinwedge_path
    except ImportError as exc:
        raise RuntimeError(
            f"Installed {RUNTIME_DISTRIBUTION_NAME} package does not expose bundled_thinwedge_path."
        ) from exc

    thinwedge_path = bundled_thinwedge_path()
    if not thinwedge_path.exists():
        raise RuntimeError(f"Pinned ThinWedge runtime binary not found at {thinwedge_path}.")
    return thinwedge_path
...
def normalize_thinwedge_version(version: str) -> str:
    ...
    if not re.fullmatch(r"[0-9]+(?:\.[0-9]+)*(?:(?:a|b|rc)[0-9]+)?", normalized):
        raise RuntimeError(f"Could not normalize ThinWedge version {version!r} to a PEP 440 version")
    return normalized
...
def stage_python_sdk_package(staging_dir: Path, sdk_version: str) -> Path:
    package_version = normalize_thinwedge_version(sdk_version)
    _copy_package_tree(sdk_root(), staging_dir)
    sdk_bin_dir = staging_dir / "src" / "openai_thinwedge" / "bin"
...
def stage_python_runtime_package(
    ...
    _extract_thinwedge_package_archive(package_archive, staged_runtime_package_root(staging_dir))
    return staging_dir


def _extract_thinwedge_package_archive(package_archive: Path, runtime_package_root: Path) -> None:
    if not package_archive.name.endswith(".tar.gz"):
        raise RuntimeError(f"Expected a .tar.gz ThinWedge package archive: {package_archive}")
    ...
    _validate_thinwedge_package_layout(runtime_package_root, package_archive)


def _validate_thinwedge_package_layout(package_dir: Path, package_archive: Path) -> None:
    missing_entries = []
    if not (package_dir / THINWEDGE_PACKAGE_METADATA).is_file():
        missing_entries.append(THINWEDGE_PACKAGE_METADATA)
    for entry in ("bin", "thinwedge-resources", "thinwedge-path"):
    ...
    if missing_entries:
        ...
        raise RuntimeError(f"Missing ThinWedge package layout entries in {package_archive}: {missing}")
...
def generate_schema_from_pinned_runtime(schema_dir: Path) -> Path:
    ...
    thinwedge_path = pinned_runtime_thinwedge_path()
...
def generate_v2_all(schema_dir: Path) -> None:
    ...
    out_path = sdk_root() / "src" / "openai_thinwedge" / "generated" / "v2_all.py"
...
def _notification_specs(schema_dir: Path) -> list[tuple[str, str]]:
    ...
    generated_source = (sdk_root() / "src" / "openai_thinwedge" / "generated" / "v2_all.py").read_text()
...
def generate_notification_registry(schema_dir: Path) -> None:
    ...
    out = sdk_root() / "src" / "openai_thinwedge" / "generated" / "notification_registry.py"
...
def _load_public_fields(
    ...
    if module_name == "openai_thinwedge.generated.v2_all":
...
def _load_generated_v2_all_module() -> types.ModuleType:
    ...
    module_name = "_openai_thinwedge_generated_v2_all_for_artifacts"
    sys.modules.pop(module_name, None)
    module_path = sdk_root() / "src" / "openai_thinwedge" / "generated" / "v2_all.py"
...
def _render_thinwedge_block(
    ...
    lines = [
        ...
        *_kw_signature_lines(thread_start_fields),
        "    ) -> Thread:",
        '        """Create a new ThinWedge conversation thread."""',
...
def generate_public_api_flat_methods() -> None:
    ...
    public_api_path = src_dir / "openai_thinwedge" / "api.py"
    ...
    thread_start_fields = _load_public_fields(
        "openai_thinwedge.generated.v2_all",
        "ThreadStartParams",
...
def generate_types() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="thinwedge-python-schema-") as td:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    stage_runtime_parser.add_argument(
        "package_archive",
        type=Path,
        help="Path to a ThinWedge package .tar.gz archive for this platform.",
    )
    stage_runtime_parser.add_argument(
        "--thinwedge-version",
        required=True,
...
def run_command(args: argparse.Namespace, ops: CliOps) -> None:
    if args.command == "generate-types":
        ...
        ops.stage_python_sdk_package(
            args.staging_dir,
            normalize_thinwedge_version(args.sdk_version),
        )
    elif args.command == "stage-runtime":
        ops.stage_python_runtime_package(
            args.staging_dir,
            normalize_thinwedge_version(args.thinwedge_version),
