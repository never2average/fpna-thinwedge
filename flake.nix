{
  description = "Development Nix flake for ThinWedge CLI";
  ...
  outputs = { self, nixpkgs, rust-overlay, ... }:
    let
      ...
      cargoToml = builtins.fromTOML (builtins.readFile ./thinwedge-rs/Cargo.toml);
    ...
    in
    {
      packages = forAllSystems (system:
        let
          ...
          thinwedge-rs = pkgs.callPackage ./thinwedge-rs {
            inherit version;
        ...
        in
        {
          thinwedge-rs = thinwedge-rs;
          default = thinwedge-rs;
        }
      );
