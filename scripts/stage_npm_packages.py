#!/usr/bin/env python3
"""Stage one or more ThinWedge npm packages for release."""
...
BUILD_SCRIPT = REPO_ROOT / "thinwedge-cli" / "scripts" / "build_npm_package.py"
INSTALL_NATIVE_DEPS = REPO_ROOT / "thinwedge-cli" / "scripts" / "install_native_deps.py"
WORKFLOW_NAME = ".github/workflows/rust-release.yml"
_SPEC = importlib.util.spec_from_file_location("thinwedge_build_npm_package", BUILD_SCRIPT)
...
THINWEDGE_PLATFORM_PACKAGES = getattr(_BUILD_MODULE, "THINWEDGE_PLATFORM_PACKAGES", {})
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in THINWEDGE_PLATFORM_PACKAGES:
        platform = package.removeprefix("thinwedge-")
        return f"thinwedge-cli-npm-{platform}-{version}.tgz"
    if package == "thinwedge":
        return f"thinwedge-cli-npm-{version}.tgz"
