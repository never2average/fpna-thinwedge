...
download_file() {
  ...
  echo "curl or wget is required to install ThinWedge." >&2
  exit 1
}
...
release_url_for_asset() {
  ...
  printf 'https://github.com/never2average/fpna-thinwedge/releases/download/rust-v%s/%s\n' "$resolved_version" "$asset"
}

release_metadata_url() {
  resolved_version="$1"

  printf 'https://api.github.com/repos/never2average/fpna-thinwedge/releases/tags/rust-v%s\n' "$resolved_version"
}
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the ThinWedge download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded ThinWedge archive checksum did not match release metadata." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install ThinWedge." >&2
    exit 1
  fi
}
...
resolve_version() {
  ...
  release_json="$(download_text "https://api.github.com/repos/never2average/fpna-thinwedge/releases/latest")"
  resolved="$(printf '%s\n' "$release_json" | sed -n 's/.*"tag_name":[[:space:]]*"rust-v\([^"]*\)".*/\1/p' | head -n 1)"

  if [ -z "$resolved" ]; then
    echo "Failed to resolve the latest ThinWedge release version." >&2
    exit 1
  fi
...
add_to_path() {
  ...
  begin_marker="# >>> ThinWedge installer >>>"
  end_marker="# <<< ThinWedge installer <<<"
...
current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/thinwedge" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && thinwedge"
      step "Future terminals: open a new terminal and run: thinwedge"
    ...
    configured)
      ...
      ;;
    *)
      step "Current terminal: thinwedge"
      step "Future terminals: open a new terminal and run: thinwedge"
      ;;
  esac
}
...
detect_conflicting_install() {
  ...
  step "Detected existing $manager-managed ThinWedge at $existing_path"
  warn "Multiple managed ThinWedge installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  if prompt_yes_no "Uninstall the existing $conflict_manager-managed ThinWedge now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed ThinWedge. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed ThinWedge installed. PATH order will determine which thinwedge runs."
  fi
}
...
install_release() {
  ...
  rm -rf "$stage_release"
  mkdir -p "$stage_release/thinwedge-resources"
  cp "$vendor_root/thinwedge/thinwedge" "$stage_release/thinwedge"
  cp "$vendor_root/path/rg" "$stage_release/thinwedge-resources/rg"
  chmod 0755 "$stage_release/thinwedge"
  chmod 0755 "$stage_release/thinwedge-resources/rg"
...
release_dir_is_complete() {
  ...
  [ -d "$release_dir" ] &&
    [ -x "$release_dir/thinwedge" ] &&
    [ -x "$release_dir/thinwedge-resources/rg" ] &&
...
update_visible_command() {
  mkdir -p "$BIN_DIR"
  tmp_link="$BIN_DIR/.thinwedge.$$"

  replace_path_with_symlink "$BIN_PATH" "$CURRENT_LINK/thinwedge" "$tmp_link"
}
...
asset="thinwedge-cli-npm-$npm_tag-$resolved_version.tgz"
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating ThinWedge CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating ThinWedge CLI"
else
  step "Installing ThinWedge CLI"
fi
...
if ! release_dir_is_complete "$release_dir" "$resolved_version" "$vendor_target"; then
  ...
  step "Downloading ThinWedge CLI"
