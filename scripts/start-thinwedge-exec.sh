...
local_exec_server_port="${THINWEDGE_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${THINWEDGE_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_thinwedge_rs="$remote_repo_root/thinwedge-rs"

cd "${remote_thinwedge_rs}"
cargo build -p thinwedge-cli --bin thinwedge

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/thinwedge exec-server --listen ws://127.0.0.1:0 \
...
printf '  THINWEDGE_EXEC_SERVER_URL=ws://127.0.0.1:%s thinwedge -C %q\n' \
